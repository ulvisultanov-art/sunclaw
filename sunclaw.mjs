...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `sunclaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[sunclaw] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.SUNCLAW_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.SUNCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    SUNCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["sunclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:sunclaw/sunclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g sunclaw@latest`.");
  return lines.join("\n");
};
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.SUNCLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.SUNCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.SUNCLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "sunclaw.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".sunclaw", "sunclaw.json"),
    path.join(homeDir, ".sunclaw", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "sunclaw.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.SUNCLAW_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.SUNCLAW_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.SUNCLAW_CONTAINER)) {
      return false;
    }
    ...
    process.stdout.write(commit ? `SunClaw ${version} (${commit})\n` : `SunClaw ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.SUNCLAW_BUNDLED_VERSION) ?? "0.0.0";
}
