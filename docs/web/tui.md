...
```bash
sunclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `sunclaw chat` and `sunclaw terminal` are aliases for `sunclaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `sunclaw` and `sunclaw crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- Turn delivery on:
  ...
  - or start with `sunclaw tui --deliver`
...
- Local shell commands receive `SUNCLAW_SHELL=tui-local` in their environment.
...
If `sunclaw config validate` is already failing, start with `sunclaw configure`
or `sunclaw doctor --fix` first. `sunclaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `sunclaw config set` or `sunclaw configure`, then rerun `!sunclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!sunclaw doctor --fix`.

Tips:

- Prefer `sunclaw config set` or `sunclaw configure` over hand-editing `sunclaw.json`.
- `sunclaw docs "<query>"` searches the live docs index from the same machine.
- `sunclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `SUNCLAW_THEME=light` before launching `sunclaw tui`.
- To force the original dark palette instead, set `SUNCLAW_THEME=dark`.
...
- Check the Gateway logs: `sunclaw logs --follow`.
- Confirm the agent can run: `sunclaw status` and `sunclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `sunclaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `sunclaw.json`
