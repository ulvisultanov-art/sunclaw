...
SunClaw routes replies **back to the channel where a message came from**. The
...
SunClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, SunClaw still records inbound session metadata, but it
...
when a guarded path must not create a new SunClaw session. In that mode,
SunClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when SunClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.sunclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.sunclaw`):

- `~/.sunclaw/agents/<agentId>/sessions/sessions.json`
