...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when SunClaw would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When SunClaw would reply in this chat, it will run all three agents.
...
<Tabs>
  <Tab title="Alfred's context">
    ...
    Workspace: /Users/user/sunclaw-alfred/
  ...
  <Tab title="Bärbel's context">
    ...
    Workspace: /Users/user/sunclaw-baerbel/
    Tools: read only
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Agents not responding">
    ...
    **Debug:**
    ...
    ```bash
    tail -f ~/.sunclaw/logs/gateway.log | grep broadcast
    ```
