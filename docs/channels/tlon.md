...
Tlon is a decentralized messenger built on Urbit. SunClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current SunClaw releases, so normal packaged
...
```bash
sunclaw plugins install @sunclaw/tlon
```
...
```bash
sunclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, SunClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `sunclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SunClaw replies in-thread.
