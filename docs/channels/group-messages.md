...
Goal: let SunClaw sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
- Sender surfacing: every group batch now ends with `[from: Sender Name (+E164)]` so SunClaw knows who is speaking.
- Ephemeral/view-once: we unwrap those before extracting text/mentions, so pings inside them still trigger.
- Group system prompt: on the first turn of a group session (and whenever `/activation` changes the mode) we inject a short blurb into the system prompt like `You are replying inside the WhatsApp group "<subject>". Group members: Alice (+44...), Bob (+43...), ... Activation: trigger-only ... Address the specific sender noted in the message context.` If metadata isn't available we still tell the agent it's a group chat.

## Config example (WhatsApp)

Add a `groupChat` block to `~/.sunclaw/sunclaw.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?sunclaw", "\\+?15555550123"],
        },
      },
    ],
  },
...
1. Add your WhatsApp account (the one running SunClaw) to the group.
2. Say `@sunclaw …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@sunclaw` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.sunclaw/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn't triggered a run yet.
