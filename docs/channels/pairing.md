...
read_when:
  ...
  - Reviewing SunClaw security posture
title: "Pairing"
---

"Pairing" is SunClaw's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `sunclaw-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.sunclaw/credentials/`:
...
3. On your phone, open the SunClaw iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, SunClaw keeps the existing approval as-is and creates a fresh pending upgrade request. Use `sunclaw devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `sunclaw nodes pending|approve|reject|remove|rename`) is a
