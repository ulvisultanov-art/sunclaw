...
This is handy when you need to toggle obscure settings without editing `sunclaw.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[sunclaw]"
...
Use `SUNCLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
SUNCLAW_PLUGIN_LIFECYCLE_TRACE=1 sunclaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm sunclaw ...`
...
`SUNCLAW_RUN_NODE_CPU_PROF_DIR`:

```bash
SUNCLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm sunclaw status
```
...
```bash
SUNCLAW_TRACE_SYNC_IO=1 pnpm sunclaw gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `SUNCLAW_TRACE_SYNC_IO=1` when you explicitly want Node
...
`sunclaw-gateway-watch-main` (or a profile/port-specific variant such as
`sunclaw-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t sunclaw-gateway-watch-main
```
...
# or
SUNCLAW_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
SUNCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`SUNCLAW_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`SUNCLAW_PROFILE`, `SUNCLAW_CONFIG_PATH`, `SUNCLAW_STATE_DIR`,
`SUNCLAW_GATEWAY_PORT`, and `SUNCLAW_SKIP_CHANNELS` into the pane. Put
...
`sunclaw doctor --fix --non-interactive` once and restarts the Gateway child.
Use `SUNCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `sunclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.sunclaw-dev` and
...
```bash
pnpm gateway:dev
SUNCLAW_PROFILE=dev sunclaw tui
```

If you don't have a global install yet, run the CLI via `pnpm sunclaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `SUNCLAW_PROFILE=dev`
   - `SUNCLAW_STATE_DIR=~/.sunclaw-dev`
   - `SUNCLAW_CONFIG_PATH=~/.sunclaw-dev/sunclaw.json`
   - `SUNCLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`SUNCLAW_SKIP_CHANNELS=1`).
...
```bash
SUNCLAW_PROFILE=dev sunclaw gateway --dev --reset
```
...
## Raw stream logging (SunClaw)

SunClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.sunclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
SUNCLAW_RAW_STREAM=1
SUNCLAW_RAW_STREAM_PATH=~/.sunclaw/logs/raw-stream.jsonl
```

Default file:

`~/.sunclaw/logs/raw-stream.jsonl`
...
```bash
SUNCLAW_RAW_STREAM_PATH=~/.sunclaw/logs/raw-openai-completions.jsonl
```

Default file:

`~/.sunclaw/logs/raw-openai-completions.jsonl`
...
- If you need to use the built SunClaw CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./sunclaw.mjs` or create a shell alias like `alias sunclaw-build="node $(pwd)/sunclaw.mjs"`
