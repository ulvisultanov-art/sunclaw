...
<Warning>
The TCP bridge has been **removed**. Current SunClaw builds do not ship the bridge listener and `bridge.*` config keys are no longer in the schema. This page is kept for historical reference only. Use the [Gateway Protocol](/gateway/protocol) for all node/operator clients.
</Warning>
...
- Bind the bridge to a tailnet IP: `bridge.bind: "tailnet"` in
  `~/.sunclaw/sunclaw.json` (historical only; `bridge.*` is no longer valid).
