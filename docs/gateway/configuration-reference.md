---
summary: "Gateway config reference for core SunClaw keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.sunclaw/sunclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main SunClaw config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `sunclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - SunClaw uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full SunClaw agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `sunclaw_agent_consult`
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. SunClaw probes the configured health endpoint, starts
...
SunClaw-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded SunClaw and other runtime adapters. The `sunclaw mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `sunclaw mcp set` and
  `sunclaw doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded SunClaw MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `sunclaw mcp login <name>` to store tokens under SunClaw state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is SunClaw metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed SunClaw agent ids.
  ...
  `default_tools_approval_mode` for that server. SunClaw strips the `codex`
...
See [MCP](/cli/mcp#sunclaw-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.sunclaw.install`
...
- Loaded from package or bundle directories under `~/.sunclaw/extensions` and `<workspace>/.sunclaw/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native SunClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native SunClaw plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central SunClaw option registry.
...
It does not enable Codex plugins for SunClaw provider runs, ACP
...
- Enabled Claude bundle plugins can also contribute embedded SunClaw defaults from `settings.json`; SunClaw applies those as sanitized agent settings, not as raw SunClaw config patches.
...
`commitments` controls inferred follow-up memory: SunClaw can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want SunClaw to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise SunClaw treats the loopback port as a
...
- Local managed `sunclaw` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SUNCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SUNCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.serviceName`: optional Tailscale Service name for Serve mode, such
  as `svc:sunclaw`. When set, SunClaw passes it to `tailscale serve
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, SunClaw
...
- `SUNCLAW_APNS_RELAY_BASE_URL` / `SUNCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `SUNCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `SUNCLAW_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
SUNCLAW_CONFIG_PATH=~/.sunclaw/a.json \
SUNCLAW_STATE_DIR=~/.sunclaw-a \
sunclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.sunclaw-dev` + port `19001`), `--profile <name>` (uses `~/.sunclaw-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/sunclaw/tls/server.crt",
      keyPath: "/etc/sunclaw/tls/server.key",
      caPath: "/etc/sunclaw/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.sunclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-sunclaw-token: <token>`.
Query-string hook tokens are rejected.

Validation and safety notes:

- `hooks.enabled=true` requires a non-empty `hooks.token`.
- `hooks.token` should be distinct from active Gateway shared-secret auth (`gateway.auth.token` / `SUNCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `SUNCLAW_GATEWAY_PASSWORD`); startup logs a non-fatal security warning when it detects reuse.
- `sunclaw security audit` flags hook/Gateway auth reuse as a critical finding, including Gateway password auth supplied only at audit time (`--auth password --password <password>`). Run `sunclaw doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new hook token.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.sunclaw/hooks/transforms`; workspace skill directories are rejected. If `sunclaw doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SUNCLAW_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.sunclaw/workspace/canvas",
            liveReload: true,
            // enabled: false, // or SUNCLAW_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__sunclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__sunclaw__/a2ui/`
...
- Also serves A2UI at `/__sunclaw__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `sunclaw`. Override with `SUNCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.sunclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `sunclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.sunclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SUNCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `sunclaw.json` credential paths.
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `sunclaw doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.sunclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/sunclaw/sunclaw-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). SunClaw keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.sunclaw/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, SunClaw uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `sunclaw.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `SUNCLAW_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. SunClaw then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$SUNCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one SunClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `SUNCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `sunclaw doctor --fix` can strip unknown keys).
...
- `webhook`: deprecated legacy fallback webhook URL (http/https) used by `sunclaw doctor --fix` to migrate stored jobs that still have `notify: true`; runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `sunclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Paths must not contain null bytes and must be strictly shorter than 4096 characters before and after resolution.
- SunClaw-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `sunclaw.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for SunClaw-owned writes; those writes fail closed instead of flattening the config.
