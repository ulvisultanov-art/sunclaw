...
- During heartbeat runs, SunClaw treats `HEARTBEAT_OK` as an ack when it appears at the **start or end** of the reply. The token is stripped and the reply is dropped if the remaining content is **≤ `ackMaxChars`** (default: 300).
...
- Explicit session key (copy from `sunclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
<AccordionGroup>
  ...
  <Accordion title="Visibility and skip behavior">
    ...
    - If only alert delivery is disabled, SunClaw can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
    - If the resolved heartbeat target supports typing, SunClaw shows typing while the heartbeat run is active. This uses the same target the heartbeat would send chat output to, and it is disabled by `typingMode: "never"`.
...
If **all three** are false, SunClaw skips the heartbeat run entirely (no model call).
...
If `HEARTBEAT.md` exists but is effectively empty (only blank lines and markdown headers like `# Heading`), SunClaw skips the heartbeat run to save API calls. That skip is reported as `reason=empty-heartbeat-file`. If the file is missing, the heartbeat still runs and the model decides what to do.
...
<AccordionGroup>
  <Accordion title="Behavior">
    - SunClaw parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
sunclaw system event --text "Check for urgent follow-ups" --mode now
```
...
If a heartbeat previously left an existing session on a smaller local model, for example an Ollama model with a 32k window, and the next main-session turn reports context overflow, reset the session runtime model back to the configured primary model. SunClaw's reset message calls this out when the last runtime model matches configured `heartbeat.model`.
