---
summary: "Schema-accurate configuration examples for common SunClaw setups"
read_when:
  - Learning how to configure SunClaw
...
Save to `~/.sunclaw/sunclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.sunclaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/sunclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.sunclaw/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
