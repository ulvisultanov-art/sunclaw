...
service via the external `sunclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.sunclaw.gateway`
  (or `ai.sunclaw.<profile>` when using `--profile`/`SUNCLAW_PROFILE`; legacy `com.sunclaw.*` is supported).
