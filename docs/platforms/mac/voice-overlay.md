...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `ai.sunclaw`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.sunclaw" AND category CONTAINS "voicewake"' --level info --style compact
  ```
