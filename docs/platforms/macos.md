---
summary: "SunClaw macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for SunClaw. It owns permissions,
...
- Installs the global CLI (`sunclaw`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `sunclaw gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.sunclaw.gateway`
(or `ai.sunclaw.<profile>` when using `--profile`/`SUNCLAW_PROFILE`; legacy `com.sunclaw.*` still unloads).
...
Replace the label with `ai.sunclaw.<profile>` when running a named profile.
...
The app registers the `sunclaw://` URL scheme for local actions.
...
```bash
open 'sunclaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **SunClaw.app**.
...
Avoid putting your SunClaw state dir in iCloud or other cloud-synced folders.
...
If `sunclaw doctor` detects state under:
...
```bash
cd apps/macos
swift run sunclaw-mac connect --json
swift run sunclaw-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `sunclaw gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
