...
title: "Google (Gemini)"
read_when:
  - You want to use Google Gemini models with SunClaw
    ...
    **Best for:** standard Gemini API access through Google AI Studio.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        sunclaw onboard --auth-choice gemini-api-key
        ```
      ...
      <Step title="Verify the model is available">
        ```bash
        sunclaw models list --provider google
        ```
      </Step>
    </Steps>
    ...
    **Best for:** reusing an existing Gemini CLI login via PKCE OAuth instead of a separate API key.
    ...
    <Steps>
      <Step title="Install the Gemini CLI">
        ...
        SunClaw supports both Homebrew installs and global npm installs, including
    ...
    Gemini 3.1 Pro's Gemini API model id is `gemini-3.1-pro-preview`. SunClaw accepts the shorter `google/gemini-3.1-pro` as a convenience alias and normalizes it before provider calls.

    **Environment variables:**

    - `SUNCLAW_GEMINI_OAUTH_CLIENT_ID`
    - `SUNCLAW_GEMINI_OAUTH_CLIENT_SECRET`
...
<Tip>
Gemini 3 models use `thinkingLevel` rather than `thinkingBudget`. SunClaw maps
...
a fixed SunClaw level. Gemini 3 and Gemini 3.1 omit a fixed `thinkingLevel` so
Google can choose the level; Gemini 2.5 sends Google's dynamic sentinel
`thinkingBudget: -1`.

Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. SunClaw
...
SunClaw adapts telephony/Meet bridge audio to Gemini's PCM Live API stream and
keeps tool calls on the shared realtime voice contract. Leave `temperature`
unset unless you need sampling changes; SunClaw omits non-positive values
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), SunClaw
    ...
    - Gemini cache-hit usage is normalized into SunClaw `cacheRead` from
  ...
  <Accordion title="Gemini CLI JSON usage notes">
    When using the `google-gemini-cli` OAuth provider, SunClaw normalizes
