---
summary: "Use NovitaAI's OpenAI-compatible API with SunClaw"
read_when:
  - You want to run SunClaw with NovitaAI models
...
API. In SunClaw it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. SunClaw handles
...
```bash
sunclaw onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for SunClaw model selection. Your account,
...
```bash
sunclaw models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `sunclaw onboard --auth-choice novita-api-key` if the stored profile is
  stale.
