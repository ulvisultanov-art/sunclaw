---
summary: "Use OpenAI via API keys or Codex subscription in SunClaw"
read_when:
  - You want to use OpenAI models in SunClaw
...
ChatGPT-plan coding agent through OpenAI's Codex clients. SunClaw uses one
provider id, `openai`, for both auth shapes.

SunClaw uses `openai/*` as the canonical OpenAI model route. Embedded agent
...
- **Legacy config** - legacy Codex model refs are repaired by
  `sunclaw doctor --fix` to `openai/*` plus the Codex runtime.

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like SunClaw.
...
`auth.order.openai` for config; `sunclaw doctor --fix` rewrites legacy
...
SunClaw runtime config remains available as an opt-in compatibility route. When SunClaw is
explicitly selected with an `openai` OAuth profile, SunClaw keeps the
public model ref as `openai/*` and routes internally through the Codex-auth
transport. Run `sunclaw doctor --fix` to repair stale
...
SunClaw install; Realtime is the one route that needs Platform billing.
</Note>

## Memory embeddings

SunClaw can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. SunClaw forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        sunclaw onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        sunclaw onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        sunclaw models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    | `openai/gpt-5.5`      | provider/model `agentRuntime.id: "sunclaw"`              | SunClaw embedded runtime      | Selected `openai` profile |
    ...
    non-agent OpenAI API surfaces. Run `sunclaw doctor --fix` to migrate older
    ...
    SunClaw normalizes incompatible OpenAI text-verbosity overrides for this
    model.

    <Warning>
    SunClaw does **not** expose `gpt-5.3-codex-spark` on the direct OpenAI API-key route. It is available only through Codex subscription catalog entries when your signed-in account exposes it.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        sunclaw onboard --auth-choice openai
        ```
        ...
        ```bash
        sunclaw models auth login --provider openai --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        sunclaw config set agents.defaults.model.primary openai/gpt-5.5
        ```

        No runtime config is required for the default path. OpenAI agent turns
        select the native Codex app-server runtime automatically, and SunClaw
    ...
    | `openai/gpt-5.5` | provider/model `agentRuntime.id: "sunclaw"` | SunClaw embedded runtime with internal Codex-auth transport | Selected `openai` OAuth profile |
    ...
    legacy Codex GPT refs are legacy SunClaw routes, not the native Codex runtime
    path; run `sunclaw doctor --fix` when you want to migrate them to canonical
    ...
    auth order under `openai`. SunClaw will try the subscription first, then
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — SunClaw manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    sunclaw models auth list --provider openai
    sunclaw config get agents.defaults.model --json
    sunclaw config get models.providers.openai.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    sunclaw models status --agent <id>
    sunclaw models auth list --agent <id> --provider openai
    ```
    ...
    ```bash
    sunclaw models auth login --provider openai
    sunclaw models status --probe --probe-provider openai
    ```
    ...
    `sunclaw doctor --fix` to migrate older legacy OpenAI Codex prefix profile ids and
    ...
    config explicitly pins SunClaw.

    ### Doctor warning

    If legacy Codex model refs or stale OpenAI runtime pins remain in config or
    session state, `sunclaw doctor --fix` rewrites them to `openai/*` with the
    Codex runtime unless SunClaw is explicitly configured.

    ### Context window cap

    SunClaw treats model metadata and the runtime context cap as separate values.
    ...
    SunClaw uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while
    the account is authenticated, SunClaw synthesizes that OAuth model row so
...
still account-based. SunClaw selects auth in this order:
...
profile is selected, SunClaw also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
Codex usage limit, SunClaw can rotate to the next ordered `openai:*` API-key
...
still accepted. SunClaw also protects the public OpenAI and
...
`sunclaw infer image edit` when starting from an input file.
...
`openai` OAuth profile is configured, SunClaw resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; SunClaw keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for SunClaw on macOS" size=3840x2160 count=1
```
...
SunClaw adds a shared GPT-5 prompt contribution for GPT-5-family runs on SunClaw-assembled prompt surfaces. It applies by model id, so SunClaw/provider routes such as legacy pre-repair refs (legacy Codex GPT-5.5 ref), `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.

The bundled native Codex harness does not receive this SunClaw GPT-5 overlay through Codex app-server developer instructions. Native Codex keeps Codex-owned base, model, and project-doc behavior, while SunClaw disables Codex's built-in personality for native threads so agent workspace personality files stay authoritative. SunClaw contributes only runtime context such as channel delivery, SunClaw dynamic tools, ACP delegation, workspace context, and SunClaw skills.

The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification on matching SunClaw-assembled prompts. Channel-specific reply and silent-message behavior stays in the shared SunClaw system prompt and outbound delivery policy. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    sunclaw config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after SunClaw's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    SunClaw's media-understanding transcription surface.
  ...
  <Accordion title="Realtime voice">
    ...
    model has emitted audio in that session. SunClaw currently exposes the
...
generation by overriding the base URL. On the image-generation path, SunClaw
...
SunClaw recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, SunClaw:
...
SunClaw 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in SunClaw
...
SunClaw. If an Azure request fails with a validation error, check the
...
SunClaw's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    SunClaw uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.

    In `"auto"` mode, SunClaw:
  ...
  <Accordion title="Fast mode">
    SunClaw exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, SunClaw maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in SunClaw:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, SunClaw leaves `service_tier` untouched.
    </Warning>

  </Accordion>

  <Accordion title="Server-side compaction (Responses API)">
    For direct OpenAI Responses models (`openai/*` on `api.openai.com`), the OpenAI plugin's SunClaw stream wrapper auto-enables server-side compaction:

    - Forces `store: true` (unless model compat sets `supportsStore: false`)
    - Injects `context_management: [{ type: "compaction", compact_threshold: ... }]`
    - Default `compact_threshold`: 70% of `contextWindow` (or `80000` when unavailable)

    This applies to the built-in SunClaw runtime path and to OpenAI provider hooks used by embedded runs. The native Codex app-server harness manages its own context through Codex and is configured by OpenAI's default agent route or provider/model runtime policy.
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, SunClaw can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    SunClaw treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
