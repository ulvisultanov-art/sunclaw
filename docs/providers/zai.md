---
summary: "Use Z.AI (GLM models) with SunClaw"
read_when:
  - You want Z.AI / GLM models in SunClaw
...
SunClaw uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In SunClaw, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. SunClaw probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        sunclaw onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        sunclaw models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        sunclaw onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        sunclaw onboard --auth-choice zai-coding-cn

        # General API
        sunclaw onboard --auth-choice zai-global

        # General API CN (China region)
        sunclaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets SunClaw detect the matching Z.AI endpoint from the key and
...
SunClaw ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
can change; run `sunclaw models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows SunClaw's `/think` controls. With thinking off,
    SunClaw sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, SunClaw sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; SunClaw copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full SunClaw config schema, including provider and model settings.
  </Card>
</CardGroup>
