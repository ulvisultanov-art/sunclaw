---
summary: "Run SunClaw with Ollama (cloud and local models)"
read_when:
  - You want to run SunClaw with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

SunClaw integrates with Ollama's native API (`/api/chat`) for hosted cloud models and local/self-hosted Ollama servers. You can use Ollama in three modes: `Cloud + Local` through a reachable Ollama host, `Cloud only` against `https://ollama.com`, or `Local only` against a reachable Ollama host.

SunClaw also registers `ollama-cloud` as a first-class hosted provider id for
...
<Warning>
**Remote Ollama users**: Do not use the `/v1` OpenAI-compatible URL (`http://host:11434/v1`) with SunClaw. This breaks tool calling and models may output raw tool JSON as plain text. Use the native Ollama API URL instead: `baseUrl: "http://host:11434"` (no `/v1`).
</Warning>

Ollama provider config uses `baseUrl` as the canonical key. SunClaw also accepts `baseURL` for compatibility with OpenAI SDK-style examples, but new config should prefer `baseUrl`.

## Auth rules

<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Local and LAN Ollama hosts do not need a real bearer token. SunClaw uses the local `ollama-local` marker only for loopback, private-network, `.local`, and bare-hostname Ollama base URLs.
  </Accordion>
    ...
    **Best for:** fastest path to a working Ollama cloud or local setup.
    ...
    <Steps>
      ...
      <Step title="Verify the model is available">
        ```bash
        sunclaw models list --provider ollama
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Manual setup">
    **Best for:** full control over cloud or local setup.
    ...
    <Steps>
      ...
      <Step title="Enable Ollama for SunClaw">
        ...
        sunclaw config set models.providers.ollama.apiKey "OLLAMA_API_KEY"
        ```
      </Step>
      <Step title="Inspect and set your model">
        ```bash
        sunclaw models list
        sunclaw models set ollama/gemma4
        ```
...
<Tabs>
  <Tab title="Cloud + Local">
    ...
    Use **Cloud + Local** during setup. SunClaw prompts for the Ollama base URL, discovers local models from that host, and checks whether the host is signed in for cloud access with `ollama signin`. When the host is signed in, SunClaw also suggests hosted cloud defaults such as `kimi-k2.5:cloud`, `minimax-m2.7:cloud`, and `glm-5.1:cloud`.

    If the host is not signed in yet, SunClaw keeps the setup local-only until you run `ollama signin`.
  ...
  <Tab title="Cloud only">
    ...
    Use **Cloud only** during setup. SunClaw prompts for `OLLAMA_API_KEY`, sets `baseUrl: "https://ollama.com"`, and seeds the hosted cloud model list. This path does **not** require a local Ollama server or `ollama signin`.

    The cloud model list shown during `sunclaw onboard` is populated live from `https://ollama.com/api/tags`, capped at 500 entries, so the picker reflects the current hosted catalog rather than a static seed. If `ollama.com` is unreachable or returns no models at setup time, SunClaw falls back to the previous hardcoded suggestions so onboarding still completes.

    You can also configure the first-class cloud provider directly:

    ```bash
    sunclaw onboard --auth-choice ollama-cloud
    sunclaw models set ollama-cloud/kimi-k2.5:cloud
    ```

  </Tab>

  <Tab title="Local only">
    In local-only mode, SunClaw discovers models from the configured Ollama instance. This path is for local or self-hosted Ollama servers.

    SunClaw currently suggests `gemma4` as the local default.
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama` or another custom remote provider with `api: "ollama"`, SunClaw discovers models from the local Ollama instance at `http://127.0.0.1:11434`.
...
full `ollama/<model>:cloud` ref, SunClaw validates that exact missing model with
...
That path still uses SunClaw's configured provider, auth, and native Ollama
...
For speech recognition, use `sunclaw infer audio transcribe` instead.

When you switch a conversation with `/model ollama/<model>`, SunClaw treats
...
Ollama provider and `/api/tags` is unreachable, SunClaw records that cron run
...
```bash
SUNCLAW_LIVE_TEST=1 SUNCLAW_LIVE_OLLAMA=1 SUNCLAW_LIVE_OLLAMA_WEB_SEARCH=0 \
...
SUNCLAW_LIVE_OLLAMA_MODEL=glm-5.1:cloud \
SUNCLAW_LIVE_OLLAMA_WEB_SEARCH=1 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```
...
`/api/embed`. Set `SUNCLAW_LIVE_OLLAMA_EMBEDDINGS=1` when you explicitly want
...
The bundled Ollama plugin registers Ollama as an image-capable media-understanding provider. This lets SunClaw route explicit image-description requests and configured image-model defaults through local or hosted Ollama vision models.
...
```bash
sunclaw infer image describe \
  --file ./photo.jpg \
  --model ollama/qwen2.5vl:7b \
  --json
```

`--model` must be a full `<provider/model>` ref. When it is set, `sunclaw infer image describe` runs that model directly instead of skipping description because the model supports native vision.

Use `infer image describe` when you want SunClaw's image-understanding provider flow, configured `agents.defaults.imageModel`, and image-description output shape. Use `infer model run --file` when you want a raw multimodal model probe with a custom prompt and one or more images.
...
Prefer the full `ollama/<model>` ref. If the same model is listed under `models.providers.ollama.models` with `input: ["text", "image"]` and no other configured image provider exposes that bare model ID, SunClaw also normalizes a bare `imageModel` ref such as `qwen2.5vl:7b` to `ollama/qwen2.5vl:7b`. If more than one configured image provider has the same bare ID, use the provider prefix explicitly.
...
```bash
SUNCLAW_LIVE_TEST=1 SUNCLAW_LIVE_OLLAMA_IMAGE=1 \
...
SunClaw rejects image-description requests for models that are not marked image-capable. With implicit discovery, SunClaw reads this from Ollama when `/api/show` reports a vision capability.
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and SunClaw will fill it for availability checks.
    </Tip>
...
Use these as starting points and replace model IDs with the exact names from `ollama list` or `sunclaw models list --provider ollama`.

<AccordionGroup>
  <Accordion title="Local model with auto-discovery">
    Use this when Ollama runs on the same machine as the Gateway and you want SunClaw to discover the installed models automatically.
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextWindow` is the SunClaw-side context budget. `params.num_ctx` is sent to Ollama for the request. Keep them aligned when your hardware cannot run the model's full advertised context.
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    When SunClaw sends the request, the active provider prefix is stripped so `ollama-large/qwen3.5:27b` reaches Ollama as `qwen3.5:27b`.
...
provider prefix, such as `ollama-spark/qwen3:32b`, SunClaw strips only that
...
# SunClaw catalog and selected model
sunclaw models list --provider ollama
...
For remote hosts, replace `127.0.0.1` with the host used in `baseUrl`. If `curl` works but SunClaw does not, check whether the Gateway runs on a different machine, container, or service account.

## Ollama Web Search

SunClaw supports **Ollama Web Search** as a bundled `web_search` provider.
...
Choose **Ollama Web Search** during `sunclaw onboard` or `sunclaw configure --section web`, or set:
...
For a signed-in local daemon, SunClaw uses the daemon's `/api/experimental/web_search` proxy. For `https://ollama.com`, it calls the hosted `/api/web_search` endpoint directly.
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in OpenAI-compatible mode.** Use this mode only if you need OpenAI format for a proxy and do not depend on native tool calling behavior.
    ...
    When `api: "openai-completions"` is used with Ollama, SunClaw injects `options.num_ctx` by default so Ollama does not silently fall back to a 4096 context window. If your proxy/upstream rejects unknown `options` fields, disable this behavior:
  ...
  <Accordion title="Thinking control">
    For native Ollama models, SunClaw forwards thinking control as Ollama expects it: top-level `think`, not `options.think`. Auto-discovered models whose `/api/show` response includes the `thinking` capability expose `/think low`, `/think medium`, `/think high`, and `/think max`; non-thinking models expose only `/think off`.

    ```bash
    sunclaw agent --model ollama/gemma4 --thinking off
    sunclaw agent --model ollama/gemma4 --thinking low
    ```
    ...
    Per-model `params.think` or `params.thinking` can disable or force Ollama API thinking for a specific configured model. SunClaw preserves those explicit model params when the active run only has the implicit default `off`; non-off runtime commands such as `/think medium` still override the active run.

  </Accordion>

  <Accordion title="Reasoning models">
    SunClaw treats models with names such as `deepseek-r1`, `reasoning`, or `think` as reasoning-capable by default.

    ```bash
    ollama pull deepseek-r1:32b
    ```

    No additional configuration is needed. SunClaw marks them automatically.
  ...
  <Accordion title="Memory embeddings">
    ...
    SunClaw's guarded direct path instead of the managed forward proxy. The
  ...
  <Accordion title="Streaming configuration">
    SunClaw's Ollama integration uses the **native Ollama API** (`/api/chat`) by default, which fully supports streaming and tool calling simultaneously. No special configuration is needed.

    For native `/api/chat` requests, SunClaw also forwards thinking control directly to Ollama: `/think off` and `sunclaw agent --thinking off` send top-level `think: false` unless an explicit model `params.think`/`params.thinking` value is configured, while `/think low|medium|high` send the matching top-level `think` effort string. `/think max` maps to Ollama's highest native effort, `think: "high"`.
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    SunClaw logs a startup warning when it detects WSL2, `ollama.service` enabled with `Restart=always`, and visible CUDA markers.
  ...
  <Accordion title="Remote host works with curl but not SunClaw">
  ...
  <Accordion title="Kimi or GLM returns garbled symbols">
    ...
    ```bash
    sunclaw infer model run --model ollama/kimi-k2.5:cloud --prompt "Reply with exactly: ok" --json
    sunclaw models set ollama/gemma4
    ```
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    Many Ollama models advertise contexts that are larger than your hardware can run comfortably. Native Ollama uses Ollama's own runtime context default unless you set `params.num_ctx`. Cap both SunClaw's budget and Ollama's request context when you want predictable first-token latency:
    ...
    Lower `contextWindow` first if SunClaw is sending too much prompt. Lower `params.num_ctx` if Ollama is loading a runtime context that is too large for the machine. Lower `maxTokens` if generation runs too long.
