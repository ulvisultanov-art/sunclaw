---
summary: "Use Ollama Cloud directly with SunClaw"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets SunClaw call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. SunClaw registers it
...
- You want the same native Ollama chat API shape SunClaw uses for local
...
SunClaw discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
sunclaw models list --provider ollama-cloud
sunclaw models set ollama-cloud/kimi-k2.6
```
...
SUNCLAW_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `sunclaw models list --provider ollama-cloud` and
