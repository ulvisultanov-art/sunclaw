...
read_when:
  - You need a beginner-friendly overview of SunClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

SunClaw has two main log surfaces:
...
SunClaw keeps up to five numbered archives beside the active file, such as
`sunclaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.sunclaw/sunclaw.json`:

```json
{
  "logging": {
    "file": "/path/to/sunclaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `sunclaw logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `sunclaw logs --follow` is stricter: on Linux it uses the active
...
```bash
sunclaw channels logs --channel whatsapp
```
...
SunClaw preserves the original structured log arguments alongside these fields
...
`sunclaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
sunclaw gateway
sunclaw gateway --verbose --ws-log compact
sunclaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.sunclaw/sunclaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/sunclaw/sunclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`SUNCLAW_LOG_LEVEL`** environment variable (e.g. `SUNCLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `sunclaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `sunclaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
SUNCLAW_DEBUG_MODEL_TRANSPORT=1 sunclaw gateway
SUNCLAW_DEBUG_MODEL_PAYLOAD=tools SUNCLAW_DEBUG_SSE=events sunclaw gateway
```

Available flags:

- `SUNCLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `SUNCLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `SUNCLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `SUNCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `SUNCLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `SUNCLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `SUNCLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal SunClaw logging, so `sunclaw logs --follow`
...
SunClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
SunClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. SunClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `SUNCLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `sunclaw doctor` first.
