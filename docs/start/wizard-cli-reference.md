...
read_when:
  - You need detailed behavior for sunclaw onboard
  - You are debugging onboarding results or integrating onboarding clients
title: "CLI setup reference"
sidebarTitle: "CLI reference"
---

This page is the full reference for `sunclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.sunclaw/sunclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `sunclaw doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.sunclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `sunclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, SunClaw falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node (recommended; required for WhatsApp and Telegram). Bun is not recommended.

  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `sunclaw health`.
    - `sunclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported.
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth">
    ...
    recommended xAI path for most users. SunClaw stores the resulting auth
...
- Auth profiles (API keys + OAuth): `~/.sunclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.sunclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
`$SUNCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
is only a legacy import source.
</Note>

## Outputs and internals

Typical fields in `~/.sunclaw/sunclaw.json`:
...
`sunclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.sunclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.sunclaw/agents/<agentId>/sessions/`.
...
- Stores it under `~/.sunclaw/tools/signal-cli/<version>/`
...
- Command reference: [`sunclaw onboard`](/cli/onboard)
