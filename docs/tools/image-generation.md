...
SunClaw records a background task, returns the task id immediately, and wakes
...
generated images are still missing from the completion reply, SunClaw sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, SunClaw routes image requests
...
nearby geometry option instead of the exact requested one, SunClaw remaps to
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when SunClaw can
  ...
  <Accordion title="Timeouts">
    ...
    honor the same timeout budget when configured, bounded by SunClaw's 600000
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, SunClaw reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. SunClaw
    ...
    SunClaw forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible SunClaw maps
    ...
    transparency-capable OpenAI image model. SunClaw routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    SunClaw forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. SunClaw sends:
    ...
    Krea; SunClaw maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as SunClaw-managed image attachments

    SunClaw intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for SunClaw image generation" size=3840x2160 count=1
```
  </Tab>
...
`sunclaw infer image edit`; `--openai-background` remains as an
