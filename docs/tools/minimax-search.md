...
SunClaw supports MiniMax as a `web_search` provider through the MiniMax
...
SunClaw also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, SunClaw resolves
