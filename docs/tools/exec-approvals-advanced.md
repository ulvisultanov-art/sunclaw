...
- allowlist entries live in host-local `~/.sunclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `sunclaw approvals allowlist ...`).
- `sunclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `sunclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, SunClaw still rejects the `env` builtin in safe-bin
...
session, SunClaw also resumes that session with an internal followup so the agent observes that
the command did not run instead of later repairing a missing result.

### Followup delivery behavior

After an approved async exec finishes, SunClaw sends a followup `agent` turn to the same session.
...
the originating channel, SunClaw keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, SunClaw sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.sunclaw.approval` custom event content on the first prompt
  event so SunClaw-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. SunClaw first tries a private route on the
...
topics, SunClaw preserves the topic for the approval prompt and the post-approval follow-up.
