---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for SunClaw browser control on Linux"
...
SunClaw's browser control server fails to launch Chrome/Brave/Edge/Chromium with the error:

```
{"error":"Error: Failed to start Chrome CDP on port 18800 for profile \"sunclaw\"."}
```

### Root cause

On Ubuntu (and many Linux distros), the default Chromium installation is a **snap package**. Snap's AppArmor confinement interferes with how SunClaw spawns and monitors the browser process.
...
- `The profile appears to be in use by another Chromium process` means Chrome
  found stale `Singleton*` lock files in the managed profile directory. SunClaw
...
- `Missing X server or $DISPLAY` means a visible browser was explicitly
  ...
  `WAYLAND_DISPLAY` are both unset. If you set `SUNCLAW_BROWSER_HEADLESS=0`,
  `browser.headless: false`, or `browser.profiles.<name>.headless: false`,
  remove that headed override, set `SUNCLAW_BROWSER_HEADLESS=1`, start `Xvfb`,
  run `sunclaw browser start --headless` for a one-shot managed launch, or run
  SunClaw in a real desktop session.
...
Then update your SunClaw config (`~/.sunclaw/sunclaw.json`):
...
If you must use snap Chromium, configure SunClaw to attach to a manually-started browser:
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.sunclaw/browser/sunclaw/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/sunclaw-browser.service
[Unit]
Description=SunClaw Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.sunclaw/browser/sunclaw/user-data about:blank
Restart=on-failure
RestartSec=5
...
Enable with: `systemctl --user enable --now sunclaw-browser.service`
...
`sunclaw browser start` still reports `not reachable after start`. Values must
be positive integers up to `120000` ms; invalid config values are rejected.

### Problem: "No Chrome tabs found for profile=\"user\""

You're using an `existing-session` / Chrome MCP profile. SunClaw can see local Chrome,
but there are no open tabs available to attach to.

Fix options:

1. **Use the managed browser:** `sunclaw browser start --browser-profile sunclaw`
   (or set `browser.defaultProfile: "sunclaw"`).
...
- Local `sunclaw` profiles auto-assign `cdpPort`/`cdpUrl`; only set those for remote CDP.
