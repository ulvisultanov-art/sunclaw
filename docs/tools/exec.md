...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling SunClaw filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
`tools.exec.security` and `~/.sunclaw/exec-approvals.json`; elevated mode can
...
- `gateway`/`node` approvals are controlled by `~/.sunclaw/exec-approvals.json`.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, SunClaw uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- On non-Windows gateway hosts, bash and zsh exec commands use a startup snapshot. SunClaw captures sourceable
  aliases/functions and a small safe environment set from shell startup files into
  `$SUNCLAW_STATE_DIR/cache/shell-snapshots/`, then sources that snapshot before each exec command.
  Secret-looking variables are excluded; sandbox and node exec do not use this snapshot. Set
  `SUNCLAW_EXEC_SHELL_SNAPSHOT=0` in the Gateway process environment to disable this snapshot path.
- Host execution (`gateway`/`node`) rejects `env.PATH` and loader overrides (`LD_*`/`DYLD_*`) to
  prevent binary hijacking or injected code.
- SunClaw sets `SUNCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- `sunclaw channels login` is blocked from `exec` because it is an interactive channel-auth flow; run it in a terminal on the gateway host, or use the channel-native login tool from chat when one exists.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.sunclaw/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#yolo-mode-no-approval).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  SunClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`sunclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `sunclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`sunclaw security audit` and `sunclaw doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
