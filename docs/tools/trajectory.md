---
summary: "Export redacted trajectory bundles for debugging an SunClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an SunClaw session
...
Trajectory capture is SunClaw's per-session flight recorder. It records a
...
SunClaw writes the bundle under the workspace:

```text
.sunclaw/trajectory-exports/sunclaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.sunclaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, SunClaw sends the
...
Trajectory capture is on by default for SunClaw agent runs.
...
SunClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `SUNCLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export SUNCLAW_TRAJECTORY_DIR=/var/lib/sunclaw/trajectories
```

When this variable is set, SunClaw writes one JSONL file per session id in that
directory.
...
Set `SUNCLAW_TRAJECTORY=0` before starting SunClaw:

```bash
export SUNCLAW_TRAJECTORY=0
```
...
SunClaw flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`SUNCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting SunClaw:

```bash
export SUNCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when SunClaw logs an `sunclaw-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `SUNCLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
SunClaw redacts sensitive values before writing export files:
...
- confirm SunClaw was started without `SUNCLAW_TRAJECTORY=0`
- check whether `SUNCLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.sunclaw/trajectory-exports/`
