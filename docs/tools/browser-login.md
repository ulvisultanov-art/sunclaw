...
When a site requires login, **sign in manually** in the **host** browser profile (the sunclaw browser).
...
SunClaw controls a **dedicated Chrome profile** (named `sunclaw`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `sunclaw` browser.
...
sunclaw browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `sunclaw`).
...
```bash
sunclaw browser open https://x.com --browser-profile sunclaw
```
