---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, SunClaw ACP, OpenCode) through the ACP backend"
read_when:
...
let SunClaw run external coding harnesses (for example Claude Code,
Cursor, Copilot, Droid, SunClaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing SunClaw channel conversations, use
[`sunclaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
SunClaw only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; SunClaw copies trusted project entries plus safe model/provider routing config from the host Codex config, while auth, notifications, and hooks stay on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. SunClaw owns routing,
...
SunClaw plugin tools and built-in SunClaw tools are **not** exposed to
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the SunClaw session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from SunClaw's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up SunClaw-owned wrapper and adapter process trees after `close`, and reaps stale SunClaw-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    SunClaw dynamic tools still execute through SunClaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, SunClaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through SunClaw approvals. Codex `Stop` hooks are relayed to
    SunClaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    SunClaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    SunClaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an SunClaw agent
    is configured with `runtime.type="acp"`, SunClaw uses
...
plugin is enabled. Use **sub-agents** when you want SunClaw-native
delegated runs.
...
1. SunClaw ACP session control plane.
2. Official `@sunclaw/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state SunClaw routes to.
...
spawned ACP session - no child thread, same chat surface. SunClaw keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; SunClaw returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, SunClaw inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by SunClaw even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - SunClaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, SunClaw returns a clear
...
- SunClaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, SunClaw inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, SunClaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, SunClaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  `session/set_model`; otherwise SunClaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What SunClaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. SunClaw agents get SunClaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_SUNCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in SunClaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. SunClaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see SunClaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, SunClaw uses an agent-to-agent (A2A) follow-up path
    ...
    SunClaw skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an SunClaw channel session key; SunClaw still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new SunClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
- SunClaw's sandbox policy does **not** wrap ACP harness execution.
- SunClaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced SunClaw-native work.
...
If no target resolves, SunClaw returns a clear error
...
aliases), the plugin-tools and SunClaw-tools MCP bridges, and ACP
...
native tool call, restart the Codex app-server or SunClaw Gateway instead of
...
- [`sunclaw acp` (bridge mode)](/cli/acp)
