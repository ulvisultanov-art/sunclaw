...
SunClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, SunClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `sunclaw configure --section web`. It stores the key in
`~/.sunclaw/sunclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.sunclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
