---
summary: "Public SunClaw App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "SunClaw App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to SunClaw
...
The **SunClaw App SDK** is the public client API for apps outside the
SunClaw process. Use `@sunclaw/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@sunclaw/sdk` talks to the Gateway from outside SunClaw.
  `sunclaw/plugin-sdk/*` is only for plugins that run inside SunClaw and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@sunclaw/sdk` ships with:
...
`AgentRunParams`, `RunResult`, `RunStatus`, `SunClawEvent`,
`SunClawEventType`, `GatewayEvent`, `SunClawTransport`,
...
```typescript
import { SunClaw } from "@sunclaw/sdk";
...
`new SunClaw({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `SunClawTransport`:
...
The SDK normalizes raw Gateway events into a stable `SunClawEvent` envelope:
...
Task helpers use the durable task ledger that also backs `sunclaw tasks`:
...
Use the App SDK when code lives outside SunClaw:
...
Use the Plugin SDK when code runs inside SunClaw:
...
App SDK code should import from `@sunclaw/sdk`. Plugin code should import from
documented `sunclaw/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [SunClaw App SDK API design](/reference/sunclaw-sdk-api-design)
