...
The private QA stack is meant to exercise SunClaw in a more realistic,
...
Every QA flow runs under `pnpm sunclaw qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm sunclaw qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                   |
| `qa coverage`                                       | Print the markdown scenario-coverage inventory (`--json` for machine output).                                                                                                                                                                                           |
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files and write the agentic parity report, or use `--runtime-axis --token-efficiency` to write Codex-vs-SunClaw runtime parity and token-efficiency reports from one runtime-pair summary.                                          |
...
```bash
pnpm sunclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`sunclaw.run`, `sunclaw.harness.run`, a latest GenAI semantic-convention
model-call span, `sunclaw.context.assembled`, and `sunclaw.message.delivery`
...
`sunclaw.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm sunclaw qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `SUNCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm sunclaw qa telegram
pnpm sunclaw qa discord
pnpm sunclaw qa slack
```
...
```bash
pnpm sunclaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent SunClaw Slack gateway
...
```bash
pnpm sunclaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `sunclaw infer image describe`
...
`sunclaw/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm sunclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds SunClaw
...
- `SUNCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `SUNCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `SUNCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `SUNCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm sunclaw qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.

Output artifacts:

- `telegram-qa-report.md`
- `telegram-qa-summary.json` - includes per-reply RTT (driver send → observed SUT reply) starting with the canary.
- `telegram-qa-observed-messages.json` - bodies redacted unless `SUNCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
`SUNCLAW_NPM_TELEGRAM_WARM_SAMPLES` and
`SUNCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS`, so `result.json` remains comparable
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child SunClaw gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `SUNCLAW_QA_DISCORD_GUILD_ID`
- `SUNCLAW_QA_DISCORD_CHANNEL_ID`
- `SUNCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `SUNCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `SUNCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `SUNCLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `SUNCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `SUNCLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child SunClaw gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `SUNCLAW_QA_SLACK_CHANNEL_ID`
- `SUNCLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `SUNCLAW_QA_SLACK_SUT_BOT_TOKEN`
- `SUNCLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `SUNCLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `SUNCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `SUNCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-observed-messages.json` - bodies redacted unless `SUNCLAW_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `SUNCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "SunClaw QA Driver",
    "description": "Test driver bot for SunClaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "SunClaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "SunClaw QA SUT",
    "description": "SunClaw QA SUT connector for SunClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "SunClaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#sunclaw-qa`) and invite both bots from inside the channel:

```
/invite @SunClaw QA Driver
/invite @SunClaw QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `SUNCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `SUNCLAW_QA_CONVEX_SITE_URL` and `SUNCLAW_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm sunclaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm sunclaw qa credentials list --kind slack --status all --json
```
...
```bash
pnpm sunclaw qa whatsapp
```

Targets two dedicated WhatsApp Web accounts: a driver account controlled by
the harness and a SUT account started by the child SunClaw gateway through the
bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `SUNCLAW_QA_WHATSAPP_DRIVER_PHONE_E164`
- `SUNCLAW_QA_WHATSAPP_SUT_PHONE_E164`
- `SUNCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `SUNCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `SUNCLAW_QA_WHATSAPP_GROUP_JID` enables `whatsapp-mention-gating`.
- `SUNCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
...
- `whatsapp-qa-observed-messages.json` - bodies redacted unless `SUNCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1`.

### Convex credential pool

Telegram, Discord, Slack, and WhatsApp lanes can lease credentials from a shared Convex pool instead of reading the env vars above. Pass `--credential-source convex` (or set `SUNCLAW_QA_CREDENTIAL_SOURCE=convex`); QA Lab acquires an exclusive lease, heartbeats it for the duration of the run, and releases it on shutdown. Pool kinds are `"telegram"`, `"discord"`, `"slack"`, and `"whatsapp"`.
...
- `mock-openai` is the scenario-aware SunClaw mock. It remains the default
...
- how `sunclaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `sunclaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `sunclaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm sunclaw qa coverage` (add `--json` for machine-readable output).
When choosing focused proof for a touched behavior or file path, run `pnpm sunclaw qa coverage --match <query>`.
...
```bash
pnpm sunclaw qa character-eval \
