---
summary: "Mantis is the visual end-to-end verification system for reproducing SunClaw bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for SunClaw bugs
...
Mantis is the SunClaw end-to-end verification system for bugs that need a real
...
Mantis lives in the SunClaw QA stack.

- SunClaw owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm sunclaw qa mantis`.
...
- SunClaw agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in SunClaw, machine scheduling in
...
```bash
pnpm sunclaw qa mantis discord-smoke \
...
thread, calls SunClaw's `message.thread-reply` action with a repo-local
...
```bash
pnpm sunclaw qa mantis desktop-browser-smoke \
...
`SUNCLAW_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `SUNCLAW_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `SUNCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `SUNCLAW_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `SUNCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `SUNCLAW_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm sunclaw qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT SunClaw gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable SunClaw
home at `$HOME/.sunclaw-mantis/slack-sunclaw`, patches Slack Socket Mode
configuration for the selected channel, starts `sunclaw gateway run` on port
...
- `SUNCLAW_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `SUNCLAW_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `SUNCLAW_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `SUNCLAW_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent SunClaw Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `SUNCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/sunclaw-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`sunclaw/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm sunclaw qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm sunclaw qa telegram --credential-source convex
...
`@sunclaw-mantis telegram desktop proof`, from the Actions UI with freeform
...
SunClaw with the leased Telegram SUT bot token, starts `sunclaw gateway run`
...
token never logs Telegram Desktop in; it only configures SunClaw. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring SunClaw.
...
comment or creates one. The workflows publish to `sunclaw-crabbox-artifacts`
with public URLs under `https://artifacts.docs.sunclaw.complex.az`. They provide bucket,
...
```text
@sunclaw-mantis discord status reactions
```
...
```text
@sunclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child SunClaw Gateway with an isolated state directory.
...
- It exercises a real SunClaw Gateway, Discord bot auth, message dispatch,
...
- `pnpm sunclaw qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or SunClaw browser control captures screenshots.
...
- Node 22 and pnpm
- SunClaw checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one SunClaw Gateway, one browser, and one model run
...
- `SUNCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `SUNCLAW_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `SUNCLAW_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `SUNCLAW_QA_CONVEX_SITE_URL`
- `SUNCLAW_QA_CONVEX_SECRET_CI`
- `SUNCLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `SUNCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`SUNCLAW_QA_REDACT_PUBLIC_METADATA=1` for this reason.
