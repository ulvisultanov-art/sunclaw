...
<CardGroup cols={2}>
  ...
  <Card title="Agent runtimes" href="/concepts/agent-runtimes">
    SunClaw, Codex, and other agent loop runtimes.
  </Card>
...
SunClaw selects models in this order:
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    - `agents.defaults.models` is the allowlist/catalog of models SunClaw can use (plus aliases). Use `provider/*` entries to limit visible providers while keeping provider discovery dynamic.
...
- Auto fallback selections are temporary recovery state. They are stored with `modelOverrideSource: "auto"` so later turns can keep using the fallback chain without probing a known-bad primary every time; SunClaw periodically probes the original primary again, clears the auto selection when it recovers, and announces fallback/recovery transitions once per state change.
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, SunClaw fails visibly instead of falling through to another configured model.
...
<AccordionGroup>
  <Accordion title="Clobber protection rules">
    `sunclaw config set` protects model/provider maps from accidental clobbers. A plain object assignment to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` is rejected when it would remove existing entries. Use `--merge` for additive changes; use `--replace` only when the provided value should become the complete target value.

    Interactive provider setup and `sunclaw configure --section model` also merge provider-scoped selections into the existing allowlist, so adding Codex, Ollama, or another provider does not drop unrelated model entries. Configure preserves an existing `agents.defaults.model.primary` when provider auth is re-applied. Explicit default-setting commands such as `sunclaw models auth login --provider <id> --set-default` and `sunclaw models set <model>` still replace `agents.defaults.model.primary`.
...
If `agents.defaults.models` is set, it becomes the **allowlist** for `/model` and for session overrides. When a user selects a model that isn't in that allowlist, SunClaw returns:
...
exact provider/model shown by `sunclaw models list --provider <provider>`.
...
<AccordionGroup>
  <Accordion title="Picker behavior">
    ...
    - On Telegram, `/models` picker selections are session-scoped; they do not change the agent's persistent default in `sunclaw.json`.
  ...
  <Accordion title="Persistence and live switching">
    ...
    - If a run is already active, SunClaw marks a live switch as pending and only restarts into the new model at a clean retry point.
  ...
  <Accordion title="Ref parsing">
    ...
    - If you omit the provider, SunClaw resolves the input in this order:
      1. alias match
      2. unique configured-provider match for that exact unprefixed model id
      3. deprecated fallback to the configured default provider — if that provider no longer exposes the configured default model, SunClaw instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
  </Accordion>
</AccordionGroup>
...
sunclaw models set <provider/model>
sunclaw models set-image <provider/model>

sunclaw models aliases list
sunclaw models aliases add <alias> <provider/model>
sunclaw models aliases remove <alias>

sunclaw models fallbacks list
sunclaw models fallbacks add <provider/model>
sunclaw models fallbacks remove <provider/model>
sunclaw models fallbacks clear

sunclaw models image-fallbacks list
sunclaw models image-fallbacks add <provider/model>
sunclaw models image-fallbacks remove <provider/model>
sunclaw models image-fallbacks clear
```

`sunclaw models` (no subcommand) is a shortcut for `models status`.
...
`sunclaw models scan` inspects OpenRouter's **free model catalog** and can optionally probe models for tool and image support.
...
When live probes run in a TTY, you can select fallbacks interactively. In non-interactive mode, pass `--yes` to accept defaults. Metadata-only results are informational; `--set-default` and `--set-image` require live probes so SunClaw does not configure an unusable keyless OpenRouter model.

## Models registry (`models.json`)

Custom providers in `models.providers` are written into `models.json` under the agent directory (default `~/.sunclaw/agents/<agentId>/agent/models.json`). Provider-plugin catalogs are stored as generated plugin-owned catalog shards under the agent's plugin state and loaded automatically. This file is merged by default unless `models.mode` is set to `replace`.
...
<Note>
Marker persistence is source-authoritative: SunClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. This applies whenever SunClaw regenerates `models.json`, including command-driven paths like `sunclaw agent`.
</Note>

## Related

- [Agent runtimes](/concepts/agent-runtimes) — SunClaw, Codex, and other agent loop runtimes
