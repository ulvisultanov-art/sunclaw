...
Linux desktop, VNC rescue, Slack Web, a real SunClaw gateway, screenshots,
...
- Mantis artifacts: owned by the SunClaw run. They live under
...
- remote logs such as `slack-desktop-command.log`, `sunclaw-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, SunClaw install/build or
...
remote status after Mantis has copied metadata proving that either the SunClaw
