...
SunClaw manages sessions end-to-end across these areas:
...
SunClaw is designed around a single **Gateway process** that owns session state.
...
SunClaw persists sessions in two layers:
...
SunClaw resolves these via `src/config/sessions.ts`.
...
SunClaw no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `sunclaw doctor --fix` removes it from older configs.
...
`SUNCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `SUNCLAW_SESSION_WRITE_LOCK_STALE_MS`, and
`SUNCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
In `mode: "warn"`, SunClaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
sunclaw sessions cleanup --dry-run
sunclaw sessions cleanup --enforce
```
...
- **Parent fork policy** uses SunClaw's active branch when creating a thread or subagent fork. If that branch is too large, SunClaw starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `sunclaw doctor --fix`.
...
Transcripts are managed by `sunclaw/plugin-sdk/agent-sessions`'s `SessionManager`.
...
SunClaw intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
SunClaw does not append AGENTS.md excerpts on top of the compaction summary.

Compaction is **persistent** (unlike session pruning). See [/concepts/session-pruning](/concepts/session-pruning).

## Compaction chunk boundaries and tool pairing

When SunClaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, SunClaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  SunClaw preserves that pending tool block and keeps the unsummarized tail
  intact.
- Aborted/error tool-call blocks do not hold a pending split open.

---

## When auto-compaction happens (SunClaw runtime)

In the embedded SunClaw agent, auto-compaction triggers in two cases:
...
exceeded`, and similar provider-shaped variants) → compact → retry.
   When the provider reports the attempted token count, SunClaw forwards that
   observed count into overflow recovery compaction. If the provider confirms
   overflow but does not expose a parseable count, SunClaw passes a minimally
   over-budget synthetic count to compaction engines and diagnostics.
   If overflow recovery still fails, SunClaw surfaces explicit guidance to the
...
These are SunClaw runtime semantics.

SunClaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: SunClaw still runs normal semantic compaction,
and it requires `truncateAfterCompaction` so the compacted summary can become a
new successor transcript.

For embedded SunClaw runs, `agents.defaults.compaction.midTurnPrecheck.enabled: true`
adds an opt-in tool-loop guard. After a tool result is appended and before the
next model call, SunClaw estimates the prompt pressure using the same preflight
budget logic used at turn start. If the context no longer fits, the guard does
not compact inside SunClaw runtime's `transformContext` hook. It raises a structured
...
before a turn opens, while mid-turn precheck runs later in the embedded SunClaw tool
...
SunClaw runtime's compaction settings live in agent settings:
...
SunClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SunClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, SunClaw leaves it alone.
- Manual `/compact` honors an explicit `agents.defaults.compaction.keepRecentTokens`
  and keeps SunClaw runtime's recent-tail cut point. Without an explicit keep budget,
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  SunClaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, SunClaw falls back to built-in LLM summarization automatically.
...
- `sunclaw status` (CLI)
- `sunclaw sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `sunclaw logs --follow`): `embedded run auto-compaction start` + `complete`
...
SunClaw supports "silent" turns for background tasks where the user should not see intermediate output.
...
- SunClaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, SunClaw also suppresses **draft/typing streaming** when a
...
SunClaw uses the **pre-threshold flush** approach:

1. Monitor session context usage.
2. When it crosses a "soft threshold" (below SunClaw runtime's compaction threshold), run a silent
...
- The flush runs only for embedded SunClaw sessions (CLI backends skip it).
- The flush is skipped when the session workspace is read-only (`workspaceAccess: "ro"` or `"none"`).
- See [Memory](/concepts/memory) for the workspace file layout and write patterns.

SunClaw also exposes a `session_before_compact` hook in the extension API, but SunClaw's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `sunclaw status`.
