---
summary: "Reference design for the public SunClaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "SunClaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public SunClaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level SunClaw App SDK wrapper
---

This page is the detailed API reference design for the public
[SunClaw App SDK](/concepts/sunclaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@sunclaw/sdk` is the external app/client package for talking to the
  Gateway. `sunclaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `SunClaw`, `Agent`, `Session`, `Run`,
...
- source: SunClaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
The repo already has `sunclaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `SunClaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [SunClaw App SDK](/concepts/sunclaw-sdk)
