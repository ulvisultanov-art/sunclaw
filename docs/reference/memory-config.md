...
This page lists every configuration knob for SunClaw memory search. For conceptual overviews, see:
...
All memory search settings live under `agents.defaults.memorySearch` in `sunclaw.json` unless noted otherwise.
...
When `provider` is not set, SunClaw uses OpenAI embeddings. Set `provider`
...
SunClaw pauses vector search and reports an index identity warning instead of
automatically re-embedding everything. Rebuild when you are ready with
`sunclaw memory status --index --agent <id>` or
`sunclaw memory index --force --agent <id>`.
</Warning>
...
`memorySearch.provider` can point at a custom `models.providers.<id>` entry for memory-specific provider adapters such as `ollama`, or for OpenAI-compatible model APIs such as `openai-responses` / `openai-completions`. SunClaw resolves that provider's `api` owner for the embedding adapter while preserving the custom provider id for endpoint, auth, and model-prefix handling. This lets multi-GPU or multi-host setups dedicate memory embeddings to a specific local endpoint:
...
<AccordionGroup>
  <Accordion title="Gemini">
    ...
    <Warning>
    Changing model or `outputDimensionality` changes the index identity. SunClaw
  ...
  <Accordion title="Bedrock">
    ...
    Bedrock uses the AWS SDK default credential chain — no API keys needed. If SunClaw runs on EC2 with a Bedrock-enabled instance role, just set the provider and model:
  ...
  <Accordion title="Local (GGUF + node-llama-cpp)">
    ...
    ```bash
    sunclaw memory status --deep --agent main
    sunclaw memory index --force --agent main
    ```
...
When sqlite-vec is unavailable, SunClaw falls back to in-process cosine similarity automatically.
...
`searchMode: "search"` is lexical/BM25-only. SunClaw does not run semantic vector readiness probes or QMD embedding maintenance for that mode, including during `memory status --deep`; `vsearch` and `query` continue to require QMD vector readiness and embeddings.

SunClaw prefers current QMD collection and MCP query shapes, but keeps older QMD releases working by trying compatible collection pattern flags and older MCP tool names when needed. When QMD advertises support for multiple collection filters, same-source collections are searched with one QMD process; older QMD builds keep the per-collection compatibility path. Same-source means durable memory collections are grouped together, while session transcript collections remain a separate group so source diversification still has both inputs.

<Note>
QMD model overrides stay on the QMD side, not SunClaw config. If you need to override QMD's models globally, set environment variables such as `QMD_EMBED_MODEL`, `QMD_RERANK_MODEL`, and `QMD_GENERATE_MODEL` in the gateway runtime environment.
</Note>
