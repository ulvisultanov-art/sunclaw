...
This is the full reference for `sunclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.sunclaw/sunclaw.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `sunclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though SunClaw now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.sunclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.sunclaw/credentials/oauth.json` is legacy import-only.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `sunclaw pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `sunclaw configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `sunclaw health`.
    - Tip: `sunclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
