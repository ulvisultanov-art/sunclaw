...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SunClaw
...
Running SunClaw via Node with `tsx` fails at startup with:

```
[sunclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `sunclaw status` and `gateway:watch` worked.
