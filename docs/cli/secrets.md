---
summary: "CLI reference for `sunclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `sunclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
sunclaw secrets reload
sunclaw secrets reload --json
sunclaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan SunClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `sunclaw.json` refs)
...
```bash
sunclaw secrets audit
sunclaw secrets audit --check
sunclaw secrets audit --json
...
```bash
sunclaw secrets configure
sunclaw secrets configure --plan-out /tmp/sunclaw-secrets-plan.json
sunclaw secrets configure --apply --yes
sunclaw secrets configure --providers-only
sunclaw secrets configure --skip-provider-setup
sunclaw secrets configure --agent ops
sunclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `sunclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, SunClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
sunclaw secrets apply --from /tmp/sunclaw-secrets-plan.json --json
```
...
- `sunclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.sunclaw/.env` known secret keys whose values were migrated
