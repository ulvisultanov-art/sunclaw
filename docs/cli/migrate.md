---
summary: "CLI reference for `sunclaw migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into SunClaw
...
```bash
sunclaw migrate list
sunclaw migrate claude --dry-run
sunclaw migrate codex --dry-run
sunclaw migrate codex --skill gog-vault77-google-workspace
sunclaw migrate codex --plugin google-calendar --dry-run
sunclaw migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
sunclaw migrate hermes --dry-run
sunclaw migrate hermes
sunclaw migrate apply codex --yes --skill gog-vault77-google-workspace
sunclaw migrate apply codex --yes --plugin google-calendar
sunclaw migrate apply codex --yes
sunclaw migrate apply claude --yes
sunclaw migrate apply hermes --yes
sunclaw migrate apply hermes --include-secrets --yes
...
<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `sunclaw migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local SunClaw state exists.
</ParamField>
...
`sunclaw migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `sunclaw migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an SunClaw backup before applying the migration. If no local SunClaw state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the SunClaw agent workspace.
...
- Claude command Markdown files converted into SunClaw skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. SunClaw does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the SunClaw Codex harness and you want to
...
Running `sunclaw migrate codex` in an interactive terminal previews the full
...
start checked unless the target SunClaw Codex plugin config already has that
...
```bash
sunclaw migrate codex --dry-run --skill gog-vault77-google-workspace
sunclaw migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
sunclaw migrate codex --dry-run --plugin google-calendar
sunclaw migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  SunClaw agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to SunClaw provider runs,
...
- `SOUL.md` and `AGENTS.md` into the SunClaw agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for SunClaw file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that SunClaw cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live SunClaw config or credentials. This preserves opaque or unsafe state without pretending SunClaw can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `sunclaw.plugin.json`:
...
Provider plugins can use `sunclaw/plugin-sdk/migration` for item construction and summary counts, plus `sunclaw/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `sunclaw onboard --flow import` and `sunclaw setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh SunClaw setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move SunClaw to a new machine.
