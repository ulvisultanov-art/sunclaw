...
read_when:
  - Adding or modifying `sunclaw infer` commands
  - Designing stable headless capability automation
title: "Inference CLI"
---

`sunclaw infer` is the canonical headless surface for provider-backed inference workflows.
...
```text
Read https://docs.sunclaw.complex.az/cli/infer, then create a skill that routes my common workflows to `sunclaw infer`.
...
- prefer `sunclaw infer ...` in examples and suggestions
...
- `sunclaw infer audio transcribe`
- `sunclaw infer tts convert`
- `sunclaw infer web search`
- `sunclaw infer embedding create`

## Why use infer

`sunclaw infer` provides one consistent CLI for provider-backed inference tasks inside SunClaw.

Benefits:

- Use the providers and models already configured in SunClaw instead of wiring up one-off wrappers for each backend.
...
- Prefer a first-party SunClaw surface when the task is fundamentally "run inference."
- Use the normal local path without requiring the gateway for most infer commands.

For end-to-end provider checks, prefer `sunclaw infer ...` once lower-level
...
| Describe an image file or URL | `sunclaw infer image describe --file ./image.png --prompt "..." --json`                      | `--model` must be an image-capable `<provider/model>` |
| Transcribe audio              | `sunclaw infer audio transcribe --file ./memo.m4a --json`                                    | `--model` must be `<provider/model>`                  |
| Synthesize speech             | `sunclaw infer tts convert --text "..." --output ./speech.mp3 --json`                        | `tts status` is gateway-oriented                      |
| Generate a video              | `sunclaw infer video generate --prompt "..." --json`                                         | Supports provider hints such as `--resolution`        |
| Describe a video file         | `sunclaw infer video describe --file ./clip.mp4 --json`                                      | `--model` must be `<provider/model>`                  |
| Search the web                | `sunclaw infer web search --query "..." --json`                                              |                                                       |
| Fetch a web page              | `sunclaw infer web fetch --url https://example.com --json`                                   |                                                       |
| Create embeddings             | `sunclaw infer embedding create --text "..." --json`                                         |                                                       |

## Behavior

- `sunclaw infer ...` is the primary CLI surface for these workflows.
...
```bash
sunclaw infer model run --prompt "Reply with exactly: smoke-ok" --json
sunclaw infer model run --prompt "Summarize this changelog entry" --model openai/gpt-5.4 --json
sunclaw infer model run --prompt "Describe this image in one sentence" --file ./photo.jpg --model google/gemini-2.5-flash --json
sunclaw infer model run --prompt "Use more reasoning here" --thinking high --json
sunclaw infer model providers --json
sunclaw infer model inspect --name gpt-5.5 --json
```
...
```bash
sunclaw infer model run --local --model anthropic/claude-sonnet-4-6 --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model cerebras/zai-glm-4.7 --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model google/gemini-2.5-flash --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model groq/llama-3.1-8b-instant --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model mistral/mistral-medium-3-5 --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model mistral/mistral-small-latest --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model openai/gpt-5.5 --prompt "Reply with exactly: pong" --json
sunclaw infer model run --local --model ollama/qwen2.5vl:7b --prompt "Describe this image." --file ./photo.jpg --json
```
...
- Codex Responses local probes are the narrow exception: SunClaw adds a minimal system instruction so the transport can populate its required `instructions` field, without adding full agent context, tools, memory, or session transcript.
...
- `model run --file` is best when you want to test the selected multimodal text model directly. Use `infer image describe` when you want SunClaw's image-understanding provider selection and default image-model routing.
...
- Use `model run --gateway` when you need to test Gateway routing, agent-runtime setup, or Gateway-managed provider state while keeping the model input raw. Use `sunclaw agent` or chat surfaces when you want the full agent context, tools, memory, and session transcript.
...
```bash
sunclaw infer image generate --prompt "friendly lobster illustration" --json
sunclaw infer image generate --prompt "cinematic product photo of headphones" --json
sunclaw infer image generate --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "simple red circle sticker on a transparent background" --json
sunclaw infer image generate --prompt "slow image backend" --timeout-ms 180000 --json
sunclaw infer image edit --file ./logo.png --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "keep the logo, remove the background" --json
sunclaw infer image edit --file ./poster.png --prompt "make this a vertical story ad" --size 2160x3840 --aspect-ratio 9:16 --resolution 4K --json
sunclaw infer image describe --file ./photo.jpg --json
sunclaw infer image describe --file https://example.com/photo.png --json
sunclaw infer image describe --file ./receipt.jpg --prompt "Extract the merchant, date, and total" --json
sunclaw infer image describe-many --file ./before.png --file ./after.png --prompt "Compare the screenshots and list visible UI changes" --json
sunclaw infer image describe --file ./ui-screenshot.png --model openai/gpt-5.4-mini --json
sunclaw infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --prompt "Describe the image in one sentence" --timeout-ms 300000 --json
```
...
```bash
sunclaw infer audio transcribe --file ./memo.m4a --json
sunclaw infer audio transcribe --file ./team-sync.m4a --language en --prompt "Focus on names and action items" --json
sunclaw infer audio transcribe --file ./memo.m4a --model openai/whisper-1 --json
```
...
```bash
sunclaw infer tts convert --text "hello from sunclaw" --output ./hello.mp3 --json
sunclaw infer tts convert --text "Your build is complete" --output ./build-complete.mp3 --json
sunclaw infer tts providers --json
sunclaw infer tts status --json
```
...
```bash
sunclaw infer video generate --prompt "cinematic sunset over the ocean" --json
sunclaw infer video generate --prompt "slow drone shot over a forest lake" --resolution 768P --duration 6 --json
sunclaw infer video describe --file ./clip.mp4 --json
sunclaw infer video describe --file ./clip.mp4 --model openai/gpt-5.4-mini --json
```
...
```bash
sunclaw infer web search --query "SunClaw docs" --json
sunclaw infer web search --query "SunClaw infer web providers" --json
sunclaw infer web fetch --url https://docs.sunclaw.complex.az/cli/infer --json
sunclaw infer web providers --json
```
...
```bash
sunclaw infer embedding create --text "friendly lobster" --json
sunclaw infer embedding create --text "customer support ticket: delayed shipment" --model openai/text-embedding-3-large --json
sunclaw infer embedding providers --json
```
...
For generated media commands, `outputs` contains files written by SunClaw. Use
...
```bash
# Bad
sunclaw infer media image generate --prompt "friendly lobster"

# Good
sunclaw infer image generate --prompt "friendly lobster"
```

```bash
# Bad
sunclaw infer audio transcribe --file ./memo.m4a --model whisper-1 --json
...
- `sunclaw capability ...` is an alias for `sunclaw infer ...`.
