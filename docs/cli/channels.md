---
summary: "CLI reference for `sunclaw channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `sunclaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
sunclaw channels add --channel telegram --token <bot-token>
sunclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
sunclaw channels remove --channel telegram --delete
```

<Tip>
`sunclaw channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, SunClaw uses the channel's default install source without opening the interactive plugin install prompt.

When you run `sunclaw channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `sunclaw agents bindings`, `sunclaw agents bind`, and `sunclaw agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, SunClaw promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `sunclaw status --deep` for a broad probe.
- Use `sunclaw doctor` for guided fixes.
- `sunclaw channels list` no longer prints model provider usage/quota snapshots. For those, use `sunclaw status` (overview) or `sunclaw models list` (per-provider).
- `sunclaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
sunclaw channels resolve --channel slack "#general" "@jane"
sunclaw channels resolve --channel discord "My Server/#support" "@someone"
sunclaw channels resolve --channel matrix "Project Room"
```
