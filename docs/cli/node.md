---
summary: "CLI reference for `sunclaw node` (headless node host)"
read_when:
...
`sunclaw node run` and `sunclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `SUNCLAW_GATEWAY_TOKEN` / `SUNCLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `sunclaw.json` config
key.
`sunclaw node install` persists it into the supervised node service when it is
...
```bash
sunclaw node install --host <gateway-host> --port 18789
```
...
sunclaw node uninstall
```

Use `sunclaw node run` for a foreground node host (no service).
...
- `sunclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, SunClaw prepares a canonical `systemRunPlan`
