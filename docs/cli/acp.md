...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an SunClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`sunclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to SunClaw channel
conversations instead of hosting an ACP harness session, use
[`sunclaw mcp serve`](/cli/mcp) instead.
...
- SunClaw acts as an ACP server
- an IDE or ACP client connects to SunClaw
- SunClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where SunClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to SunClaw: use `sunclaw acp`
- SunClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include SunClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                 |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the SunClaw gateway or agent instead.                                                                                                                                      |
...
# Remote Gateway
sunclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
sunclaw acp --url wss://gateway-host:18789 --token-file ~/.sunclaw/gateway.token

# Attach to an existing session key
sunclaw acp --session agent:main:main

# Attach by label (must already exist)
sunclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
sunclaw acp --session agent:main:main --reset-session
```
...
sunclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.sunclaw/gateway.token

# Override the server command (default: sunclaw)
sunclaw acp client --server "node" --server-args sunclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run SunClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`sunclaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `sunclaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an SunClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `sunclaw acp` over stdio.

Example config (persisted):

```bash
sunclaw config set gateway.remote.url wss://gateway-host:18789
sunclaw config set gateway.remote.token <token>
```
...
```bash
sunclaw acp --session agent:main:main
sunclaw acp --session agent:design:main
sunclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see SunClaw plugin tools or selected
...
[SunClaw tools MCP bridge](/tools/acp-agents-setup#sunclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
SunClaw bot over ACP, use `acpx` with its built-in `sunclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx sunclaw` at `sunclaw acp`.
3. Target the SunClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default SunClaw ACP session
acpx sunclaw exec "Summarize the active SunClaw session state."

# Persistent named session for follow-up turns
acpx sunclaw sessions ensure --name codex-bridge
acpx sunclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my SunClaw work agent for recent context relevant to this repo."
```

If you want `acpx sunclaw` to target a specific Gateway and session key every
time, override the `sunclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "sunclaw": {
      "command": "env SUNCLAW_HIDE_BANNER=1 SUNCLAW_SUPPRESS_NOTES=1 sunclaw acp --url ws://127.0.0.1:18789 --token-file ~/.sunclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local SunClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env SUNCLAW_HIDE_BANNER=1 SUNCLAW_SUPPRESS_NOTES=1 node sunclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an SunClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "SunClaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`SUNCLAW_GATEWAY_TOKEN`, `SUNCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`SUNCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `SUNCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `sunclaw acp client` sets `SUNCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `sunclaw`).
