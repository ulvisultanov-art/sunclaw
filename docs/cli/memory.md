---
summary: "CLI reference for `sunclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
sunclaw memory status
sunclaw memory status --deep
sunclaw memory status --fix
sunclaw memory index --force
sunclaw memory search "meeting notes"
sunclaw memory search --query "deployment" --max-results 20
sunclaw memory promote --limit 10 --min-score 0.75
sunclaw memory promote --apply
sunclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
...
sunclaw memory status --json
sunclaw memory status --deep --index
sunclaw memory status --deep --index --verbose
sunclaw memory status --agent main
sunclaw memory index --agent main --verbose
```
...
```bash
sunclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `sunclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
sunclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
sunclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
