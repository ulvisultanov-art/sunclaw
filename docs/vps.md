---
summary: "Run SunClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for SunClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the SunClaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install SunClaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep SunClaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/sunclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
For the standard `sunclaw onboard --install-daemon` path, edit the user unit:
...
`sunclaw-gateway.service` via `sudo systemctl edit sunclaw-gateway.service`.
