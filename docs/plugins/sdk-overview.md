...
read_when:
  ...
  - You want a reference for all registration methods on SunClawPluginApi
...
<Note>
  This page is for plugin authors using `sunclaw/plugin-sdk/*` inside
  SunClaw. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [SunClaw App SDK](/concepts/sunclaw-sdk) and the `@sunclaw/sdk` package
  instead.
</Note>
...
prefer `sunclaw/plugin-sdk/channel-core`; keep `sunclaw/plugin-sdk/core` for
...
`sunclaw.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. SunClaw's
...
<Warning>
  ...
  `sunclaw/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`sunclaw/plugin-sdk/discord` and `sunclaw/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `SunClawPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main SunClaw prompt.", surfaces: ["sunclaw_main"] },
];
```

Structured `surfaces` may include `sunclaw_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `sunclaw_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `sunclaw nodes` |
...
targeted runtime, for example `["sunclaw", "codex"]`. External plugins
cannot register this middleware; keep normal SunClaw plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. SunClaw calls the
...
`sunclaw nodes canvas` explicit plugin-owned node features.
...
- User config still wins. SunClaw merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping SunClaw thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `sunclaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep SunClaw as the source of truth for due checks and execution.
...
| `api.config`             | `SunClawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `sunclaw/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when SunClaw is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through SunClaw's plugin
...
- **`@sunclaw/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@sunclaw/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `sunclaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `sunclaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
