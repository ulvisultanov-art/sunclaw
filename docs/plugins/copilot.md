---
summary: "Run SunClaw embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / sunclaw / copilot) and want it to run through the Copilot CLI
---

The external `@sunclaw/copilot` plugin lets SunClaw run embedded subscription
...
SunClaw still owns chat channels, session files, model selection, SunClaw
...
[Side questions (`/btw`)](#side-questions-btw)), and `sunclaw doctor`.
...
- SunClaw with the `@sunclaw/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@sunclaw/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `~/.sunclaw/agents/<agentId>/copilot` for full per-agent isolation. The
...
`sunclaw doctor` runs the plugin
...
The Copilot runtime is an external plugin so the core `sunclaw` package does
...
```bash
sunclaw plugins install @sunclaw/copilot
```
...
Without the opt-in, sunclaw uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@sunclaw/copilot`
   package.
2. The well-known fallback dir `~/.sunclaw/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `SUNCLAW_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the SunClaw harness without disturbing
...
(isolating SDK state from SunClaw's `models.json` / `auth-profiles.json` in
the same directory), or `~/.sunclaw/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, SunClaw uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  SunClaw's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `infiniteSessionConfig` — optional override for the SDK
  ...
  leave as-is.
- `hooksConfig` — optional bridge config exposing SunClaw
...
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged SunClaw tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through SunClaw's
...
Nothing in the rest of SunClaw needs to know about these fields. Other
...
The SunClaw side transcript mirror (see below) continues to receive the
...
deliberately leaves `harness.runSideQuestion` undefined, so SunClaw's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/ulvisultanov-art/sunclaw/blob/main/extensions/copilot/harness.test.ts)
...
that hosts (including `sunclaw doctor`) can call to verify the environment:
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the SunClaw channel/TUI prompt path; the
...
Permission enforcement for bridged SunClaw tools happens **inside the
...
`createSunClawCodingTools` to every coding tool: loop detection,
...
`createSunClawCodingTools` — identity (`senderIsOwner`,
