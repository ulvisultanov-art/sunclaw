...
read_when:
  - Building or debugging native SunClaw plugins
...
This is the **deep architecture reference** for the SunClaw plugin system. For practical guides, start with one of the focused pages below.
...
Capabilities are the public **native plugin** model inside SunClaw. Every native SunClaw plugin registers against one or more capability types:
...
SunClaw classifies every loaded plugin into a shape based on its actual registration behavior (not just static metadata):
...
Use `sunclaw plugins inspect <id>` to see a plugin's shape and capability breakdown. See [CLI reference](/cli/plugins#inspect) for details.
...
When you run `sunclaw doctor` or `sunclaw plugins inspect <id>`, you may see one of these labels:
...
Neither `hook-only` nor `before_agent_start` will break your plugin today: `hook-only` is advisory, and `before_agent_start` only triggers a warning. These signals also appear in `sunclaw status --all` and `sunclaw plugins doctor`.

## Architecture overview

SunClaw's plugin system has four layers:

<Steps>
  <Step title="Manifest + discovery">
    SunClaw finds candidate plugins from configured paths, workspace roots, global plugin roots, and bundled plugins. Discovery reads native `sunclaw.plugin.json` manifests plus supported bundle manifests first.
  </Step>
  ...
  <Step title="Runtime loading">
    Native SunClaw plugins are loaded in-process and register capabilities into a central registry. Packaged JavaScript loads through native `require`; third-party local source TypeScript is the emergency Jiti fallback. Compatible bundles are normalized into registry records without importing runtime code.
  </Step>
  <Step title="Surface consumption">
    The rest of SunClaw reads the registry to expose tools, channels, provider setup, hooks, HTTP routes, CLI commands, and services.
  </Step>
</Steps>
...
That keeps plugin-owned CLI code inside the plugin while still letting SunClaw reserve root command names before parsing.
...
That split lets SunClaw validate config, explain missing/disabled plugins, and build UI/schema hints before the full runtime is active.
...
Channel plugins do not need to register a separate send/edit/react tool for normal chat actions. SunClaw keeps one shared `message` tool in core, and channel plugins own the channel-specific discovery and execution behind it.
...
Bundled plugins follow the same rule. A bundled plugin's `runtime-api.ts` should not re-export its own branded `sunclaw/plugin-sdk/<plugin-id>` facade. Those branded facades remain compatibility shims for external plugins and older consumers, but bundled plugins should use local exports plus narrow generic SDK subpaths such as `sunclaw/plugin-sdk/channel-policy`, `sunclaw/plugin-sdk/runtime-store`, or `sunclaw/plugin-sdk/webhook-ingress`. New code should not add plugin-id-specific SDK facades unless the compatibility boundary for an existing external ecosystem requires it.
...
SunClaw treats a native plugin as the ownership boundary for a **company** or a **feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's SunClaw-facing surfaces
...
So if SunClaw adds a new domain such as video, the first question is not "which provider should hardcode video handling?" The first question is "what is the core video capability contract?" Once that contract exists, vendor plugins can register against it and channel/feature plugins can consume it.
...
A company plugin should feel cohesive from the outside. If SunClaw has shared contracts for models, speech, realtime transcription, realtime voice, media understanding, image generation, video generation, web fetch, and web search, a vendor can own all of its surfaces in one place:

```ts
import type { SunClawPluginDefinition } from "sunclaw/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "sunclaw/plugin-sdk/media-understanding";

const plugin: SunClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
SunClaw already treats image/audio/video understanding as one shared capability. The same ownership model applies there:
...
The plugin API surface is intentionally typed and centralized in `SunClawPluginApi`. That contract defines the supported registration points and the runtime helpers a plugin may rely on.
...
<AccordionGroup>
  ...
  <Accordion title="Contract tests">
    Bundled plugins are captured in contract registries during test runs so SunClaw can assert ownership explicitly. Today this is used for model providers, speech providers, web search providers, and bundled registration ownership.
  </Accordion>
</AccordionGroup>

The practical effect is that SunClaw knows, up front, which plugin owns which surface. That lets core and channels compose seamlessly because ownership is declared, typed, and testable rather than implicit.
...
<Tabs>
  ...
  <Tab title="Bad contracts">
    ...
    - ad hoc runtime objects that are not part of `SunClawPluginApi` or `api.runtime`
...
Native SunClaw plugins run **in-process** with the Gateway. They are not sandboxed. A loaded native plugin has the same process-level trust boundary as core code.

<Warning>
Native plugin implications: a plugin can register tools, network handlers, hooks, and services; a plugin bug can crash or destabilize the gateway; and a malicious native plugin is equivalent to arbitrary code execution inside the SunClaw process.
</Warning>

Compatible bundles are safer by default because SunClaw currently treats them as metadata/content packs. In current releases, that mostly means bundled skills.
...
For bundled workspace package names, keep the plugin id anchored in the npm name: `@sunclaw/<id>` by default, or an approved typed suffix such as `-provider`, `-plugin`, `-speech`, `-sandbox`, or `-media-understanding` when the package intentionally exposes a narrower plugin role.
...
SunClaw exports capabilities, not implementation convenience.
