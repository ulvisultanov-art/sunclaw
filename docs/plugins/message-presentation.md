...
Message presentation is SunClaw's shared contract for rich outbound chat UI.
...
```bash
sunclaw message send --channel slack \
  --target channel:C123 \
...
```bash
sunclaw message send --channel telegram \
...
Use helpers from `sunclaw/plugin-sdk/interactive-runtime` when bridging older
code:
