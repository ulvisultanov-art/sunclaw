...
An **agent harness** is the low level executor for one prepared SunClaw agent
...
runtime and the normal SunClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the SunClaw
  session transcript
...
Before a harness is selected, SunClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the SunClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an SunClaw-owned
policy bundle for runtime decisions that must stay shared across SunClaw and native
harnesses:
...
Harnesses may use the plan for decisions that need to match SunClaw behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `sunclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "sunclaw/plugin-sdk/agent-harness";
...
SunClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, SunClaw uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, SunClaw does not
...
`agents.list[].agentRuntime`, and `SUNCLAW_AGENT_RUNTIME`. `/status` shows the
...
SunClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: SunClaw sends the bare model id to Codex and lets the
...
SunClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
SunClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning SunClaw to the newer tested stable line.
...
seam is for async tool-result transforms that must run before SunClaw or Codex feeds
...
`sunclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so SunClaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded SunClaw
...
`sunclaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. SunClaw still owns the chat channel,
...
By default, SunClaw uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "sunclaw"`.
...
Keep that binding explicitly associated with the SunClaw session, and keep
mirroring user-visible assistant/tool output into the SunClaw transcript.

The SunClaw transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in SunClaw harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so SunClaw can
clear it when the owning SunClaw session is reset.

## Tool and media results

Core constructs the SunClaw tool list and passes it into the prepared attempt.
...
on the same delivery path as SunClaw-backed runs.
