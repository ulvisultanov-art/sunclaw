...
read_when:
  ...
  - You are defining plugin config schemas or package.json sunclaw metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`sunclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `sunclaw` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/sunclaw-my-channel",
      "version": "1.0.0",
      "type": "module",
      "sunclaw": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json sunclaw-clawhub-package.json
    {
...
`sunclaw.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`sunclaw.install` is package metadata, not manifest metadata.
...
<AccordionGroup>
  ...
  <Accordion title="Pinned npm installs">
    ...
    ```json
    {
      "sunclaw": {
        "install": {
          "npmSpec": "@wecom/wecom-sunclaw-plugin@1.2.3",
...
When enabled, SunClaw loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `sunclaw.plugin.json` in the package root. SunClaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to SunClaw",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that SunClaw loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `sunclaw/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When SunClaw uses setupEntry instead of the full entry">
...
CLI wizard locale (`SUNCLAW_LOCALE`, then system locale variables) and falls
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so SunClaw can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "sunclaw/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `sunclaw.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `sunclaw onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "sunclaw/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `sunclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `sunclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `sunclaw/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "sunclaw/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/sunclaw-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    sunclaw plugins install @myorg/sunclaw-my-plugin
    ```
  ...
  <Tab title="ClawHub only">
    ```bash
    sunclaw plugins install clawhub:@myorg/sunclaw-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    ...
    ```bash
    sunclaw plugins install npm:@myorg/sunclaw-my-plugin
    ```
...
```bash
sunclaw plugins install <package-name>
```

<Info>
For npm-sourced installs, `sunclaw plugins install` installs the package into a per-plugin project under `~/.sunclaw/npm/projects` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
...
Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged SunClaw startup never repairs or mirrors plugin dependencies.
