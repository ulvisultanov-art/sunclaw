...
read_when:
  - You want an SunClaw agent to join a Google Meet call
  - You want an SunClaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for SunClaw — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured SunClaw agent answers, and regular SunClaw TTS speaks into Meet.
...
provider plus regular SunClaw TTS. OpenAI is the default transcription
...
```bash
sunclaw googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `sunclaw googlemeet setup --json` for
...
```bash
sunclaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
sunclaw googlemeet create --transport chrome-node --mode agent
```
...
```bash
sunclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. SunClaw uses the
  ...
  the SunClaw Chrome profile on the node to already be signed in to Google.
...
SunClaw's microphone/camera permission grant and avoid the Meet **Use
...
Use `sunclaw googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in SunClaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by SunClaw. For
...
You do **not** need a full SunClaw Gateway or model API key inside a macOS VM
...
- Gateway host: SunClaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: SunClaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update SunClaw in the VM, then enable the bundled plugin there:

```bash
sunclaw plugins enable google-meet
```

Start the node host in the VM:

```bash
sunclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  sunclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  sunclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
sunclaw node restart
```

`SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`sunclaw.json` setting. `sunclaw node install` stores it in the LaunchAgent
...
```bash
sunclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, SunClaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, SunClaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `sunclaw node run` in the VM,
  approve pairing, and make sure `sunclaw plugins enable google-meet` and
  `sunclaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses SunClaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. SunClaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by SunClaw; use separate virtual devices or Loopback-style routing
...
SunClaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with SunClaw, review BlackHole's
...
Chrome transport opens the Meet URL through SunClaw browser control and joins
as the signed-in SunClaw browser profile. On macOS, the plugin checks for
...
```bash
sunclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
sunclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local SunClaw audio
...
meeting; SunClaw does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an SunClaw agent. Be brief.",
...
sunclaw plugins list | grep -E 'google-meet|voice-call'
sunclaw googlemeet setup
```
...
`SUNCLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes SunClaw requests:
...
`meetings.space.readonly` lets SunClaw resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets SunClaw pass `SpaceConfig` settings such as
...
```bash
sunclaw googlemeet auth login --json
```
...
```bash
SUNCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
SUNCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
sunclaw googlemeet auth login --json
```
...
SUNCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
SUNCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
sunclaw googlemeet auth login --json --manual
```
...
support existed, rerun `sunclaw googlemeet auth login --json` so the refresh
...
```bash
sunclaw googlemeet doctor --oauth --json
```
...
```bash
sunclaw googlemeet doctor --oauth --create-space --json
sunclaw googlemeet create --no-join --json
```
...
```bash
sunclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
sunclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `sunclaw googlemeet auth login
...
- `SUNCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `SUNCLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `SUNCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `SUNCLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `SUNCLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
sunclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
sunclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
sunclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
sunclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
sunclaw googlemeet latest --today
sunclaw googlemeet calendar-events --today --json
sunclaw googlemeet artifacts --event "Weekly sync"
sunclaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
sunclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
sunclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
sunclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
sunclaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
SunClaw accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops SunClaw's local/session
...
```bash
sunclaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
sunclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
sunclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
sunclaw googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
SUNCLAW_LIVE_TEST=1 \
SUNCLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
sunclaw googlemeet setup --transport chrome-node --mode transcribe
sunclaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `SUNCLAW_LIVE_TEST=1` enables guarded live tests.
- `SUNCLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `SUNCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `SUNCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `SUNCLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `SUNCLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the SunClaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the SunClaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, SunClaw reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured SunClaw TTS provider.
...
- `chrome.guestName: "SunClaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through SunClaw browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  SunClaw agent in a per-meeting sub-agent session, and spoken back through the
  normal SunClaw TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `sunclaw_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `sunclaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional SunClaw agent id for
  `sunclaw_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `sunclaw_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured SunClaw
agent produces the answer, and regular SunClaw TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means SunClaw did
...
transcripts are routed through the configured SunClaw agent, and the answer is
spoken through the normal SunClaw TTS runtime. Set `mode: "bidi"` when you want
...
The consult tool runs the regular SunClaw agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
SunClaw sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated SunClaw agent workspace, model defaults,
...
```bash
sunclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `sunclaw logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
sunclaw plugins list | grep google-meet
sunclaw googlemeet setup
```
...
```bash
sunclaw plugins enable google-meet
sunclaw plugins enable browser
SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  sunclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; SunClaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, SunClaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `SUNCLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `sunclaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the SunClaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. SunClaw should click **Use microphone** or, for
...
```bash
sunclaw googlemeet setup
sunclaw googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> SunClaw agent -> TTS talk-back path,
...
run `sunclaw googlemeet status --json <session-id>` after participants speak
...
utterance but SunClaw did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  SunClaw. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
sunclaw googlemeet recover-tab
sunclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
- Run `sunclaw googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `sunclaw voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `sunclaw voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `sunclaw logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `sunclaw googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `sunclaw logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: SunClaw owns the
