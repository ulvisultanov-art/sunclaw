...
If the active memory plugin exposes shared recall artifacts, SunClaw can search
...
currently exposing public bridge inputs yet. Run `sunclaw wiki doctor` first,
...
`sunclaw wiki status`, `sunclaw wiki doctor`, and `sunclaw wiki bridge
...
sunclaw wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
