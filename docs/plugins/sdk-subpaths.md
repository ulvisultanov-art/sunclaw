...
`sunclaw/plugin-sdk/`. This page catalogs the commonly used subpaths grouped by
...
SunClaw's Vitest-backed test-helper subpaths are repo-local only and are no
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `sunclaw.json` Zod schema export (`SunClawSchema`) |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled SunClaw channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@sunclaw/discord@2026.3.13` and tracked owner compatibility; new plugins should use generic channel SDK subpaths |
...
and pairing-path families.
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`, `ensureApiKeyFromOptionEnvOrPrompt`, `upsertAuthProfile`, `upsertApiKeyProfile`, `writeOAuthCredentials`, OpenAI Codex auth-import helpers, deprecated `resolveSunClawAgentDir` compatibility export |
...
SunClaw displays that summary text in status output; use `error` only when the
usage endpoint failed or returned no usable usage data.
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-contracts` | Focused type-only config surface for plugin config shapes such as `SunClawConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, SunClaw tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
    ...
    | `plugin-sdk/agent-runtime` | Agent dir/identity/workspace helpers, including `resolveAgentDir`, `resolveDefaultAgentDir`, and deprecated `resolveSunClawAgentDir` compatibility export |
  ...
  <Accordion title="Capability and testing subpaths">
    | Subpath | Key exports |
    | --- | --- |
    | `plugin-sdk/media-runtime` | Shared media fetch/transform/store helpers including `saveRemoteMedia`, `saveResponseMedia`, `readRemoteMediaBuffer`, and deprecated `fetchRemoteMedia`; prefer store helpers before buffer reads when a URL should become SunClaw media |
    ...
    | `plugin-sdk/testing` | Repo-local deprecated compatibility barrel for legacy SunClaw tests. New repo tests should import focused local test subpaths such as `plugin-sdk/agent-runtime-test-contracts`, `plugin-sdk/plugin-test-runtime`, `plugin-sdk/channel-test-helpers`, `plugin-sdk/test-env`, or `plugin-sdk/test-fixtures` instead |
  ...
  <Accordion title="Reserved bundled-helper subpaths">
    ...
    | `plugin-sdk/codex-native-task-runtime` | Bundled Codex plugin helper for mirroring Codex app-server native subagents into SunClaw task state |
