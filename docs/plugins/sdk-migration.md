...
read_when:
  - You see the SUNCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the SUNCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before SunClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external SunClaw plugin
---

SunClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`sunclaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`sunclaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`sunclaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`sunclaw/extension-api`** - a bridge that gave plugins direct access to
...
SunClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`sunclaw/plugin-sdk/\<subpath\>`)
...
`sunclaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `sunclaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `sunclaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `SunClawConfig` | `sunclaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `sunclaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `sunclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `sunclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `sunclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `sunclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `sunclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `sunclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `sunclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // SunClaw and Codex runtime dynamic tools
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `sunclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `sunclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "sunclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "sunclaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "sunclaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "sunclaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "sunclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "sunclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `sunclaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `sunclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `sunclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `sunclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `sunclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `sunclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `sunclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `sunclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `sunclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `sunclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `sunclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `sunclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `sunclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `sunclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `sunclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `sunclaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `sunclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `sunclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `sunclaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `sunclaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `SunClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | SunClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@sunclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`sunclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`sunclaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "sunclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "sunclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `sunclaw/plugin-sdk/channel-inbound` or
    `sunclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `sunclaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `sunclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `sunclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `sunclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    SunClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. SunClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="SunClawSchemaType alias → SunClawConfig">
    `SunClawSchemaType` re-exported from `sunclaw/plugin-sdk` is now a
    one-line alias for `SunClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { SunClawSchemaType } from "sunclaw/plugin-sdk";
    // After
    import type { SunClawConfig } from "sunclaw/plugin-sdk/config-schema";
    ```
...
```bash
SUNCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 sunclaw gateway run
SUNCLAW_SUPPRESS_EXTENSION_API_WARNING=1 sunclaw gateway run
```
