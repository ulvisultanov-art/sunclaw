---
summary: "How SunClaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged SunClaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

SunClaw keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the SunClaw
package directory.
...
- SDK/core imports are peer or supplied SunClaw imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into SunClaw-owned package roots

SunClaw owns only the plugin lifecycle:
...
SunClaw uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.sunclaw/npm/projects/<encoded-package>`
- git packages clone under `~/.sunclaw/git`
...
```bash
cd ~/.sunclaw/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`sunclaw plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. SunClaw reads the tarball's npm
...
`node_modules` beside the plugin package. SunClaw scans the managed project
...
publishable lockfile during install, and SunClaw's managed npm project root
supports it through the normal npm install path. SunClaw-owned publishable
...
SunClaw does not require it for community packages, but npm will respect it
when present.

SunClaw-owned npm plugin packages can also publish with explicit
...
with `sunclaw.release.bundleRuntimeDependencies: false`; those packages still
...
`sunclaw` package does not bundle its full dependency tree.

Plugins that import `sunclaw/plugin-sdk/*` declare `sunclaw` as a peer
dependency. SunClaw does not let npm install a separate registry copy of the
...
resolution/materialization and SunClaw reasserts plugin-local
`node_modules/sunclaw` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. SunClaw does not
...
sunclaw plugins install <source>
sunclaw doctor --fix
```

`doctor --fix` can clean legacy SunClaw-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of SunClaw.
...
In source checkouts, SunClaw treats the repository as a pnpm monorepo. After
...
Older SunClaw versions generated bundled-plugin dependency roots at startup or
...
`.sunclaw-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.sunclaw/npm/node_modules` root.
