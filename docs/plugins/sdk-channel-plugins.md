---
summary: "Step-by-step guide to building a messaging channel plugin for SunClaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect SunClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects SunClaw to a
...
<Info>
  If you have not built any SunClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. SunClaw keeps one
...
`defineChannelMessageAdapter` from `sunclaw/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `sunclaw/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`sunclaw/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `sunclaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `sunclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `sunclaw/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `sunclaw/plugin-sdk/approval-auth-runtime`
- `sunclaw/plugin-sdk/approval-client-runtime`
- `sunclaw/plugin-sdk/approval-delivery-runtime`
- `sunclaw/plugin-sdk/approval-gateway-runtime`
- `sunclaw/plugin-sdk/approval-handler-adapter-runtime`
- `sunclaw/plugin-sdk/approval-handler-runtime`
- `sunclaw/plugin-sdk/approval-native-runtime`
- `sunclaw/plugin-sdk/approval-reply-runtime`
- `sunclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `sunclaw/plugin-sdk/setup-runtime`,
`sunclaw/plugin-sdk/setup-runtime`,
`sunclaw/plugin-sdk/reply-runtime`,
`sunclaw/plugin-sdk/reply-dispatch-runtime`,
`sunclaw/plugin-sdk/reply-reference`, and
`sunclaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `sunclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `sunclaw/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `sunclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `sunclaw.setupEntry` in
...
- use the broader `sunclaw/plugin-sdk/setup` seam only when you also need the
...
- `sunclaw/plugin-sdk/account-core`,
  `sunclaw/plugin-sdk/account-id`,
  `sunclaw/plugin-sdk/account-resolution`, and
  `sunclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `sunclaw/plugin-sdk/inbound-envelope` and
  `sunclaw/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `sunclaw/plugin-sdk/channel-targets` for target parsing helpers
- `sunclaw/plugin-sdk/outbound-media` for media loading and
  `sunclaw/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `sunclaw/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `sunclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `sunclaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `sunclaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `sunclaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `sunclaw/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`sunclaw/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#sunclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/sunclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "sunclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect SunClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { SunClawConfig } from "sunclaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "sunclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so SunClaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "sunclaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    SunClaw loads this instead of the full entry when the channel is disabled
    ...
    `sunclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    SunClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to SunClaw.
...
├── package.json              # sunclaw.channel metadata
├── sunclaw.plugin.json      # Manifest with config schema
