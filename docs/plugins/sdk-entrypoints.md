...
when SunClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, SunClaw will use a
...
**Import:** `sunclaw/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the SunClaw tool-result format, and
exposes static metadata that `sunclaw plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, SunClaw uses a strict empty object
...
- Tool names are static. `sunclaw plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `sunclaw.plugin.json` and `package.json` `sunclaw.extensions`; SunClaw does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `sunclaw/plugin-sdk/plugin-entry`
...
- SunClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `sunclaw/plugin-sdk/channel-core`
...
- Discovery registration is non-activating, not import-free. SunClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and SunClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `sunclaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; SunClaw resolves it to the
...
SunClaw loads this instead of the full entry when a channel is disabled,
...
- `sunclaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `sunclaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `sunclaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`sunclaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "sunclaw/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so SunClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want SunClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; SunClaw rejects descriptor names outside
...
SunClaw classifies loaded plugins by their registration behavior:
...
Use `sunclaw plugins inspect <id>` to see a plugin's shape.
