...
summary: "Specification for making the bundled Codex app-server harness honor SunClaw context-engine plugins"
read_when:
  ...
  - You are comparing embedded SunClaw and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same SunClaw context-engine
lifecycle contract that embedded SunClaw turns already honor.
...
SunClaw-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in SunClaw attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in SunClaw-controlled code. The external
boundary is the Codex app-server protocol itself: SunClaw can control what it
...
Built-in SunClaw attempts call the context-engine lifecycle directly:
...
For Codex harness turns, SunClaw should preserve this lifecycle:

1. Read the mirrored SunClaw session transcript.
...
8. Mirror the Codex result back into the SunClaw transcript.
...
11. Preserve Codex native compaction signals and SunClaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. SunClaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

SunClaw's transcript mirror remains the source for SunClaw features:
...
The context-engine interface returns SunClaw `AgentMessage[]`, not a Codex
...
- Label it clearly as SunClaw assembled context.
...
inside SunClaw and preserves context-engine semantics.
...
we need strict SunClaw parity, run context-engine assembly before hook
...
1. SunClaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit SunClaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
SunClaw compaction should prefer the context engine's `compact()` result for
the SunClaw transcript mirror and plugin state.
...
the SunClaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
SunClaw session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in SunClaw semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole SunClaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in SunClaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for SunClaw/plugin state.
