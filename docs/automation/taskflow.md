...
read_when:
  ...
  - You encounter Task Flow or sunclaw tasks flow in release notes or docs
...
- Browser availability and profile choice, for example `sunclaw` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
...
`sunclaw tasks flow cancel` sets a sticky cancel intent on the flow. Active tasks within the flow are cancelled, and no new steps are started. The cancel intent persists across restarts, so a cancelled flow stays cancelled even if the gateway restarts before all child tasks have terminated.
...
Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `sunclaw tasks` to inspect individual task records and `sunclaw tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) — the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) — CLI command reference for `sunclaw tasks flow`
