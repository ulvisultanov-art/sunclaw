---
summary: "Deploy SunClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying SunClaw to Render
...
Deploy SunClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/ulvisultanov-art/sunclaw)
...
repository configures everything needed to run SunClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: SUNCLAW_GATEWAY_PORT
        value: "8080"
      - key: SUNCLAW_STATE_DIR
        value: /data/.sunclaw
      - key: SUNCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: SUNCLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means SunClaw state
...
`SUNCLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original SunClaw repository, Render will not auto-deploy your SunClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For SunClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `SUNCLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `SUNCLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `sunclaw backup create` in the Render shell.
