---
summary: "Host SunClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up SunClaw on Oracle Cloud
  - Looking for free VPS hosting for SunClaw
  - Want 24/7 SunClaw on a small server
title: "Oracle Cloud"
---

Run a persistent SunClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname sunclaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=sunclaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@sunclaw`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    sunclaw config set gateway.bind loopback
    ...
    sunclaw config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://sunclaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.sunclaw` to restrict credential file permissions.
- `sunclaw security audit` for an SunClaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most SunClaw features work fine; a small number of native binaries need ARM builds:
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@sunclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=sunclaw --reset` to re-authenticate.
