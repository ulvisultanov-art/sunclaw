---
summary: "Updating SunClaw safely (global install or source), plus rollback strategy"
read_when:
  - Updating SunClaw
...
## Recommended: `sunclaw update`

The fastest way to update. It detects your install type (npm or git), fetches the latest version, runs `sunclaw doctor`, and restarts the gateway.
...
sunclaw update --dry-run   # preview without applying
```

`sunclaw update` does not accept `--verbose`. For update diagnostics, use
`--dry-run` to preview the planned actions, `--json` for structured results, or
`sunclaw update status --json` to inspect channel and availability state. The
...
updates, `--tag main` maps to `github:sunclaw/sunclaw#main` for one run, and
...
state, config, credentials, and workspace in `~/.sunclaw`; it only changes
which SunClaw code install the CLI and gateway use.
...
```bash
sunclaw update --channel dev --dry-run
sunclaw update --channel stable --dry-run
```
...
gateway is already installed, `sunclaw update` refreshes the service metadata
and restarts it unless you pass `--no-restart`.

For package installs with a managed Gateway service, `sunclaw update` targets
the package root used by that service. If the shell `sunclaw` command comes
...
If `sunclaw update` fails after the npm package install phase, re-run the
...
```bash
curl -fsSL https://docs.sunclaw.complex.az/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
```

## Alternative: manual npm, pnpm, or bun

```bash
npm i -g sunclaw@latest
```

Prefer `sunclaw update` for supervised installs because it can coordinate the
...
For a root-owned Linux system-global install, if `sunclaw update` fails with
`EACCES` and you recover with system npm, keep the Gateway stopped through the
manual package replacement. Use the same `sunclaw` profile flags or environment
...
sudo /usr/bin/npm i -g sunclaw@latest
...
sunclaw gateway status --deep --json
sunclaw doctor --lint --json
```

When `sunclaw update` manages a global npm install, it installs the target into
...
SunClaw retries once with `--omit=optional`. That retry helps hosts where native
optional dependencies cannot compile, while keeping the original failure visible
if the fallback also fails.

SunClaw-managed npm update and plugin-update commands also clear npm
...
quarantine policies, but an explicit SunClaw update means "install the selected
SunClaw release now."
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    SunClaw treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in SunClaw-owned npm/git roots under the user config directory, and Gateway startup does not mutate the SunClaw package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/sunclaw`. SunClaw supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give SunClaw write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:

    ```ini
    ReadWritePaths=/var/lib/sunclaw /home/sunclaw/.sunclaw /tmp
    ```

  </Accordion>
  <Accordion title="Disk-space preflight">
    Before package updates and explicit plugin installs, SunClaw tries a best-effort disk-space check for the target volume. Low space produces a warning with the checked path, but does not block the update because filesystem quotas, snapshots, and network volumes can change after the check. The actual package-manager install and post-install verification remain authoritative.
  </Accordion>
</AccordionGroup>

## Auto-updater

The auto-updater is off by default. Enable it in `~/.sunclaw/sunclaw.json`:
...
normal `sunclaw update --yes --json` CLI path stop the service, replace the
...
```bash
npm i -g sunclaw@<version>
sunclaw doctor
sunclaw gateway restart
```

<Tip>
`npm view sunclaw version` shows the current published version.
</Tip>
...
- Run `sunclaw doctor` again and read the output carefully.
- For `sunclaw update --channel dev` on source checkouts, the updater auto-bootstraps `pnpm` when needed. If you see a pnpm/corepack bootstrap error, install `pnpm` manually (or re-enable `corepack`) and rerun the update.
