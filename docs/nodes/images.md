...
- Send media with optional captions via `sunclaw message send --media`.
- Allow auto-replies from the web inbox to include media alongside text.
- Keep per-type limits sane and predictable.

## CLI Surface

- `sunclaw message send --media <path-or-url> [--message <caption>]`
...
- When media is present, the web sender resolves local paths or URLs using the same pipeline as `sunclaw message send`.
- Multiple media entries are sent sequentially if provided.

## Inbound Media To Commands

- When inbound web messages include media, SunClaw downloads to a temp file and exposes templating variables:
...
- Media understanding (if configured via `tools.media.*` or shared `tools.media.models`) runs before templating and can insert `[Image]`, `[Audio]`, and `[Video]` blocks into `Body`.
  ...
  - If the active primary image model already supports vision natively, SunClaw skips the `[Image]` summary block and passes the original image to the model instead.
