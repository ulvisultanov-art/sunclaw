...
SunClaw supports **camera capture** for agent workflows:
...
```bash
sunclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
sunclaw nodes camera snap --node <id> --facing front
sunclaw nodes camera clip --node <id> --duration 3000
sunclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`sunclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `sunclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
sunclaw nodes camera list --node <id>            # list camera ids
sunclaw nodes camera snap --node <id>            # prints saved path
sunclaw nodes camera snap --node <id> --max-width 1280
sunclaw nodes camera snap --node <id> --delay-ms 2000
sunclaw nodes camera snap --node <id> --device-id <id>
sunclaw nodes camera clip --node <id> --duration 10s          # prints saved path
sunclaw nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
sunclaw nodes camera clip --node <id> --device-id <id>
sunclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `sunclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
sunclaw nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
