    ...
    if (showDiagnostics) {
      Surface(
        ...
        Column(
          ...
          if (pairingRequired) {
            Text(
              "Approve this phone on the gateway. SunClaw retries automatically while this screen stays open.",
              style = mobileCallout,
              color = mobileTextSecondary,
            )
            CommandBlock("sunclaw devices list")
            CommandBlock("sunclaw devices approve <requestId>")
          }
          Text("SunClaw Android ${sunClawAndroidVersionLabel()}", style = mobileCaption1, color = mobileTextSecondary)
          Button(
            onClick = {
    ...
    AnimatedVisibility(visible = advancedOpen) {
      Surface(
        ...
        Column(
          ...
          CommandBlock("sunclaw qr --setup-code-only")
          CommandBlock("sunclaw qr --json")
          Text(
