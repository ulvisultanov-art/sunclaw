...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    // Command strings come from SunClawProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      SunClawCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      SunClawCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      SunClawCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      SunClawCanvasA2UICommand.Push.rawValue, SunClawCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      SunClawCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      SunClawCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      SunClawCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      SunClawLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      SunClawDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      SunClawDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      SunClawDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      SunClawDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      SunClawDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      SunClawNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      SunClawNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      SunClawSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      SunClawTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      SunClawTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      SunClawTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      SunClawTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.sunclaw.app.protocol.SunClawPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      SunClawContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      SunClawContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      SunClawCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      SunClawCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      SunClawMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      SunClawMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      SunClawSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      SunClawSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      SunClawCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
