...
internal fun sunClawAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this SunClaw Android gateway connection failure.
    ...
    - tell me whether `sunclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `sunclaw qr --json`, `sunclaw devices list`, and `sunclaw nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${sunClawAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("SunClaw gateway diagnostics", report))
