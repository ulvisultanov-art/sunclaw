...
class SecurePrefs(
  ...
  companion object {
    val defaultWakeWords: List<String> = listOf("sunclaw", "claude")
    private const val displayNameKey = "node.displayName"
    private const val locationModeKey = "location.enabledMode"
    private const val voiceWakeModeKey = "voiceWake.mode"
    private const val plainPrefsName = "sunclaw.node"
    private const val securePrefsName = "sunclaw.node.secure"
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block SunClaw's own notifications in blocklist mode to prevent forwarding loops.
