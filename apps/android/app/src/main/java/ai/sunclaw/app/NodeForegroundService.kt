...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "SunClaw Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "SunClaw Node · Talk"
              state.connected -> "SunClaw Node · Connected"
              else -> "SunClaw Node"
            }
          val text =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "SunClaw node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.sunclaw.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.sunclaw.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.sunclaw.app.extra.VOICE_CAPTURE_MODE"
