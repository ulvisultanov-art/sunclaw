...
import ai.sunclaw.app.SensitiveFeatureConfig
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep SunClaw secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with SunClaw. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    ...
    message = "Approval is in progress.\nSunClaw will reconnect automatically.",
    canAutoRetry = true,
  ),
  Finishing(
    title = "Finishing Setup",
    message = "Gateway approved this phone.\nSunClaw is bringing the node online.",
    canAutoRetry = true,
  ),
  Failed(
...
private fun recoveryGatewayDetail(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      "Gateway approval is in progress. SunClaw will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOf(
      "SunClaw Android gateway diagnostic",
      "Status: $statusText",
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("SunClaw gateway diagnostic", diagnostic))
