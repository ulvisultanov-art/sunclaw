...
val androidStoreFile = providers.gradleProperty("SUNCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("SUNCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("SUNCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("SUNCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set SUNCLAW_ANDROID_STORE_FILE, " +
      "SUNCLAW_ANDROID_STORE_PASSWORD, SUNCLAW_ANDROID_KEY_ALIAS, and " +
      "SUNCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/SunClawKit/Sources/SunClawKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.sunclaw.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "sunclaw-$versionName-$buildType.apk"
          } else {
            "sunclaw-$versionName-$flavorName-$buildType.apk"
          }
