...
private let webChatSwiftLogger = Logger(subsystem: "ai.sunclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "sunclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: SunClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> SunClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(SunClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            SunClawChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return SunClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SunClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SunClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    SunClawSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: SunClawProtocol.ModelChoice) -> SunClawChatModelChoice {
        SunClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SunClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SunClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SunClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: SunClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SunClawChatView>) -> NSViewController
    {
