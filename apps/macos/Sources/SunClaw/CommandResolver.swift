...
enum CommandResolver {
    private static let projectRootDefaultsKey = "sunclaw.gatewayProjectRootPath"
    private static let helperName = "sunclaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let sunclawEntry = root.appendingPathComponent("sunclaw.mjs").path
        if FileManager().isReadableFile(atPath: sunclawEntry) { return sunclawEntry }
        let binEntry = root.appendingPathComponent("bin/sunclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/sunclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let sunclawPaths = self.sunclawManagedPaths(home: home)
        if !sunclawPaths.isEmpty {
            extras.insert(contentsOf: sunclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + sunclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func sunclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".sunclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func sunclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSunClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("sunclaw.mjs").path,
            root.appendingPathComponent("bin/sunclaw.js").path,
        ]
    ...
    static func hasAnySunClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.sunclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func sunclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let sunclawPath = self.projectSunClawExecutable(projectRoot: root) {
            return [sunclawPath, subcommand] + extraArgs
        }
        if let sunclawPath = self.sunclawExecutable(searchPaths: searchPaths) {
            return [sunclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "sunclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            sunclaw entrypoint missing (looked for dist/index.js or sunclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real sunclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/sunclaw"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/sunclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/sunclaw.mjs"
            node "$PRJ/sunclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/sunclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/sunclaw.js"
            node "$PRJ/bin/sunclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent sunclaw"
          pnpm --silent sunclaw \(quotedArgs);
        else
          echo "sunclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SunClawConfigFile.loadDict()
