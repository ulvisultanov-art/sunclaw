...
enum DebugActions {
    private static let verboseDefaultsKey = "sunclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "sunclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SunClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SunClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SunClawPaths.configURL
        guard
