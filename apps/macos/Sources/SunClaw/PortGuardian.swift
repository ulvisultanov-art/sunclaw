...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("SunClaw", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "sunclaw", "tsx", "pnpm", "bun"]
    ...
    private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("sunclaw-gateway")
                || cmd.contains("sunclaw-gateway")
            {
                return true
            }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("sunclaw"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
