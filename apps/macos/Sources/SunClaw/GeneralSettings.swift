...
struct GeneralSettings: View {
    ...
    private var generalPage: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsPageHeader(
                title: "General",
                subtitle: "Everyday SunClaw app behavior.")

            self.sunClawStatusPanel

            SettingsCardGroup("App") {
                SettingsCardToggleRow(
                    title: "Launch at login",
                    subtitle: "Automatically start SunClaw after you sign in.",
                    binding: self.$state.launchAtLogin)

                SettingsCardToggleRow(
                    title: "Show Dock icon",
                    subtitle: "Keep SunClaw visible in the Dock. When off, windows still show the Dock icon while open.",
    ...
    private var sunClawStatusPanel: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(self.state.isPaused ? "SunClaw paused" : "SunClaw active")
                    .font(.headline)
            ...
            Toggle("SunClaw active", isOn: self.activeBinding)
                .labelsHidden()
                .toggleStyle(.switch)
        }
    ...
    private var connectionStatusSubtitle: String {
        switch self.state.connectionMode {
        case .local:
            return "SunClaw starts and monitors the Gateway on this Mac."
        case .remote:
        ...
        SettingsCardGroup("Advanced") {
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    self.advancedTextField(
                        "Project root",
                        placeholder: "/home/you/Projects/sunclaw",
                        text: self.$state.remoteProjectRoot)
                    self.advancedTextField(
                        "CLI path",
                        placeholder: "/Applications/SunClaw.app/.../sunclaw",
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for sunclaw logs in /tmp/sunclaw/.
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/sunclaw"
        state.remoteCliPath = "/tmp/sunclaw"
