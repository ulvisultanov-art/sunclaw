...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SunClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SunClawKit.AnyCodable]()) {
                $0[$1.key] = SunClawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking SunClaw Local Network access.
                    Allow SunClaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SunClawProtocol.AnyCodable?) -> [String: SunClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SunClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SunClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SunClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("sunclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("sunclaw.control.agent")
}
