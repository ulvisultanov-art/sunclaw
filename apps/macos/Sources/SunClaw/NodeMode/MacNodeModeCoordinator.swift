...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = SunClawConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SunClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: SunClawLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        var caps: [String] = [SunClawCapability.canvas.rawValue, SunClawCapability.screen.rawValue]
        if browserControlEnabled, connectionMode == .local {
            caps.append(SunClawCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(SunClawCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(SunClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: SunClawConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: SunClawLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SunClawSystemCommand.which.rawValue,
            SunClawSystemCommand.run.rawValue,
            SunClawSystemCommand.execApprovalsGet.rawValue,
            SunClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SunClawCapability.browser.rawValue) {
            commands.append(SunClawBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(SunClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(SunClawCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: SunClawConfigFile.loadDict(),
