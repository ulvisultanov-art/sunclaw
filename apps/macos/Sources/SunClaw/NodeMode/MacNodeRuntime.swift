...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case SunClawBrowserCommand.proxy.rawValue:
            ...
            case SunClawLocationCommand.get.rawValue:
            ...
            case SunClawSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case SunClawSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case SunClawSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case SunClawSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case SunClawSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SunClawCanvasA2UICommand.push.rawValue,
             SunClawCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard SunClawConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SunClawNodeError(
    ...
    private func handleScreenSnapshotInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let res: (data: Data, format: SunClawScreenSnapshotFormat, width: Int, height: Int)
        do {
    ...
    private static func resolveA2UIHostUrl(from raw: String?) -> String? {
        ...
        return baseUrl.appendingPathComponent("__sunclaw__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SunClawSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SunClawSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> SunClawLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: SunClawNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: SunClawCanvasSnapshotFormat,
        maxWidth: Int?,
