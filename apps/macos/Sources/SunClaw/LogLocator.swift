...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SUNCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/sunclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("sunclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("sunclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/sunclaw/ (rolling or stdout), or nil if none exist.
