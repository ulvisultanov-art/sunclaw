...
final class AppState {
    private static let logger = Logger(subsystem: "ai.sunclaw", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = SunClawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            SunClawConfigFile.canonicalHostForComparison(host) ==
                SunClawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = SunClawConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: SunClawConfigFile.loadDict(),
            draft: .init(
        ...
        guard SunClawConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/sunclaw"
        state.remoteCliPath = ""
        return state
    }
}
