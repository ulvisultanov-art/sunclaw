...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "sunclaw-bundled":
            "Bundled"
        case "sunclaw-managed":
            "Managed"
        case "sunclaw-workspace":
            "Workspace"
        case "sunclaw-extra":
            "Extra"
        case "sunclaw-plugin":
...
private struct EnvEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Saved to sunclaw.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in sunclaw.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in sunclaw.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
