...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.sunclaw.mac`) even if the shipped
    ...
    {
        if environment["SUNCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "sunclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "sunclaw.nixMode") { return true }
