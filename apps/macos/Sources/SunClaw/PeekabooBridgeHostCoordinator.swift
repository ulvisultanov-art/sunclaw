...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SunClawPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["clawdbot", "clawdis", "moltbot"]

    private static var sunclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "SunClaw", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = SunClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.sunclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.sunclawSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.sunclawSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.sunclawSocketPath)
        } catch {
...
@MainActor
private final class SunClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.sunclaw.peekaboo")
