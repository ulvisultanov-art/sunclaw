...
enum SunClawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = SunClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SunClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SunClawLogging.bootstrapIfNeeded()
        let label = SunClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SunClawOSLogHandler: AppLogLevelBackedHandler {
...
struct SunClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SunClawLogging.parseLabel(self.label)
