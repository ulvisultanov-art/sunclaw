...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in SunClawBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: SunClawBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.sunclaw.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = SunClawBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = SunClawBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = SunClawBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (SunClaw)", with: "")
