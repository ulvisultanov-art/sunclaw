...
struct SunClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("sunclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      sunclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      sunclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      sunclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      sunclaw-mac connect
      sunclaw-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      sunclaw-mac connect --url ws://127.0.0.1:18789 --json
      sunclaw-mac discover --timeout 3000 --json
      sunclaw-mac wizard --mode local
    """)
}
