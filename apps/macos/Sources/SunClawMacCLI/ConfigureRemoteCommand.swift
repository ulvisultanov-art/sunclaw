...
private let appDefaultsSuites = ["ai.sunclaw.mac", "ai.sunclaw.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              sunclaw-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              sunclaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote SunClaw checkout for CLI commands.
              --cli-path <path>   Remote sunclaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func configureSSHRemote(_ opts: ConfigureRemoteOptions) throws -> ConfigureRemoteOutput {
    ...
    let configURL = sunClawConfigURL()
...
private func sunClawConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["SUNCLAW_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".sunclaw/sunclaw.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "sunclaw.connectionMode")
        setDefaultString(defaults, key: "sunclaw.remoteTarget", value: target)
        defaults.set(true, forKey: "sunclaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "sunclaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "sunclaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "sunclaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "sunclaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("SunClaw macOS Remote Config")
