...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SunClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SunClawProtocol.AnyCodable] = [
            "presence": .init([SunClawProtocol.AnyCodable(entry)]),
        ]
        let payload = SunClawProtocol.AnyCodable(payloadMap)
