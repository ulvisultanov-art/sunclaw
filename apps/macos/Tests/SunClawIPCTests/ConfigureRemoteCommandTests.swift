...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("sunclaw-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.sunclaw.mac", "ai.sunclaw.mac.debug"]
        let keys = [
            "sunclaw.connectionMode",
            "sunclaw.remoteTarget",
            "sunclaw.onboardingSeen",
            "sunclaw.onboardingVersion",
            "sunclaw.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["SUNCLAW_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "sunclaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "sunclaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "sunclaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "sunclaw.remoteCliPath") == "/opt/homebrew/bin/sunclaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("sunclaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("sunclaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("sunclaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["SUNCLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
