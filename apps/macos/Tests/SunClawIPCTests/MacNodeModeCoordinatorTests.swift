...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(SunClawCapability.browser.rawValue))
        #expect(!commands.contains(SunClawBrowserCommand.proxy.rawValue))
        #expect(commands.contains(SunClawCanvasCommand.present.rawValue))
