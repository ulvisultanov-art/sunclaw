...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `sunclaw --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("SunClaw 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  SunClaw 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("SunClaw 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("SunClaw 2026.4.2-1 (d74a122)")
    ...
    @Test func `gateway port defaults and respects override`() async {
        ...
        await TestIsolation.withIsolatedState(
            env: ["SUNCLAW_CONFIG_PATH": configPath],
