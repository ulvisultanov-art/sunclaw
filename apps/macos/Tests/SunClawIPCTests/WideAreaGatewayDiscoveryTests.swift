...
import Foundation
import Testing
@testable import SunClawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("SUNCLAW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("SUNCLAW_WIDE_AREA_DOMAIN", "sunclaw.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("SUNCLAW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("SUNCLAW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._sunclaw-gw._tcp.sunclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.sunclaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (SunClaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/sunclaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (SunClaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/sunclaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._sunclaw-gw._tcp.sunclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
