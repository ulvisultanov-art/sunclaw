...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["SUNCLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "sunclaw-gateway",
                executablePath: "/tmp/sunclaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 sunclaw-gateway @ /tmp/sunclaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
