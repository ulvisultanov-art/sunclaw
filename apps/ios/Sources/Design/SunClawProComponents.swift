...
struct SunClawProBackground: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        LinearGradient(
            colors: SunClawBrand.canvasColors(for: self.colorScheme),
            startPoint: .top,
            endPoint: .bottom)
            .ignoresSafeArea()
            .overlay(alignment: .top) {
                if self.colorScheme == .light {
                    LinearGradient(
                        colors: [
                            SunClawBrand.accent.opacity(0.05),
                            .clear,
                        ],
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = SunClawProMetric.cardRadius
    ...
    private var borderStyle: AnyShapeStyle {
        ...
        let gradient = LinearGradient(
            colors: [
                ...
                (self.tint ?? SunClawBrand.accent).opacity(0.10),
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = SunClawProMetric.cardRadius,
...
struct SunClawProMark: View {
    ...
    var body: some View {
        Image("SunClawIcon")
            ...
            .shadow(color: SunClawBrand.accent.opacity(0.28), radius: self.shadowRadius, y: self.shadowRadius / 2)
            .accessibilityLabel("SunClaw")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = SunClawBrand.accentHot
        ...
        Button(action: self.action) {
            HStack(spacing: 12) {
                Image(systemName: self.systemImage)
                    ...
                    .background(SunClawBrand.accentHot, in: RoundedRectangle(cornerRadius: 13, style: .continuous))
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundStyle(SunClawBrand.accentHot)
            }
            .padding(12)
            .proGlassSurface(
                fill: self.colorScheme == .dark ? Color.white.opacity(0.045) : Color.white.opacity(0.68),
                stroke: SunClawBrand.accent.opacity(self.colorScheme == .dark ? 0.22 : 0.14),
                radius: 18,
                isProminent: true,
                interactive: true)
        }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ProIconBadge(
                ...
                color: self.done ? SunClawBrand.ok : SunClawBrand.warn)
            VStack(alignment: .leading, spacing: 3) {
                Text(self.title)
