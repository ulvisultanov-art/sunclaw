...
extension SettingsProTab {
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(self.gatewayConnected ? "Connected" : self.appModel.gatewayDisplayStatusText)
                    .font(.caption)
                    .foregroundStyle(self.gatewayConnected ? SunClawBrand.ok : .secondary)
            }
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            ...
            self.settingsListRow(
                icon: "info.circle",
                title: "About",
                detail: DeviceInfoHelper.sunClawVersionString(),
                route: .about)
        }
        .padding(.horizontal, SunClawProMetric.pagePadding)
    }
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while SunClaw is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while SunClaw is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? SunClawBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayConnected ? SunClawBrand.ok : SunClawBrand.warn)
            ...
            self.detailListCard {
                ...
                Divider()
                self.detailRow("App", value: DeviceInfoHelper.sunClawVersionString())
                Divider()
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context SunClaw can expose to the gateway.",
    ...
    var notificationsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "bell",
                title: "Notifications",
                detail: "Approvals and event alerts from SunClaw.",
                value: self.notificationStatusText,
                color: self.notificationStatusText == "Allowed" ? SunClawBrand.ok : .secondary)
            ...
            ProCard(radius: SettingsLayout.cardRadius) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("SunClaw uses notifications for approval prompts and mirrored event alerts.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    var aboutDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                value: DeviceInfoHelper.sunClawVersionString(),
                color: SunClawBrand.accent)

            self.detailListCard {
                self.detailRow("Version", value: DeviceInfoHelper.sunClawVersionString())
                Divider()
                self.detailRow("Device", value: DeviceInfoHelper.deviceFamily())
                Divider()
                self.detailRow("Platform", value: DeviceInfoHelper.platformStringForDisplay())
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? SunClawBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == SunClawLocationMode.off.rawValue ? .secondary : SunClawBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(SunClawLocationMode.off.rawValue)
                    Text("While Using").tag(SunClawLocationMode.whileUsing.rawValue)
                    Text("Always").tag(SunClawLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
