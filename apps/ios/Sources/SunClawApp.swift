...
@MainActor
enum SunClawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class SunClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.sunclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.sunclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.sunclaw.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? SunClawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "sunclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "sunclaw.watch.promptId"
    static let sessionKeyKey = "sunclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "sunclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "sunclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "sunclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "sunclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "sunclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "sunclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "sunclaw.watch.action."
    static let actionIDKeyPrefix = "sunclaw.watch.action.id."
    static let actionLabelKeyPrefix = "sunclaw.watch.action.label."
    static let categoryPrefix = "sunclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SunClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SunClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SunClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SunClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SunClawWatchAction]) -> [UNNotificationAction] {
...
struct SunClawApp: App {
    ...
    init() {
        ...
        SunClawAppModelRegistry.appModel = appModel
...
extension SunClawApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("SunClaw: installing uncaught exception handler")
