...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "sunclaw.exec-approval"
    static let reviewActionIdentifier = "sunclaw.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.sunClawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.sunClawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func sunClawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
