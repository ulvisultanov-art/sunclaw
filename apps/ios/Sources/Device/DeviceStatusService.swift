...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> SunClawDeviceStatusPayload {
        ...
        return SunClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> SunClawDeviceInfoPayload {
        ...
        return SunClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> SunClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SunClawBatteryState = switch device.batteryState {
        ...
        return SunClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SunClawThermalStatusPayload {
        let state: SunClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SunClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SunClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SunClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
