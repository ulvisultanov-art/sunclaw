...
final class RemindersService: RemindersServicing {
    func list(params: SunClawRemindersListParams) async throws -> SunClawRemindersListPayload {
        ...
        let payload: [SunClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return SunClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SunClawRemindersAddParams) async throws -> SunClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SunClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SunClawRemindersAddPayload(reminder: payload)
    }
