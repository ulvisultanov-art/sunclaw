...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SUNCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SunClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.sunclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SunClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SunClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return SunClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SunClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SunClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SunClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SunClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SunClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SunClawTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = SunClawChatEventText.assistantText(from: chatEvent) {
