...
public enum GatewayConnectionProblemMapper {
    ...
    private static func map(_ authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
        ...
        case .authTokenMissing:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    ...
                    fallback: "https://docs.sunclaw.complex.az/gateway/authentication"),
        ...
        case .authTokenNotConfigured:
            return self.problem(
                ...
                actionCommand: authError.actionCommand ?? "sunclaw config set gateway.auth.token <new-token>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.sunclaw.complex.az/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            return self.problem(
                ...
                actionLabel: authError.actionLabel ?? "Fix on gateway",
                actionCommand: authError.actionCommand ?? "sunclaw config set gateway.auth.password <new-password>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.sunclaw.complex.az/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.sunclaw.complex.az/platforms/ios"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        case .authDeviceTokenMismatch:
            return self.problem(
                ...
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.sunclaw.complex.az/gateway/pairing"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        ...
        case .deviceAuthSignatureExpired:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.sunclaw.complex.az/gateway/troubleshooting"),
                requestId: authError.requestId,
                retryable: true,
                pauseReconnect: true,
                authError: authError)
        ...
        case .authTailscaleIdentityMissing:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.sunclaw.complex.az/gateway/tailscale"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.sunclaw.complex.az/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "SunClaw could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                actionCommand: nil,
                docsURL: URL(string: "https://docs.sunclaw.complex.az/gateway/troubleshooting"),
                retryable: true,
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost,
                ...
                return GatewayConnectionProblem(
                    ...
                    message: "SunClaw could not reach the gateway over the current network.",
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "sunclaw devices approve \(requestId)"
        }
        return "sunclaw devices list"
    }
