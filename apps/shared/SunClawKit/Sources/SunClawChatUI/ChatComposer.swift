...
public struct SunClawChatTalkControl {
...
@MainActor
struct SunClawChatComposer: View {
    @Bindable var viewModel: SunClawChatViewModel
    let style: SunClawChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: SunClawChatView.ComposerChrome
    let messagePlaceholder: String?
    let talkControl: SunClawChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(SunClawChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(SunClawChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(SunClawChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SunClawPendingAttachment.id)
                { (att: SunClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SunClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(SunClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(SunClawChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: SunClawChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: SunClawChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: SunClawChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(SunClawChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return SunClawChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: SunClawChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return SunClawChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: SunClawChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: SunClawChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(SunClawChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? SunClawChatTheme.userBubble
    }
