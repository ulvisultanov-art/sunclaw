...
private let chatUILogger = Logger(subsystem: "ai.sunclaw", category: "SunClawChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class SunClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [SunClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [SunClawChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [SunClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [SunClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SunClawChatPendingToolCall] = []
    public private(set) var sessions: [SunClawChatSessionEntry] = []
    private let transport: any SunClawChatTransport
    private var sessionDefaults: SunClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: SunClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any SunClawChatTransport,
    ...
    public var sessionChoices: [SunClawChatSessionEntry] {
    ...
    private static let baseThinkingLevelOptions: [SunClawChatThinkingLevelOption] = [
        SunClawChatThinkingLevelOption(id: "off", label: "off"),
        SunClawChatThinkingLevelOption(id: "minimal", label: "minimal"),
        SunClawChatThinkingLevelOption(id: "low", label: "low"),
        SunClawChatThinkingLevelOption(id: "medium", label: "medium"),
        SunClawChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: SunClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SunClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SunClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SunClawChatMessage) -> SunClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SunClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SunClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: SunClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: SunClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: SunClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: SunClawChatMessage) -> Bool {
        ...
        updated[matchIndex] = SunClawChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [SunClawChatMessage],
        incoming: [SunClawChatMessage]) -> [SunClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SunClawChatMessage]) -> [SunClawChatMessage] {
        var result: [SunClawChatMessage] = []
    ...
    private static func dedupeKey(for message: SunClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SunClawChatMessageContent] = [
            SunClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SunClawChatAttachmentPayload in
            SunClawChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "SunClawChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: SunClawChatSessionEntry?) -> [SunClawChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: SunClawChatSessionEntry,
        defaults: SunClawChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [SunClawChatThinkingLevelOption]?) -> [SunClawChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return SunClawChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [SunClawChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return SunClawChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [SunClawChatThinkingLevelOption],
        current: String) -> [SunClawChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [SunClawChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [SunClawChatThinkingLevelOption]) -> [SunClawChatThinkingLevelOption]
    {
        var result: [SunClawChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> SunClawChatSessionEntry {
        SunClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = SunClawChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: SunClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: SunClawSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: SunClawChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: SunClawChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = SunClawChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: SunClawChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: SunClawChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: SunClawChatMessage) -> SunClawChatMessage {
    ...
    private func handleAgentEvent(_ evt: SunClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SunClawChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: SunClawAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: SunClawAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
