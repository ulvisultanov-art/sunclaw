...
struct ChatMessageBubble: View {
    let message: SunClawChatMessage
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SunClawChatTheme.userText : SunClawChatTheme.assistantText
    ...
    private var inlineAttachments: [SunClawChatMessageContent] {
    ...
    private var toolCalls: [SunClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SunClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SunClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SunClawChatTheme.onboardingAssistantBubble
        }
        return SunClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SunClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SunClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SunClawChatTheme.userText : SunClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SunClawChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SunClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SunClawChatTheme.assistantText)
            }
        }
    }
}
