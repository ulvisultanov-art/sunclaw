...
@MainActor
public struct SunClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let talkControl: SunClawChatTalkControl?
    ...
    public init(
        viewModel: SunClawChatViewModel,
        ...
        talkControl: SunClawChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [SunClawChatMessage] {
        let base: [SunClawChatMessage]
    ...
    private func mergeToolResults(in messages: [SunClawChatMessage]) -> [SunClawChatMessage] {
    ...
    private func isToolResultMessage(_ message: SunClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: SunClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: SunClawChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return SunClawChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: SunClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: SunClawChatMessage) -> [SunClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: SunClawChatMessage) -> [SunClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: SunClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: SunClawChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(SunClawChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(SunClawChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
