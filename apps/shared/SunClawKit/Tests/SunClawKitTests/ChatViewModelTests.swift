...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> SunClawChatHistoryPayload
{
    SunClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> SunClawChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> SunClawChatThinkingLevelOption {
    SunClawChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> SunClawChatSessionEntry
{
    SunClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> SunClawChatModelChoice {
    SunClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [SunClawChatHistoryPayload],
    sessionsResponses: [SunClawChatSessionsListResponse] = [],
    modelResponses: [[SunClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, SunClawChatViewModel)
{
...
private func sendUserMessage(_ vm: SunClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, SunClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SunClawChatHistoryPayload]
    private let sessionsResponses: [SunClawChatSessionsListResponse]
    private let modelResponses: [[SunClawChatModelChoice]]
    ...
    private let stream: AsyncStream<SunClawChatTransportEvent>
    private let continuation: AsyncStream<SunClawChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<SunClawChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> SunClawChatCreateSessionResponse
    {
        ...
        return SunClawChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SunClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SunClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [SunClawChatAttachmentPayload]) async throws -> SunClawChatSendResponse
    {
        await self.state.sentSessionKeysAppend(sessionKey)
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return SunClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SunClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SunClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SunClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func displaysErrorMessageFallbackOnlyForAssistantErrorTurns() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> SunClawChatMessage {
        ...
        #expect(
            SunClawChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                SunClawSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: SunClawChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = SunClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = SunClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: SunClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = SunClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SunClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(SunClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == SunClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = SunClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SunClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = SunClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = SunClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = SunClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func decodesGatewayThinkingMetadataFromSessionList() throws {
        ...
        let decoded = try JSONDecoder().decode(
            SunClawChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        ...
        let vm = await MainActor.run { SunClawChatViewModel(sessionKey: "main", transport: transport) }
