...
export const ja_JP: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "モバイルを使用していますか？ デスクトップで sunclaw dashboard --no-open から完全な URL（#token=... を含む）をコピーしてください。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — sunclaw models auth で再認証してください",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "sunclaw dashboard --no-open のトークンを貼り付けるか、構成済みのパスワードを入力します。",
        stepGenerate:
          "トークンが構成されていない場合は、Gateway ホストで sunclaw doctor --generate-gateway-token を実行します。",
      ...
      authFailed: {
        ...
        stepDashboard:
          "sunclaw dashboard --no-open を実行し、新しい URL を開くか、そのトークンを貼り付けます。",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Gateway ホストで sunclaw devices list を実行します。",
        stepApproveId: "このリクエストを承認します: sunclaw devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "sunclaw dashboard で提供元の dashboard を開き直し、UI と Gateway が同じインストールから来るようにします。",
        stepDevUi:
          "pnpm ui:dev を使用している場合は、現在の checkout に対して開発 UI を再ビルドまたは再起動します。",
        stepRestart: "SunClaw 更新後に Gateway を再起動し、現在のプロトコルを提供させます。",
      },
      network: {
        title: "接続できません",
        summary:
        ...
        stepGateway:
          "sunclaw status または sunclaw gateway run で Gateway が実行中であることを確認します。",
        stepUrl:
          "WebSocket URL を確認し、Gateway が HTTPS/Tailscale Serve の背後にある場合は wss:// を使用します。",
        stepDashboard:
          "sunclaw dashboard --no-open で dashboard を開き直し、現在の URL と認証詳細を再コピーします。",
      },
    },
  },
  chat: {
