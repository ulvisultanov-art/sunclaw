...
async function readIndexHtmlWithDelay(delayMs: number): Promise<string> {
  ...
  return html.replace(
    'data-sunclaw-mount-timeout-ms="12000"',
    `data-sunclaw-mount-timeout-ms="${delayMs}"`,
  );
}
...
function installFallbackShell(window: TestWindow, html: string): void {
  ...
  const sentinel = Array.from(parsed.querySelectorAll<HTMLScriptElement>("script:not([src])")).find(
    (script) => script.textContent?.includes("sunclaw-mount-fallback"),
  );
...
describe("Control UI mount fallback", () => {
  ...
  it("shows the static troubleshooting panel when the app element is never registered", async () => {
    ...
    expect(frameWindow.customElements.get("sunclaw-app")).toBeUndefined();
    ...
    const fallback = requireElementById(
      frameWindow,
      "sunclaw-mount-fallback",
      frameWindow.HTMLElement,
    );
    expect(fallback.hidden).toBe(false);
    expect([...frameWindow.document.body.classList]).toEqual(["sunclaw-mount-fallback-active"]);
  ...
  it("keeps the fallback hidden when the app element registers before the timeout", async () => {
    ...
    if (!frameWindow.customElements.get("sunclaw-app")) {
      frameWindow.customElements.define("sunclaw-app", class extends frameWindow.HTMLElement {});
    }
    await frameWindow.customElements.whenDefined("sunclaw-app");
