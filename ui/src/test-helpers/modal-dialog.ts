...
import type { SunClawModalDialog } from "../ui/components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<SunClawModalDialog>("sunclaw-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected sunclaw-modal-dialog");
  }
