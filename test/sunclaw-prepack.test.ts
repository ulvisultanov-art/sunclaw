...
import {
  ...
  runPrepackCommand,
} from "../scripts/sunclaw-prepack.ts";
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ SUNCLAW_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ SUNCLAW_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ SUNCLAW_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid SUNCLAW_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
