...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeSunClawRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const includeFromEnv = loadPatternListFromEnv("SUNCLAW_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeSunClawRuntimeSetup === false ? [] : ["test/setup-sunclaw-runtime.ts"]),
    ]),
