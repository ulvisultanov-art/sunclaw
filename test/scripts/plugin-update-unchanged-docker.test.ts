...
function runProbe(command: string, payload: unknown): void {
  const root = mkdtempSync(path.join(tmpdir(), "sunclaw-plugin-update-probe-"));
...
describe("plugin update unchanged Docker E2E", () => {
  ...
  it("bounds the update command and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("SUNCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS");
    expect(script).toContain(
      'sunclaw_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update',
    );
  ...
  it("waits for the local registry process during cleanup", () => {
    ...
    expect(script).toContain('sunclaw_e2e_stop_process "${registry_pid:-}"');
  ...
  it("bounds corrupt plugin update commands and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("SUNCLAW_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS");
    expect(
      script.match(/sunclaw_e2e_maybe_timeout "\$\{update_timeout_seconds\}s" \\/gu)?.length,
    ).toBe(2);
    expect(script).toContain("--channel beta");
    expect(script).toContain("SUNCLAW_UPDATE_POST_CORE=1");
        ...
        outcomes: [
          {
            ...
            status: "skipped",
            message: `Disabled "${CORRUPT_PLUGIN_ID}" after plugin update failure; SunClaw will continue without it. Failed to update ${CORRUPT_PLUGIN_ID}: registry timeout`,
          },
        ],
      },
    };
        ...
        warnings: [
          {
            ...
            message:
              ...
              `Run sunclaw plugins inspect ${CORRUPT_PLUGIN_ID} --runtime --json for details.`,
          },
        ],
      }),
    ).not.toThrow();
