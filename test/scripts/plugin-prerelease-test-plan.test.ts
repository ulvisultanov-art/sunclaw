...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@sunclaw/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@sunclaw/kitchen-sink@beta");
    expect(script).toContain("clawhub:@sunclaw/kitchen-sink@latest");
    expect(script).toContain("clawhub:@sunclaw/kitchen-sink@beta");
    expect(script).toContain("SUNCLAW_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-clawhub|clawhub:@sunclaw/kitchen-sink@latest|sunclaw-kitchen-sink-fixture|clawhub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
    ...
    expect(sweepScript).toContain("SUNCLAW_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("SUNCLAW_ENTRY=/app/sunclaw.mjs");
    expect(script).toContain("SUNCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltSunClawEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(assertionsScript).toContain("assertClawHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "sunclaw');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('sunclaw: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(inspectorRun.env).toEqual({
      SUNCLAW_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      SUNCLAW_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@sunclaw/plugin-inspector@${SUNCLAW_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/sunclaw-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "sunclaw-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg SUNCLAW_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
  ...
  it("keeps runtime tool coverage blocking in release checks", () => {
    const releaseChecksSource = readFileSync(
      ".github/workflows/sunclaw-release-checks.yml",
      "utf8",
    );
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm sunclaw qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, SUNCLAW_DISCORD_TOKEN)");
