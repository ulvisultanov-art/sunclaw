...
describe("test perf budget script", () => {
  it("parses numeric budget env vars strictly before running Vitest", () => {
    expect(
      testing.parseArgs([], {
        SUNCLAW_TEST_PERF_BASELINE_WALL_MS: "1000",
        SUNCLAW_TEST_PERF_MAX_REGRESSION_PCT: "12.5",
        SUNCLAW_TEST_PERF_MAX_WALL_MS: "1500",
      }),
    ).toMatchObject({
      baselineWallMs: 1000,
    ...
    expect(() =>
      testing.parseArgs([], {
        SUNCLAW_TEST_PERF_MAX_WALL_MS: "1000ms",
      }),
    ).toThrow("SUNCLAW_TEST_PERF_MAX_WALL_MS must be a non-negative number");
  });
