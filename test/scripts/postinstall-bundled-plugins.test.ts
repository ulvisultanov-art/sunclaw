...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/sunclaw/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/sunclaw/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "sunclaw-cache");
    ...
    expect(removed).not.toContain(path.join(configuredBase, "sunclaw"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune SunClaw compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "sunclaw-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "sunclaw-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("patches the Baileys upload helper dispatcher guard", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-baileys-postinstall-");
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-source-checkout-state-skip-");
    const home = await createTempDirAsync("sunclaw-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".sunclaw", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { SUNCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-postinstall-registry-");
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["SUNCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] SUNCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { SUNCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { SUNCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-");
  ...
  it("omits unpacked plugin-sdk test helpers from the package dist inventory", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-inventory-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("sunclaw-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "sunclaw");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("sunclaw-packaged-home-");
    const stateOverride = path.join(home, "custom-state");
    const systemState = path.join(home, "system-state");
    const defaultLegacyRoot = path.join(home, ".sunclaw", "plugin-runtime-deps");
    ...
    runBundledPluginPostinstall({
      env: {
        HOME: home,
        SUNCLAW_STATE_DIR: stateOverride,
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.sunclaw/plugin-runtime-deps: Error: locked",
    );
  });

  it("resolves legacy plugin runtime deps roots from SunClaw state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          SUNCLAW_HOME: "/srv/sunclaw-home",
          SUNCLAW_CONFIG_PATH: "~/profile/sunclaw.json",
          SUNCLAW_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/sunclaw",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/sunclaw-home/.clawdbot/plugin-runtime-deps",
      "/srv/sunclaw-home/.sunclaw/plugin-runtime-deps",
      "/srv/sunclaw-home/profile/plugin-runtime-deps",
      "/srv/sunclaw-home/state/plugin-runtime-deps",
      "/var/lib/sunclaw/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expectPathMissing(
      path.join(packageRoot, "dist", "extensions", "qa-channel", "sunclaw.plugin.json"),
    );
    await expectPathMissing(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("sunclaw-packaged-install-dist-prune-");
