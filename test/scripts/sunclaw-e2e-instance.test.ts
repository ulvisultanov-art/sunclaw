...
const helperPath = path.resolve("scripts/lib/sunclaw-e2e-instance.sh");
const hostPath = [
  ...
  return spawnSync(
    ...
    [
      ...
      [
        ...
        `sunclaw_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
        'printf "value=%s" "${SUNCLAW_E2E_INSTANCE_TEST:-unset}"',
      ].join("; "),
    ],
    { encoding: "utf8" },
  );
...
function writePackageFixture(packagePath: string): void {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-package-"));
  try {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "sunclaw-e2e-fixture", version: "0.0.0" }),
    );
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/sunclaw-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/sunclaw-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export SUNCLAW_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid SunClaw test-state base64 payload");
  });
  ...
  it("wraps package installs with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-"));
    try {
      ...
      const packagePath = path.join(tempDir, "sunclaw.tgz");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `sunclaw_e2e_install_package ${shellQuote(logPath)} ${shellQuote("fixture package")} ${shellQuote(prefixPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            ...
            SUNCLAW_CURRENT_PACKAGE_TGZ: packagePath,
            SUNCLAW_E2E_NPM_INSTALL_TIMEOUT: "42s",
            SUNCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            SUNCLAW_TEST_NPM_ARGS: npmArgsPath,
            SUNCLAW_TEST_NPM_BIN: path.join(tempDir, "npm"),
          }),
        },
      );
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-plain-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-gtimeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-no-timeout-"));
    try {
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "sunclaw.tgz");
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-node-watchdog-"));
    try {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `sunclaw_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      expect(result.stderr).toContain("SunClaw E2E command timed out after 200ms");
    } finally {
      fs.rmSync(tempDir, { force: true, recursive: true });
    }
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "sunclaw-e2e-instance-node-watchdog-signal-"),
      );
      try {
        writeNodeShim(tempDir);
...
set -euo pipefail
source ${shellQuote(helperPath)}
export SUNCLAW_E2E_TIMEOUT_KILL_GRACE_MS=100
sunclaw_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-gateway-terminate-"));
    try {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$SUNCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
sunclaw_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
sunclaw_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$SUNCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = spawnSync("/bin/bash", ["-c", script], {
        ...
        env: shellTestEnv({
          ...
          SUNCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        }),
        timeout: 5_000,
      });
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(sunclaw_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
sunclaw_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-http-probe-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source ${shellQuote(helperPath)}`,
            'sunclaw_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
          ].join("; "),
        ],
  ...
  it("wraps logged SunClaw E2E commands with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-run-logged-"));
    ...
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `sunclaw_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
            `printf "%s" "$SUNCLAW_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            SUNCLAW_E2E_LOG_DIR: logDir,
            SUNCLAW_E2E_COMMAND_TIMEOUT: "17s",
            SUNCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            SUNCLAW_TEST_COMMAND_ARGS: commandArgsPath,
            SUNCLAW_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
          }),
        },
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^sunclaw-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("installs the trash shim under isolated test state", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-trash-shim-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `printf "%s" "$SUNCLAW_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
      ...
      expect(binDir).not.toBe("/tmp/sunclaw-bin");
  ...
  it("wraps package-installed SunClaw CLI calls with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-sunclaw-cli-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "sunclaw-args.txt");
      fs.writeFileSync(
        path.join(tempDir, "timeout"),
        [
          "#!/usr/bin/env bash",
          ...
          'if [ "${1:-}" = "--kill-after=1s" ]; then exit 0; fi',
          'printf "%s\\n" "$*" >"$SUNCLAW_TEST_TIMEOUT_ARGS"',
          `while [ "$#" -gt 0 ] && [ "$1" != ${shellQuote(path.join(tempDir, "sunclaw"))} ]; do shift; done`,
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$SUNCLAW_TEST_SUNCLAW_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
        path.join(tempDir, "sunclaw"),
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          'printf "%s\\n" "$*" >"$SUNCLAW_TEST_COMMAND_ARGS"',
      ...
      fs.chmodSync(path.join(tempDir, "timeout"), 0o755);
      fs.chmodSync(path.join(tempDir, "sunclaw"), 0o755);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            SUNCLAW_E2E_COMMAND_TIMEOUT: "23s",
            SUNCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            SUNCLAW_TEST_COMMAND_ARGS: commandArgsPath,
            SUNCLAW_TEST_SUNCLAW_BIN: path.join(tempDir, "sunclaw"),
          }),
        },
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "sunclaw")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-e2e-instance-pty-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `sunclaw_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            SUNCLAW_E2E_COMMAND_TIMEOUT: "31s",
            SUNCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            SUNCLAW_TEST_SCRIPT_ARGS: scriptArgsPath,
            SUNCLAW_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
          }),
        },
      );
