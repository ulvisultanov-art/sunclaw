...
const WORKFLOW = ".github/workflows/sunclaw-performance.yml";
...
describe("SunClaw performance workflow", () => {
  it("uses the clawgrit reports token for every report repo push path", () => {
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/sunclaw/clawgrit-reports.git"',
    );
    expect(publish.run).toContain(
      'remote set-url origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/sunclaw/clawgrit-reports.git"',
    );
