...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "sunclaw-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "sunclaw-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$SUNCLAW_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$SUNCLAW_TEST_CONTRIB"',
    ],
    {
      SUNCLAW_PR_CHANGELOG_SH: changelogScriptPath,
      SUNCLAW_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$SUNCLAW_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      SUNCLAW_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @sunclaw.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @SunClaw.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "sunclaw", text: "- Org-owned fix. Thanks @sunclaw." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "sunclaw", text: "- Mixed credit. Thanks @contributor and @SunClaw." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @sunclaw and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "sunclaw",
        text: "- Mixed credit (#123). Thanks @sunclaw and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("sunclaw")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("sunclaw-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("sunclaw-clawsweeper[bot]")).toBe(true);
    ...
    try {
      const output = run(
        ...
        [
          ...
          `
set -euo pipefail
source "$SUNCLAW_PR_COMMON_SH"
source "$SUNCLAW_PR_CHANGELOG_SH"
source "$SUNCLAW_PR_GATES_SH"
...
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$SUNCLAW_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$SUNCLAW_TEST_CALLS"; }

prepare_gates 123
`,
        ],
        {
          SUNCLAW_PR_COMMON_SH: commonScriptPath,
          SUNCLAW_PR_CHANGELOG_SH: changelogScriptPath,
          SUNCLAW_PR_GATES_SH: gatesScriptPath,
          SUNCLAW_TEST_CALLS: callsPath,
        },
      );
