...
function credentialBrokerEnv(port: number) {
  return {
    ...
    SUNCLAW_QA_CONVEX_SECRET_MAINTAINER: "test-secret",
    SUNCLAW_QA_CONVEX_SITE_URL: `http://127.0.0.1:${port}`,
    SUNCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "1000",
    SUNCLAW_QA_CREDENTIAL_OWNER_ID: "test-owner",
    SUNCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE: "maintainer",
  };
}

describe("RTT harness", () => {
  it("validates SunClaw package specs", () => {
    expect(validateSunClawPackageSpec("sunclaw@main")).toBe("sunclaw@main");
    expect(validateSunClawPackageSpec("sunclaw@alpha")).toBe("sunclaw@alpha");
    ...
    expect(() => validateSunClawPackageSpec("@sunclaw/sunclaw@beta")).toThrow(
      /Package spec must be/,
    );
    expect(() => validateSunClawPackageSpec("sunclaw@next")).toThrow(/Package spec must be/);
  });

  it("builds stable run labels", () => {
    expect(safeRunLabel("sunclaw@beta")).toBe("sunclaw_beta");
    expect(
      buildRunId({
    ...
    ).toBe("2026-05-01T030405678Z-sunclaw_beta-2");
  });
  ...
  it("constructs harness env without dropping caller env", () => {
    ...
    expect(env.SUNCLAW_QA_TELEGRAM_GROUP_ID).toBe("-100123");
    expect(env.SUNCLAW_NPM_TELEGRAM_PACKAGE_SPEC).toBe("sunclaw@beta");
    expect(env.SUNCLAW_NPM_TELEGRAM_PACKAGE_LABEL).toBe("sunclaw@beta (2026.4.30-beta.1)");
    expect(env.SUNCLAW_NPM_TELEGRAM_PROVIDER_MODE).toBe("mock-openai");
    expect(env.SUNCLAW_NPM_TELEGRAM_SCENARIOS).toBe("telegram-mentioned-message-reply");
    expect(env.SUNCLAW_NPM_TELEGRAM_OUTPUT_DIR).toBe(".artifacts/rtt/run/raw");
    expect(env.SUNCLAW_NPM_TELEGRAM_FAST).toBe("0");
    expect(env.SUNCLAW_NPM_TELEGRAM_WARM_SAMPLES).toBe("20");
    expect(env.SUNCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("30000");
    expect(env.SUNCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS).toBe("180000");
    expect(env.SUNCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS).toBe("180000");
  });

  it("forwards Convex credential controls without dropping RTT sample controls", () => {
    const env = createHarnessEnv({
      baseEnv: {
        SUNCLAW_QA_CONVEX_SITE_URL: "https://qa-credentials.example.convex.site",
        SUNCLAW_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
    ...
    expect(env.SUNCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE).toBe("convex");
    expect(env.SUNCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE).toBe("maintainer");
    expect(env.SUNCLAW_NPM_TELEGRAM_WARM_SAMPLES).toBe("7");
    expect(env.SUNCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("45000");
    expect(() =>
  ...
  it("exports the Telegram bot token after Convex credentials are sourced", async () => {
    ...
    expect(script).not.toContain('export TELEGRAM_BOT_TOKEN="$SUNCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"');
  });
  ...
  it("keeps RTT Docker artifacts isolated by default", async () => {
    ...
    expect(script).not.toContain(
      'OUTPUT_DIR="${SUNCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-rtt}"',
    );
  });
  ...
  it("bounds Convex credential broker response bodies", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rtt-credentials-"));
      ...
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...
            SUNCLAW_QA_CREDENTIAL_HTTP_MAX_BODY_BYTES: "16",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("does not start another credential acquire after retry delay exhausts the deadline", async () => {
    ...
    const { port } = await listenOnLoopback(server);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rtt-credentials-retry-"));
    tempDirs.push(tempDir);
    ...
    try {
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...credentialBrokerEnv(port),
            SUNCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "75",
            SUNCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "250",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("caps credential acquire HTTP retries to the remaining acquire deadline", async () => {
    ...
    const { port } = await listenOnLoopback(server);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rtt-credentials-cap-"));
    tempDirs.push(tempDir);
    const startedAt = Date.now();
    ...
    try {
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...credentialBrokerEnv(port),
            SUNCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "100",
            SUNCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "900",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("generates final-only Telegram RTT delivery config for release packages", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rtt-config-test-"));
    tempDirs.push(tempDir);
  ...
  it("builds normalized result JSON", async () => {
    ...
    expect(result).toStrictEqual({
      ...
      package: { spec: "sunclaw@beta", version: "2026.4.30-beta.1" },
      run: {
        durationMs: 12_000,
  ...
  it("appends JSONL rows", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rtt-test-"));
    tempDirs.push(tempDir);
  ...
  it("parses CLI options", () => {
    ...
    expect(parsed.spec).toBe("sunclaw@latest");
    expect(parsed.options).toStrictEqual({
      packageTgz: "/tmp/sunclaw.tgz",
      credentialRole: "ci",
      credentialSource: "convex",
