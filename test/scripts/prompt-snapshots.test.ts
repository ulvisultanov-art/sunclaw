...
describe("happy path prompt snapshots", () => {
  ...
  it("deletes stale generated snapshot artifacts", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-prompt-snapshot-stale-"));
    try {
  ...
  it("renders the Codex model-bound prompt layers", async () => {
    ...
    expect(telegram).toContain("SunClaw runtime context for this turn:");
  ...
  it("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    const agentSoulHeading = "## SunClaw Agent Soul";
    ...
    expect(direct).not.toContain("This is an SunClaw heartbeat turn.");
    expect(group).not.toContain("This is an SunClaw heartbeat turn.");

    expect(heartbeat).toContain('"collaborationMode": {');
    expect(heartbeat).toContain('"developer_instructions": "This is an SunClaw heartbeat turn.');
    expect(heartbeat).toContain(agentSoulHeading);
    const sunClawRuntimeInstructions = renderedPromptSection(
      heartbeat,
    ...
    expect(sunClawRuntimeInstructions).not.toContain(heartbeatPhrase);
    ...
    expect(collaborationModeInstructions).toContain(
      "/tmp/sunclaw-happy-path/workspace/HEARTBEAT.md",
    );
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {
