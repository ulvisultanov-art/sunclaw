...
} from "../../scripts/resolve-sunclaw-package-candidate.mjs";
...
describe("resolve-sunclaw-package-candidate", () => {
  it("accepts only SunClaw release package specs for npm candidates", () => {
    for (const spec of [
      "sunclaw@beta",
      "sunclaw@alpha",
      "sunclaw@latest",
      "sunclaw@2026.4.27",
      "sunclaw@2026.4.27-1",
      "sunclaw@2026.4.27-beta.2",
      "sunclaw@2026.4.27-alpha.2",
    ]) {
      expect(validateSunClawPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateSunClawPackageSpec("@evil/sunclaw@1.0.0")).toThrow(
      "package_spec must be sunclaw@alpha",
    );
    expect(() => validateSunClawPackageSpec("sunclaw@canary")).toThrow(
      "package_spec must be sunclaw@alpha",
    );
    expect(() => validateSunClawPackageSpec("sunclaw@2026.04.27")).toThrow(
      "package_spec must be sunclaw@alpha",
    );
    expect(() => validateSunClawPackageSpec("sunclaw@npm:other-package")).toThrow(
      "package_spec must be sunclaw@alpha",
    );
    expect(() => validateSunClawPackageSpec("sunclaw@file:../other-package.tgz")).toThrow(
      "package_spec must be sunclaw@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "sunclaw-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "sunclaw@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "sunclaw@2026.5.26-beta.1",
      "C:\\sunclaw\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack sunclaw@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\sunclaw\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-sunclaw-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/sunclaw-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/sunclaw-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/sunclaw/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "sunclaw.tgz");

    await expect(
      downloadUrl("http://packages.example/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/sunclaw/sunclaw.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/sunclaw/sunclaw.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/sunclaw/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/sunclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/sunclaw/sunclaw.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/sunclaw/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/sunclaw.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/sunclaw.tgz", target, {
      fetchImpl: async () =>
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "sunclaw" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "sunclaw.tgz");
