...
} from "../../scripts/sunclaw-cross-os-release-checks.ts";
...
describe("scripts/sunclaw-cross-os-release-checks", () => {
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: sunclaw gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("gives the Windows packaged updater wrapper enough headroom for SunClaw timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });

  it("records packaged-fresh phase timings for release-check summaries", () => {
    const source = readFileSync("scripts/sunclaw-cross-os-release-checks.ts", "utf8");
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-agent-output-"));
    try {
  ...
  it("detects embedded fallback agent turns as non-gateway proof", () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-agent-fallback-"));
    const logPath = join(dir, "agent.log");
    expect(
      agentTurnUsedEmbeddedFallback({
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-agent-skip-retry-"));
    try {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        SUNCLAW_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        SUNCLAW_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.5", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/sunclaw-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "SUNCLAW_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.SUNCLAW_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.5' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("sunclaw@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "sunclaw@2026.5.2",
      "--omit=dev",
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/sunclaw-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/sunclaw-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "sunclaw" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'sunclaw.cmd')");
    expect(script).toContain("$cmd = Get-Command sunclaw -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        SUNCLAW_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        SUNCLAW_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        SUNCLAW_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("sunclaw-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-static-server-"));
    const filePath = join(dir, "sunclaw-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-static-server-close-"));
    const filePath = join(dir, "sunclaw-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "sunclaw/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "SUNCLAW_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/sunclaw/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/sunclaw/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/sunclaw-cross-os-release-checks.ts", "utf8")).toContain(
      "SUNCLAW_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view sunclaw@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\SunClaw Prefix`, "sunclaw.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\SunClaw Prefix\sunclaw.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-run-command-"));
    try {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-run-command-output-"));
    try {
      const logPath = join(dir, "command.log");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-run-command-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.SUNCLAW_TEST_CHILD_PID, String(child.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");

      const command = runCommand(process.execPath, ["-e", parentScript], {
        cwd: dir,
        env: { ...process.env, SUNCLAW_TEST_CHILD_PID: childPidPath },
        logPath,
        timeoutMs: 500,
      });
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            SUNCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "25",
            SUNCLAW_TEST_CHILD_PID: childPidPath,
          },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/sunclaw", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "sunclaw");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "sunclaw" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "sunclaw"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "sunclaw"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "sunclaw"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-sunclaw-cache",
        SUNCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
      }),
    ).toEqual({
      FOO: "bar",
      SUNCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[sunclaw] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/sunclaw/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\sunclaw-upgrade-q9DsA7\\prefix\\node_modules\\sunclaw\\sunclaw.mjs update --tag http://127.0.0.1:49951/sunclaw-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\sunclaw\\sunclaw.mjs update --tag http://127.0.0.1:49951/sunclaw-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node sunclaw.mjs update --tag sunclaw@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.sunclaw-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("sunclaw@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("sunclaw@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "sunclaw")
          : join(prefixDir, "lib", "node_modules", "sunclaw");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".SunClaw-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".SunClaw-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "sunclaw-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "sunclaw-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
