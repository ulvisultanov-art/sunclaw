...
import { downloadUrl } from "../../scripts/resolve-sunclaw-package-candidate.mjs";
...
describe("package URL IPv6 transition address blocking", () => {
  it.each([
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "sunclaw-package-ip-bypass-"));
    tempDirs.push(dir);
  ...
  it.each([
    ["IPv4-mapped loopback dotted", `https://[::ffff:${dotted(127, 0, 0, 1)}]/sunclaw.tgz`],
    ["IPv4-compatible loopback dotted", `https://[::${dotted(127, 0, 0, 1)}]/sunclaw.tgz`],
