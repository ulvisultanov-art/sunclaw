...
import { createSunClawTestInstance, type SunClawTestInstance } from "./sunclaw-test-instance.js";
...
export type GatewayInstance = SunClawTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createSunClawTestInstance({ name });
  try {
