...
import { createSunClawTestInstance, testing } from "./sunclaw-test-instance.js";
...
describe("sunclaw test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createSunClawTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".sunclaw"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "sunclaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.SUNCLAW_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.SUNCLAW_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.SUNCLAW_SKIP_CRON).toBe("0");
