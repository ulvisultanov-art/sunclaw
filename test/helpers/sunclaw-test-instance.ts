...
import {
  ...
  type SunClawTestStateOptions,
} from "../../src/test-utils/sunclaw-test-state.js";
import { sleep } from "../../src/utils.js";

export type SunClawTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<SunClawTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type SunClawTestInstanceCommandResult = {
  code: number | null;
...
export type SunClawTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<SunClawTestInstanceCommandResult>;
...
export async function createSunClawTestInstance(
  options: SunClawTestInstanceOptions,
): Promise<SunClawTestInstance> {
  ...
  const instance: SunClawTestInstance = {
    name: options.name,
    port,
