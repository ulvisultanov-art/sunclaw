...
import { createSunClawCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/sunclaw-happy-path/workspace";
const AGENT_DIR = "/tmp/sunclaw-happy-path/agent";
const SESSION_FILE = "/tmp/sunclaw-happy-path/session.jsonl";
...
const CODEX_WORKSPACE_THREAD_DEVELOPER_INSTRUCTIONS = [
  "## SunClaw Workspace Instructions",
  "",
...
const baseConfig: SunClawConfig = {
  messages: {
    groupChat: {
...
const dynamicToolsConfig: SunClawConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(codexApi: CodexPromptSnapshotApi): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "SunClaw maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "SunClaw",
    ConversationLabel: "SunClaw/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@SunClaw please verify the Codex happy path too.",
      },
    ],
    Body: "@SunClaw can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to SunClaw/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const sunClawDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack SunClaw can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, Codex thread config instructions when present, SunClaw developer instructions, turn-scoped collaboration-mode instructions when SunClaw provides them, turn input with SunClaw runtime context, and the SunClaw dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
      ...
      stableJson({
        ...
        sunClawRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input SunClaw runtime context",
    ...
    markdownFence(
      ...
      stableJson({
        ...
        sunClawDeveloperInstructions: textStats(sunClawDeveloperInstructions),
    ...
    "",
    "### Developer: SunClaw Runtime Instructions",
    "",
    markdownFence("text", sunClawDeveloperInstructions),
    "",
...
function buildCodexSunClawRuntimeContext(): string {
  return [
    ...
    "",
    "## SunClaw Workspace Context",
    "",
    CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT,
  ].join("\n");
}

function prependCodexSunClawRuntimeContext(prompt: string): string {
  return [buildCodexSunClawRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexSunClawRuntimeContext(scenario.prompt);
  ...
  return [
    ...
    "- This captures the SunClaw-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, SunClaw developer instructions, turn input with simulated SunClaw workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how SunClaw routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path SunClaw tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after SunClaw sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
