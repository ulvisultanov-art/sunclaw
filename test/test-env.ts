...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.SUNCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "SUNCLAW_TEST_FAST", value: process.env.SUNCLAW_TEST_FAST },
    {
      key: "SUNCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.SUNCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "SUNCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.SUNCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "SUNCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.SUNCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "SUNCLAW_STATE_DIR", value: process.env.SUNCLAW_STATE_DIR },
    { key: "SUNCLAW_CONFIG_PATH", value: process.env.SUNCLAW_CONFIG_PATH },
    { key: "SUNCLAW_GATEWAY_PORT", value: process.env.SUNCLAW_GATEWAY_PORT },
    { key: "SUNCLAW_BRIDGE_ENABLED", value: process.env.SUNCLAW_BRIDGE_ENABLED },
    { key: "SUNCLAW_BRIDGE_HOST", value: process.env.SUNCLAW_BRIDGE_HOST },
    { key: "SUNCLAW_BRIDGE_PORT", value: process.env.SUNCLAW_BRIDGE_PORT },
    { key: "SUNCLAW_CANVAS_HOST_PORT", value: process.env.SUNCLAW_CANVAS_HOST_PORT },
    { key: "SUNCLAW_TEST_HOME", value: process.env.SUNCLAW_TEST_HOME },
    { key: "SUNCLAW_AGENT_DIR", value: process.env.SUNCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SUNCLAW_TEST_HOME = tempHome;
  process.env.SUNCLAW_TEST_FAST = "1";
  process.env.SUNCLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.SUNCLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.SUNCLAW_BRIDGE_ENABLED;
  delete process.env.SUNCLAW_BRIDGE_HOST;
  delete process.env.SUNCLAW_BRIDGE_PORT;
  delete process.env.SUNCLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.SUNCLAW_STATE_DIR = path.join(tempHome, ".sunclaw");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.SUNCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.SUNCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".sunclaw");
  const priorIsolatedHome = params.env.SUNCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".sunclaw")
  ) {
    realStateDir = path.join(params.realHome, ".sunclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".sunclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.SUNCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.SUNCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "sunclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "sunclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.SUNCLAW_LIVE_TEST === "1" ||
    process.env.SUNCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.SUNCLAW_LIVE_USE_REAL_HOME);
