...
const RUN_NODE_SKIP_DTS_BUILD = process.env.SUNCLAW_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.SUNCLAW_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return (
    id === "ws" ||
    id.startsWith("ws/") ||
    id === "@sunclaw/gateway-protocol" ||
    id.startsWith("@sunclaw/gateway-protocol/")
  );
}
...
function shouldExternalizeSpeechCoreDependency(id: string): boolean {
  return id === "sunclaw" || id.startsWith("sunclaw/");
}
...
function shouldExternalizeLlmRuntimeDependency(id: string): boolean {
  return id === "@sunclaw/llm-core" || id.startsWith("@sunclaw/llm-core/");
}
