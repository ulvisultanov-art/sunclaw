...
steps:
  - name: emits protected low-cardinality prometheus metrics
    actions:
      ...
      - set: gatewayToken
        value:
          expr: "String(env.gateway.token ?? env.gateway.runtimeEnv.SUNCLAW_GATEWAY_TOKEN ?? '')"
      - assert:
      ...
      - assert:
          expr: "prometheusText.includes('# TYPE sunclaw_run_completed_total counter')"
          message: "missing run completion counter"
      - assert:
          expr: "prometheusText.includes('# TYPE sunclaw_run_duration_seconds histogram')"
          message: "missing run duration histogram"
      - assert:
          expr: "prometheusText.includes('# TYPE sunclaw_model_call_total counter')"
          message: "missing model call counter"
      - assert:
          expr: "prometheusText.includes('# TYPE sunclaw_harness_run_total counter')"
      ...
      - assert:
          expr: "!prometheusText.includes('sunclaw.content.')"
          message: "prometheus output leaked content attributes"
      - assert:
          expr: "!/sunclaw_prometheus_series_dropped_total(?:\\{[^}]*\\})?\\s+(?!0(?:\\.0+)?(?:\\s|$))/.test(prometheusText)"
