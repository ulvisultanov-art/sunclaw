---
name: node-connect
description: "Diagnose SunClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify SunClaw is advertising that route, then fix pairing/auth.
...
- whether `sunclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `sunclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
sunclaw config get gateway.mode
sunclaw config get gateway.bind
sunclaw config get gateway.tailscale.mode
sunclaw config get gateway.remote.url
sunclaw config get gateway.auth.mode
sunclaw config get gateway.auth.allowTailscale
sunclaw config get plugins.entries.device-pair.config.publicUrl
sunclaw qr --json
sunclaw devices list
sunclaw nodes status
```

If this SunClaw instance is pointed at a remote gateway, also run:

```bash
sunclaw qr --remote --json
```
...
If `sunclaw qr --json` says `Gateway is only bound to loopback`:
...
- `sunclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run sunclaw qr again, rescan, then approve the pending device pairing.`
