...
import { resolveNpmCommandInvocation } from "./sunclaw-npm-release-check.ts";
...
function main(): void {
  ...
  if (!tarballPath) {
    throw new Error(
      "Usage: node --import tsx scripts/sunclaw-npm-prepublish-verify.ts <tarball.tgz> [expected-version]",
    );
  }

  const workingDir = mkdtempSync(join(tmpdir(), "sunclaw-prepublish-"));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed sunclaw binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `sunclaw-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`sunclaw-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
