...
const DISABLE_POSTINSTALL_ENV = "SUNCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "SUNCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallSunClawHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.SUNCLAW_HOME?.trim();
  return override ? pathResolve(resolvePostinstallTildePath(override, osHome)) : osHome;
}

function resolvePostinstallUserPath(input, sunClawHome) {
  return pathResolve(resolvePostinstallTildePath(input, sunClawHome));
}
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.sunclaw-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const sunClawHome = resolvePostinstallSunClawHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  if (stateOverride) {
    addStateRoot(resolvePostinstallUserPath(stateOverride, sunClawHome));
  }
  const configPath = env?.SUNCLAW_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, sunClawHome)));
  }
  addStateRoot(join(sunClawHome, ".sunclaw"));
  addStateRoot(join(sunClawHome, ".clawdbot"));

  for (const entry of splitPostinstallPathList(env?.STATE_DIRECTORY)) {
    addStateRoot(resolvePostinstallUserPath(entry, sunClawHome));
  }
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.sunclaw-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneSunClawCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune SunClaw compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
