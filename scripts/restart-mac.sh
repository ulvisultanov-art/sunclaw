#!/usr/bin/env bash
# Reset SunClaw like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${SUNCLAW_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="SunClaw.app/Contents/MacOS/SunClaw"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/SunClaw"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/SunClaw"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/SunClaw"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.sunclaw.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/sunclaw-restart-${LOCK_KEY}"
...
GATEWAY_WAIT_SECONDS="${SUNCLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.sunclaw/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SUNCLAW_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node sunclaw.mjs daemon install --force --runtime node"
      log "  node sunclaw.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.sunclaw/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.sunclaw.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing SunClaw instances"
kill_all_sunclaw
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product SunClaw"
...
choose_app_bundle() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    ...
    fail "SUNCLAW_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi

  if [[ -d "${ROOT_DIR}/dist/SunClaw.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SunClaw.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SunClaw.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/SunClaw.app" ]]; then
    APP_BUNDLE="/Applications/SunClaw.app"
    return 0
  fi

  fail "App bundle not found. Set SUNCLAW_APP_BUNDLE to your installed SunClaw.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node sunclaw.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node sunclaw.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".sunclaw", "sunclaw.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: SunClaw is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.sunclaw.gateway.plist' | head -n 40 || true"
fi
