...
const dangerous = hasFlag("--dangerous") || process.env.SUNCLAW_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "sunclaw ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "sunclaw-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "SunClaw E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
