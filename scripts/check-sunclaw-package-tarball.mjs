...
function usage() {
  return "Usage: node scripts/check-sunclaw-package-tarball.mjs <sunclaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`SunClaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.SUNCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-sunclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "sunclaw") {
      errors.push("npm-shrinkwrap.json root name must be sunclaw");
    }
    ...
    if (!rootPackage || rootPackage.name !== "sunclaw") {
      errors.push("npm-shrinkwrap.json packages root must name sunclaw");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`SunClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`SunClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("SunClaw package tarball integrity passed.");
