#!/usr/bin/env bash
# ClawDock - Docker helpers for SunClaw
# Inspired by Simon Willison's "Running SunClaw in Docker"
# https://til.simonwillison.net/llms/sunclaw-docker
#
# Installation:
...
# Common paths to check for SunClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/sunclaw"
  "${HOME}/workspace/sunclaw"
  "${HOME}/projects/sunclaw"
  "${HOME}/dev/sunclaw"
  "${HOME}/code/sunclaw"
  "${HOME}/src/sunclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SunClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/sunclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SunClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/ulvisultanov-art/sunclaw.git ~/sunclaw"
    echo "  cd ~/sunclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SUNCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d sunclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart sunclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f sunclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.sunclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show sunclaw.json
  if [[ -f "${config_dir}/sunclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/sunclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/sunclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No sunclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec sunclaw-gateway \
    bash -c 'echo "alias sunclaw=\"./sunclaw.mjs\"" > /tmp/.bashrc_sunclaw && bash --rcfile /tmp/.bashrc_sunclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm sunclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build sunclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d sunclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build sunclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SUNCLAW_GATEWAY_TOKEN=$token" sunclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" sunclaw-gateway \
    bash -c './sunclaw.mjs config set gateway.remote.token "$TOKEN" && ./sunclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec sunclaw-gateway \
    bash -c "./sunclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart sunclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm sunclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec sunclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'sunclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SunClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (sunclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to sunclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.sunclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'sunclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'sunclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.sunclaw.complex.az${_CLR_RESET}"
  echo ""
}
