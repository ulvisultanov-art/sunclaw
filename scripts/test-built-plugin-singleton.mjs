...
const { clearPluginCommands, getPluginCommandSpecs, loadSunClawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadSunClawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@sunclaw/build-smoke-plugin",
      type: "module",
      sunclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'sunclaw/plugin-sdk';",
...
const registry = loadSunClawPlugins({
  ...
  env: {
    ...process.env,
    SUNCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
