...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${SUNCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$SUNCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.SUNCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `SUNCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${SUNCLAW_INSTALL_SMOKE_IMAGE:-sunclaw-install-smoke:local}"
NONROOT_IMAGE="${SUNCLAW_INSTALL_NONROOT_IMAGE:-sunclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${SUNCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${SUNCLAW_INSTALL_URL:-https://sunclaw.bot/install.sh}"
CLI_INSTALL_URL="${SUNCLAW_INSTALL_CLI_URL:-https://sunclaw.bot/install-cli.sh}"
PACKAGE_NAME="${SUNCLAW_INSTALL_PACKAGE:-sunclaw}"
SKIP_NONROOT="${SUNCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${SUNCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${SUNCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${SUNCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${SUNCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
SKIP_FRESHNESS="${SUNCLAW_INSTALL_SMOKE_SKIP_FRESHNESS:-0}"
FRESHNESS_INSTALL_URL="${SUNCLAW_INSTALL_SMOKE_FRESHNESS_INSTALL_URL:-file:///tmp/sunclaw-install.sh}"
# npm min-release-age is days; 10000 keeps the control failure independent of normal release cadence.
FRESHNESS_MIN_RELEASE_AGE="${SUNCLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE:-10000}"
FRESHNESS_NPM_VERSION="${SUNCLAW_INSTALL_FRESHNESS_NPM_VERSION:-11.14.1}"
UPDATE_BASELINE_VERSION="${SUNCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${SUNCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${SUNCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${SUNCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${SUNCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${SUNCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
...
NPM_CACHE_DIR="${SUNCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
NPM_CACHE_PREPARED=0
NPM_CACHE_DOCKER_ARGS=()
INSTALL_SCRIPT_DOCKER_ARGS=(
  -v "$ROOT_DIR/scripts/install.sh:/tmp/sunclaw-install.sh:ro"
  -v "$ROOT_DIR/scripts/install-cli.sh:/tmp/sunclaw-install-cli.sh:ro"
)
...
for env_name in \
  ...
  SUNCLAW_INSTALL_SMOKE_SKIP_PREVIOUS; do
  env_value="${!env_name:-}"
...
' "$pack_json_file"
  ...
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-sunclaw-package-tarball.mjs "${UPDATE_DIR}/${UPDATE_TGZ_FILE}"
  fi
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (SUNCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    -e SUNCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SUNCLAW_INSTALL_METHOD=npm \
    -e SUNCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SUNCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e SUNCLAW_INSTALL_LATEST_OUT="/out/latest" \
  ...
  run_install_smoke_container --rm -t \
    ...
    ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
    -e SUNCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SUNCLAW_INSTALL_SMOKE_MODE=update \
    -e SUNCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e SUNCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e SUNCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SUNCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e SUNCLAW_NO_ONBOARD=1 \
    -e SUNCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (SUNCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    run_install_smoke_container --rm -t \
      ...
      ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
      -e SUNCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e SUNCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e SUNCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e SUNCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e SUNCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e SUNCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_FRESHNESS" == "1" ]]; then
  echo "==> Skip installer npm freshness smoke (SUNCLAW_INSTALL_SMOKE_SKIP_FRESHNESS=1)"
else
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
    ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
    -e SUNCLAW_INSTALL_URL="$FRESHNESS_INSTALL_URL" \
    -e SUNCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SUNCLAW_INSTALL_SMOKE_MODE=freshness \
    -e SUNCLAW_INSTALL_FRESHNESS_VERSION="${SUNCLAW_INSTALL_FRESHNESS_VERSION:-latest}" \
    -e SUNCLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE="$FRESHNESS_MIN_RELEASE_AGE" \
    -e SUNCLAW_INSTALL_FRESHNESS_NPM_VERSION="$FRESHNESS_NPM_VERSION" \
    -e SUNCLAW_NO_ONBOARD=1 \
    -e SUNCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (SUNCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  run_install_smoke_container --rm -t \
    ...
    -e SUNCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SUNCLAW_INSTALL_METHOD=npm \
    -e SUNCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e SUNCLAW_NO_ONBOARD=1 \
    -e SUNCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${SUNCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (SUNCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
run_install_smoke_container --rm -t \
  ...
  "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
  -e SUNCLAW_INSTALL_URL="$INSTALL_URL" \
  -e SUNCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e SUNCLAW_NO_ONBOARD=1 \
  -e SUNCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
