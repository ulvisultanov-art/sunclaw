...
Optional custom relay:
  SUNCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-beta-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/SunClaw.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.docs.sunclaw.complex.az"
PUSH_RELAY_BASE_URL="${SUNCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid SUNCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid SUNCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid SUNCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid SUNCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(SUNCLAW_URL_SLASH)$(SUNCLAW_URL_SLASH)#g'
)"
...
SUNCLAW_CODE_SIGN_STYLE = Automatic
SUNCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
SUNCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
SUNCLAW_APP_BUNDLE_ID = ai.sunclaw.client
SUNCLAW_SHARE_BUNDLE_ID = ai.sunclaw.client.share
SUNCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.sunclaw.client.activitywidget
SUNCLAW_WATCH_APP_BUNDLE_ID = ai.sunclaw.client.watchkitapp
SUNCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.sunclaw.client.watchkitapp.extension
SUNCLAW_APP_PROFILE =
SUNCLAW_SHARE_PROFILE =
SUNCLAW_PUSH_TRANSPORT = relay
SUNCLAW_PUSH_DISTRIBUTION = official
SUNCLAW_URL_SLASH = /
SUNCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
SUNCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
