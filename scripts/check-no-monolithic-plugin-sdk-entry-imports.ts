...
import { discoverSunClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']sunclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']sunclaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']sunclaw\/plugin-sdk\/channel-runtime["']/,
    label: "sunclaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']sunclaw\/plugin-sdk\/config-runtime["']/,
    label: "sunclaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']sunclaw\/plugin-sdk\/infra-runtime["']/,
    label: "sunclaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic sunclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy sunclaw/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused sunclaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
