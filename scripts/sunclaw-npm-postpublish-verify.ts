...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./sunclaw-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `sunclaw@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`sunclaw@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
export function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@sunclaw/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "sunclaw.cmd")
    : pathPosix.join(prefixDir, "bin", "sunclaw");
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `sunclaw-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "sunclaw");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed sunclaw binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`sunclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error("Usage: node --import tsx scripts/sunclaw-npm-postpublish-verify.ts <version>");
  }
  ...
  console.log(
    `sunclaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`sunclaw-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
