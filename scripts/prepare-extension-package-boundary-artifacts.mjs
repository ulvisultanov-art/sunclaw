...
const ROOT_SHIMS_MAX_OLD_SPACE_SIZE =
  process.env.SUNCLAW_ROOT_SHIMS_MAX_OLD_SPACE_SIZE?.trim() || "8192";
...
export function resolveBoundaryRootShimsTimeoutMs(env = process.env) {
  const raw = env.SUNCLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS?.trim();
  if (!raw) {
    return 300_000;
  }
  return parsePositiveInt(raw, "SUNCLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS");
}
