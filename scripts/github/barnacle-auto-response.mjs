...
const thirdPartyExtensionMessage =
  "Please publish this as a third-party plugin on [ClawHub](https://clawhub.complex.az) instead of adding it to the core repo. Docs: https://docs.sunclaw.complex.az/plugin and https://docs.sunclaw.complex.az/clawhub";
...
const rules = [
  {
    ...
    close: true,
    message:
      "Thanks for the contribution! New skills should be published on [ClawHub](https://clawhub.complex.az) for everyone to use. We’re keeping the core lean on skills, so I’m closing this out.",
  },
  {
    label: "r: support",
    close: true,
    message:
      "Please use [our support server](https://discord.gg/clawd) and ask in #help or #users-helping-users to resolve this, or follow the stuck FAQ at https://docs.sunclaw.complex.az/help/faq#im-stuck-whats-the-fastest-way-to-get-unstuck.",
  },
  {
    label: "r: false-positive",
    close: true,
    message:
      "Closing this because it looks like a false positive or reclassification-only report rather than an actionable SunClaw bug. If this is still a real issue, please open a fresh report with concrete reproduction steps and current-version details.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    message:
      "BlueBubbles is deprecated and no longer ships as a bundled SunClaw channel. Use iMessage via `imsg` instead: https://docs.sunclaw.complex.az/channels/imessage. If this needs to stay BlueBubbles-backed, publish it as a third-party plugin on ClawHub instead of adding it back to core.",
  },
  {
    label: "r: moltbook",
    close: true,
    ...
    message:
      "SunClaw is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  ...
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for SunClaw.",
  },
  "r: spam": {
    color: "B60205",
...
const candidateActionRules = [
  {
    ...
    close: true,
    message:
      "Closing this PR because it does not include real behavior proof. Please reopen or resubmit with after-fix evidence from a real SunClaw setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count. Unit tests, mocks, snapshots, lint, typechecks, and CI are supplemental only.",
  },
  {
    label: candidateLabels.mockOnlyProof,
    close: true,
    message:
      "Closing this PR because the proof only shows tests, mocks, snapshots, lint, typechecks, or CI. Please reopen or resubmit with after-fix evidence from a real SunClaw setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the docs-only change is too low-signal for the core repo. Please reopen or resubmit with a concrete SunClaw docs gap and linked context.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the template is mostly blank and does not describe a concrete SunClaw problem, fix, or test plan. Please reopen or resubmit with the missing context filled in.",
  },
];
...
function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/sunclaw\/sunclaw\/(?:issues|pull)\/\d+)/i.test(text);
}
...
export function classifyPullRequestCandidateLabels(pullRequest, files) {
  ...
  const addsPluginManifest = files.some(
    (file) =>
      file.status === "added" && /^extensions\/[^/]+\/sunclaw\.plugin\.json$/i.test(file.filename),
  );
  if (
    !clearDesignContext &&
    (addsPluginManifest ||
