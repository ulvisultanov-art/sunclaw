...
REMOTE_URL="${SUNCLAW_REF_REMOTE:-https://github.com/ulvisultanov-art/sunclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-sunclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves SunClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve SunClaw ref: ${REF}" >&2
exit 1
