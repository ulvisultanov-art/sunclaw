...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.SUNCLAW_VITEST_MAX_WORKERS || env.SUNCLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      SUNCLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.SUNCLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.SUNCLAW_TEST_PROJECTS_PARALLEL &&
        !baseEnv.SUNCLAW_VITEST_MAX_WORKERS &&
        !baseEnv.SUNCLAW_TEST_WORKERS &&
