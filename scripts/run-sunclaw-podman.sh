#!/usr/bin/env bash
# Rootless SunClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-sunclaw-podman.sh launch        # Start gateway
#   ./scripts/run-sunclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   sunclaw --container sunclaw dashboard --no-open
#   sunclaw --container sunclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      SUNCLAW_GATEWAY_TOKEN|SUNCLAW_PODMAN_CONTAINER|SUNCLAW_PODMAN_IMAGE|SUNCLAW_IMAGE|SUNCLAW_PODMAN_PULL|SUNCLAW_PODMAN_RUN_TIMEOUT|SUNCLAW_PODMAN_GATEWAY_HOST_PORT|SUNCLAW_GATEWAY_PORT|SUNCLAW_PODMAN_BRIDGE_HOST_PORT|SUNCLAW_BRIDGE_PORT|SUNCLAW_GATEWAY_BIND|SUNCLAW_PODMAN_USERNS|SUNCLAW_BIND_MOUNT_OPTIONS|SUNCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-sunclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${SUNCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.sunclaw}"
ENV_FILE="${SUNCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${SUNCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.sunclaw}"
ENV_FILE="${SUNCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${SUNCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${SUNCLAW_PODMAN_CONTAINER:-sunclaw}"
SUNCLAW_IMAGE="${SUNCLAW_PODMAN_IMAGE:-${SUNCLAW_IMAGE:-sunclaw:local}}"
PODMAN_PULL="${SUNCLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${SUNCLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${SUNCLAW_PODMAN_GATEWAY_HOST_PORT:-${SUNCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${SUNCLAW_PODMAN_BRIDGE_HOST_PORT:-${SUNCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${SUNCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$SUNCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v sunclaw >/dev/null 2>&1; then
    return 0
  fi
  SUNCLAW_CONTAINER="" SUNCLAW_CONFIG_DIR="$config_dir" \
    sunclaw config get gateway.bind 2>/dev/null || true
}
...
# SUNCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${SUNCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'SUNCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v sunclaw >/dev/null 2>&1; then
    echo "Warning: sunclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    SUNCLAW_CONTAINER="" SUNCLAW_CONFIG_DIR="$config_dir" \
      sunclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    SUNCLAW_CONTAINER="" SUNCLAW_CONFIG_DIR="$config_dir" \
      sunclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  SUNCLAW_CONTAINER="" SUNCLAW_CONFIG_DIR="$config_dir" \
    sunclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${SUNCLAW_GATEWAY_TOKEN:-}" ]]; then
  export SUNCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "SUNCLAW_GATEWAY_TOKEN" "$SUNCLAW_GATEWAY_TOKEN"
  echo "Generated SUNCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/sunclaw.json"
...
PODMAN_USERNS="${SUNCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported SUNCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (SUNCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${SUNCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${SUNCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$SUNCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.sunclaw/.npm \
    -e SUNCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.sunclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.sunclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$SUNCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME sunclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME sunclaw devices approve --latest  # if pairing required"
