...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/sunclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SunClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/sunclaw/scripts/mobile-reauth.sh'" --id sunclaw-auth
        ;;
    *)
