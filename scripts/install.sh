...
# SunClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one SunClaw."
...
resolve_sunclaw_effective_home() {
    local sunclaw_home="${SUNCLAW_HOME:-}"
    if [[ -z "$sunclaw_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$sunclaw_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$sunclaw_home" == \~/* ]]; then
        echo "${HOME}${sunclaw_home:1}"
        return
    fi
    echo "$sunclaw_home"
}
...
GUM_VERSION="${SUNCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 SunClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://docs.sunclaw.complex.az/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$SUNCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.sunclaw.complex.az/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_sunclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.sunclaw-* "$npm_root"/sunclaw 2>/dev/null || true
}

extract_sunclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_sunclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/sunclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/sunclaw"|"/usr/local/bin/sunclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/sunclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale sunclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing sunclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing SunClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing SunClaw package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_sunclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "SunClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*sunclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_sunclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_sunclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "SunClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an sunclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${SUNCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${SUNCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((SUNCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${SUNCLAW_NO_ONBOARD:-0}
NO_PROMPT=${SUNCLAW_NO_PROMPT:-0}
DRY_RUN=${SUNCLAW_DRY_RUN:-0}
INSTALL_METHOD=${SUNCLAW_INSTALL_METHOD:-}
SUNCLAW_VERSION=${SUNCLAW_VERSION:-latest}
USE_BETA=${SUNCLAW_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_sunclaw_effective_home)/sunclaw"
GIT_DIR=${SUNCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${SUNCLAW_GIT_UPDATE:-1}
NPM_LOGLEVEL="${SUNCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${SUNCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${SUNCLAW_VERIFY_INSTALL:-0}"
SUNCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
SunClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/sunclaw)
...
Environment variables:
  SUNCLAW_INSTALL_METHOD=git|npm
  SUNCLAW_VERSION=latest|next|<semver>|<spec>
  SUNCLAW_BETA=0|1
  SUNCLAW_GIT_DIR=...
  SUNCLAW_GIT_UPDATE=0|1
  SUNCLAW_NO_PROMPT=1
  SUNCLAW_VERIFY_INSTALL=1
  SUNCLAW_DRY_RUN=1
  SUNCLAW_NO_ONBOARD=1
  SUNCLAW_VERBOSE=1
  SUNCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected SunClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a SunClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_sunclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"sunclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://docs.sunclaw.complex.az/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future SunClaw updates; use npm i -g sunclaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_sunclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/sunclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/sunclaw" ]]; then
        ln -sf "$npm_root/sunclaw/dist/entry.js" "${npm_bin}/sunclaw"
        ui_info "Created sunclaw bin link at ${npm_bin}/sunclaw"
    fi
    return 0
}

# Check for existing SunClaw installation
check_existing_sunclaw() {
    if [[ -n "$(type -P sunclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing SunClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_sunclaw_ref() {
    local requested="${SUNCLAW_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "sunclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_sunclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${SUNCLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_sunclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/sunclaw"
        ...
        local version=""
        version="$(sunclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_sunclaw_npm_root_candidates)
}

warn_duplicate_sunclaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_sunclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple SunClaw global installs detected"
    echo "  Different Node/npm environments can run different SunClaw versions."

    local active_node active_npm active_sunclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_sunclaw="${SUNCLAW_BIN:-}"
    if [[ -z "$active_sunclaw" ]]; then
        active_sunclaw="$(type -P sunclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active sunclaw: ${INFO}${active_sunclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make sunclaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

sunclaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "sunclaw"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${SUNCLAW_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_sunclaw_not_found() {
    ui_warn "Installed, but sunclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t sunclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named sunclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_sunclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P sunclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/sunclaw" ]]; then
        echo "${npm_bin}/sunclaw"
        return 0
    fi
...
install_sunclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/ulvisultanov-art/sunclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing SunClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing SunClaw from GitHub (${repo_url})"
    fi
    ...
    local git_ref
    git_ref="$(resolve_git_sunclaw_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_sunclaw_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building SunClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/sunclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/sunclaw"
    ui_success "SunClaw wrapper installed to \$HOME/.local/bin/sunclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view sunclaw dist-tags.beta 2>/dev/null || true)"
...
is_sunclaw_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#sunclaw@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:sunclaw/sunclaw($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/sunclaw/sunclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]sunclaw/sunclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/sunclaw/sunclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:sunclaw/sunclaw(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:sunclaw/sunclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_sunclaw() {
    local package_name="sunclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            SUNCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="sunclaw"
        else
            SUNCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${SUNCLAW_VERSION}" ]]; then
        SUNCLAW_VERSION="latest"
    fi

    if is_sunclaw_source_package_install_spec "${SUNCLAW_VERSION}"; then
        ui_error "npm installs do not support SunClaw GitHub source targets like '${SUNCLAW_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${SUNCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${SUNCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing SunClaw v${resolved_version}"
    else
        ui_info "Installing SunClaw (${SUNCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${SUNCLAW_VERSION}")"

    if ! install_sunclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_sunclaw_paths
        install_sunclaw_npm "${install_spec}"
    fi

    if [[ "${SUNCLAW_VERSION}" == "latest" && "${package_name}" == "sunclaw" ]]; then
        if ! resolve_sunclaw_bin &> /dev/null; then
            ui_warn "npm install sunclaw@latest failed; retrying sunclaw@next"
            cleanup_npm_sunclaw_paths
            install_sunclaw_npm "sunclaw@next"
        fi
    fi

    ensure_sunclaw_bin_link || true

    ui_success "SunClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${SUNCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_sunclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (sunclaw not on PATH yet)"
        warn_sunclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${SUNCLAW_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_sunclaw_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.sunclaw/workspace-${profile}"
    else
        echo "${effective_home}/.sunclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_sunclaw_effective_home)"
    local config_path="${SUNCLAW_CONFIG_PATH:-$effective_home/.sunclaw/sunclaw.json}"
    local legacy_config_path="${HOME}/.sunclaw/sunclaw.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${SUNCLAW_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(sunclaw_command_for_user "${SUNCLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but sunclaw not on PATH; skipping onboarding"
        warn_sunclaw_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(sunclaw_command_for_user "$claw")"
...
if ! declare -F extract_sunclaw_semver >/dev/null 2>&1; then
...
resolve_sunclaw_version() {
    ...
    local claw="${SUNCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v sunclaw &> /dev/null; then
        claw="$(command -v sunclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_sunclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/sunclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/sunclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "sunclaw daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: sunclaw not on PATH yet"
        warn_sunclaw_not_found
        return 1
    fi

    run_quiet_step "Checking SunClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: sunclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_sunclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found SunClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set SUNCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_sunclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing SunClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g sunclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g sunclaw 2>/dev/null || true
        ...
        install_sunclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/sunclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/sunclaw"
    ...
    SUNCLAW_BIN="$(resolve_sunclaw_bin || true)"
    warn_duplicate_sunclaw_global_installs || true
    ...
    installed_version=$(resolve_sunclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 SunClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 SunClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/sunclaw"
        ui_kv "Update command" "sunclaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://docs.sunclaw.complex.az/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running sunclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                SUNCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                SUNCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                SUNCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (sunclaw not on PATH yet)"
                    warn_sunclaw_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v sunclaw &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (sunclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if SUNCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: sunclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${SUNCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
