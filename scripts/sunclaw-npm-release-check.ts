...
const EXPECTED_REPOSITORY_URL = "https://github.com/ulvisultanov-art/sunclaw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@sunclaw/fs-safe";
...
const skipPackValidationEnv = "SUNCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "SUNCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "sunclaw") {
    errors.push(`package.json name must be "sunclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.sunclaw !== "sunclaw.mjs") {
    errors.push(
      `package.json bin.sunclaw must be "sunclaw.mjs"; found "${pkg.bin?.sunclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`sunclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `sunclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
