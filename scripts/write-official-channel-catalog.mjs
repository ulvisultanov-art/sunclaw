...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.sunclaw) ? packageJson.sunclaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.sunclaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.sunclaw?.channel?.id) || left.name;
    const rightId = trimString(right.sunclaw?.channel?.id) || right.name;
