...
export function resolveWindowsExtensionChunkSize(env = process.env) {
  return resolvePositiveEnvIntWithFallback(
    env,
    "SUNCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE",
...
export function shouldRunOxlintShardsSerial({
  ...
  const explicitMode = env.SUNCLAW_OXLINT_SHARDS_SERIAL?.trim();
  if (explicitMode === "1") {
    return true;
  }
  if (platform === "win32") {
  ...
  const localCheckMode = env.SUNCLAW_LOCAL_CHECK_MODE?.trim().toLowerCase();
...
export async function main(extraArgs = process.argv.slice(2), runtimeEnv = process.env) {
  ...
  const releaseLock =
    env.SUNCLAW_OXLINT_SKIP_LOCK === "1"
      ? () => {}
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "SUNCLAW_OXLINT_SHARD_CONCURRENCY");
...
export function resolveShardHeartbeatMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "SUNCLAW_OXLINT_SHARD_HEARTBEAT_MS",
...
export function resolveShardKillGraceMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "SUNCLAW_OXLINT_SHARD_KILL_GRACE_MS",
