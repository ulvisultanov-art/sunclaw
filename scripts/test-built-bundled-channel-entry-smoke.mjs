...
process.env.SUNCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "SUNCLAW_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "SUNCLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/sunclaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "sunclaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.sunclaw?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.sunclaw.extensions) && packageJson.sunclaw.extensions.length > 0
    ...
    const setupEntry = packageJson.sunclaw.setupEntry;
