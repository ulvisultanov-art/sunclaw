...
const DEFAULT_OUTPUT_NAME = "sunclaw-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "SUNCLAW_TRUSTED_PACKAGE_TOKEN";
...
export const SUNCLAW_PACKAGE_SPEC_RE =
  /^sunclaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-sunclaw-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateSunClawPackageSpec(spec) {
  if (!SUNCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be sunclaw@alpha, sunclaw@beta, sunclaw@latest, or an exact SunClaw release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateSunClawPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an SunClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `sunclaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^sunclaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no SunClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking SunClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-sunclaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `SunClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "sunclaw") {
    throw new Error(`package candidate must be named "sunclaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
