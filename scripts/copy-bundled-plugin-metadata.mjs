...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.SUNCLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "sunclaw.plugin.json");
    ...
    if (packageJson.sunclaw && "extensions" in packageJson.sunclaw) {
      packageJson.sunclaw = {
        ...packageJson.sunclaw,
        extensions: rewritePackageExtensions(packageJson.sunclaw.extensions),
        ...(typeof packageJson.sunclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.sunclaw.setupEntry) }
          : {}),
      };
    }
