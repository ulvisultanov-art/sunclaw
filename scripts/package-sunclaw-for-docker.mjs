#!/usr/bin/env node
// Builds the SunClaw package artifact used by Docker E2E.
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building SunClaw package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "SUNCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestSunClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed SunClaw tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

export async function packSunClawPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing SunClaw package");
  ...
  try {
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "SUNCLAW_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestSunClawTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing SunClaw package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "SUNCLAW_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packSunClawPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking SunClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-sunclaw-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "SUNCLAW_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
