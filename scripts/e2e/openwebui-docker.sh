#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized SunClaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-openwebui-e2e" SUNCLAW_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${SUNCLAW_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${SUNCLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${SUNCLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${SUNCLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${SUNCLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${SUNCLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="sunclaw-openwebui-e2e-$$"
GW_NAME="sunclaw-openwebui-gateway-$$"
OW_NAME="sunclaw-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${SUNCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${SUNCLAW_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${SUNCLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${SUNCLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${SUNCLAW_TESTBOX_PROFILE_FILE:-$HOME/.sunclaw-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/sunclaw-openwebui-config.XXXXXX.json)"
    SUNCLAW_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    sunclaw_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "SUNCLAW_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="SunClaw E2E" \
  -e WEBUI_SECRET_KEY="sunclaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="SunClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="sunclaw/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "SUNCLAW_HTTP_PROBE_BEARER='$TOKEN' SUNCLAW_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> SunClaw smoke..."
