...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${SUNCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${SUNCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${SUNCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${SUNCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "SUNCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (SUNCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set SUNCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sunclaw-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack SunClaw tarball"
...
main() {
  ...
  echo "==> Bun global install packed SunClaw"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  sunclaw_bin="$BUN_INSTALL/bin/sunclaw"
  if [ ! -x "$sunclaw_bin" ]; then
    sunclaw_bin="$(command -v sunclaw || true)"
  fi
  if [ -z "$sunclaw_bin" ] || [ ! -x "$sunclaw_bin" ]; then
    echo "Bun global install did not create an executable sunclaw binary" >&2
    exit 1
  fi

  echo "==> SunClaw version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$sunclaw_bin" --version

  echo "==> SunClaw image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$sunclaw_bin" infer image providers --json)"
  SUNCLAW_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}
