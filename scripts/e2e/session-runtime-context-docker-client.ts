...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".sunclaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "sunclaw.runtime-context",
  ...
  assert(
    !userText.includes("SUNCLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.SUNCLAW_STATE_DIR = path.join(root, ".sunclaw");
  process.env.SUNCLAW_CONFIG_PATH = path.join(process.env.SUNCLAW_STATE_DIR, "sunclaw.json");
  try {
    ...
    if (process.env.SUNCLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
