...
function makeParams(commandBody: string, cfg: SunClawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: SunClawConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("sunclaw-crestodian-");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env SUNCLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/sunclaw-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/sunclaw-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as SunClawConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "SUNCLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/sunclaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/sunclaw-work",
    ),
