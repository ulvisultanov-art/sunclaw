#!/usr/bin/env bash
# Installs SunClaw from a prepared package tarball, installs @sunclaw/codex
...
CANDIDATE_ROOT="${SUNCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-codex-npm-plugin-live-e2e" SUNCLAW_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${SUNCLAW_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${SUNCLAW_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${SUNCLAW_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${SUNCLAW_CODEX_NPM_PLUGIN_PROFILE_FILE:-${SUNCLAW_TESTBOX_PROFILE_FILE:-$HOME/.sunclaw-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${SUNCLAW_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${SUNCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "SUNCLAW_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires SUNCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sunclaw-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/sunclaw-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      cat /tmp/sunclaw-codex-plugin-pack.log >&2 || true
      exit 1
    fi
...
SUNCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e SUNCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${SUNCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e SUNCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${SUNCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e SUNCLAW_CODEX_NPM_PLUGIN_MODEL="${SUNCLAW_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e SUNCLAW_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${SUNCLAW_CODEX_NPM_PLUGIN_SPEC:?missing SUNCLAW_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${SUNCLAW_CODEX_NPM_PLUGIN_MODEL:?missing SUNCLAW_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="SUNCLAW-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${SUNCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  sunclaw_e2e_dump_logs \
    ...
    /tmp/sunclaw-codex-plugin-install.log \
    /tmp/sunclaw-codex-plugin-enable.log \
    /tmp/sunclaw-codex-plugins-list.json \
    /tmp/sunclaw-codex-plugin-inspect.json \
    /tmp/sunclaw-codex-preflight.log \
    /tmp/sunclaw-codex-agent.json \
    /tmp/sunclaw-codex-agent.err \
    /tmp/sunclaw-codex-agent-turn1.json \
    /tmp/sunclaw-codex-agent-turn1.err \
    /tmp/sunclaw-codex-agent-turn2.json \
    /tmp/sunclaw-codex-agent-turn2.err \
    /tmp/sunclaw-codex-plugin-uninstall.log \
    /tmp/sunclaw-codex-plugins-list-after-uninstall.json \
    /tmp/sunclaw-codex-agent-after-uninstall.json \
    /tmp/sunclaw-codex-agent-after-uninstall.err
}
...
sunclaw plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/sunclaw-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
sunclaw plugins enable codex >/tmp/sunclaw-codex-plugin-enable.log 2>&1

sunclaw plugins list --json >/tmp/sunclaw-codex-plugins-list.json
sunclaw plugins inspect codex --runtime --json >/tmp/sunclaw-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/sunclaw-codex-preflight.log 2>&1 </dev/null
...
echo "Running SunClaw local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the SunClaw Codex plugin." \
  /tmp/sunclaw-codex-agent-turn1.json \
  /tmp/sunclaw-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
sunclaw plugins uninstall codex --force >/tmp/sunclaw-codex-plugin-uninstall.log 2>&1
sunclaw plugins list --json >/tmp/sunclaw-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if sunclaw agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/sunclaw-codex-agent-after-uninstall.json 2>/tmp/sunclaw-codex-agent-after-uninstall.err; then
  echo "Expected SunClaw agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/sunclaw-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/sunclaw-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  cat /tmp/sunclaw-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
