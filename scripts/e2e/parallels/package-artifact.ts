...
export function resolveSunClawRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("sunclaw@") ? rawValue.slice("sunclaw@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`sunclaw@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "sunclaw", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no sunclaw registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packSunClaw(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "sunclaw-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `sunclaw-main-${shortHead}.tgz`);
...
async function acquirePackageLock(lockDir: string, ownerToken: string): Promise<void> {
  const timeoutMs = readPositiveIntEnv("SUNCLAW_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("SUNCLAW_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
