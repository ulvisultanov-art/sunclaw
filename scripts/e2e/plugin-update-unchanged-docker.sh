#!/usr/bin/env bash
# Verifies `sunclaw plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-plugin-update-e2e" SUNCLAW_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${SUNCLAW_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${SUNCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
SUNCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
