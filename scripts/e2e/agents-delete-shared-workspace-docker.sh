...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-agents-delete-shared-workspace-e2e:local" SUNCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE)"
SKIP_BUILD="${SUNCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD:-0}"
DOCKER_COMMAND_TIMEOUT="${SUNCLAW_AGENTS_DELETE_SHARED_WORKSPACE_DOCKER_COMMAND_TIMEOUT:-300s}"
SUNCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 agents-delete-shared-workspace empty)"
...
run_logged agents-delete-shared-workspace docker_e2e_docker_cmd run --rm \
  ...
  -e SUNCLAW_SKIP_BROWSER_CONTROL_SERVER=1 \
  -e SUNCLAW_SKIP_ACPX_RUNTIME=1 \
  -e SUNCLAW_SKIP_ACPX_RUNTIME_PROBE=1 \
  -e SUNCLAW_GATEWAY_TOKEN=agents-delete-shared-workspace-token \
...
run_sunclaw() {
  ...
  if [ -f /app/sunclaw.mjs ]; then
    node /app/sunclaw.mjs "$@"
    return
  fi
  echo "sunclaw CLI not found in Docker image" >&2
  exit 1
}
...
mkdir -p "$SUNCLAW_STATE_DIR" "$SHARED_WORKSPACE"
node scripts/e2e/lib/fixture.mjs agents-delete-config

run_sunclaw agents delete ops --force --json > "$output_file"
