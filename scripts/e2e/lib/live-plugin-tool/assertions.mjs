...
function agentOutputPath() {
  return process.env.SUNCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/sunclaw-agent.json";
}

function agentErrorPath() {
  return process.env.SUNCLAW_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/sunclaw-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/sunclaw-plugin-inspect.json")
    ? readJson("/tmp/sunclaw-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "sunclaw.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const inspect = readJson("/tmp/sunclaw-plugin-inspect.json");
