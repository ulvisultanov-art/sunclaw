...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/sunclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/sunclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${SUNCLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${SUNCLAW_SECRET_ENV_NAME:-}" ] && [ -n "${SUNCLAW_SECRET_FILE:-}" ] && [ -f "$SUNCLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$SUNCLAW_SECRET_FILE")"
  rm -f "$SUNCLAW_SECRET_FILE"
  export "${SUNCLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "SUNCLAW_SECRET_ENV_NAME=$api_key_env" \
      "SUNCLAW_SECRET_FILE=$secret_path" \
      "SUNCLAW_WRAPPER_FILE=$wrapper_path" \
