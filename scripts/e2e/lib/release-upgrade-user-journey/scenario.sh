...
export OPENAI_API_KEY="sk-sunclaw-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="SUNCLAW_E2E_OK_RELEASE_UPGRADE"
...
BASELINE_SPEC="${SUNCLAW_RELEASE_UPGRADE_BASELINE_SPEC:-sunclaw@latest}"
export SUCCESS_MARKER MOCK_REQUEST_LOG CLICKCLACK_STATE

candidate_version="$(
  tar -xOf "${SUNCLAW_CURRENT_PACKAGE_TGZ:?missing SUNCLAW_CURRENT_PACKAGE_TGZ}" package/package.json |
...
if ! sunclaw_e2e_maybe_timeout "${SUNCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v sunclaw >/dev/null
baseline_root="$(sunclaw_e2e_package_root)"
baseline_entry="$(sunclaw_e2e_package_entrypoint "$baseline_root")"
...
sunclaw plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
sunclaw release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
sunclaw_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate SunClaw package"
...
sunclaw release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
