...
function writePluginDemo([dir]) {
  ...
  writePluginManifest(path.join(dir, "sunclaw.plugin.json"), "demo-plugin", {
...
function writePlugin([dir, id, version, method, name]) {
  ...
  writePluginManifest(path.join(dir, "sunclaw.plugin.json"), id);
}
...
function writePluginMarketplace(args) {
  ...
  writeJson(path.join(process.env.HOME, ".claude", "plugins", "known_marketplaces.json"), {
    "claude-fixtures": {
      ...
      source: { type: "github", repo: "sunclaw/fixture-marketplace" },
    },
  });
}
