...
export OPENAI_API_KEY="sk-sunclaw-release-media-memory"
export SUNCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="SUNCLAW_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/sunclaw-release-media-memory.XXXXXX)"
...
entry="$(sunclaw_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'entry=%s\n' "$entry"
  printf 'HOME=%s\n' "$HOME"
  printf 'SUNCLAW_HOME=%s\n' "$SUNCLAW_HOME"
  printf 'SUNCLAW_STATE_DIR=%s\n' "$SUNCLAW_STATE_DIR"
  printf 'SUNCLAW_CONFIG_PATH=%s\n' "$SUNCLAW_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(sunclaw_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$SUNCLAW_CONFIG_PATH" "$CONFIG_JSON"
sunclaw plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$SUNCLAW_STATE_DIR/workspace/memory"
...
cat >"$SUNCLAW_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

sunclaw memory index --force >"$INDEX_LOG" 2>&1
sunclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
sunclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
