...
const inspect = readJson("/tmp/sunclaw-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@sunclaw/codex")) {
  throw new Error(`expected @sunclaw/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @sunclaw/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@sunclaw/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@sunclaw/codex");
...
const list = readJson("/tmp/sunclaw-plugins-list.json");
...
if (authRaw.includes("sk-sunclaw-codex-on-demand-e2e")) {
