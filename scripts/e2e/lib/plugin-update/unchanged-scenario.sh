...
sunclaw_e2e_install_package /tmp/sunclaw-install.log "mounted SunClaw package" /tmp/npm-prefix
...
SUNCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/sunclaw-e2e-registry.log 2>&1 &
registry_pid=$!
trap 'sunclaw_e2e_stop_process "${registry_pid:-}"' EXIT

if ! node "$probe" wait-registry; then
  echo "Local npm metadata registry failed to start"
  cat /tmp/sunclaw-e2e-registry.log || true
  exit 1
fi

before_config_hash=""
if [ "$SUNCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$SUNCLAW_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${SUNCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"

node "$probe" snapshot > /tmp/plugin-update-before.json

set +e
sunclaw_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update @example/lossless-claw > /tmp/plugin-update-output.log 2>&1
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$SUNCLAW_CONFIG_PATH" | awk '{print $1}')"
