...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-live-plugin-tool-e2e" SUNCLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${SUNCLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${SUNCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${SUNCLAW_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${SUNCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${SUNCLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${SUNCLAW_TESTBOX_PROFILE_FILE:-$HOME/.sunclaw-testbox-live.profile}}"
run_log=""
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${SUNCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "SUNCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires SUNCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
SUNCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e SUNCLAW_LIVE_PLUGIN_TOOL_MODEL="${SUNCLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "SUNCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${SUNCLAW_LIVE_PLUGIN_TOOL_MODEL:?missing SUNCLAW_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@sunclaw/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="SunClaw E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  sunclaw_e2e_dump_logs \
    ...
    /tmp/sunclaw-plugin-install.log \
    /tmp/sunclaw-plugin-enable.log \
    /tmp/sunclaw-plugins-list.json \
    /tmp/sunclaw-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/sunclaw-live-plugin-tool.XXXXXX)"
...
sunclaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/sunclaw-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
sunclaw plugins enable "$PLUGIN_ID" >/tmp/sunclaw-plugin-enable.log 2>&1
sunclaw plugins list --json >/tmp/sunclaw-plugins-list.json
sunclaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/sunclaw-plugin-inspect.json
...
sunclaw agent --local \
  ...
  --thinking off \
  --timeout "${SUNCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/sunclaw-agent.json 2>/tmp/sunclaw-agent.err
