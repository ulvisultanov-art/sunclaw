...
# git installs. Both fixtures come from the same prepared SunClaw npm tarball.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "$ROOT_DIR/scripts/lib/docker-e2e-image.sh"
source "$ROOT_DIR/scripts/lib/docker-e2e-package.sh"
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-doctor-install-switch-e2e" SUNCLAW_DOCTOR_INSTALL_SWITCH_E2E_IMAGE)"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz doctor-switch "${SUNCLAW_CURRENT_PACKAGE_TGZ:-}")"
