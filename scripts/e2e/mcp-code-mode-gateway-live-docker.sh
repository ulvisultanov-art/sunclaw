...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-mcp-code-mode-gateway-live-e2e" SUNCLAW_IMAGE)"
PORT="${SUNCLAW_MCP_CODE_MODE_LIVE_GATEWAY_PORT:-18789}"
TOKEN="mcp-code-mode-live-e2e-$(date +%s)-$$"
CONTAINER_NAME="sunclaw-mcp-code-mode-live-e2e-$$"
CLIENT_LOG="$(mktemp -t sunclaw-mcp-code-mode-live-log.XXXXXX)"
PROFILE_FILE="${SUNCLAW_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${SUNCLAW_TESTBOX_PROFILE_FILE:-$HOME/.sunclaw-testbox-live.profile}}"
...
# The profile is only a credential source. Keep this lane's SunClaw runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset SUNCLAW_TESTBOX

docker_e2e_build_or_reuse "$IMAGE_NAME" mcp-code-mode-gateway-live
SUNCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "SUNCLAW_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "SUNCLAW_MCP_CODE_MODE_MODEL=${SUNCLAW_MCP_CODE_MODE_LIVE_MODEL:-sunclaw/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(sunclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    sunclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480
