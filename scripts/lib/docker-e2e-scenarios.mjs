...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "SUNCLAW_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${SUNCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; SUNCLAW_LIVE_DOCKER_REPO_ROOT="\${SUNCLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
function createPackageUpdateMaintenanceLanes() {
  return [
    npmLane("doctor-switch", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
      stateScenario: "empty",
      weight: 3,
    }),
    npmLane(
      "update-channel-switch",
      "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
      {
    ...
    npmLane("skill-install", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install", {
    ...
    npmLane(
      ...
      "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
      {
        stateScenario: "upgrade-survivor",
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `SUNCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} SUNCLAW_BUNDLED_PLUGIN_SWEEP_INDEX=${index} SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
function livePluginToolLane() {
  return liveLane(
    "live-plugin-tool",
    "SUNCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=300 SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-plugin-tool",
    {
...
function liveOpenAiChatToolsLane() {
  return liveLane(
    "openai-chat-tools",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-chat-tools",
    {
...
function liveMcpCodeModeGatewayLane() {
  return liveLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-mcp-code-mode-gateway",
    {
...
function kitchenSinkRpcLane() {
  return serviceLane(
    "kitchen-sink-rpc",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
    {
      resources: ["npm"],
      stateScenario: "empty",
...
export const mainLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "SUNCLAW_IMAGE=sunclaw:local-live-gateway SUNCLAW_DOCKER_BUILD_EXTENSIONS=matrix SUNCLAW_LIVE_GATEWAY_PROVIDERS=claude-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "SUNCLAW_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
  ...
  serviceLane("onboard", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane("codex-on-demand", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand", {
    resources: ["service"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  npmLane(
    ...
    "SUNCLAW_NPM_ONBOARD_CHANNEL=discord SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "SUNCLAW_NPM_ONBOARD_CHANNEL=slack SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "release-user-journey",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-typed-onboarding",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-media-memory",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-upgrade-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
  ...
  npmLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-plugin-marketplace",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  serviceLane("gateway-network", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  serviceLane(
    "agents-delete-shared-workspace",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-code-mode-gateway",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  lane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:agent-bundle-mcp-tools",
    {
      stateScenario: "empty",
    },
  ),
  lane("crestodian-rescue", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-rescue", {
    stateScenario: "empty",
  }),
  ...
  lane("plugins", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "SUNCLAW_PLUGINS_E2E_CLAWHUB=0 SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  lane("openai-image-auth", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "crestodian-first-run",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane(
    "plugin-binding-command-escape",
    "SUNCLAW_SKIP_DOCKER_BUILD=0 pnpm test:docker:plugin-binding-command-escape",
    {
      e2eImageKind: false,
      resources: ["npm"],
      stateScenario: "empty",
  ...
  lane("commitments-safety", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:commitments-safety", {
    stateScenario: "empty",
  }),
...
export const tailLanes = [
  serviceLane(
    ...
    "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "SUNCLAW_LIVE_CODEX_HARNESS_AUTH=api-key SUNCLAW_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE=1 SUNCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 SUNCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0 SUNCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0 SUNCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-codex-harness-docker.sh",
      "SUNCLAW_LIVE_CODEX_BIND=1 SUNCLAW_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts",
    ),
    {
      cacheKey: "codex-harness",
      provider: "codex-cli",
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "SUNCLAW_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "SUNCLAW_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "SUNCLAW_LIVE_ACP_BIND_AGENT=droid SUNCLAW_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "SUNCLAW_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "SUNCLAW_LIVE_ACP_BIND_AGENT=opencode SUNCLAW_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "SUNCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `SUNCLAW_DOCKER_ALL_CHUNK must be one of: ${[
