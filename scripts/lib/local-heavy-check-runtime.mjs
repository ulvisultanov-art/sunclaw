...
export function isLocalCheckEnabled(env) {
  const raw = env.SUNCLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.SUNCLAW_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.SUNCLAW_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.SUNCLAW_TSGO_PPROF_DIR);
  }
...
export function shouldAcquireLocalHeavyCheckLockForOxlint(
  ...
  if (env.SUNCLAW_OXLINT_FORCE_LOCK === "1") {
    return true;
  }
...
export function shouldAcquireLocalHeavyCheckLockForTsgo(args, env = process.env) {
  if (env.SUNCLAW_TSGO_FORCE_LOCK === "1") {
    return true;
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const timeoutMs = readPositiveInt(
    env.SUNCLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
    "SUNCLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS",
  );
  const pollMs = readPositiveInt(
    env.SUNCLAW_HEAVY_CHECK_LOCK_POLL_MS,
    DEFAULT_LOCK_POLL_MS,
    "SUNCLAW_HEAVY_CHECK_LOCK_POLL_MS",
  );
  const progressMs = readPositiveInt(
    env.SUNCLAW_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
    "SUNCLAW_HEAVY_CHECK_LOCK_PROGRESS_MS",
  );
  const staleLockMs = readPositiveInt(
    env.SUNCLAW_HEAVY_CHECK_STALE_LOCK_MS,
    DEFAULT_STALE_LOCK_MS,
    "SUNCLAW_HEAVY_CHECK_STALE_LOCK_MS",
  );
...
function resolveHeavyCheckLocksDir(cwd, env) {
  const lockScope = env.SUNCLAW_HEAVY_CHECK_LOCK_SCOPE?.trim().toLowerCase();
  if (lockScope === "worktree") {
    return path.join(resolveGitWorktreeRoot(cwd), ".artifacts", "sunclaw-local-checks");
  }

  return path.join(resolveGitCommonDir(cwd), "sunclaw-local-checks");
}
...
function readLocalCheckMode(env, defaultMode) {
  const raw = env.SUNCLAW_LOCAL_CHECK_MODE?.trim().toLowerCase();
