 ...
 * `sunclaw update`. Use it to verify the completion installation flow
...
\`sunclaw update\`, \`sunclaw doctor\`, and \`sunclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SunClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`sunclaw completion --install\` later to enable.`));
    return;
  }
