...
const PUBLISHED_INSTALLER_BASE_URL = "https://docs.sunclaw.complex.az";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "SUNCLAW_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "SUNCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `SUNCLAW_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.SUNCLAW_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "sunclaw" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.sunclaw-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "SUNCLAW_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "SUNCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("SUNCLAW_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.SUNCLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.SUNCLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.SUNCLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `sunclaw@${previousVersion}` : "sunclaw@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.SUNCLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.SUNCLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        SUNCLAW_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `sunclaw-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".sunclaw");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    SUNCLAW_HOME: lane.homeDir,
    SUNCLAW_STATE_DIR: lane.stateDir,
    SUNCLAW_CONFIG_PATH: join(lane.stateDir, "sunclaw.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.SUNCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]sunclaw\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/sunclaw[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "sunclaw@latest") {
    return "";
  }
  if (trimmed.startsWith("sunclaw@")) {
    return trimmed.slice("sunclaw@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "sunclaw@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view sunclaw@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'sunclaw.cmd'),
      (Join-Path $npmPrefix 'sunclaw.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command sunclaw -ErrorAction Stop
...
Write-Output "__SUNCLAW_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__SUNCLAW_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed sunclaw path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v sunclaw >/dev/null 2>&1",
    'printf "__SUNCLAW_PATH__=%s\\n" "$(command -v sunclaw)"',
    "sunclaw --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__SUNCLAW_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed sunclaw path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `sunclaw update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.sunclaw/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.SUNCLAW_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.SUNCLAW_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.SUNCLAW_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.SUNCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "sunclaw/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.SUNCLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.SUNCLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.SUNCLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing SUNCLAW_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.SUNCLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.SUNCLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      SUNCLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      SUNCLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>SunClaw Control</title>") &&
          html.includes("<sunclaw-app></sunclaw-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>SunClaw Control</title>")} app=${html.includes("<sunclaw-app></sunclaw-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runSunClaw(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "sunclaw.mjs");
}
