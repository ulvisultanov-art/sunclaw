...
IMAGE_NAME="$(docker_e2e_resolve_image "sunclaw-code-mode-namespace-live-e2e" SUNCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_IMAGE)"
SKIP_BUILD="${SUNCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_SKIP_BUILD:-0}"
PROFILE_FILE="${SUNCLAW_CODE_MODE_NAMESPACE_LIVE_PROFILE_FILE:-${SUNCLAW_TESTBOX_PROFILE_FILE:-$HOME/.sunclaw-testbox-live.profile}}"
run_log=""
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "SUNCLAW_CODE_MODE_LIVE_MODEL=${SUNCLAW_CODE_MODE_LIVE_MODEL:-gpt-5.4-mini}" \
  -e "SUNCLAW_CODE_MODE_LIVE_TASKS=${SUNCLAW_CODE_MODE_LIVE_TASKS:-3}" \
  -v "$ROOT_DIR":/src:ro \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
