...
#
# Suggested cron: */30 * * * * /home/admin/sunclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SunClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., sunclaw-alerts)
...
STATE_FILE="$HOME/.sunclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SunClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use sunclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SunClaw to $NOTIFY_PHONE..."
            sunclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SunClaw is down. Run: ssh l36 '~/sunclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
