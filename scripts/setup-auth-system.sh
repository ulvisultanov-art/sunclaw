#!/bin/bash
# Setup SunClaw Auth Management System
...
echo "=== SunClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'sunclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SunClaw message: Send warning via SunClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/sunclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/sunclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/sunclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now sunclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status sunclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
