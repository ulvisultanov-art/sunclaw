...
```bash
export SUNCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.sunclaw/sunclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env SUNCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `sunclaw` wrapper for guest `message send/read`; `node sunclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/sunclaw-parallels-smoke.*`
...
- host inbound nonce appears in `sunclaw message read` output
