---
name: sunclaw-qa-testing
description: Run, watch, debug, extend, or explain SunClaw qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
SUNCLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
...
5. If the user wants to watch the live UI, find the current `sunclaw-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm sunclaw qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
SUNCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `SUNCLAW_QA_PROVIDER_MODE`
  - `SUNCLAW_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `SunClaw`
  - item: `SUNCLAW_QA_CONVEX_SITE_URL`
  - item: `SUNCLAW_QA_CONVEX_SECRET_MAINTAINER`
  - item: `SUNCLAW_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `SUNCLAW QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `SUNCLAW_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `SUNCLAW_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `SunClaw/SUNCLAW_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
SUNCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
SUNCLAW_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
SUNCLAW_QA_PROVIDER_MODE="mock-openai" \
SUNCLAW_NPM_TELEGRAM_PACKAGE_SPEC="sunclaw@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `SUNCLAW_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `SUNCLAW_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `SUNCLAW_QA_PROVIDER_MODE`, map it explicitly to `SUNCLAW_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo sunclaw/sunclaw --ref main \
  -f package_spec=sunclaw@YYYY.M.D-beta.N \
  -f package_label=sunclaw@YYYY.M.D-beta.N \
...
```bash
gh api repos/sunclaw/sunclaw/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal SunClaw WhatsApp accounts in the shared pool.
...
- Validate with `pnpm sunclaw qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal SunClaw identity combines with the character.
...
```bash
pnpm sunclaw qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `SUNCLAW_HOME` sandboxed.
