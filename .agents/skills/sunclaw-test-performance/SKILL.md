---
name: sunclaw-test-performance
description: Benchmark, diagnose, and optimize SunClaw test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# SunClaw Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `SUNCLAW_VITEST_IMPORT_DURATIONS=1 SUNCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- SUNCLAW_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `release-sunclaw-plugin-testing` and Package Acceptance rather than
...
```bash
SUNCLAW_TEST_MEMORY_TRACE=1 \
SUNCLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
SUNCLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
SUNCLAW_TEST_WORKERS=2 \
SUNCLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `sunclaw-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `SUNCLAW_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  SUNCLAW_VITEST_IMPORT_DURATIONS=1 \
  SUNCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
