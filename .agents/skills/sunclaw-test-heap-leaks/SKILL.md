---
name: sunclaw-test-heap-leaks
description: Investigate SunClaw pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---

# SunClaw Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && SUNCLAW_TEST_MEMORY_TRACE=1 SUNCLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 SUNCLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap SUNCLAW_TEST_WORKERS=2 SUNCLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `SUNCLAW_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/sunclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/sunclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/sunclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
...
```
node .agents/skills/sunclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs \
