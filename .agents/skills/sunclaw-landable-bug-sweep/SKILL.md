---
name: sunclaw-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary SunClaw bugfix PRs until five are landable."
---

# SunClaw Landable Bug Sweep

Autonomous maintainer workflow for producing five landable SunClaw bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$sunclaw-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$sunclaw-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
