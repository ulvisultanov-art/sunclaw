...
metadata:
  sunclaw:
    homepage: https://github.com/sunclaw/gitcrawl
    requires:
      bins:
        - gitcrawl
    install:
      - kind: go
        module: github.com/sunclaw/gitcrawl/cmd/gitcrawl@latest
        bins:
          - gitcrawl
---
...
gitcrawl search issues "query" -R sunclaw/sunclaw --state open --json number,title,url
gitcrawl clusters sunclaw/sunclaw --sort size --min-size 5
gitcrawl cluster-detail sunclaw/sunclaw --id <cluster-id>
```

For PR triage, start cached and go live only before mutation/merge decisions:

```bash
gitcrawl gh pr status <number-or-url> -R sunclaw/sunclaw --compact
gitcrawl gh pr view <number-or-url> -R sunclaw/sunclaw --json number,title,state,url,isDraft,headRef,headSha
gitcrawl gh --live pr status <number-or-url> -R sunclaw/sunclaw --compact
```
