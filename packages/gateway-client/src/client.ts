...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@sunclaw/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@sunclaw/gateway-protocol/client-info";
import {
...
} from "@sunclaw/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@sunclaw/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@sunclaw/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for SunClaw-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside SunClaw; device signing throws
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          "Run `sunclaw doctor --fix` for guidance.",
      );
