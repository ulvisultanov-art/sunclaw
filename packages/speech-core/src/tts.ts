...
import { resolveChannelTtsVoiceDelivery } from "sunclaw/plugin-sdk/channel-targets";
import type {
  SunClawConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "sunclaw/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "sunclaw/plugin-sdk/media-runtime";
import { clampTimerTimeoutMs } from "sunclaw/plugin-sdk/number-runtime";
import {
...
import { isVerbose, logVerbose } from "sunclaw/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredSunClawTmpDir } from "sunclaw/plugin-sdk/sandbox";
...
import { stripMarkdown } from "sunclaw/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "sunclaw/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.SUNCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function resolveConfiguredSpeechVoiceModelRefs(cfg: SunClawConfig | undefined): VoiceModelRef[] {
...
function sortSpeechProvidersForAutoSelection(cfg?: SunClawConfig) {
...
function resolveTtsRuntimeConfig(cfg: SunClawConfig): SunClawConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: SunClawConfig): TtsProvider[] {
  const effectiveCfg = cfg ? resolveTtsRuntimeConfig(cfg) : undefined;
