...
import { createMockServerResponse } from "sunclaw/plugin-sdk/test-env";
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { SunClawConfig } from "../api.js";
import type { SunClawPluginApi, SunClawPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("sunclaw-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<SunClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: SunClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: SunClawConfig = {
      gateway: {
        port: 18789,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/sunclaw",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<SunClawPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as SunClawPluginApi);
