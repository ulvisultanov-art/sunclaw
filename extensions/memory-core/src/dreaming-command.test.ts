...
function resolveStoredDreaming(config: SunClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: SunClawConfig = {}) {
  const registered: { command?: SunClawPluginCommandDefinition } = {};
  let runtimeConfig: SunClawConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: SunClawConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: SunClawConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: SunClawPluginCommandDefinition) => {
