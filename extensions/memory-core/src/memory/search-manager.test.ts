...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "sunclaw/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("sunclaw/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): SunClawConfig {
  return {
    agents: {
      defaults: {
        workspace: "/tmp/workspace",
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: SunClawConfig;
  secondCfg: SunClawConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("sunclaw.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-qmd-workspace-"));
