...
import type { MemoryEmbeddingProbeResult } from "sunclaw/plugin-sdk/memory-core-host-engine-storage";
import {
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: sunclaw memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: SunClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredSunClawTmpDir(), "sunclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: SunClawConfig): string {
...
function resolveAgent(cfg: SunClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: SunClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: sunclaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredSunClawTmpDir(), "sunclaw-rem-backfill-"),
      );
      try {
