...
import { peekSystemEventEntries } from "sunclaw/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: SunClawPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: SunClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): SunClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as SunClawConfig;
