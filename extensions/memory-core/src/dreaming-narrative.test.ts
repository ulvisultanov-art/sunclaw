...
import * as memoryCoreHostRuntimeCoreModule from "sunclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "sunclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "sunclaw/plugin-sdk/session-store-runtime";
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("sunclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("sunclaw-dreaming-backfill-");
    ...
    expect(content.match(/sunclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("sunclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- sunclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- sunclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- sunclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- sunclaw:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("sunclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("sunclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("sunclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("sunclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("sunclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("sunclaw-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("sunclaw-dreaming-detach-");
    const logger = createMockLogger();
