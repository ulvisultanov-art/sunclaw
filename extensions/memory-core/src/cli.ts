    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sunclaw memory status", "Show index and provider status."],
          [
            "sunclaw memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["sunclaw memory status --deep", "Probe embedding provider readiness."],
          ["sunclaw memory index --force", "Force a full reindex."],
          ['sunclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'sunclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `sunclaw memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "sunclaw memory rem-backfill --path ./memory",
          ...
          ["sunclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.sunclaw.complex.az/cli/memory")}\n`,
    );
