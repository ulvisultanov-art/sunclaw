import { getExecApprovalReplyMetadata } from "sunclaw/plugin-sdk/approval-runtime";
import {
...
import { toGatewayAccount, writeSunClawConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped sunclaw.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // sunclaw.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeSunClawConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe sunclaw.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeSunClawConfigThroughRuntime(getQQBotRuntime(), nextCfg as SunClawConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as SunClawConfig, accountId);
