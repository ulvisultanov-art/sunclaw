...
function isQQBotAccountConfigured(cfg: SunClawConfig, accountId: string): boolean {
...
export async function finalizeQQBotSetup(params: {
  ...
  if (mode === "qr") {
    ...
    await params.prompter.note(
      ["您可以稍后运行以下命令重新选择 QQ Bot 进行配置：", "  sunclaw channels add"].join("\n"),
      "QQ Bot",
    );
  }
