...
let sunclawVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; SunClaw/${sunclawVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; sunclawVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.sunclawVersion) {
    sunclawVersion = options.sunclawVersion;
  }
}

/** Update the SunClaw framework version in the User-Agent (called after runtime injection). */
export function setSunClawVersion(version: string): void {
  if (version) {
