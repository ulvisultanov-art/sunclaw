...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<SunClawPluginApi["registerService"]>[0];
};
type MockCallSource = {
  mock: {
...
function setup(config: Record<string, unknown>): Registered {
  ...
  const api = createTestPluginApi({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: vi.fn() } } as unknown as SunClawPluginApi["runtime"],
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("sunclaw.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("sunclaw.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("sunclaw.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("SUNCLAW_CLI", "1");
    process.argv = ["node", "sunclaw", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "sunclaw doctor --fix"');
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager["initiateCall"]).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "SunClaw voice call smoke test.",
        mode: "notify",
      });
