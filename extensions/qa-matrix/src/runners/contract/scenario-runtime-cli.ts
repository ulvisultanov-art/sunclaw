import { spawn as startSunClawCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `sunclaw ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaSunClawCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startSunClawCliProcess>,
...
export function startMatrixQaSunClawCli(params: {
  ...
  const distEntryPath = resolveMatrixQaSunClawCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startSunClawCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaSunClawCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaSunClawCli(params).wait();
}
...
export async function createMatrixQaSunClawCliRuntime(params: {
  accountId: string;
  accessToken: string;
