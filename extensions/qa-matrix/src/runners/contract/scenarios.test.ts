...
const {
  ...
  resolveMatrixQaSunClawCliEntryPath,
  runMatrixQaSunClawCli,
  startMatrixQaSunClawCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `sunclaw ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaSunClawCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaSunClawCli: vi.fn(),
  startMatrixQaSunClawCli: vi.fn(),
}));
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "sunclaw.sqlite");
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaSunClawCli.mockReset();
    startMatrixQaSunClawCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "SunClaw Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          SUNCLAW_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          SUNCLAW_CONFIG_PATH: "/tmp/gateway-config.json",
          SUNCLAW_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaSunClawCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaSunClawCli, "startMatrixQaSunClawCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaSunClawCli, "startMatrixQaSunClawCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "SunClaw Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaSunClawCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaSunClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaSunClawCli, "runMatrixQaSunClawCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaSunClawCli, "startMatrixQaSunClawCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.SUNCLAW_STATE_DIR).toContain("sunclaw-matrix-cli-qa-");
      expect(cliEnv?.SUNCLAW_CONFIG_PATH).toContain("sunclaw-matrix-cli-qa-");
      const configPath = String(cliEnv?.SUNCLAW_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.SUNCLAW_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSunClawCli.mockImplementation(async ({ args, env }) => {
        if (env.SUNCLAW_CONFIG_PATH) {
      ...
      expect(runMatrixQaSunClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "SunClaw Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSunClawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.SUNCLAW_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.SUNCLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(
        (
          mockObjectArg(startMatrixQaSunClawCli, "startMatrixQaSunClawCli").env as Record<
            string,
            unknown
          >
        ).SUNCLAW_CONFIG_PATH,
      ).toContain("sunclaw-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSunClawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.SUNCLAW_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SunClaw Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "SunClaw Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "SunClaw Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
