...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__sunclaw_channel_id";
...
type InteractionCallbackConfig = Pick<SunClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "sunclaw-mattermost-interactions").update(botToken).digest("hex");
}
