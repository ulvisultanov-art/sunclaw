...
const { stopSunClawChromeMock } = vi.hoisted(() => ({
  stopSunClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopSunClawChrome: stopSunClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopSunClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["sunclaw", "user"]);
    ...
    expect(stopMap.sunclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopSunClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("sunclaw browser stop failed: Error: oops");
  });
});
