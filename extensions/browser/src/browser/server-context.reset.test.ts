...
function localSunClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "sunclaw",
...
function createLocalSunClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localSunClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveSunClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-reset-"));
    const profileDir = path.join(tempRoot, "sunclaw");
    ...
    const ops = createLocalSunClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveSunClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "sunclaw");
    fs.mkdirSync(profileDir, { recursive: true });
