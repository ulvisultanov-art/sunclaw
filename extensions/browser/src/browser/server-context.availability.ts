...
function formatLocalPortOwnershipHint(profile: ResolvedBrowserProfile): string {
  const resetHint =
    `If SunClaw should own this local profile, run action=reset-profile profile=${profile.name} ` +
  ...
  return (
    ...
    `set browser.profiles.${profile.name}.attachOnly=true so SunClaw attaches without trying ` +
    "to manage the local process. For Browserless Docker, set EXTERNAL to the same WebSocket " +
    "endpoint SunClaw can reach via browser.profiles.<name>.cdpUrl."
  );
}
...
export function createProfileAvailability({
  ...
  const formatChromeMcpAttachFailure = (lastError: unknown): string => {
    ...
    if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome")) {
      return (
        ...
        'If you do not need your signed-in browser session, use the managed "sunclaw" profile instead.' +
        detail
      );
    }
    return (
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopSunClawChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchSunClawChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const launchManagedChrome = async (
    ...
    try {
      return await launchSunClawChrome(current.resolved, profile, launchOptions);
    } catch (err) {
  ...
  const ensureBrowserAvailableOnce = async (options?: BrowserEnsureOptions): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback SunClaw browser is still
      ...
      try {
        ...
        await stopSunClawChrome(launched).catch(() => {});
        setProfileRunning(null);
        recordManagedLaunchFailure(profileState, err);
        throw err;
      }
    ...
    if (!profileState.running) {
      const detail = await describeCdpFailure(PROFILE_ATTACH_RETRY_TIMEOUT_MS);
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by sunclaw. ` +
          `${formatLocalPortOwnershipHint(profile)} ${detail}`,
      );
    }

    await stopSunClawChrome(profileState.running);
    setProfileRunning(null);
