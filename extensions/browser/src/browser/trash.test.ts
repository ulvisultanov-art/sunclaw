...
const resolvePreferredSunClawTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/sunclaw"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "sunclaw-browser-")));
    ...
    resolvePreferredSunClawTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "sunclaw-demo");
