...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        sunclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SUNCLAW_GATEWAY_PORT;
    process.env.SUNCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SUNCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SUNCLAW_GATEWAY_PASSWORD;
