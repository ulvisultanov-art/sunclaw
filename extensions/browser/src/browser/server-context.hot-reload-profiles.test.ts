...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", () => {
    // Start with only sunclaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing sunclaw.json)
  ...
  it("forProfile refreshes existing profile config after getRuntimeConfig cache updates", () => {
    ...
    mockState.cfgProfiles.sunclaw = { cdpPort: 19999, color: "#FF4500" };
    ...
    expect(state.resolved.profiles.sunclaw?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", () => {
    const cfg = getRuntimeConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const sunclawProfile = requireValue(
      resolveProfile(resolved, "sunclaw"),
      "sunclaw profile missing",
    );
    ...
    const runtime = requireValue(state.profiles.get("sunclaw"), "sunclaw runtime missing");
  ...
  it("marks local managed runtime state for reconcile when profile headless changes", () => {
    ...
    expect(sunclawProfile.headless).toBe(true);
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("marks local managed runtime state for reconcile when profile executablePath changes", () => {
    ...
    expect(sunclawProfile.executablePath).toBe("/usr/bin/chrome-old");
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("does not reconcile remote cdp runtime when only headless changes", () => {
    ...
    expect(remoteProfile.driver).toBe("sunclaw");
    ...
    const runtime = requireValue(state.profiles.get("remote"), "remote runtime missing");
    expect(runtime.profile.driver).toBe("sunclaw");
