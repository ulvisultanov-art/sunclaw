...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!sunclaw) {
      throw new Error("expected sunclaw profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...sunclaw, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["sunclaw", "stale-removed", "user"]);
  });
});
