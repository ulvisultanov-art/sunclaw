...
import { resolveSunClawUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveSunClawUserDataDir: vi.fn(() => "/tmp/sunclaw-test/sunclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "sunclaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveSunClawUserDataDir).mockReturnValue(userDataDir);
