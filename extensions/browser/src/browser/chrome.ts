...
import { prepareOomScoreAdjustedSpawn } from "sunclaw/plugin-sdk/process-runtime";
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set SUNCLAW_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.sunclaw/browser/${params.profile.name}/user-data.`,
    );
  }
...
export type RunningChrome = {
   ...
   * through stopSunClawChrome without type churn.
   */
...
export function resolveSunClawUserDataDir(profileName = DEFAULT_SUNCLAW_BROWSER_PROFILE_NAME) {
...
export function buildSunClawChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
export async function launchSunClawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_SUNCLAW_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredSunClawTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 sunclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`sunclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`sunclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 sunclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopSunClawChrome(
