...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage SunClaw's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via SunClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated SunClaw-managed browser (`sunclaw`).",
...
function createBrowserToolOptions(ctx: SunClawPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: SunClawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: SunClawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): SunClawPluginService {
  let service: SunClawPluginService | null = null;
...
export function registerBrowserPlugin(api: SunClawPluginApi) {
  api.registerTool(((ctx: SunClawPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as SunClawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
