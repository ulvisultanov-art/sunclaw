...
- Extension production code should import from `sunclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `sunclaw.plugin.json` and the package
  `sunclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `sunclaw/plugin-sdk/*`.
