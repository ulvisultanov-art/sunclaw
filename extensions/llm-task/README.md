...
`sunclaw.invoke --each`) without adding new SunClaw code per workflow.
...
This extension depends on SunClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SunClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.sunclaw/extensions` as a standalone plugin directory.
