...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    regressionRefs: ["sunclaw/sunclaw#74698"],
    timeoutMs: 45_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        expectReply: true,
        input: `/status@${sutUsername}`,
        expectedTextIncludes: ["SunClaw", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["SunClaw", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@SunClawQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["sunclaw/sunclaw#87392"],
    timeoutMs: 90_000,
    buildRun: (sutUsername) => {
  ...
  },
  {
    ...
    regressionRefs: ["sunclaw/sunclaw#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        allowAnySutReply: true,
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "SUNCLAW_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "SUNCLAW_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "SUNCLAW_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "SUNCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "SUNCLAW_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("SUNCLAW_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "SUNCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "SUNCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledSunClawTelegramOnboardingPreflight(params: {
  sunClawCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredSunClawTmpDir(), "sunclaw-npm-telegram-"),
  );
  ...
  const env = {
    ...process.env,
    HOME: homeDir,
    SUNCLAW_HOME: stateDir,
    SUNCLAW_CONFIG_PATH: path.join(stateDir, "sunclaw.json"),
    SUNCLAW_STATE_DIR: stateDir,
    SUNCLAW_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-sunclaw-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
      params.sunClawCommand,
      [
        "onboard",
        "--non-interactive",
        "--accept-risk",
    ...
    await execFileAsync(params.sunClawCommand, ["doctor", "--non-interactive"], { env });
  } finally {
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutSunClawCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledSunClawTelegramOnboardingPreflight({
        sunClawCommand: params.sutSunClawCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutSunClawCommand
        ? {
            executablePath: params.sutSunClawCommand,
