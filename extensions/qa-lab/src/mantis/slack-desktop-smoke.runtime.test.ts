...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      SUNCLAW_QA_SLACK_CHANNEL_ID: "C123",
      SUNCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      SUNCLAW_QA_SLACK_SUT_APP_TOKEN: "app-token",
      SUNCLAW_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    expect(
      commands.every((entry) => entry.env?.SUNCLAW_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(runArgs).toContain("sunclaw/sunclaw#85141");
    ...
    expect(remoteScript).toContain("sunclaw qa slack");
    ...
    expect(remoteScript).toContain("SUNCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR");
    ...
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/sunclaw-mantis-slack-desktop-2026-05-04T13-00-00-000Z/",
    );
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/sunclaw-mantis-slack-desktop-2026-05-04T13-00-00-000Z/slack-qa/",
    );
  ...
  it("runs approval checkpoint mode with default approval scenarios and records screenshots", async () => {
    ...
    expect(remoteScript).toContain('export SUNCLAW_QA_SLACK_CHANNEL_ID="$slack_channel_id"');
    ...
    expect(remoteScript).toContain("SUNCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR");
    expect(remoteScript).toContain("SUNCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS");
    ...
    expect(remoteScript).toContain("SUNCLAW_MANTIS_APPROVAL_BROWSER_BIN");
  ...
  it("leases Convex Slack credentials for gateway setup and maps them into the VM env", async () => {
    ...
    expect(runCommand?.env?.SUNCLAW_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.SUNCLAW_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
    expect(runCommand?.env?.SUNCLAW_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.SUNCLAW_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.SUNCLAW_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.SUNCLAW_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
    ...
    expect(remoteScript).toContain('</dev/null >"$out/sunclaw-gateway.log"');
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        SUNCLAW_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        SUNCLAW_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
