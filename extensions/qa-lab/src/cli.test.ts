...
import type { QaRunnerCliContribution } from "sunclaw/plugin-sdk/qa-runner-runtime";
...
vi.mock("sunclaw/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    await program.parseAsync([
      ...
      "--browser-url",
      "https://docs.sunclaw.complex.az/docs",
      "--html-file",
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "sunclaw", "qa", "aimock", "--port", "44080"]);
  ...
  it("normalizes signed decimal QA numeric option values through the shared parser", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "aimock", "--port", "+044080"]);
  ...
  it.each([
    ...
    await expect(invalidProgram.parseAsync(["node", "sunclaw", ...args])).rejects.toThrow(message);
  });
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "sunclaw", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "sunclaw", "qa", "suite", "--pack", "personal-agent"]);
