...
description: Maintain the SunClaw memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `sunclaw wiki ingest`, `sunclaw wiki compile`, and `sunclaw wiki lint` as the default maintenance loop.
- In `bridge` mode, run `sunclaw wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `sunclaw wiki unsafe-local import` only when the user explicitly opted into private local path access.
