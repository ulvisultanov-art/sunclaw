...
describe("kimi web search provider", () => {
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    await withEnvAsync({ KIMI_API_KEY: undefined, MOONSHOT_API_KEY: undefined }, async () => {
      ...
      const result = await tool.execute({ query: "SunClaw docs" });
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
    const toolArguments = JSON.stringify({
      query: "SunClaw GitHub repository",
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "SunClaw is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "SunClaw is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "SunClaw", url: "https://github.com/ulvisultanov-art/sunclaw" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "SunClaw is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "SunClaw is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/ulvisultanov-art/sunclaw"]);
      expect(result).not.toHaveProperty("error");
    });
  });
