...
import {
  ...
  resolveSunClawExecModeForCodexAppServer,
  resolveSunClawExecModeFromConfig,
  resolveSunClawExecPolicyForCodexAppServer,
...
describe("Codex app-server config", () => {
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        SUNCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUNCLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { SUNCLAW_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { SUNCLAW_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("SUNCLAW_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          SUNCLAW_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          SUNCLAW_CODEX_COMPUTER_USE: "1",
          SUNCLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          SUNCLAW_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          SUNCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            SUNCLAW_CODEX_COMPUTER_USE: "1",
            SUNCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { SUNCLAW_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("maps normalized SunClaw auto exec mode to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
    });
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        SUNCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUNCLAW_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
    });
  ...
  it("preserves explicit read-only app-server sandbox for auto mode", () => {
    ...
    const envRuntime = resolveRuntimeForTest({
      ...
      execMode: "auto",
      env: {
        SUNCLAW_CODEX_APP_SERVER_MODE: "yolo",
        SUNCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUNCLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it.each(["deny", "allowlist"] as const)(
    "blocks Codex app-server local execution for normalized SunClaw %s exec mode",
    (execMode) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("maps normalized SunClaw ask exec mode away from Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "ask",
    });
  ...
  it("fails closed when normalized SunClaw ask mode cannot use user approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "ask",
  ...
  it.each([
    ...
    "fails closed when normalized SunClaw $execMode mode can only use $policies approvals",
    ({ execMode, policies }) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("keeps normalized SunClaw full exec mode on default Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "full",
    });
  ...
  it("fails closed when normalized SunClaw auto mode can only use on-failure approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("fails closed when normalized SunClaw auto mode cannot force prompting over yolo", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("fails closed when normalized SunClaw auto mode cannot use an auto reviewer", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("keeps normalized SunClaw auto mode when legacy app-server yolo was schema-defaulted", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          command: "codex",
  ...
  it("forces guarded policy fields for normalized SunClaw auto mode", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
  ...
  it("resolves agent-scoped normalized SunClaw exec mode for Codex app-server mapping", () => {
    const config = {
      tools: {
        exec: {
          mode: "ask",
    ...
    expect(resolveSunClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("auto");
    expect(resolveSunClawExecModeFromConfig({ config, agentId: "other-agent" })).toBe("ask");
  });

  it("keeps legacy exec security overrides ahead of normalized SunClaw exec mode", () => {
    expect(
      resolveSunClawExecModeFromConfig({
        config: {
          tools: {
  ...
  it.each(["always"] as const)(
    ...
    (ask) => {
      ...
      const execPolicy = resolveSunClawExecPolicyForCodexAppServer({ config });

      expect(resolveSunClawExecModeForCodexAppServer({ config })).toBe("ask");
      expectRuntimePolicy(
        resolveRuntimeForTest({
          pluginConfig: {
            appServer: {
  ...
  it("keeps legacy full exec security with ask=on-miss on default Codex yolo", () => {
    ...
    expect(resolveSunClawExecModeForCodexAppServer({ config })).toBe("full");
  ...
  it("fails closed when legacy full exec with ask cannot use full Codex sandbox", () => {
    ...
    expect(() =>
      resolveRuntimeForTest({
        execPolicy: resolveSunClawExecPolicyForCodexAppServer({ config }),
        requirementsToml: 'allowed_sandbox_modes = ["read-only", "workspace-write"]\n',
      }),
    ).toThrow("legacy full exec security with ask requires Codex app-server danger-full-access");
  });

  it("clamps legacy full exec with ask when an SunClaw sandbox is active", () => {
    const config = {
      tools: {
        exec: {
          security: "full",
  ...
  it("applies host exec approval security floors before starting Codex app-server", () => {
    const execPolicy = resolveSunClawExecPolicyForCodexAppServer({
      config: {
        tools: {
          exec: {
            mode: "full",
  ...
  it("treats ask-only legacy overrides as normalized mode overrides", () => {
    ...
    expect(resolveSunClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("allowlist");
    const execMode = resolveSunClawExecModeForCodexAppServer({
      config,
      agentId: "main",
      execOverrides: {
        ask: "always",
  ...
  it("ignores removed SUNCLAW_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { SUNCLAW_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../sunclaw.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
