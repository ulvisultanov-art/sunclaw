...
import { wrapToolWithBeforeToolCallHook } from "sunclaw/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installSunClawOwnedToolHooks,
  mediaToolResult,
  resetSunClawOwnedToolHooks,
  textToolResult,
...
describe("SunClaw-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetSunClawOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installSunClawOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installSunClawOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installSunClawOwnedToolHooks();
