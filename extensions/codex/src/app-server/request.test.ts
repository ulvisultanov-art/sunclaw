...
describe("requestCodexAppServerJson sandbox guard", () => {
  ...
  it("fails closed before raw app-server bypass methods in sandboxed sessions", async () => {
    await expect(
      ...
      "Codex-native app-server method `command/exec` is unavailable because SunClaw sandboxing is active for this session.",
    );
  ...
  it("fails closed before raw app-server bypass methods when exec host=node is active", async () => {
    for (const method of ["command/exec", "process/spawn"]) {
      await expect(
        ...
        `Codex-native app-server method \`${method}\` is unavailable because SunClaw exec host=node is active for this session.`,
      );
    }
  ...
  it("fails closed for config-level exec host=node even without a session key", async () => {
    await expect(
      ...
      "Codex-native app-server method `command/exec` is unavailable because SunClaw exec host=node is active for this session.",
    );
  ...
  it("fails closed for MCP reload when config-level exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native app-server method `config/mcpServer/reload` is unavailable because SunClaw exec host=node is active for this session.",
    );
  ...
  it("allows sandbox-pinned thread starts in sandboxed sessions", async () => {
    ...
    const params = {
      cwd: "/workspace",
      environments: [{ environmentId: "sunclaw-sandbox-abc123", cwd: "/workspace" }],
    };
  ...
  it("blocks thread starts with sandbox environments when exec host=node is active", async () => {
    ...
    await expect(
      ...
      "Codex-native app-server method `thread/start` is unavailable because SunClaw exec host=node is active for this session.",
    );
