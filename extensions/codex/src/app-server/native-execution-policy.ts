...
import { getSessionEntry, type SessionEntry } from "sunclaw/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<SunClawConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "SunClaw exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected SunClaw node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because SunClaw exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected SunClaw node.",
    "Use a normal Codex harness turn so SunClaw exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: SunClawConfig): AgentEntry[] {
