...
import { isSilentReplyPayloadText } from "sunclaw/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-codex-delivery-contract-"));
