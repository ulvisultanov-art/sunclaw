import {
  ...
  isPendingSunClawDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingSunClawDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingSunClawDynamicToolCompletionNotification(
    notification,
    params.pendingSunClawDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after SunClaw hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served SunClaw
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingSunClawDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
