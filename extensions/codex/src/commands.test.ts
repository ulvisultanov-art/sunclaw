...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`SunClaw session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`SunClaw session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-codex-command-"));
    vi.stubEnv("SUNCLAW_STATE_DIR", tempDir);
  });
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "sunclaw.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in sunclaw.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in sunclaw.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this SunClaw session to Codex thread thread-123.",
    });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because SunClaw sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because SunClaw exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(startCodexConversationThread).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because SunClaw exec host=node is active for this session.",
    );
    expect(startCodexConversationThread).not.toHaveBeenCalled();
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this SunClaw session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("SunClaw session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("SunClaw session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("SunClaw session key: `agent:main:discord:two`");
    expect(request.text).toContain("SunClaw session id: `session-two`");
