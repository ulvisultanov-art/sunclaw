...
export function createCodexCliSessionNodeHostCommands(): SunClawPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): SunClawPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredSunClawTmpDir(), "sunclaw-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
