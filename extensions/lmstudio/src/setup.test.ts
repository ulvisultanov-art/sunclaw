...
} from "sunclaw/plugin-sdk/provider-setup";
import type { WizardPrompter } from "sunclaw/plugin-sdk/setup";
...
vi.mock("sunclaw/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("sunclaw/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("sunclaw/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("sunclaw/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("sunclaw/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): SunClawConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.SUNCLAW_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("SUNCLAW_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
