...
import { canResolveEnvSecretRefInReadOnlyPath } from "sunclaw/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "sunclaw/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "sunclaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<SunClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<SunClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SunClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: SunClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: SunClawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: SunClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: SunClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: SunClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: SunClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: SunClawConfig, override?: number): number {
