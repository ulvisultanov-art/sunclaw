...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("sunclaw/plugin-sdk/runtime-env", () => ({
  ...
  try {
    ...
    vi.doUnmock("sunclaw/plugin-sdk/runtime-env");
    vi.doUnmock("openai");
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "sunclaw-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("sunclaw/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/sunclaw-agent");
      ...
      vi.doUnmock("sunclaw/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "sunclaw", "ltm", "list", "--limit", "+03"]);
  ...
  test("sanitizeForMemoryCapture strips sender label from real room-label envelope shapes", () => {
    ...
    expect(
      sanitizeForMemoryCapture(
        "[Discord SunClaw #dev channel id:456 +5m] Alice: I prefer dark mode",
      ),
    ).toBe("I prefer dark mode");
    expect(sanitizeForMemoryCapture("[Telegram SunClaw id:-100] Alice: I prefer dark mode")).toBe(
      "I prefer dark mode",
    );
  ...
  test("escapeMemoryForPrompt strips media attached annotations before escaping", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent image [media attached: /Users/alex/.sunclaw/media/photo.jpg (image/jpeg)] and said hello",
      ),
