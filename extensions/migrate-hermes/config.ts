  ...
  if (params.hasMemoryFiles || memoryProvider) {
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Use SunClaw built-in file memory for imported Hermes memory files.",
        conflict:
    ...
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Select the default SunClaw memory plugin for imported file memory.",
        conflict:
          !params.ctx.overwrite &&
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes used Honcho memory. SunClaw keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createMigrationManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known SunClaw mapping.`,
        recommendation: "Install or configure an equivalent SunClaw memory plugin manually.",
      }),
    );
  }
