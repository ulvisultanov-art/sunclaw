  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes auth.json contains legacy OAuth credentials. SunClaw no longer imports those into live auth during Hermes migration.",
        recommendation:
          "Run sunclaw models auth login --provider openai after migration, or run sunclaw doctor --fix for existing SunClaw legacy auth state.",
      }),
    );
  }
