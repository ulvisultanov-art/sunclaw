  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const assertPrivateApiEnabled = async () => {
      ...
      if (!privateApiStatus?.available) {
        ...
        log.warn(
          `iMessage ${action} blocked: private API bridge unavailable (accountId=${account.accountId}, cliPath=${cliPathForProbe}). Run \`imsg launch\` to re-inject the dylib, then \`sunclaw channels status\` to refresh.`,
        );
        throw new Error(
          `iMessage ${action} requires the imsg private API bridge. Run imsg launch, then sunclaw channels status to refresh capability detection.`,
        );
      }
    };
    const opts = {
    ...
    if (action === "reply") {
      ...
      if (attachment) {
        ...
        // (sunclaw/imsg#114). Older imsg builds reject the option, so
        ...
        if (privateApiStatus?.cliCapabilities?.sendRichSupportsAttachment !== true) {
          throw new Error(
            "iMessage reply with an attachment needs an imsg build that exposes `send-rich --file` " +
              "(sunclaw/imsg#114). Upgrade imsg, or use action 'upload-file' (with filePath/filename) " +
