...
describe("iMessage sent-message echo cache", () => {
  ...
  it("matches persisted echoes written before the monitor cache is created", () => {
    rememberPersistedIMessageEcho({
      ...
      text: "SunClaw imsg live test",
      messageId: "guid-1",
    });
    const cache = createSentMessageCache();

    expect(cache.has("acct:imessage:+1555", { text: "SunClaw imsg live test" })).toBe(true);
    expect(cache.has("acct:imessage:+1666", { text: "SunClaw imsg live test" })).toBe(false);
