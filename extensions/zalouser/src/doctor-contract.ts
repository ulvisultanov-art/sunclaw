...
type ZalouserChannelsConfig = NonNullable<SunClawConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: SunClawConfig): ChannelDoctorConfigMutation {
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "sunclaw doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "sunclaw doctor --fix".',
