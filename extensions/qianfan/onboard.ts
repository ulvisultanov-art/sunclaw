...
function resolveQianfanPreset(cfg: SunClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: SunClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: SunClawConfig): SunClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: SunClawConfig): SunClawConfig {
