import { clearLiveCatalogCacheForTests } from "sunclaw/plugin-sdk/provider-catalog-shared";
import {
...
const SUPPORTED_DURATIONS_HINT = Symbol.for("sunclaw.videoGeneration.supportedDurations");
...
vi.mock("sunclaw/plugin-sdk/provider-http", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/provider-http")>(
    "sunclaw/plugin-sdk/provider-http",
  );
  return {
    ...actual,
