...
import { coerceSecretRef } from "sunclaw/plugin-sdk/secret-input";
import {
...
const DEFAULT_POLICY_SANDBOX_BROWSER_NETWORK = "sunclaw-sandbox-browser";
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://sunclaw.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://sunclaw.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://sunclaw.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://sunclaw.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://sunclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://sunclaw.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://sunclaw.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://sunclaw.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://sunclaw.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://sunclaw.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://sunclaw.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://sunclaw.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://sunclaw.config/channels/${ocPathSegment(channel)}`;
...
export function scanPolicyGatewayExposure(
  ...
  entries.push({
    ...
    kind: "bind",
    source: "oc://sunclaw.config/gateway/bind",
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://sunclaw.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://sunclaw.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://sunclaw.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://sunclaw.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://sunclaw.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://sunclaw.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://sunclaw.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
  );
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://sunclaw.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://sunclaw.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://sunclaw.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://sunclaw.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
export function scanPolicyAgentWorkspace(
  ...
  pushAgentWorkspaceEvidence(entries, {
    ...
    tools: defaultTools,
    inheritedTools: {},
    workspaceSourceBase: "oc://sunclaw.config/agents/defaults",
    inheritedWorkspaceSourceBase: "oc://sunclaw.config/agents/defaults",
    toolsSourceBase: "oc://sunclaw.config/tools",
    inheritedToolsSourceBase: "oc://sunclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushAgentWorkspaceEvidence(entries, {
      ...
      workspaceSourceBase: `oc://sunclaw.config/agents/list/#${index}`,
      inheritedWorkspaceSourceBase: "oc://sunclaw.config/agents/defaults",
      toolsSourceBase: `oc://sunclaw.config/agents/list/#${index}/tools`,
      inheritedToolsSourceBase: "oc://sunclaw.config/tools",
    });
  });
  return entries.toSorted((a, b) => a.source.localeCompare(b.source) || a.id.localeCompare(b.id));
...
export function scanPolicySandboxPosture(
  ...
  pushSandboxPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://sunclaw.config/agents/defaults/sandbox",
    inheritedSourceBase: "oc://sunclaw.config/agents/defaults/sandbox",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushSandboxPostureEvidence(entries, {
      ...
      sourceBase: `oc://sunclaw.config/agents/list/#${index}/sandbox`,
      inheritedSourceBase: "oc://sunclaw.config/agents/defaults/sandbox",
    });
  });
...
export function scanPolicyToolPosture(
  ...
  pushToolPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://sunclaw.config/tools",
    inheritedSourceBase: "oc://sunclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushToolPostureEvidence(entries, {
      ...
      inheritedSandbox: defaultSandbox,
      sourceBase: `oc://sunclaw.config/agents/list/#${index}/tools`,
      inheritedSourceBase: "oc://sunclaw.config/tools",
    });
  });
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://sunclaw.config/auth/profiles/${ocPathSegment(id)}`,
...
function pushMemorySessionTranscriptIndexing(
  ...
  if (defaultSessionMemory !== undefined) {
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: "oc://sunclaw.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "global",
  ...
  agents.list.forEach((rawAgent, index) => {
    ...
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: explicit
        ? `oc://sunclaw.config/agents/list/#${index}/memorySearch/experimental/sessionMemory`
        : "oc://sunclaw.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "agent",
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://sunclaw.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://sunclaw.config/${path.map(ocPathSegment).join("/")}`;
}
  ...
  const sandboxModeSource =
    explicitSandboxMode !== undefined
      ...
      : inheritedSandboxMode !== undefined
        ...
        : "oc://sunclaw.config/agents/defaults/sandbox/mode";
    ...
    source:
      explicitWorkspaceAccess !== undefined
        ...
        : inheritedWorkspaceAccess !== undefined
          ...
          : "oc://sunclaw.config/agents/defaults/sandbox/workspaceAccess",
    scope: params.scope,
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://sunclaw.config/gateway/http/endpoints/${endpoint}`;
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://sunclaw.config/agents/defaults/models",
    );
  }
  ...
  for (const [index, agent] of list.entries()) {
    ...
    collectModelRefsFromModelMap(
      refs,
      agent.models,
      `oc://sunclaw.config/agents/list/#${index}/models`,
    );
  }
}
