...
metadata: { "sunclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SunClaw `sessions_spawn`
- **File I/O** == SunClaw `read`/`write`
- **Remote fetch** == SunClaw `web_fetch` (or `exec` with curl when POST is required)
