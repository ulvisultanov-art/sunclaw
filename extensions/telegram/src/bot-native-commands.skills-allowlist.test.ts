...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "sunclaw/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("sunclaw-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg: cfgLocal, agentIds }: { cfg: SunClawConfig; agentIds?: string[] }) =>
