...
export function shouldShowTelegramDmAccessWarning(cfg: SunClawConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`sunclaw config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`sunclaw config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
