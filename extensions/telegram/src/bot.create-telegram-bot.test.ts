import { escapeRegExp, formatEnvelopeTimestamp } from "sunclaw/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "sunclaw/plugin-sdk/config-contracts";
import type { GetReplyOptions, MsgContext } from "sunclaw/plugin-sdk/reply-runtime";
...
const pluginStateTestRuntime = await import("sunclaw/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("sunclaw/plugin-sdk/conversation-runtime");
const configMutation = await import("sunclaw/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("sunclaw/plugin-sdk/session-store-runtime");
...
describe("createTelegramBot", () => {
  ...
  it.each(["stop", "/stop@sunclaw_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "SunClaw Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("sunclaw pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "SunClaw", username: "sunclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "SunClaw", username: "sunclaw_bot" },
          text: "Binding: Review pull request 54118 (sunclaw)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "SunClaw", username: "sunclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "SunClaw Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "SunClaw", username: "sunclaw_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "sunclaw_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "sunclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/sunclaw-telegram-group-activation.json";
