...
import { resolveChannelConfigWrites } from "sunclaw/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "sunclaw/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "sunclaw/plugin-sdk/command-auth-native";
...
import { applyModelOverrideToSessionEntry } from "sunclaw/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "sunclaw/plugin-sdk/models-provider-runtime";
...
import { danger, logVerbose, warn } from "sunclaw/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in sunclaw.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
