...
import { resetInboundDedupe } from "sunclaw/plugin-sdk/reply-runtime";
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("sunclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("sunclaw/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "sunclaw-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as SunClawConfig) as TelegramBotDeps["getRuntimeConfig"];
