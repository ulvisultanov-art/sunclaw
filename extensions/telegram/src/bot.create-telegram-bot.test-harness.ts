...
type GetRuntimeConfigFn =
  typeof import("sunclaw/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("sunclaw/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("sunclaw/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("sunclaw/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}sunclaw-telegram-${
      process.pid
...
function resolveDefaultModelForAgentForTest(params: { cfg: SunClawConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: SunClawConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: SunClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: SunClawConfig) => {
