...
function resolveDefaultAgentId(cfg: SunClawConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: SunClawConfig): string[] {
...
function listBoundAccountIds(cfg: SunClawConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: SunClawConfig, channelId: string): string | null {
...
function hasImplicitDefaultTelegramAccount(cfg: SunClawConfig): boolean {
...
export function listTelegramAccountIds(cfg: SunClawConfig): string[] {
...
export function resolveDefaultTelegramAccountSelection(cfg: SunClawConfig): {
  accountId: string;
  accountIds: string[];
...
export function resolveDefaultTelegramAccountId(cfg: SunClawConfig): string {
