...
type ResolveConfiguredBindingRouteFn =
  typeof import("sunclaw/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("sunclaw/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("sunclaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("sunclaw/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("sunclaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/conversation-runtime")>(
    "sunclaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("sunclaw/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/session-store-runtime")>(
    "sunclaw/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("sunclaw/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/command-auth-native")>(
    "sunclaw/plugin-sdk/command-auth-native",
  );
...
vi.mock("sunclaw/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/plugin-runtime")>(
    "sunclaw/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/sunclaw-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/sunclaw-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/sunclaw-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/sunclaw-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as SunClawConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/sunclaw-sessions/sessions.json",
        sessionsDir: "/tmp/sunclaw-sessions",
        fallbackSessionFile: path.resolve("/tmp/sunclaw-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/sunclaw-sessions", "sess-topic-topic-42.jsonl"),
