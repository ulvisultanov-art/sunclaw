...
import { redactMigrationPlan } from "sunclaw/plugin-sdk/migration";
...
describe("Claude migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when SUNCLAW_HOME is set", () => {
    const previous = process.env.SUNCLAW_HOME;
    process.env.SUNCLAW_HOME = path.join(path.sep, "tmp", "sunclaw-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.SUNCLAW_HOME = previous;
      }
    }
  });
