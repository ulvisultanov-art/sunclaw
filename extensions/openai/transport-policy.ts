...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-sunclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-sunclaw-turn-id": turnId,
      "x-sunclaw-turn-attempt": attempt,
    },
    metadata: {
      sunclaw_session_id: sessionHeaders["x-sunclaw-session-id"] ?? "",
      sunclaw_turn_id: turnId,
      sunclaw_turn_attempt: attempt,
      sunclaw_transport: ctx.transport,
    },
  };
}
