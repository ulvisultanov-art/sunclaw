...
import { resolveClosestSize } from "sunclaw/plugin-sdk/media-generation-runtime";
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.SUNCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: SunClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: SunClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: SunClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: SunClawConfig | undefined): SunClawConfig | undefined {
