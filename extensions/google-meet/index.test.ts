...
import type { RealtimeTranscriptionProviderPlugin } from "sunclaw/plugin-sdk/realtime-transcription";
import type { RealtimeVoiceProviderPlugin } from "sunclaw/plugin-sdk/realtime-voice";
...
describe("google-meet plugin", () => {
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(config.realtime.instructions).toContain("sunclaw_agent_consult");
  ...
  it("declares advanced config metadata in the plugin entry and manifest", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./sunclaw.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        SUNCLAW_GOOGLE_MEET_CLIENT_ID: "client-id",
        GOOGLE_MEET_CLIENT_SECRET: "client-secret",
        SUNCLAW_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
        GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
        GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
        SUNCLAW_GOOGLE_MEET_PREVIEW_ACK: "true",
      },
    );
  ...
  it.each(["0x10", "1e3"])("ignores non-decimal env numeric fallbacks: %s", (expiresAt) => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        SUNCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: expiresAt,
      },
    );
  ...
  it("uses a provider-safe flat tool parameter schema", () => {
    ...
    expect(properties.mode).toEqual({
      ...
      description:
        "Join mode. agent uses realtime transcription, the configured SunClaw agent, and regular TTS. bidi uses the realtime voice model directly. transcribe joins observe-only.",
    });
  });
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["sunclaw-audio-bridge", "capture"],
          audioOutputCommand: ["sunclaw-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "sunclaw-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "sunclaw-google-meet-tool-dry-run-"));
          ...
          if (request.path === "/act") {
            return {
              result: JSON.stringify({
                ...
                manualActionMessage: "Admit the SunClaw browser participant in Google Meet.",
                title: "Meet",
                url: "https://meet.google.com/abc-defg-hij",
              }),
            };
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__sunclawMeetCaptions";
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the SunClaw browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("sunclaw node run");
  });
  ...
  it("pipes Chrome command-pair audio through the realtime provider", async () => {
    ...
    expect(progressPayload.tool).toBe("sunclaw_agent_consult");
    ...
    expect(callbacks.tools?.map((tool) => tool.name)).toContain("sunclaw_agent_consult");
    await vi.waitFor(() => {
