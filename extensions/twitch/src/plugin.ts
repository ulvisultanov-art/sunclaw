...
} from "sunclaw/plugin-sdk/channel-pairing";
import type { SunClawConfig } from "sunclaw/plugin-sdk/config-contracts";
import {
  buildPassiveProbedChannelStatusSummary,
  runStoppablePassiveMonitor,
} from "sunclaw/plugin-sdk/extension-shared";
import {
 ...
 * for SunClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: SunClawConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: SunClawConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: SunClawConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: SunClawConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("sunclaw/plugin-sdk/runtime-env").RuntimeEnv;
