...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "sunclaw/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "sunclaw/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "sunclaw/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "sunclaw/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "sunclaw/plugin-sdk/runtime-env";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/sunclaw-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/sunclaw-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("sunclaw/plugin-sdk/reply-runtime", () => ({
...
vi.mock("sunclaw/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/sunclaw-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/sunclaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/sunclaw-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/sunclaw-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
