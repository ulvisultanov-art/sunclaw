...
import { VERSION } from "sunclaw/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "sunclaw/plugin-sdk/fetch-runtime";
import { danger, success } from "sunclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "sunclaw/plugin-sdk/runtime-env";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("sunclaw channels login")}`,
              ),
            );
          }
        }
