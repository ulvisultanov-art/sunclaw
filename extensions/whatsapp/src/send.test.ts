...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "sunclaw/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("sunclaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("sunclaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: SunClawConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("sunclaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `sunclaw-outbound-${crypto.randomUUID()}.log`);
