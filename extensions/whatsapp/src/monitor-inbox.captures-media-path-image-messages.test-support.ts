...
vi.mock("sunclaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("sunclaw/plugin-sdk/logging-core")>(
    "sunclaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("SunClaw WhatsApp listener close");
