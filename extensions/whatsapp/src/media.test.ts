...
import { withMockedWindowsPlatform, withRestoredMocks } from "sunclaw/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "sunclaw/plugin-sdk/web-media";
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub SUNCLAW_STATE_DIR.
    // Also keep it outside the SunClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["SUNCLAW_STATE_DIR"]);
    process.env.SUNCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "sunclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSunClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/sunclaw` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredSunClawTmpDir();
  ...
  it("allows default SunClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default SunClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
