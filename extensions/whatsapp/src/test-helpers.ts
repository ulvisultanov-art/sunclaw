...
import { formatEnvelopeTimestamp } from "sunclaw/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("sunclaw:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("sunclaw:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[sunclaw]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/sunclaw-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("sunclaw:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("sunclaw:lastSocket")];
  if (typeof getter === "function") {
