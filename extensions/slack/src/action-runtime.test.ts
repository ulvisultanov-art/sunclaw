...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): SunClawConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as SunClawConfig;
  ...
  function expectLastSlackSend(content: string, cfg: SunClawConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: SunClawConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: SunClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/sunclaw-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/sunclaw-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/sunclaw-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
