import { CURRENT_MESSAGE_MARKER } from "sunclaw/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "sunclaw/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "sunclaw/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bsunclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("sunclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("sunclaw: hello <@U2>");
  });
