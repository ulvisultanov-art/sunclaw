...
import { expectChannelInboundContextContract as expectInboundContextContract } from "sunclaw/plugin-sdk/channel-contract-testing";
...
import { resolveThreadSessionKeys } from "sunclaw/plugin-sdk/routing";
...
describe("slack prepareSlackMessage inbound contract", () => {
  const storeFixture = createSlackSessionStoreFixture("sunclaw-slack-thread-");
  ...
  function createThreadSlackCtx(params: { cfg: SunClawConfig; replies: unknown }) {
...
Second paragraph should still reach the agent after Slack's preview cutoff.`;
  ...
  it("records skipped no-mention room images as pending history media", async () => {
    ...
    try {
      const slackCtx = createInboundSlackCtx({
        cfg: { channels: { slack: { enabled: true } } } as SunClawConfig,
  ...
  it("keeps a root app mention and URL-only Slack thread follow-up on one parent session", async () => {
    ...
    slackCtx.resolveChannelName = async () => ({ name: "proj-sunclaw", type: "channel" });
    ...
    const followUp = await prepareSlackMessage({
      ...
      message: {
        ...
        channel_type: "channel",
        user: "U_BEK",
        text: "https://github.com/ulvisultanov-art/sunclaw/issues/50621",
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/sunclaw" } },
  ...
  it("prefixes channel bodies with sender label and annotates Slack mention tokens", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/sunclaw", enabled: true },
    });
...
describe("slack thread.requireExplicitMention", () => {
  const storeFixture = createSlackSessionStoreFixture("sunclaw-slack-explicit-mention-");
  ...
  it("drops thread reply without explicit mention when requireExplicitMention is true", async () => {
    ...
    const { storePath } = storeFixture.makeTmpStorePath();
    vi.spyOn(
      await import("sunclaw/plugin-sdk/session-store-runtime"),
      "resolveStorePath",
