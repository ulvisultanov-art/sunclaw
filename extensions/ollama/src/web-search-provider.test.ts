...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): SunClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): SunClawConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
      ...
      response: new Response(
        JSON.stringify({
          results: [
            {
              title: "SunClaw",
              url: "https://docs.sunclaw.complex.az/docs",
    ...
    const result = await tool.execute({ query: "sunclaw docs", count: 3 });
    ...
    expect(result.query).toBe("sunclaw docs");
    expect(result.provider).toBe("ollama");
    expect(result.count).toBe(1);
    expectSingleSearchResultUrl(result.results, "https://docs.sunclaw.complex.az/docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest sunclaw release" })).rejects.toThrow(
      "ollama signin",
    );
  });
