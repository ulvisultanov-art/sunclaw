...
import type { SunClawPluginApi, SunClawPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<SunClawPluginApi> = {}): SunClawPluginApi {
...
function fakeCtx(overrides: Partial<SunClawPluginToolContext> = {}): SunClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: SunClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
