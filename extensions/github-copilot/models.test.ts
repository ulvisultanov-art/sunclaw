import { createProviderUsageFetch, makeResponse } from "sunclaw/plugin-sdk/test-env";
...
vi.mock("sunclaw/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("sunclaw/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/sunclaw-state",
}));

import type { ProviderResolveDynamicModelContext } from "sunclaw/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/sunclaw-state/credentials/github-copilot.token.json";
