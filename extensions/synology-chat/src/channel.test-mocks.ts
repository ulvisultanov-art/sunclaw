...
vi.mock("sunclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("sunclaw/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("sunclaw/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("sunclaw/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("sunclaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("sunclaw/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/sunclaw/synology-chat-sessions.json"),
