...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: SunClawConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.SUNCLAW_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("SunClaw");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.SUNCLAW_BOT_NAME = "TestBot";
