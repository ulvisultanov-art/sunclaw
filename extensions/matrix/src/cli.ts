...
import { parseStrictInteger, timestampMsToIsoString } from "sunclaw/plugin-sdk/number-runtime";
import type { ChannelSetupInput } from "sunclaw/plugin-sdk/setup";
...
import { isSunClawManagedMatrixDevice } from "./matrix/device-health.js";
...
function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["sunclaw", "matrix", ...parts];
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleSunClawDeviceIds: addedDevices
        .filter((device) => !device.current && isSunClawManagedMatrixDevice(device.displayName))
...
function printMatrixVerificationSummary(summary: MatrixCliVerificationSummary): void {
  ...
  console.log(`Initiated by SunClaw: ${summary.initiatedByMe ? "yes" : "no"}`);
...
function buildVerificationGuidance(
  ...
  if (status.serverDeviceKnown === false) {
    nextSteps.add(
      `This Matrix device is no longer listed on the homeserver. Create a new SunClaw Matrix device with ${formatMatrixCliCommand("account add --homeserver <url> --user-id <@user:server> --password <password> --device-name SunClaw-Gateway", accountId)}. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run ${formatMatrixCliCommand("account add --homeserver <url> --access-token <token>", accountId)}.`,
    );
  }
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.sunclaw.complex.az/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleSunClawDeviceIds.length > 0) {
              const staleDeviceIds = result.deviceHealth.staleSunClawDeviceIds
                .map((deviceId) => formatMatrixCliText(deviceId))
                .join(", ");
              console.log(
                `Matrix device hygiene warning: stale SunClaw devices detected (${staleDeviceIds}). Run ${formatMatrixCliCommand("devices prune-stale", result.accountId)}.`,
              );
            }
            ...
            const bindHint = `sunclaw agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale SunClaw-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
