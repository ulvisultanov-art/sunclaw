...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  try {
    ...
    const homeDir = env.SUNCLAW_HOME?.trim() || env.HOME?.trim() || os.homedir();
    return path.join(homeDir, ".sunclaw");
  }
}
...
function shouldReadLegacyCredentialsForAccount(accountId?: string | null): boolean {
  ...
  const cfg = getMatrixRuntime().config.current() as SunClawConfig;
