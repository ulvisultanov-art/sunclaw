...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleSunClawDevices: [] as MatrixManagedDeviceInfo[],
      currentSunClawDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale SunClaw devices detected for @bot:example.org: DEV123. Run 'sunclaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
