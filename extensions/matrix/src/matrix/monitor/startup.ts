...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleSunClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale SunClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleSunClawDevices.map((device) => device.deviceId).join(", ")}. Run 'sunclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
  ...
  try {
    ...
    if (startupVerification.kind === "verified") {
      ...
      params.logger.info(
        "matrix: device not verified — run 'sunclaw matrix verify device <key>' to enable E2EE",
      );
