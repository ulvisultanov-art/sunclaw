...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'sunclaw matrix verify status --verbose --account ${accountId}' and 'sunclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another SunClaw runtime is using the same account. " +
    `Check 'sunclaw matrix verify status --verbose --account ${accountId}' and 'sunclaw matrix devices list --account ${accountId}'.`
  );
}
