...
import { resolveRequiredHomeDir, resolveStateDir } from "sunclaw/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "sunclaw-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("sunclaw/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("sunclaw/plugin-sdk/runtime")).createBackupArchive;
