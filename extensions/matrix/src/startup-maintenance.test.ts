  ...
  it("skips snapshot creation when startup only has warning-only migration state", async () => {
    await withTempHome(async (home) => {
      ...
      expect(harness.log.warn).toHaveBeenCalledWith(
        `matrix: Legacy Matrix state detected at ${path.join(home, ".sunclaw", "matrix")}, but the new account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix). Start the gateway once with a working Matrix login, or rerun "sunclaw doctor --fix" after cached credentials are available.`,
      );
    });
  });
