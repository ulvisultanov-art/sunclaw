...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<SunClawConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<SunClawConfig["channels"]>["matrix"]>>,
): SunClawConfig {
  return {
    channels: {
      matrix: {
