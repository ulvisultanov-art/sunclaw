...
import { SILENT_REPLY_TOKEN } from "sunclaw/plugin-sdk/reply-chunking";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredSunClawTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredSunClawTmpDir(), "msteams-retry-"));
