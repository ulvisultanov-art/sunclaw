...
import type { SunClawPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/sunclaw-diagnostics-otel-test";
...
const ORIGINAL_SUNCLAW_OTEL_PRELOADED = process.env.SUNCLAW_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<SunClawPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): SunClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.SUNCLAW_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_SUNCLAW_OTEL_PRELOADED === undefined) {
      delete process.env.SUNCLAW_OTEL_PRELOADED;
    } else {
      process.env.SUNCLAW_OTEL_PRELOADED = ORIGINAL_SUNCLAW_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["sunclaw.provider"]).toBe("openai");
    for (const key of [
      "sunclaw.callId",
      "sunclaw.call_id",
      "sunclaw.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("sunclaw.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "telegram",
      "sunclaw.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("sunclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "sunclaw.channel": "telegram",
      "sunclaw.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("sunclaw.message.queued")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "telegram",
      "sunclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("sunclaw.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("sunclaw.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "sunclaw.channel": "telegram",
      "sunclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("sunclaw.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "sunclaw.lane": "main",
    });
    expect(telemetryState.counters.get("sunclaw.message.processed")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "telegram",
      "sunclaw.outcome": "completed",
    });
    expect(telemetryState.counters.get("sunclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "telegram",
      "sunclaw.source": "webhook",
    });
    expect(telemetryState.counters.get("sunclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "unknown",
      "sunclaw.source": "unknown",
    });
    expect(
      telemetryState.counters.get("sunclaw.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("sunclaw.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("sunclaw.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "sunclaw.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("sunclaw.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "sunclaw.channel": "telegram",
      "sunclaw.outcome": "completed",
    });
    expect(telemetryState.histograms.get("sunclaw.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "sunclaw.lane": "main",
      },
    );
    expect(telemetryState.counters.get("sunclaw.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("sunclaw.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.state": "processing",
    });
    expect(
      telemetryState.histograms.get("sunclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "sunclaw.state": "processing",
    });
    expect(telemetryState.counters.get("sunclaw.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("sunclaw.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "sunclaw.agent": "agent.default",
        "sunclaw.channel": "telegram",
        "sunclaw.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("sunclaw.webhook.processed");
    expect(spanNames).toContain("sunclaw.message.processed");
    expect(spanNames).toContain("sunclaw.session.stuck");
    const webhookSpanOptions = startedSpanOptions("sunclaw.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("sunclaw.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("sunclaw.message.processed");
    expect(messageSpanOptions?.attributes?.["sunclaw.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["sunclaw.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("sunclaw.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("sunclaw.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.SUNCLAW_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("sunclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["sunclaw.provider"]).toBe("openai");
    expect(runDurationAttributes?.["sunclaw.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("sunclaw.run");
    expect(runSpanOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("sunclaw.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "sunclaw.exporter": "diagnostics-otel",
      "sunclaw.signal": "logs",
      "sunclaw.status": "started",
      "sunclaw.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("sunclaw.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("sunclaw.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "sunclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("sunclaw.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "sunclaw.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("sunclaw.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["sunclaw.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["sunclaw.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["sunclaw.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "sunclaw.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("sunclaw.payload.large")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.payload.action": "rejected",
      "sunclaw.payload.surface": "gateway.frame",
      "sunclaw.channel": "web",
      "sunclaw.plugin": "none",
      "sunclaw.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("sunclaw.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "sunclaw.exporter": "diagnostics-otel",
      "sunclaw.signal": "logs",
      "sunclaw.status": "failure",
      "sunclaw.reason": "emit_failed",
      "sunclaw.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("sunclaw.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("sunclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["sunclaw.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["sunclaw.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("sunclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["sunclaw.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["sunclaw.content.input_messages"]).toBe("user prompt");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["sunclaw.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "sunclaw.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "sunclaw.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "sunclaw.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/sunclaw/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/sunclaw/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["sunclaw.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["sunclaw.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `sunclaw.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "sunclaw.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "sunclaw.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "sunclaw.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "sunclaw.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "sunclaw.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "sunclaw.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("sunclaw.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "sunclaw.channel": "webchat",
      "sunclaw.agent": "ops",
      "sunclaw.provider": "openai",
      "sunclaw.model": "gpt-5.4",
      "sunclaw.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("sunclaw.tokens")?.add).toHaveBeenCalledWith(2, {
      "sunclaw.channel": "unknown",
      "sunclaw.agent": "unknown",
      "sunclaw.provider": "openai",
      "sunclaw.model": "gpt-5.4",
      "sunclaw.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("sunclaw.tokens")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("sunclaw.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "sunclaw.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "sunclaw.agent": "main",
      "sunclaw.skill.activation": "read",
      "sunclaw.skill.name": "tiny-llm-brainstorm",
      "sunclaw.skill.source": "workspace",
      "sunclaw.toolName": "read",
    };
    expect(telemetryState.counters.get("sunclaw.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "sunclaw.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("sunclaw.run");
    expect(spanNames).toContain("sunclaw.model.call");
    expect(spanNames).toContain("sunclaw.harness.run");
    expect(spanNames).toContain("sunclaw.tool.execution");

    const runOptions = startedSpanOptions("sunclaw.run");
    expect(runOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["sunclaw.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["sunclaw.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["sunclaw.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "sunclaw.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "sunclaw.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("sunclaw.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("sunclaw.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["sunclaw.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["sunclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["sunclaw.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["sunclaw.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["sunclaw.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["sunclaw.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["sunclaw.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["sunclaw.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["sunclaw.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["sunclaw.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "sunclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "sunclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "sunclaw.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("sunclaw.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["sunclaw.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["sunclaw.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["sunclaw.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["sunclaw.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["sunclaw.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "sunclaw.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "sunclaw.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("sunclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["sunclaw.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["sunclaw.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("sunclaw.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["sunclaw.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["sunclaw.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("sunclaw.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["sunclaw.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["sunclaw.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("sunclaw.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["sunclaw.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["sunclaw.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("sunclaw.model.call");
    expect(modelSpanAttributes["sunclaw.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["sunclaw.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["sunclaw.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("sunclaw.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "sunclaw.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("sunclaw.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["sunclaw.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["sunclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["sunclaw.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "sunclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "sunclaw.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("sunclaw.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["sunclaw.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "sunclaw.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "sunclaw.runId")).toBe(false);

    const toolSpan = spanByName("sunclaw.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("sunclaw.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("sunclaw.model.failover");
    expect(failoverOptions?.attributes?.["sunclaw.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["sunclaw.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["sunclaw.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["sunclaw.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["sunclaw.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["sunclaw.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["sunclaw.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["sunclaw.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "sunclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("sunclaw.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("sunclaw.model.failover")).toStrictEqual([
      1,
      {
        "sunclaw.failover.reason": "overloaded",
        "sunclaw.failover.suspended": "true",
        "sunclaw.lane": "main",
        "sunclaw.model": "claude-opus-4-6",
        "sunclaw.provider": "anthropic",
        "sunclaw.failover.to_model": "gpt-5.4",
        "sunclaw.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("sunclaw.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "sunclaw.toolName": "browser",
        "sunclaw.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "sunclaw.tool.owner": "browser-tools",
        "sunclaw.tool.params.kind": "object",
        "sunclaw.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("sunclaw.model.failover");
    expect(failoverOptions?.attributes?.["sunclaw.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "sunclaw.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("sunclaw.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    expect(span?.addEvent).toHaveBeenCalledWith("sunclaw.provider.request", {
      "sunclaw.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("sunclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["sunclaw.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "sunclaw.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("sunclaw.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "sunclaw.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["sunclaw.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["sunclaw.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["sunclaw.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["sunclaw.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["sunclaw.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["sunclaw.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["sunclaw.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["sunclaw.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["sunclaw.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["sunclaw.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["sunclaw.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["sunclaw.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["sunclaw.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("sunclaw.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.toolName": "process",
      "sunclaw.loop.level": "critical",
      "sunclaw.loop.action": "block",
      "sunclaw.loop.detector": "known_poll_no_progress",
      "sunclaw.loop.count": 20,
      "sunclaw.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("sunclaw.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["sunclaw.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["sunclaw.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["sunclaw.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["sunclaw.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["sunclaw.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["sunclaw.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "sunclaw.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("sunclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("sunclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "sunclaw.memory.level": "critical",
        "sunclaw.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("sunclaw.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.memory.level": "critical",
      "sunclaw.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("sunclaw.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["sunclaw.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["sunclaw.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["sunclaw.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["sunclaw.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["sunclaw.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["sunclaw.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["sunclaw.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["sunclaw.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["sunclaw.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["sunclaw.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "sunclaw.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("sunclaw.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "sunclaw.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "sunclaw.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "sunclaw.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["sunclaw.run"]).toBeUndefined();
    expect(parentBySpanName["sunclaw.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["sunclaw.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("sunclaw.message.processed");
    const harnessSpan = spanByName("sunclaw.harness.run");
    const runSpan = spanByName("sunclaw.run");
    const usageSpan = spanByName("sunclaw.model.usage");
    const modelCallSpan = spanByName("sunclaw.model.call");
    ...
    expect(parentBySpanName["sunclaw.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["sunclaw.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["sunclaw.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["sunclaw.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["sunclaw.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("sunclaw.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("sunclaw.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("sunclaw.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("sunclaw.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("sunclaw.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("sunclaw.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("sunclaw.message.processed")["sunclaw.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("sunclaw.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("sunclaw.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("sunclaw.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("sunclaw.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("sunclaw.run").spanContext();
    const modelParentContext = startedSpanParentContexts("sunclaw.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("sunclaw.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("sunclaw.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["sunclaw.run"]).toBeUndefined();
    expect(parentBySpanName["sunclaw.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["sunclaw.run"]).toBeUndefined();
    expect(parentBySpanName["sunclaw.model.call"]).toBeUndefined();
    expect(parentBySpanName["sunclaw.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "sunclaw.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "sunclaw.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("sunclaw.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["sunclaw.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["sunclaw.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["sunclaw.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["sunclaw.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("sunclaw.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["sunclaw.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["sunclaw.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["sunclaw.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["sunclaw.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["sunclaw.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["sunclaw.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["sunclaw.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "sunclaw.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "sunclaw.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("sunclaw.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("sunclaw.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("sunclaw.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "sunclaw.channel": "matrix",
      "sunclaw.delivery.kind": "text",
    });
    ...
    expect(deliveryDurationRecords[0]?.[1]["sunclaw.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["sunclaw.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["sunclaw.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["sunclaw.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["sunclaw.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["sunclaw.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["sunclaw.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "sunclaw.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["sunclaw.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["sunclaw.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["sunclaw.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["sunclaw.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["sunclaw.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["sunclaw.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["sunclaw.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "sunclaw.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "sunclaw.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("sunclaw.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["sunclaw.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["sunclaw.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["sunclaw.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("sunclaw.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["sunclaw.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["sunclaw.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["sunclaw.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["sunclaw.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("sunclaw.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["sunclaw.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["sunclaw.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["sunclaw.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("sunclaw.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["sunclaw.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["sunclaw.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["sunclaw.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("sunclaw.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["sunclaw.status"]).toBe("released");
    expect(telemetryState.counters.get("sunclaw.talk.event")?.add).toHaveBeenCalledWith(1, {
      "sunclaw.talk.brain": "agent-consult",
      "sunclaw.talk.event_type": "input.audio.delta",
      "sunclaw.talk.mode": "realtime",
      "sunclaw.talk.provider": "openai",
      "sunclaw.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("sunclaw.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "sunclaw.talk.brain": "agent-consult",
        "sunclaw.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("sunclaw.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "sunclaw.talk.brain": "agent-consult",
      "sunclaw.talk.event_type": "latency.metrics",
      "sunclaw.talk.mode": "realtime",
      "sunclaw.talk.provider": "openai",
      "sunclaw.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("sunclaw.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("sunclaw.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "sunclaw.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["sunclaw.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["sunclaw.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["sunclaw.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["sunclaw.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["sunclaw.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["sunclaw.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["sunclaw.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "sunclaw.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "sunclaw.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "sunclaw.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "sunclaw.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["sunclaw.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("sunclaw.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["sunclaw.reason"])).toContain("…");
    expect(typeof attrs?.["sunclaw.reason"]).toBe("string");
    expect(String(attrs?.["sunclaw.reason"])).not.toContain(
