...
import { formatPluginConfigIssue } from "sunclaw/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "sunclaw-plugin-tools";
const ACPX_SUNCLAW_TOOLS_MCP_SERVER_NAME = "sunclaw-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "sunclaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromSunClawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common SunClaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromSunClawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveSunClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const sunClawRoot = resolveSunClawRoot(pluginRoot);
  const distEntry = path.join(sunClawRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(sunClawRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveSunClawToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const sunClawRoot = resolveSunClawRoot(pluginRoot);
  const distEntry = path.join(sunClawRoot, "dist", "mcp", "sunclaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(sunClawRoot, "src", "mcp", "sunclaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.sunClawToolsMcpBridge && resolved[ACPX_SUNCLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_SUNCLAW_TOOLS_MCP_SERVER_NAME} is reserved when sunClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.sunClawToolsMcpBridge) {
    resolved[ACPX_SUNCLAW_TOOLS_MCP_SERVER_NAME] = resolveSunClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
export function resolveAcpxPluginConfig(params: {
  ...
  const sunClawToolsMcpBridge = normalized.sunClawToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    sunClawToolsMcpBridge,
