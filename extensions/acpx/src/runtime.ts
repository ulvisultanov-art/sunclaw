...
import { redactSensitiveText } from "sunclaw/plugin-sdk/security-runtime";
...
type SunClawAcpxRuntimeOptions = AcpRuntimeOptions & {
  sunclawWrapperRoot?: string;
  sunclawGatewayInstanceId?: string;
  sunclawProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  sunclawProcessCleanup?: AcpxProcessCleanupDeps;
};
type SunClawRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type SunClawRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type SunClawLeaseSessionMetadata = {
  sunclawLeaseId: string;
  sunclawGatewayInstanceId: string;
};

function withSunClawManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // SunClaw owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withSunClawLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: SunClawLeaseSessionMetadata,
): T & SunClawLeaseSessionMetadata {
  return {
    ...record,
    sunclawLeaseId: metadata.sunclawLeaseId,
    sunclawGatewayInstanceId: metadata.sunclawGatewayInstanceId,
  };
}
...
function readSunClawLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { sunclawLeaseId } = record as { sunclawLeaseId?: unknown };
  return typeof sunclawLeaseId === "string" ? sunclawLeaseId.trim() || undefined : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withSunClawLeaseSessionMetadata(record, {
        sunclawLeaseId: lease.leaseId,
        sunclawGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withSunClawLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            sunclawLeaseId: launch.leaseId,
            sunclawGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const SUNCLAW_BRIDGE_EXECUTABLE = "sunclaw";
const SUNCLAW_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_SUNCLAW_PREFIX = "openai/";
...
function isSunClawBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === SUNCLAW_BRIDGE_EXECUTABLE) {
    return parts[1] === SUNCLAW_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^sunclaw(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === SUNCLAW_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_SUNCLAW_PREFIX)) {
    value = value.slice(CODEX_ACP_SUNCLAW_PREFIX.length);
  }
...
function withAcpxSessionOptions(input: SunClawRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isSunClawBridgeCommand(command);
}
...
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: SunClawAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { sunclawProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = sunclawProcessCleanup;
    this.wrapperRoot = options.sunclawWrapperRoot;
    this.gatewayInstanceId = options.sunclawGatewayInstanceId;
    this.processLeaseStore = options.sunclawProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readSunClawLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readSunClawLeaseIdFromRecord(record);
    ...
    await cleanupSunClawOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
      wrapperRoot: this.wrapperRoot,
      deps: this.processCleanupDeps,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withSunClawManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: SunClawRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withSunClawManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
