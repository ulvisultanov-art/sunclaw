...
const { cleanupSunClawOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupSunClawOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleSunClawOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleSunClawOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupSunClawOwnedAcpxProcessTree: cleanupSunClawOwnedAcpxProcessTreeMock,
  reapStaleSunClawOwnedAcpxOrphans: reapStaleSunClawOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { SunClawPluginServiceContext } from "../runtime-api.js";
import { createAcpxRuntimeService, resolveAcpxTimerTimeoutMs } from "./service.js";

const tempDirs: string[] = [];
const previousEnv = {
  SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE: process.env.SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE,
  SUNCLAW_SKIP_ACPX_RUNTIME: process.env.SUNCLAW_SKIP_ACPX_RUNTIME,
  SUNCLAW_SKIP_ACPX_RUNTIME_PROBE: process.env.SUNCLAW_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupSunClawOwnedAcpxProcessTreeMock.mockClear();
  reapStaleSunClawOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("SUNCLAW_SKIP_ACPX_RUNTIME");
  restoreEnv("SUNCLAW_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): SunClawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".sunclaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.SUNCLAW_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupSunClawOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupSunClawOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SunClaw-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    reapStaleSunClawOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupSunClawOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleSunClawOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SunClaw-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.SUNCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.SUNCLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
