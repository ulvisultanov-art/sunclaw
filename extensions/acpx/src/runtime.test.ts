...
const DOCUMENTED_SUNCLAW_BRIDGE_COMMAND =
  "env SUNCLAW_HIDE_BANNER=1 SUNCLAW_SUPPRESS_NOTES=1 sunclaw acp --url ws://127.0.0.1:18789 --token-file ~/.sunclaw/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/sunclaw/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${SUNCLAW_ACPX_LEASE_ID_ARG} lease-close ${SUNCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "sunclaw" ? "sunclaw acp" : agentName),
        list: () => ["codex", "sunclaw"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes SunClaw Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      sunclawGatewayInstanceId: "gateway-test",
      sunclawProcessLeaseStore: leaseStore.store,
      sunclawWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        sunclawLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        sunclawLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        sunclawLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for SunClaw-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("sunclaw acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/sunclaw/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/sunclaw/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("sunclaw acp")).toBe(false);
  ...
  it("cleans up SunClaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/sunclaw/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        sunclawWrapperRoot: "/tmp/sunclaw/acpx",
      },
      {
        sunclawProcessCleanup: {
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/sunclaw/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("SUNCLAW_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("SUNCLAW_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.sunclawGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.sunclawLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.sunclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.sunclawLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.sunclawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.sunclawLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${SUNCLAW_ACPX_LEASE_ID_ARG} lease-old ${SUNCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${SUNCLAW_ACPX_LEASE_ID_ARG} lease-current ${SUNCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("routes sunclaw ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:sunclaw:acp:test",
      backend: "acpx",
  ...
  it("routes non-sunclaw sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for sunclaw sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:sunclaw:acp:test",
      backend: "acpx",
      runtimeSessionName: "sunclaw-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the sunclaw agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "sunclaw" ? "codex" : agentName),
        list: () => ["codex", "sunclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the sunclaw bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "sunclaw acp" : agentName),
        list: () => ["codex", "sunclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped sunclaw bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "sunclaw" ? DOCUMENTED_SUNCLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "sunclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node sunclaw entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "sunclaw" ? "env SUNCLAW_HIDE_BANNER=1 node sunclaw.mjs acp" : agentName,
        list: () => ["codex", "sunclaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:sunclaw:acp:test",
        agentCommand: DOCUMENTED_SUNCLAW_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:sunclaw:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:sunclaw:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to sunclaw bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
