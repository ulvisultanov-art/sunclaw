...
describe("shared/node-match", () => {
  ...
  it("matches candidates by node id, remote ip, normalized name, and long prefix", () => {
    ...
    expect(resolveNodeMatches(nodes, "sunclaw")).toStrictEqual([]);
  ...
  it("prefers a unique current SunClaw client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=sunclaw-macos.*node=third-mac.*client=sunclaw-macos/,
    );
  });
