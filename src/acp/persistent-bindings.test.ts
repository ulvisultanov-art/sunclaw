...
type ConfiguredBinding = NonNullable<SunClawConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<SunClawConfig>,
): SunClawConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: SunClawConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/sunclaw" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
