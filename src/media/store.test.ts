...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("sunclaw-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal17, homeLocal4) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.sunclaw${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal9, homeInner) => {
          ...
          const targetDir = path.join(homeInner, ".sunclaw", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@sunclaw/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@sunclaw/media-core/mime")>(
          "@sunclaw/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@sunclaw/media-core/mime");
      }
    });
  });
