...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds SUNCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `sunclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service SUNCLAW_GATEWAY_TOKEN does not match gateway.auth.token in sunclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.SUNCLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.SUNCLAW_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.SUNCLAW_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by SunClaw ${serviceVersion}; current CLI is ${VERSION}.`,
