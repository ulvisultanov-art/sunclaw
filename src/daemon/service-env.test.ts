...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SUNCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.SUNCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.SUNCLAW_SERVICE_MARKER).toBe("sunclaw");
    expect(env.SUNCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SUNCLAW_SERVICE_VERSION).toBe("string");
    expect(env.SUNCLAW_SYSTEMD_UNIT).toBe("sunclaw-gateway.service");
    expect(env.SUNCLAW_WINDOWS_TASK_NAME).toBe("SunClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.SUNCLAW_LAUNCHD_LABEL).toBe("ai.sunclaw.gateway");
    }
  });

  it("sets the SunClaw-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through SUNCLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUNCLAW_WRAPPER: " /usr/local/bin/sunclaw-doppler ",
      },
      port: 18789,
    });

    expect(env.SUNCLAW_WRAPPER).toBe("/usr/local/bin/sunclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".sunclaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SUNCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SUNCLAW_SYSTEMD_UNIT).toBe("sunclaw-gateway-work.service");
    expect(env.SUNCLAW_WINDOWS_TASK_NAME).toBe("SunClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.SUNCLAW_LAUNCHD_LABEL).toBe("ai.sunclaw.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        SUNCLAW_SYSTEMD_UNIT: "sunclaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.SUNCLAW_SYSTEMD_UNIT).toBe("sunclaw-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUNCLAW_SYSTEMD_UNIT: "sunclaw-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", SUNCLAW_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUNCLAW_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.SUNCLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the SunClaw-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.SUNCLAW_LAUNCHD_LABEL).toBe("ai.sunclaw.node");
  });

  it("passes through SUNCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SUNCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.SUNCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through SUNCLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SUNCLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.SUNCLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits SUNCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.SUNCLAW_WINDOWS_TASK_NAME).toBe("SunClaw Node");
    expect(env.SUNCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.SUNCLAW_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".sunclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SUNCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".sunclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SUNCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".sunclaw"));
  });

  it("uses SUNCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SUNCLAW_STATE_DIR: "/var/lib/sunclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/sunclaw"));
  });

  it("expands ~ in SUNCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SUNCLAW_STATE_DIR: "~/sunclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/sunclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SUNCLAW_STATE_DIR: "C:\\State\\sunclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\sunclaw");
  });
});
