...
const EXTRA_MARKERS = ["sunclaw", "clawdbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["sunclaw_service_marker"];
  const kindKeys = ["sunclaw_service_kind"];
...
function isSunClawGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "sunclaw") {
    return false;
  }
  return label.startsWith("ai.sunclaw.");
}

function isSunClawGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("sunclaw-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isSunClawGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  const defaultName = normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName());
  return normalized === defaultName || normalized.startsWith("sunclaw gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "sunclaw"
        ? "sunclaw"
    ...
    if (marker === "sunclaw" && isSunClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "sunclaw" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedSunClaw?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedSunClaw ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedSunClaw &&
      marker === "sunclaw" &&
      isSunClawGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isSunClawGatewayTaskName(name)) {
        continue;
      }
