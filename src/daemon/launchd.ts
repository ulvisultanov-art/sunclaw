...
const SUNCLAW_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.sunclaw.update.";
const SUNCLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.sunclaw\.manual-update\.\d+$/;

export type StaleSunClawUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeSunClawUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(SUNCLAW_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  return SUNCLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.SUNCLAW_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.SUNCLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.SUNCLAW_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.SUNCLAW_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isSunClawUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeSunClawUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentSunClawUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.SUNCLAW_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeSunClawUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.SUNCLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.SUNCLAW_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by SunClaw. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListSunClawUpdateJobs(
  output: string,
): StaleSunClawUpdateLaunchdJob[] {
  const jobs: StaleSunClawUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeSunClawUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleSunClawUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleSunClawUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListSunClawUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function removeSunClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeSunClawUpdateLaunchdLabel(label);
...
export async function disableSunClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeSunClawUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentSunClawUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentSunClawUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableSunClawUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.SUNCLAW_GATEWAY_PORT ?? "");
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.sunclaw.complex.az/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `sunclaw gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.SUNCLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "sunclaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "sunclaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `sunclaw gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "sunclaw gateway restart",
    });
