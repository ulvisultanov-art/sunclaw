...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sunclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      SUNCLAW_LAUNCHD_LABEL: `ai.sunclaw.launchd-int-${testId}`,
      SUNCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
