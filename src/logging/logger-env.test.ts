...
const logPathTracker = createSuiteLogPathTracker("sunclaw-test-env-log-level-");

describe("SUNCLAW_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.SUNCLAW_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.SUNCLAW_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.SUNCLAW_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.SUNCLAW_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("SUNCLAW_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid SUNCLAW_LOG_LEVEL="nope"');
  });
});
