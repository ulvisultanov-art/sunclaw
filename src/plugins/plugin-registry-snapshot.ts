...
export const DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV = "SUNCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY";
const MAX_PLUGIN_REGISTRY_SNAPSHOT_MEMOS = 8;
const REGISTRY_SNAPSHOT_MEMO_ENV_KEYS = [
  "APPDATA",
  "HOME",
  "SUNCLAW_BUNDLED_PLUGINS_DIR",
  "SUNCLAW_COMPATIBILITY_HOST_VERSION",
  "SUNCLAW_CONFIG_PATH",
  "SUNCLAW_DISABLE_BUNDLED_PLUGINS",
  "SUNCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS",
...
function formatDeprecatedPersistedRegistryDisableWarning(): string {
  return `${DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV} is a deprecated break-glass compatibility switch; use \`sunclaw plugins registry --refresh\` or \`sunclaw doctor --fix\` to repair registry state.`;
}
    ...
    if (persistedReadsEnabled && persistedIndex) {
      if (
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry policy does not match current config; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry points at missing plugin files; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry points at a different bundled plugin tree; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry contains diagnostics referencing missing paths; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry is missing config-path startup metadata; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry metadata no longer matches plugin manifest or package files; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
      } else if (
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry is missing recoverable managed npm plugins; using derived plugin index. Run `sunclaw plugins registry --refresh` to update the persisted registry.",
        });
      } else {
