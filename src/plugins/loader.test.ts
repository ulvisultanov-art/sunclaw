...
import { ensureSunClawPluginSdkAlias } from "./plugin-sdk-dist-alias.js";
...
function updatePluginManifest(plugin: Pick<TempPlugin, "dir">, patch: Record<string, unknown>) {
  const manifestPath = path.join(plugin.dir, "sunclaw.plugin.json");
...
function makeSunClawDevSourceRoot() {
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".sunclaw", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ SUNCLAW_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          sunclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadSunClawPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadSunClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadSunClawPlugins>[0], "cache" | "config">,
) {
  return loadSunClawPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadSunClawPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadSunClawPlugins>;
  loadSecond: () => ReturnType<typeof loadSunClawPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadSunClawPlugins>;
  loadVariant: () => ReturnType<typeof loadSunClawPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const sunclawHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(sunclawHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
...
describe("loadSunClawPlugins", () => {
  ...
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "sunclaw.plugin.json"));
  ...
  it("loads installed plugin packages discovered from persisted install records", () => {
    ...
    const registry = withEnv({ SUNCLAW_STATE_DIR: stateDir }, () =>
      loadSunClawPlugins({
        cache: false,
        config: {
          plugins: {
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "sunclaw", "plugin-sdk");
    ...
    ensureSunClawPluginSdkAlias(distRoot);
  ...
  it("keeps private local-only plugin-sdk artifacts out of package dist aliases", () => {
    ...
    const pluginSdkDir = path.join(distRoot, "plugin-sdk");
    const aliasRoot = path.join(distRoot, "extensions", "node_modules", "sunclaw");
  ...
  it("keeps private local-only plugin-sdk artifacts out of legacy package dist aliases", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "sunclaw", version: "2026.4.22", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(pluginSdkDir, "index.js"), "export const root = true;\n", "utf8");
    fs.writeFileSync(
  ...
  it("loads bundled plugins with plugin-sdk imports from a package dist root", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "sunclaw", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    ensureSunClawPluginSdkAlias(path.join(packageRoot, "dist"));
    fs.writeFileSync(
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadSunClawPlugins>) => {
...
module.exports = { id: "manifest-surfaces-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
        ...
        fs.writeFileSync(
          path.join(plugin.dir, "sunclaw.plugin.json"),
          JSON.stringify(
            {
              id: "manifest-surfaces-plugin",
              configSchema: EMPTY_PLUGIN_SCHEMA,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "sunclaw.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__sunclaw_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__sunclaw_loader_reentry_error";
        const reenterFnMarker = "__sunclaw_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadSunClawPlugins>[0]) => loadSunClawPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadSunClawPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadSunClawPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__sunclaw_runtime_registry_reentry_marker";
        const resolverMarker = "__sunclaw_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadSunClawPlugins(options);
        const scoped = loadSunClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadSunClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("skips discovery and manifest registry loading entirely when onlyPluginIds is an explicit empty array", async () => {
    ...
    const discoverySpy = vi.spyOn(discovery, "discoverSunClawPlugins");
  ...
  it("only publishes plugin commands to the global registry during activating loads", () => {
    ...
    const active = loadSunClawPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@sunclaw/unscoped-provider",
        sunclaw: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadSunClawPlugins(options);
    ...
    const second = loadSunClawPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__sunclawDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadSunClawPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__sunclawSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__sunclawGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__sunclawGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadSunClawPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadSunClawPlugins(options);
  ...
  it("preserves the gateway-bindable hook runner across later default-mode activating loads", () => {
    ...
    const gatewayRegistry = loadSunClawPlugins({
    ...
    const defaultRegistry = loadSunClawPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadSunClawPlugins({
              ...
              env: {
                ...process.env,
                SUNCLAW_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadSunClawPlugins({
              ...
              env: {
                ...process.env,
                SUNCLAW_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(sunclawHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadSunClawPlugins(options),
          loadVariant: () =>
            loadSunClawPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadSunClawPlugins({
      env: {
        ...
        HOME: homeDir,
        SUNCLAW_HOME: undefined,
        SUNCLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers SUNCLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const sunclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "sunclaw-home-demo",
      dir: path.join(sunclawHome, "plugins", "sunclaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "sunclaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadSunClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["sunclaw-home-demo"],
          entries: {
            "sunclaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/sunclaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "sunclaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ SUNCLAW_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadSunClawPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadSunClawPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@sunclaw/nested-default-channel",
          sunclaw: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "sunclaw.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadSunClawPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadSunClawPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(workspacePluginDir, "sunclaw.plugin.json"),
      JSON.stringify(
        {
          id: "workspace-shadow",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-bundled-route-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-external-deferred-test",
        fullBlurb: "full entry should defer while configured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@sunclaw/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rolls back setup-runtime registrations when setup side effects fail", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-runtime-route-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("closes setup-runtime registration APIs after synchronous registration", async () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-runtime-late-route-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@sunclaw/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@sunclaw/setup-entry-throws-test",
          sunclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@sunclaw/setup-entry-throws-sibling-test",
          sunclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "sunclaw.plugin.json"),
      JSON.stringify(
        {
  ...
  it("records a diagnostic when registerChannel throws in the setup-entry path", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@sunclaw/register-channel-throws-test",
          sunclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
...
module.exports = {
  ...
  it("prefers built bundled plugin artifacts over source TS when requested", () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "sunclaw.plugin.json"),
      JSON.stringify(
        {
    ...
    const registry = withEnv(
      {
        SUNCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "sunclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "sunclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "sunclaw.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "sunclaw.plugin.json"),
            JSON.stringify(
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
          return withEnv({ SUNCLAW_DEV_SOURCE_ROOT: devSourceRoot }, () =>
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          return withStateDir((stateDir) => {
            const globalDir = path.join(stateDir, "node_modules", "@sunclaw", "memory-lancedb");
            mkdirSafe(globalDir);
              ...
              config: {
                plugins: {
                  ...
                  installs: {
                    "memory-lancedb": {
                      source: "npm",
                      spec: "@sunclaw/memory-lancedb",
                      resolvedName: "@sunclaw/memory-lancedb",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ SUNCLAW_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ SUNCLAW_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("sunclaw/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ SUNCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadSunClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__sunclawLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("sunclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("sunclaw/plugin-sdk");
