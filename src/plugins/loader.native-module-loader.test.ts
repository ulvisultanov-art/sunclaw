...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginModuleLoader", () => {
  it("loads bundled JavaScript without creating a module loader", async () => {
    ...
    const { loadSunClawPlugins } = await importFreshModule<typeof import("./loader.js")>(
      import.meta.url,
      "./loader.js?scope=native-module-loader",
    );

    const pluginRoot = writeBundledPluginFixture("demo");
    process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = pluginRoot;
  ...
  it("loads packaged JavaScript without creating a module loader", async () => {
    ...
    process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = makeTempDir();
