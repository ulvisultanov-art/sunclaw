...
import { normalizeSunClawVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeSunClawVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeSunClawVersionBase(plugin.version) ??
    plugin.version
  );
}
