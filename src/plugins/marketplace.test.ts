...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("sunclaw-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("sunclaw-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("sunclaw-marketplace-test-", async (homeDir) => {
      const sunClawHome = path.join(homeDir, "sunclaw-home");
      await fs.mkdir(path.join(homeDir, ".claude", "plugins"), { recursive: true });
      await fs.mkdir(sunClawHome, { recursive: true });
      await fs.writeFile(
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, SUNCLAW_HOME: sunClawHome },
