...
const diagnosticEventsStateKey = Symbol.for("sunclaw.diagnosticEvents.state.v1");
...
function loadRootAliasWithStubs(options?: {
  ...
  const localRequire = ((id: string) => {
    ...
    if (id === "node:os") {
      return {
        tmpdir: () =>
          context.process.env.TMPDIR ?? options?.defaultTmpDir ?? "/tmp/sunclaw-root-alias-test",
      };
    }
    if (id === "jiti") {
      return {
...
function ensureDiagnosticEventsStateFixture(
  ...
  const state = vm.runInNewContext(
    `({
      marker: Symbol.for("sunclaw.diagnosticEvents.state.v1"),
      enabled: true,
      seq: 0,
      listeners: new Set(),
      dispatchDepth: 0,
...
describe("plugin-sdk root alias", () => {
  ...
  it("loads legacy root exports on demand and preserves reflection", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/sunclaw-root-alias-test", "jiti", "sunclaw", "3.4.5", "12345-678"),
    );
  ...
  it("preserves jiti's tmpdir guard when root-alias TMPDIR resolves to cwd", () => {
    const lazyModule = loadRootAliasWithStubs({
      cwd: "/tmp/sunclaw-root-alias-cwd",
      defaultTmpDir: "/tmp/sunclaw-root-alias-fallback",
      env: { TMPDIR: "/tmp/sunclaw-root-alias-cwd" },
      packageVersion: "3.4.5",
    });

    expect("slowHelper" in lazyModule.moduleExports).toBe(true);
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/sunclaw-root-alias-fallback", "jiti", "sunclaw", "3.4.5", "12345-678"),
    );
  });
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(aliasMap["sunclaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["@sunclaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["sunclaw/plugin-sdk/group-access"]).toContain(
  ...
  it("keeps agent-core package imports on the source graph when llm-core dist is missing", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["@sunclaw/llm-core"]).toBe(sourceLlmCorePath);
  });
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "sunclaw/plugin-sdk/alpha",
      "@sunclaw/plugin-sdk/alpha",
      "sunclaw/plugin-sdk/group-access",
      "@sunclaw/plugin-sdk/group-access",
      "sunclaw/plugin-sdk/zeta",
      "@sunclaw/plugin-sdk/zeta",
      "@sunclaw/llm-core",
      "@sunclaw/llm-core/diagnostics",
      "@sunclaw/llm-core/event-stream",
      "@sunclaw/llm-core/types",
      "@sunclaw/llm-core/validation",
      "sunclaw/plugin-sdk",
      "@sunclaw/plugin-sdk",
    ]);
  });
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["sunclaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap["@sunclaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap).not.toHaveProperty("sunclaw/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("sunclaw/plugin-sdk/nested/path");
    expect(aliasMap).not.toHaveProperty("sunclaw/plugin-sdk/ssrf-runtime-internal");
    expect(aliasMap).not.toHaveProperty("@sunclaw/plugin-sdk/ssrf-runtime-internal");
  });
  ...
  it("keeps non-QA private local-only plugin-sdk subpaths out of the CJS root alias", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap).not.toHaveProperty("sunclaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("@sunclaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("sunclaw/plugin-sdk/qa-runtime");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["sunclaw/plugin-sdk/channel-runtime"]).toBe(
  ...
  it("bridges diagnostic listeners through shared process state when the lazy module is isolated", () => {
    ...
    const state = lazyModule.globalContext[Symbol.for("sunclaw.diagnosticEvents.state.v1")] as {
      listeners: Set<(event: { type: string }, metadata: { trusted: boolean }) => void>;
    };
