...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: SunClawPluginApi | undefined;
    let workspaceApi: SunClawPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: SunClawPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: SunClawPluginApi | undefined;
      registerTestPlugin({
