...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/sunclaw-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-network.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/sunclaw-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-config.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-io.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/sunclaw-runtime-session.ts",
] as const;
...
function collectSunClawRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = fs.readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./sunclaw-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@sunclaw/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@sunclaw/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@sunclaw/memory-host-sdk");
    ...
    expect(collectSunClawRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
