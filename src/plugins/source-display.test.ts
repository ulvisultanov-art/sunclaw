...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "sunclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".sunclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".sunclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "sunclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".sunclaw", "extensions"),
      },
    });
  });
});
