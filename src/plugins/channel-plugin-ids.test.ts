...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/sunclaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: SunClawConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: SunClawConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: SunClawConfig) => {
      if (Object.hasOwn(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        SUNCLAW_STATE_DIR: "/tmp/sunclaw-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("does not include Codex when an OpenAI model is manually pinned to SunClaw", () => {
    expectStartupPluginIdsCase({
      config: {
        agents: {
          defaults: {
  ...
  it("ignores env runtime overrides during startup planning", () => {
    expectStartupPluginIdsCase({
      ...
      env: { SUNCLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "memory-core"],
    });
  });
