...
vi.mock("../loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadSunClawPlugins: (...args: Parameters<typeof loaderMocks.loadSunClawPlugins>) =>
      loaderMocks.loadSunClawPlugins(...args),
  };
});
...
beforeEach(() => {
  loaderMocks.loadSunClawPlugins.mockReset();
});
...
describe("ensureStandaloneRuntimePluginRegistryLoaded", () => {
  it("reuses a compatible gateway startup registry for gateway-bindable dispatch load options", () => {
    ...
    expect(loaderMocks.loadSunClawPlugins).not.toHaveBeenCalled();
  });
  ...
  it("loads a fresh registry when dispatch config is not startup-compatible", () => {
    ...
    loaderMocks.loadSunClawPlugins.mockReturnValue(loadedRegistry);
    ...
    expect(loaderMocks.loadSunClawPlugins).toHaveBeenCalledOnce();
  });
});
