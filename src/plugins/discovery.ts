...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @sunclaw/voice-call -> voice-call).
...
function pushInvalidPackageExtensionDiagnostic(params: {
  ...
  if (params.resolution.status === "empty") {
    params.diagnostics.push({
      level: "error",
      source: params.source,
      message: "package.json sunclaw.extensions is empty",
    });
    return true;
  }
  return false;
...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "sunclaw doctor --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
...
export function discoverSunClawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  const scopedResult = tracePluginLifecyclePhase(
    ...
    () => {
      ...
      for (const extraPath of extra) {
        ...
        if (bundledAlias) {
          result.diagnostics.push({
            level: "warn",
            source: trimmed,
            message: `ignored plugins.load.paths entry that points at SunClaw's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run sunclaw doctor --fix`,
          });
          continue;
        }
        discoverFromPath({
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the SunClaw extensions root.
