...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before SunClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether SunClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume SunClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical SunClaw auth profile
   ...
   * `profile-only` treats the selected SunClaw auth profile as the sole auth
   ...
   * as static config, such as mapping SunClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside SunClaw's tool
