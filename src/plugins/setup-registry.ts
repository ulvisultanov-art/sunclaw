...
function collectConfiguredPluginEntryIds(config: SunClawConfig): string[] {
...
function resolveRegister(mod: SunClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: SunClawPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as SunClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: SunClawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so SunClaw ignored the plugin setup runtime entry. Remove setup-api/sunclaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
