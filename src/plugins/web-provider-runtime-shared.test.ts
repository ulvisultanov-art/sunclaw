...
vi.mock("./loader.js", () => ({
  ...
  loadSunClawPlugins: mocks.loadSunClawPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadSunClawPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadSunClawPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadSunClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadSunClawPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadSunClawPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadSunClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
