...
const fixtureRoot = makeTrackedTempDir("sunclaw-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedSunClawPackageFixture(version: string) {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "sunclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createExtensionApiAliasFixture(params?: {
  ...
  fs.writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "sunclaw", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "sunclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@sunclaw/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".sunclaw", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["sunclaw/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/sunclaw-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/sunclaw-loader.js",
      argv1: (root: string) => path.join(root, "sunclaw.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves extension-api aliases from an explicit dev source root", () => {
    ...
    expect(fs.realpathSync(aliases["sunclaw/extension-api"] ?? "")).toBe(
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".sunclaw", "npm"),
        packageName: "@sunclaw/codex",
      });
    const { packageRoot: installedOtherRoot, pluginEntry: installedOtherEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".sunclaw", "npm"),
        packageName: "@sunclaw/demo",
      });
    const shadowCodexRoot = path.join(makeTempDir(), ".sunclaw", "extensions", "codex-shadow");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@sunclaw/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "sunclaw.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an SunClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const devRootAliases = withEnv(
      ...
      () =>
        buildPluginLoaderAliasMap(
          distCodexEntry,
          path.join(fixture.root, "sunclaw.mjs"),
          undefined,
          "dist",
          devFixture.root,
        ),
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "sunclaw.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["sunclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
    ...
    expect(fs.realpathSync(devRootAliases["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
    expect(fs.realpathSync(devRootAliases["sunclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(devCodexMcpProjectionPath),
    );
    expect(
      fs.realpathSync(devRootAliases["sunclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(devCodexNativeTaskRuntimePath));
    expect(aliases["sunclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["sunclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["sunclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["sunclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(installedOtherAliases["sunclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(shadowCodexAliases["sunclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["sunclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const entryBody = [
      'import { ssrfInternal } from "sunclaw/plugin-sdk/ssrf-runtime-internal";',
    ...
    const { packageRoot: installedOllamaRoot, pluginEntry: installedOllamaEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".sunclaw", "npm"),
        packageName: "@sunclaw/ollama",
      });

    const sourceSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOllamaEntry,
      }),
    );
    const sourceBrowserSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceBrowserEntry,
      }),
    );
    const privateQaOtherSubpaths = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
    ...
    const privateQaOtherAliases = withEnv(
      { SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
    ...
    expect(fs.realpathSync(sourceAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(sourceSsrFInternalPath),
    );
    expect(
      fs.realpathSync(sourceBrowserAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(fs.realpathSync(distAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(distSsrFInternalPath),
    );
    expect(
      fs.realpathSync(distBrowserAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeBrowserAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(otherAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(privateQaOtherAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(installedAliases["sunclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
  ...
  it("aliases workspace packages to source when dist artifacts are missing", () => {
    ...
    expect(fs.realpathSync(aliases["@sunclaw/gateway-client"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/gateway-client/timeouts"] ?? "")).toBe(
      fs.realpathSync(gatewayClientTimeouts.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/gateway-protocol"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/gateway-protocol/schema"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolSchema.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/markdown-core"] ?? "")).toBe(
      fs.realpathSync(markdownCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/markdown-core/tables"] ?? "")).toBe(
      fs.realpathSync(markdownCoreTables.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-generation-core"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-generation-core/model-ref"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationModelRef.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-core"] ?? "")).toBe(
      fs.realpathSync(mediaCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-core/mime"] ?? "")).toBe(
      fs.realpathSync(mediaCoreMime.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/acp-core"] ?? "")).toBe(
      fs.realpathSync(acpCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/acp-core/runtime/types"] ?? "")).toBe(
      fs.realpathSync(acpCoreRuntimeTypes.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/normalization-core"] ?? "")).toBe(
      fs.realpathSync(normalizationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/normalization-core/string-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationStringCoerce.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/terminal-core"] ?? "")).toBe(
      fs.realpathSync(terminalCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/terminal-core/theme"] ?? "")).toBe(
      fs.realpathSync(terminalCoreTheme.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/net-policy"] ?? "")).toBe(
      fs.realpathSync(netPolicy.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/net-policy/ip"] ?? "")).toBe(
      fs.realpathSync(netPolicyIp.srcFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/model-catalog-core/provider-id"] ?? "")).toBe(
  ...
  it("aliases workspace package subpaths to dist when available", () => {
    ...
    expect(fs.realpathSync(aliases["@sunclaw/gateway-client/readiness"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.distFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/gateway-protocol/connect-error-details"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.distFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/markdown-core/render"] ?? "")).toBe(
      fs.realpathSync(markdownCore.distFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-generation-core/catalog"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.distFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/media-core/read-response-with-limit"] ?? "")).toBe(
      fs.realpathSync(mediaCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/acp-core/normalize-text"] ?? "")).toBe(
      fs.realpathSync(acpCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/normalization-core/record-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/terminal-core/links"] ?? "")).toBe(
      fs.realpathSync(terminalCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@sunclaw/net-policy/redact-sensitive-url"] ?? "")).toBe(
      fs.realpathSync(netPolicy.distFile),
    );
    expect(
      fs.realpathSync(aliases["@sunclaw/model-catalog-core/provider-model-id-normalize"] ?? ""),
  ...
  it("derives acp-core aliases from packaged root dist when package metadata is absent", () => {
    ...
    const cwdWithoutSunClawPackage = makeTempDir();

    const aliases = withCwd(cwdWithoutSunClawPackage, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(sourcePluginEntry, undefined, undefined, "dist"),
      ),
    );

    expect(fs.realpathSync(aliases["@sunclaw/acp-core/runtime/errors"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@sunclaw/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@sunclaw/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@sunclaw/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@sunclaw/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@sunclaw/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["sunclaw/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running sunclaw argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "sunclaw.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the sunclaw root
    // (e.g. single-binary distributions or custom process launchers).
    // Use sunclaw.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real sunclaw repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "sunclaw.mjs")).href;
    ...
    // runner binary, inside the sunclaw repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedSunClawRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an SunClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["sunclaw/plugin-sdk"] ?? "")).toBe(
  ...
  it("loads source runtime shims through the non-native module loading boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "@sunclaw/plugin-sdk/channel-outbound";
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "sunclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
        "@sunclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "sunclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
    ...
    expect(
      resolvePluginRuntimeModulePath({
        ...
        argv1: path.join(root, "bin", "sunclaw"),
        pluginSdkResolution: "dist",
      }),
    ).toBe(distFile);
  });
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "sunclaw-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "sunclaw");
    try {
      expect(
        resolvePluginRuntimeModulePath({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "sunclaw");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "sunclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "sunclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "sunclaw");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "sunclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "sunclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["sunclaw/plugin-sdk"]).not.toBe(aliasB["sunclaw/plugin-sdk"]);
  });
  ...
  it("returns different references when an explicit dev source root differs", () => {
    ...
    expect(fs.realpathSync(stableAliases["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(stableRootAlias),
    );
    expect(fs.realpathSync(devAliases["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ SUNCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["sunclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["sunclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["sunclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes jiti fs cache by SunClaw package version and install metadata", () => {
    const root = createTrustedSunClawPackageFixture("1.2.3-beta.4");
    ...
    expect(fsCache).toContain(path.join(tmpDir, "jiti", "sunclaw", "1.2.3-beta.4") + path.sep);
  ...
  it("preserves jiti's tmpdir guard when TMPDIR resolves to cwd", () => {
    ...
    expect(guardedFsCache).toContain(path.join("jiti", "sunclaw", "1.2.3-beta.4") + path.sep);
    expect(guardedFsCache.startsWith(path.join(root, "jiti") + path.sep)).toBe(false);
    expect(respectedFsCache).toContain(
      path.join(root, "jiti", "sunclaw", "1.2.3-beta.4") + path.sep,
    );
  });

  it("adds the versioned fs cache directory to plugin loader jiti options", () => {
    const root = createTrustedSunClawPackageFixture("2.0.0");
    const tmpDir = path.join(root, "tmp");

    const options = withEnv({ TMPDIR: tmpDir }, () =>
      buildPluginLoaderJitiOptions(
        { "sunclaw/plugin-sdk": path.join(root, "dist", "plugin-sdk", "root-alias.cjs") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(tmpDir, "jiti", "sunclaw", "2.0.0"));
  });
  ...
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "sunclaw/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "sunclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@sunclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
