...
import {
  installSunClawPluginSdkNativeResolver,
  resetSunClawPluginSdkNativeResolverForTest,
...
function writeFakeSunClawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "sunclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installSunClawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeSunClawPackage(stableRoot);
    writeFakeSunClawPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.SUNCLAW_DEV_SOURCE_ROOT;
    process.env.SUNCLAW_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installSunClawPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("sunclaw/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("sunclaw/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.SUNCLAW_DEV_SOURCE_ROOT;
      } else {
        process.env.SUNCLAW_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("sunclaw/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("sunclaw/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeSunClawPackage(root);
    ...
    expect(installedAliases).toContain("sunclaw/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("sunclaw/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve SunClaw SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeSunClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("sunclaw/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("sunclaw/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("sunclaw/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installSunClawPluginSdkNativeResolver, resetSunClawPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"sunclaw/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"sunclaw/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installSunClawPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeSunClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("sunclaw/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("sunclaw/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for SunClaw-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeSunClawPackage(root);
    ...
    expect(installedAliases).toContain("@sunclaw/normalization-core/string-coerce");
    expect(installedAliases).toContain("@sunclaw/media-core/mime");
    expect(installedAliases).toContain("@sunclaw/acp-core/runtime/types");
    expect(installedAliases).toContain("@sunclaw/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@sunclaw/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@sunclaw/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@sunclaw/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@sunclaw/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@sunclaw/normalization-core/string-coerce")).toThrow();
    expect(() => requireFromPlugin.resolve("@sunclaw/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@sunclaw/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@sunclaw/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeSunClawPackage(root);
    ...
    expect(installedAliases).toContain("sunclaw/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("sunclaw/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("sunclaw/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeSunClawPackage(root);
    ...
    expect(installedAliases).toContain("sunclaw/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("sunclaw/plugin-sdk/ssrf-runtime-internal")),
    ...
    expect(
      fs.realpathSync(requireFromBrowser.resolve("sunclaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("sunclaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("sunclaw/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
