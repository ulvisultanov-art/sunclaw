  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        sunclaw?: SunClawPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.sunclaw?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "sunclaw.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
