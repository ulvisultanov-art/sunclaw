...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "sunclaw.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "sunclaw.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "sunclaw.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "sunclaw.plugin.json");
  ...
  it.each([
    ["manifest", "sunclaw.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "sunclaw.plugin.json");
