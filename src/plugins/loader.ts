...
import { installSunClawPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
function createGuardedPluginRegistrationApi(api: SunClawPluginApi): {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<SunClawPluginDefinition["register"]>>[0],
): void {
...
function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  const devSourceRoot = resolveSunClawDevSourceRoot(env);
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadSunClawPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadSunClawPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SunClawPluginDefinition;
  register?: SunClawPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as SunClawPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as SunClawPluginDefinition;
...
export function loadSunClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into SunClaw yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: SunClawPluginModule | null = null;
      let moduleLoadMs = 0;
      ...
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as SunClawPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: SunClawPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as SunClawPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'sunclaw plugins inspect <id> --runtime --json' for runtime diagnostics, 'sunclaw plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
...
export async function loadSunClawPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: SunClawPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
