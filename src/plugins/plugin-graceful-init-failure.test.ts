...
const fixtureRoot = makeTrackedTempDir("sunclaw-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadSunClawPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "sunclaw.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("sunclaw plugins inspect <id> --runtime --json");
    expect(summary).toContain("sunclaw plugins list");
  });
});
