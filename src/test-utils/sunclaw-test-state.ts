...
type SunClawTestStateLayout = "home" | "state-only" | "split";

type SunClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type SunClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: SunClawTestStateLayout;
  scenario?: SunClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "sunclaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "sunclaw.json"),
...
function scenarioConfig(options: SunClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "sunclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: SunClawTestStateOptions): Record<string, string | undefined> {
...
export async function createSunClawTestState(
  options: SunClawTestStateOptions = {},
): Promise<SunClawTestState> {
  ...
  const state: SunClawTestState = {
    ...
    applyEnv: () => {
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed SunClaw env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withSunClawTestState<T>(
  options: SunClawTestStateOptions,
  fn: (state: SunClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createSunClawTestState(options);
  try {
