  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          installSpec: `@sunclaw/${channelId}`,
          installCommand: `sunclaw plugins install @sunclaw/${channelId}`,
          doctorFixCommand: "sunclaw doctor --fix",
          repairHint: `Install the official external plugin with: sunclaw plugins install @sunclaw/${channelId}, or run: sunclaw doctor --fix.`,
        }
      : null,
}));
...
describe("resolveMessageChannelSelection", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is unavailable: feishu. Install the official external plugin with: sunclaw plugins install @sunclaw/feishu, or run: sunclaw doctor --fix.",
    },
    {
      params: { cfg: {} as never },
      expectedMessage:
        "Channel is required (no configured channels detected). Run sunclaw channels add to configure one",
    },
    {
      setup: () => {
        mocks.resolveOutboundChannelPlugin.mockReturnValue(undefined);
      ...
      expectedMessage:
        "Channel is required (no available channels detected). Configured official external channel WhatsApp is missing its plugin. Install the official external plugin with: sunclaw plugins install @sunclaw/whatsapp, or run: sunclaw doctor --fix.",
    },
    {
      setup: () => {
        mocks.listChannelPlugins.mockReturnValue([
