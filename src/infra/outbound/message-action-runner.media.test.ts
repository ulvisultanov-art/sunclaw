...
describe("runMessageAction media behavior", () => {
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before sunclaw#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred SunClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideSunClaw = path.join(os.tmpdir(), "outside-sunclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
