...
const matrixChunkConfig: SunClawConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as SunClawConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredSunClawTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: SunClawConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as SunClawConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as SunClawConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as SunClawConfig;
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "matrix",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as SunClawConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as SunClawConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".sunclaw", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes SunClaw tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as SunClawConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as SunClawConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for sunclaw/sunclaw#70386: if a reconnect or startup drain
