...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "SUNCLAW_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "sunclaw", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("sunclaw channels login --channel whatsapp");
  });
