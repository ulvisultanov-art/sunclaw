...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", async () => {
    ...
    withEnv(
      {
        ...
        SUNCLAW_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "SUNCLAW_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: SUNCLAW_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        SUNCLAW_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "SUNCLAW_BLANK_ENV",
