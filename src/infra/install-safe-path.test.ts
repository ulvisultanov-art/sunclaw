...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@sunclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@sunclaw/matrix", id: "matrix", expected: true },
    { packageName: "@sunclaw/matrix", id: "@sunclaw/matrix", expected: true },
    { packageName: "@sunclaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@sunclaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@sunclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "sunclaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "sunclaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "sunclaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "sunclaw-install-safe-" }, async (parentDir) => {
