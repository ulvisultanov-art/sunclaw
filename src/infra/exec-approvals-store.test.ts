...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.SUNCLAW_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".sunclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".sunclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".sunclaw", "exec-approvals.sock")),
    );
  });
  ...
  it("accepts a symlinked SUNCLAW_HOME as the trusted approvals root", () => {
    ...
    process.env.SUNCLAW_HOME = linkedHome;

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".sunclaw", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".sunclaw"), "dir");
    process.env.SUNCLAW_HOME = linkedHome;
