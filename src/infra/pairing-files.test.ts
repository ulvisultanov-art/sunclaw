...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/sunclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/sunclaw-state", "devices"),
      pendingPath: path.join("/tmp/sunclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/sunclaw-state", "devices", "paired.json"),
    });
  });
