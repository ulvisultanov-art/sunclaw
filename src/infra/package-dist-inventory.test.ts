...
describe("package dist inventory", () => {
  ...
  it("honors package files exclusions when writing the dist inventory", async () => {
    await withTempDir({ prefix: "sunclaw-dist-inventory-package-files-" }, async (packageRoot) => {
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    ...
    expect(isLegacyPluginDependencyInstallStagePath("dist/extensions/.sunclaw-install-stage")).toBe(
      false,
    );
  });
