...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers SUNCLAW_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands SUNCLAW_HOME when set to ~", () => {
    const env = {
      SUNCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for SUNCLAW_HOME",
      env: { SUNCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when SUNCLAW_HOME is tilde-only and no fallback home exists",
      env: { SUNCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores SUNCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          SUNCLAW_HOME: "/srv/sunclaw-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/sunclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/sunclaw-home " },
      expected: "/srv/sunclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/sunclaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of SUNCLAW_HOME", () => {
    expect(
