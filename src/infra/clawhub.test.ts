...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.SUNCLAW_CLAWHUB_URL;
    delete process.env.SUNCLAW_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.SUNCLAW_CLAWHUB_CONFIG_PATH;
  ...
  it("treats SunClaw CalVer correction versions as stable plugin API hosts", () => {
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("SunClaw 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    await withTempDir({ prefix: "sunclaw-clawhub-config-" }, async (configRoot) => {
      const configPath = path.join(configRoot, "clawhub", "config.json");
      process.env.SUNCLAW_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    await withTempDir({ prefix: "sunclaw-clawdhub-config-" }, async (configRoot) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "sunclaw-clawhub-home-" }, async (fakeHome) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "sunclaw-clawhub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "sunclaw-clawhub-xdg-" }, async (xdgRoot) => {
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.SUNCLAW_CLAWHUB_TOKEN = "env-token-123";
  ...
  it("preserves the configured ClawHub base URL path prefix", async () => {
    process.env.SUNCLAW_CLAWHUB_URL = "https://internal.example.com/clawhub";
    let requestedUrl = "";
  ...
  it("fetches skill verification reports and lets version take precedence over tag", async () => {
    ...
    const envelope = {
      ...
      card: {
        available: true,
        url: "https://clawhub.complex.az/api/v1/skills/agentreceipt/card?version=1.2.3",
      },
      artifact: {
  ...
  it("fetches typed package readiness reports", async () => {
    ...
    await expect(
      fetchClawHubPackageReadiness({
        ...
        fetchImpl: async (input) => {
          ...
          return new Response(
            JSON.stringify({
              package: { name: "@sunclaw/diagnostics-otel", isOfficial: true },
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40sunclaw%2Fdiagnostics-otel/readiness",
    );
  });
  ...
  it("fetches typed package artifact resolver reports", async () => {
          ...
          return new Response(
            JSON.stringify({
              artifact: {
                ...
                version: "2026.3.22",
                downloadUrl: "https://clawhub.complex.az/api/v1/clawpacks/abc",
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40sunclaw%2Fdiagnostics-otel/versions/2026.3.22/artifact",
    );
  });
  ...
  it("fetches typed package security reports", async () => {
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40sunclaw%2Fdiagnostics-otel/versions/2026.3.22/security",
    );
  });
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.SUNCLAW_CLAWHUB_CONFIG_PATH = path.join(os.tmpdir(), "sunclaw-no-clawhub-config");
    await expect(
      searchClawHubSkills({
        query: "calendar",
        fetchImpl: async () =>
