...
const stateDir = "/opt/sunclaw/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/sunclaw/state/sessions/../config.jsonl` resolves to
    // `/opt/sunclaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\sunclaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/sunclaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
