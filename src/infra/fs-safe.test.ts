...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("sunclaw-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("sunclaw-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("sunclaw-fs-safe-");
  ...
  it("writes external command output within an allowed root", async () => {
    const dir = await tempDirs.make("sunclaw-fs-safe-output-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("sunclaw-fs-safe-root-");
    const sourceDir = await tempDirs.make("sunclaw-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("sunclaw-fs-safe-root-");
    const outside = await tempDirs.make("sunclaw-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    process.env.SUNCLAW_HOME = fakeHome;
    try {
    ...
    const root = await tempDirs.make("sunclaw-tilde-test-");
    ...
    const outsideRoot = await tempDirs.make("sunclaw-tilde-outside-");
    await expectRejectCode(
