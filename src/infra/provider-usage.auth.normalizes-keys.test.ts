...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "sunclaw-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".sunclaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".sunclaw", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      SUNCLAW_STATE_DIR: path.join(home, ".sunclaw"),
      ...env,
    };
