...
type LegacyPluginStateImportDatabase = Pick<SunClawStateKyselyDatabase, "plugin_state_entries">;
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runSunClawStateWriteTransaction(
      ...
      { env: { ...process.env, SUNCLAW_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
async function withPluginStateImportEnv<T>(
  ...
  process.env.SUNCLAW_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.SUNCLAW_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set SUNCLAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
async function migrateLegacySessions(
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run sunclaw doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
export async function runLegacyStateMigrations(params: {
  ...
  const acpSessionMetadata = await migrateLegacyAcpSessionMetadata({
    cfg: params.config ?? ({} as SunClawConfig),
    env: { ...process.env, SUNCLAW_STATE_DIR: detected.stateDir },
    now,
  });
...
export async function autoMigrateLegacyState(params: {
  ...
  const hasCustomAgentDir = env.SUNCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
