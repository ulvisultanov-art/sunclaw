...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "sunclaw-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      SUNCLAW_DIAGNOSTICS: "timeline",
      SUNCLAW_DIAGNOSTICS_RUN_ID: "run-1",
      SUNCLAW_DIAGNOSTICS_ENV: "env-1",
      SUNCLAW_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUNCLAW_DIAGNOSTICS: "1" } })).toBe(true);
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUNCLAW_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, SUNCLAW_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.SUNCLAW_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as SunClawConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as SunClawConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as SunClawConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("sunclaw.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.SUNCLAW_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as SunClawConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
