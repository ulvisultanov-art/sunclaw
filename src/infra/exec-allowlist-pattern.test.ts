...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevSunClawHome = process.env.SUNCLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.SUNCLAW_HOME = "/srv/sunclaw-home";
    process.env.HOME = "/home/other";
    const sunClawHome = path.join(path.resolve("/srv/sunclaw-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", sunClawHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevSunClawHome === undefined) {
        delete process.env.SUNCLAW_HOME;
      } else {
        process.env.SUNCLAW_HOME = prevSunClawHome;
      }
