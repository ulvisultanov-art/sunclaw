...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open SunClaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "sunclaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SUNCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SUNCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SUNCLAW_APNS_TEAM_ID and SUNCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SUNCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SUNCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SUNCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SUNCLAW_APNS_PRIVATE_KEY_P8 or SUNCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SUNCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
