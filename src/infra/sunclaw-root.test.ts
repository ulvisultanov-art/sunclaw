...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__sunclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "sunclaw-root");
...
function setPackageRoot(root: string, name = "sunclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./sunclaw-root.js").resolveSunClawPackageRootSync,
  asyncResolver: typeof import("./sunclaw-root.js").resolveSunClawPackageRoot,
  opts: Parameters<typeof import("./sunclaw-root.js").resolveSunClawPackageRootSync>[0],
...
vi.mock("./sunclaw-root.fs.runtime.js", () => ({
  sunClawRootFsSync: mockFsModule(),
  sunClawRootFs: mockFsPromisesModule(),
}));

describe("resolveSunClawPackageRoot", () => {
  let resolveSunClawPackageRoot: typeof import("./sunclaw-root.js").resolveSunClawPackageRoot;
  let resolveSunClawPackageRootSync: typeof import("./sunclaw-root.js").resolveSunClawPackageRootSync;
  let clearSunClawPackageRootCaches: typeof import("./sunclaw-root.js").testing.clearSunClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveSunClawPackageRoot,
      resolveSunClawPackageRootSync,
      testing: { clearSunClawPackageRootCaches },
    } = await import("./sunclaw-root.js"));
  });

  beforeEach(() => {
    clearSunClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "sunclaw");
        const pkgRoot = path.join(project, "node_modules", "sunclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "sunclaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "sunclaw.mjs")));
    ...
    {
      name: "falls through from a non-sunclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-sunclaw");
    ...
    {
      name: "returns null for non-sunclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-sunclaw");
        setPackageRoot(pkgRoot, "not-sunclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "sunclaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "sunclaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
