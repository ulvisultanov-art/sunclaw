...
import { sunClawRootFs, sunClawRootFsSync } from "./sunclaw-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["sunclaw"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await sunClawRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(sunClawRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = sunClawRootFsSync.realpathSync(normalized);
...
export async function resolveSunClawPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveSunClawPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const testing = {
  clearSunClawPackageRootCaches(): void {
