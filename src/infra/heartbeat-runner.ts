...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: SunClawConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SunClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: SunClawConfig) {
...
function resolveHeartbeatAgents(cfg: SunClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: SunClawConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: SunClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: SunClawConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: SunClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: SunClawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: SunClawConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SunClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
