...
export const POSIX_SUNCLAW_TMP_DIR = "/tmp/sunclaw";
...
export type ResolvePreferredSunClawTmpDirOptions = {
...
export function resolvePreferredSunClawTmpDir(
  options: ResolvePreferredSunClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "sunclaw" : `sunclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[sunclaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback SunClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback SunClaw temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_SUNCLAW_TMP_DIR;
