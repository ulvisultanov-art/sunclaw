...
import {
  resolveSunClawStateSqliteDir,
  resolveSunClawStateSqlitePath,
} from "./sunclaw-state-db.paths.js";
import { SUNCLAW_STATE_SCHEMA_SQL } from "./sunclaw-state-schema.generated.js";

const SUNCLAW_STATE_SCHEMA_VERSION = 1;
export const SUNCLAW_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const SUNCLAW_STATE_DIR_MODE = 0o700;
const SUNCLAW_STATE_FILE_MODE = 0o600;
const SUNCLAW_STATE_SIDECAR_SUFFIXES = ["", "-shm", "-wal"] as const;

export type SunClawStateDatabase = {
  db: DatabaseSync;
  path: string;
  walMaintenance: SqliteWalMaintenance;
};

export type SunClawStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

export type SunClawMigrationRunStatus = "completed" | "warning" | "failed";
export type SunClawBackupRunStatus = "completed" | "failed";

export type RecordSunClawStateMigrationRunOptions = SunClawStateDatabaseOptions & {
  id?: string;
  startedAt: number;
  finishedAt?: number;
  status: SunClawMigrationRunStatus;
  report: Record<string, unknown>;
};

export type RecordSunClawStateMigrationSourceOptions = SunClawStateDatabaseOptions & {
  runId: string;
...
export type RecordSunClawStateBackupRunOptions = SunClawStateDatabaseOptions & {
  id?: string;
  createdAt: number;
  archivePath: string;
  status: SunClawBackupRunStatus;
  manifest: Record<string, unknown>;
};

const cachedDatabases = new Map<string, SunClawStateDatabase>();

type SunClawStateMetadataDatabase = Pick<
  SunClawStateKyselyDatabase,
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > SUNCLAW_STATE_SCHEMA_VERSION) {
    throw new Error(
      `SunClaw state database ${pathname} uses newer schema version ${userVersion}; this SunClaw build supports ${SUNCLAW_STATE_SCHEMA_VERSION}.`,
    );
  }
}

function ensureSunClawStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveSunClawStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveSunClawStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`SunClaw state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: SUNCLAW_STATE_DIR_MODE });
  if (isDefaultStateDatabase || !dirExisted) {
    chmodSync(dir, SUNCLAW_STATE_DIR_MODE);
  }
  for (const suffix of SUNCLAW_STATE_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (existsSync(candidate)) {
      chmodSync(candidate, SUNCLAW_STATE_FILE_MODE);
    }
  }
}
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(SUNCLAW_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${SUNCLAW_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<SunClawStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: SUNCLAW_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: SunClawStateDatabaseOptions = {}): string {
  return options.path ?? resolveSunClawStateSqlitePath(options.env ?? process.env);
}

export function openSunClawStateDatabase(
  options: SunClawStateDatabaseOptions = {},
): SunClawStateDatabase {
  ...
  ensureSunClawStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  const walMaintenance = configureSqliteWalMaintenance(db, {
    databaseLabel: "sunclaw-state",
...
export function runSunClawStateWriteTransaction<T>(
  operation: (database: SunClawStateDatabase) => T,
  options: SunClawStateDatabaseOptions = {},
): T {
  const database = openSunClawStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureSunClawStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function recordSunClawStateMigrationRun(
  options: RecordSunClawStateMigrationRunOptions,
): string {
  const id = options.id ?? randomUUID();
  runSunClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<SunClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
      database.db,
...
export function recordSunClawStateMigrationSource(
  options: RecordSunClawStateMigrationSourceOptions,
): void {
  runSunClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<SunClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function recordSunClawStateBackupRun(options: RecordSunClawStateBackupRunOptions): string {
  const id = options.id ?? randomUUID();
  runSunClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<SunClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function closeSunClawStateDatabase(): void {
...
export function isSunClawStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

export const closeSunClawStateDatabaseForTest = closeSunClawStateDatabase;
