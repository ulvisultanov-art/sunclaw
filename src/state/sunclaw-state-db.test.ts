...
function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-state-db-"));
}
...
describe("sunclaw state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveSunClawStateSqlitePath({ SUNCLAW_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "sunclaw.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "sunclaw-test-state", `${process.pid}-7`, "state", "sunclaw.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./sunclaw-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "sunclaw.sqlite"));
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `sunclaw-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openSunClawStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openSunClawStateDatabase({ path: firstPath });
    const second = openSunClawStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openSunClawStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { SUNCLAW_STATE_DIR: stateDir } };
    ...
    runSunClawStateWriteTransaction((database) => {
      ...
      expect(() =>
        runSunClawStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runSunClawStateWriteTransaction(async () => {
        return "not sync";
      }, options),
