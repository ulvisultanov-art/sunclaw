...
type GatewayStartupStateEmitterMock = ReturnType<
  typeof vi.fn<(code: string, message: string, cfg: SunClawConfig) => void>
>;

const RESOLVED_GATEWAY_TOKEN = "resolved-gateway-token";

function gatewayTokenConfig(config: SunClawConfig): SunClawConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: SunClawConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/sunclaw-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: SunClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
function runtimeSecretsActivatorOptionsForTest() {
  return {
    ...
    emitStateEvent: vi.fn<(code: string, message: string, cfg: SunClawConfig) => void>(),
  };
}
...
function installDiagnosticsTimelineEnv() {
  const root = mkdtempSync(path.join(tmpdir(), "sunclaw-startup-secrets-timeline-"));
  const timelinePath = path.join(root, "timeline.jsonl");
  const previousDiagnostics = process.env.SUNCLAW_DIAGNOSTICS;
  const previousTimelinePath = process.env.SUNCLAW_DIAGNOSTICS_TIMELINE_PATH;
  process.env.SUNCLAW_DIAGNOSTICS = "timeline";
  process.env.SUNCLAW_DIAGNOSTICS_TIMELINE_PATH = timelinePath;

  return {
    timelinePath,
    cleanup: () => {
      if (previousDiagnostics === undefined) {
        delete process.env.SUNCLAW_DIAGNOSTICS;
      } else {
        process.env.SUNCLAW_DIAGNOSTICS = previousDiagnostics;
      }
      if (previousTimelinePath === undefined) {
        delete process.env.SUNCLAW_DIAGNOSTICS_TIMELINE_PATH;
      } else {
        process.env.SUNCLAW_DIAGNOSTICS_TIMELINE_PATH = previousTimelinePath;
      }
...
async function activateImportedStartupConfig(config: SunClawConfig) {
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.SUNCLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.SUNCLAW_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.SUNCLAW_SKIP_CHANNELS;
    } else {
      process.env.SUNCLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.SUNCLAW_SKIP_PROVIDERS;
    } else {
      process.env.SUNCLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
  ...
  it("activates no-SecretRef startup config without importing the full secrets runtime", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "sunclaw-startup-fast-path-"));
    const runtimeImport = vi.fn();
    const prepareRuntimeSecretsSnapshot = vi.fn(async ({ config }) => preparedSnapshot(config));
    const activateRuntimeSecretsSnapshot = vi.fn();
  ...
  it("keeps the full secrets runtime path when startup config has a SecretRef", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("sunclaw-startup-secret-ref-");
  ...
  it("keeps the full secrets runtime path when auth profile files are present", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("sunclaw-startup-auth-store-");
    writeFileSync(
