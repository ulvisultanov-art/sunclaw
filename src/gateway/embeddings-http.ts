...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== SUNCLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `sunclaw` or `sunclaw/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-sunclaw-model")) ||
