...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "sunclaw.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/sunclaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (SunClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/sunclaw"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.SUNCLAW_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.SUNCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.SUNCLAW_SSH_PORT = "2222";
  ...
  it("skips local discovery services for truthy SUNCLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.SUNCLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "sunclaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("sunclaw.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (SunClaw)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (sunclaw.internal. → /tmp/sunclaw.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
