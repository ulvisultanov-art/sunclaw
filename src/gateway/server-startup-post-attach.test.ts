...
const hoisted = vi.hoisted(() => {
  ...
  const ensureSunClawModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/sunclaw-state",
    resolveConfigPath: vi.fn(() => "/tmp/sunclaw-state/sunclaw.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/sunclaw-state"),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureSunClawModelsJson: hoisted.ensureSunClawModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("SUNCLAW_SKIP_CHANNELS", "0");
    vi.stubEnv("SUNCLAW_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureSunClawModelsJson.mockReset();
    hoisted.ensureSunClawModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-home-"));
    try {
      const sunclawHome = path.join(osHome, "sunclaw-home");
      const stateDirFromHome = path.join(sunclawHome, ".sunclaw");
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          SUNCLAW_HOME: "~/sunclaw-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\sunclaw-state`);
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          SUNCLAW_STATE_DIR: "~\\sunclaw-state",
        } as NodeJS.ProcessEnv),
      ).toBe(true);
    } finally {
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/sunclaw-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/sunclaw-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        SUNCLAW_SKIP_CHANNELS: undefined,
        SUNCLAW_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { SUNCLAW_SKIP_CHANNELS: undefined, SUNCLAW_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { SUNCLAW_SKIP_CHANNELS: "1", SUNCLAW_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (SUNCLAW_SKIP_CHANNELS=1 or SUNCLAW_SKIP_PROVIDERS=1)",
    );
  });
