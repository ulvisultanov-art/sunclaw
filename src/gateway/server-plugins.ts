...
import { clearActivatedPluginRuntimeState, loadSunClawPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "sunclaw.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "sunclaw.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: SunClawConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.sunclaw.complex.az/plugins/sdk-runtime#api-runtime-subagent and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadSunClawPlugins({
    config: resolvedConfig,
