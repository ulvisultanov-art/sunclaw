...
function createSnapshot(config: SunClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: SunClawConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: SunClawConfig) {
...
// set SUNCLAW_SKIP_CHANNELS / SUNCLAW_SKIP_PROVIDERS at module load. When a
