...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers SUNCLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.SUNCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.SUNCLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.SUNCLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.SUNCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SUNCLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.SUNCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            SUNCLAW_VERSION: " ",
            SUNCLAW_SERVICE_VERSION: "2.4.6-service",
    ...
    test("hello-ok reports persisted token scopes when reusing an existing device token", async () => {
      ...
      const deviceIdentityPath = path.join(
        os.tmpdir(),
        `sunclaw-shared-auth-scope-reuse-${randomUUID()}.json`,
      );
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `sunclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
