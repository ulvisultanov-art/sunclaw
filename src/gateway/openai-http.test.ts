  ...
  it("handles request validation and routing", async () => {
    ...
    try {
      ...
      await expectAgentSessionKeyMatch({
        body: { model: "sunclaw", messages: [{ role: "user", content: "hi" }] },
        headers: { "x-sunclaw-agent-id": "beta" },
        matcher: /^agent:beta:/,
      });
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "sunclaw", messages: [{ role: "user", content: "hi" }] },
          {
            "x-sunclaw-agent-id": "beta",
            "x-sunclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe("Invalid `model`. Use `sunclaw` or `sunclaw/<agentId>`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-sunclaw-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from SunClaw.");
      }
