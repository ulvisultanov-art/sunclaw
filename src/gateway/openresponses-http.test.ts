...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("SUNCLAW_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "sunclaw", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `sunclaw` or `sunclaw/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "sunclaw", input: "hi" },
        { "x-sunclaw-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "sunclaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "sunclaw/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "sunclaw", input: "hi" },
        { "x-sunclaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-sunclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "sunclaw", input: "hi" },
      { "x-sunclaw-scopes": "operator.admin, operator.write" },
    );
