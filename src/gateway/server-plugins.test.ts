...
const loadSunClawPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadOptions(): Record<string, unknown> {
  return requireRecord(
    getLastMockFirstArg(loadSunClawPlugins, "plugin load"),
    "plugin load options",
  );
}
...
function getLastPluginLoadLogger(): {
  ...
  const call = getLastMockFirstArg(loadSunClawPlugins, "plugin load") as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadSunClawPlugins.mockReturnValue(createRegistry([]));
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadSunClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadSunClawPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", () => {
    ...
    expect(loadSunClawPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.sunclaw.complex.az/plugins/sdk-runtime#api-runtime-subagent and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadSunClawPlugins.mockReturnValue(pluginRegistry);
