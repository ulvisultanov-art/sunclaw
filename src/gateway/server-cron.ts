...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.SUNCLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: SunClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: SunClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: SunClawConfig, requestedAgentId: string) => {
