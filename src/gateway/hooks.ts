...
export function resolveHooksConfig(cfg: SunClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SunClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-sunclaw-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-sunclaw-idempotency-key"]) ||
