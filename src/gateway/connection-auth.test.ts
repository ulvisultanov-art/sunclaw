...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: SunClawConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<SunClawConfig>): SunClawConfig {
  return input as SunClawConfig;
}
...
const DEFAULT_ENV = {
  ...
  SUNCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when SUNCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when SUNCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when SUNCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
