...
let lastCreateSunClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/sunclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSunClawToolsContext?.agentTo,
          agentThreadId: lastCreateSunClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSunClawTools: (ctx: Record<string, unknown>) => {
      lastCreateSunClawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSunClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-sunclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-sunclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateSunClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateSunClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSunClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSunClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateSunClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateSunClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateSunClawToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-sunclaw-message-to": "channel:24514",
        "x-sunclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as full operator access on /tools/invoke", async () => {
    ...
    expect(lastCreateSunClawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateSunClawToolsContext?.senderIsOwner).toBe(false);
