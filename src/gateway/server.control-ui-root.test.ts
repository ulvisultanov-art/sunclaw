...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-gateway-ui-hardlink-"));
  try {
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global SunClaw package control-ui", async () => {
