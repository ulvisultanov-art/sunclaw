...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.SUNCLAW_CONFIG_PATH) {
    configPaths.add(process.env.SUNCLAW_CONFIG_PATH);
  }
  if (process.env.SUNCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.SUNCLAW_STATE_DIR, "sunclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "sunclaw-test-no-bundled-extensions")
    : "sunclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".sunclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".sunclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".sunclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>sunclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__sunclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__sunclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["SUNCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.SUNCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SUNCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.SUNCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
