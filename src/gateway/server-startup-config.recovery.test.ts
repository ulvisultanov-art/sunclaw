...
import type { ConfigFileSnapshot, ModelDefinitionConfig, SunClawConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: SunClawConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: SunClawConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: SunClawConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/sunclaw-startup-recovery.json";
...
function buildSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: params.valid,
    config: params.config ?? ({} as SunClawConfig),
...
function buildRuntimeSnapshot(
  ...
  runtimeConfig: SunClawConfig = sourceConfig,
): ConfigFileSnapshot {
  return {
...
function expectPluginAutoEnableFor(config: SunClawConfig) {
...
function mockRuntimeAutoEnable(config: SunClawConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as SunClawConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as SunClawConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("sunclaw doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as SunClawConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "sunclaw doctor --fix" to repair, then retry.');
  });
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires SunClaw >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
    });
    vi.mocked(configIo.readConfigFileSnapshot).mockResolvedValueOnce(invalidSnapshot);
