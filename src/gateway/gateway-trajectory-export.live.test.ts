...
const CODEX_HARNESS_LIVE = process.env.SUNCLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.SUNCLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["SUNCLAW_TRAJECTORY", "SUNCLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-sunclaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.SUNCLAW_TRAJECTORY = "1";
      process.env.SUNCLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".sunclaw", "trajectory-exports", "bundle");
