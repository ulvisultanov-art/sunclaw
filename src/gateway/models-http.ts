...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([SUNCLAW_MODEL_ID, SUNCLAW_DEFAULT_MODEL_ID]);
  ids.add(`sunclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`sunclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== SUNCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
