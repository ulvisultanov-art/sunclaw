...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-session-history-"));
...
type SessionHistoryMessage = {
  ...
  __sunclaw?: { id?: string; seq?: number };
};
...
function expectSunClawMetadata(
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expectSunClawMetadata(
      (event.data as { message?: { __sunclaw?: { id?: string; seq?: number } } }).message?.[
        "__sunclaw"
      ],
      {
        id: params.id,
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectSunClawMetadata(body.messages?.[0]?.["__sunclaw"], {
        seq: 1,
      });
    });
  });
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(firstBody.messages?.map((message) => message["__sunclaw"]?.seq)).toEqual([2, 3]);
      ...
      expect(secondBody.messages?.map((message) => message["__sunclaw"]?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectSunClawMetadata(nextData.messages?.[0]?.["__sunclaw"], {
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const refreshData = refreshEvent.data as {
        messages?: Array<{ content?: Array<{ text?: string }>; __sunclaw?: { seq?: number } }>;
      };
      expect(refreshData.messages?.[0]?.content?.[0]?.text).toBe("second message");
      expect(refreshData.messages?.[0]?.["__sunclaw"]?.seq).toBe(2);
