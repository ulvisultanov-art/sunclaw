...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal SunClaw voice control result.",
    "Do not call sunclaw_agent_consult or any other tool for this message.",
    "Speak this exact SunClaw status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "SunClaw is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "SunClaw has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "SunClaw could not queue that follow-up."
    : "SunClaw could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "SunClaw finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `SunClaw is starting ${name}.` : "SunClaw is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `SunClaw finished ${name} and is continuing.`
        : "SunClaw finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `SunClaw is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "SunClaw is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `SunClaw is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "SunClaw is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "SunClaw is working on the current voice request.";
  }
