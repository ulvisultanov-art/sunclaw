...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/sunclaw-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/sunclaw-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/sunclaw-agent"}/auth-profiles.json`,
    ),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/sunclaw-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/sunclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/sunclaw-dev/sunclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/sunclaw-agent/auth-profiles.json");
  ...
  it("honors SUNCLAW_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    const previous = process.env.SUNCLAW_AGENT_DIR;
    process.env.SUNCLAW_AGENT_DIR = "/tmp/sunclaw-isolated-agent";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.SUNCLAW_AGENT_DIR = previous;
      }
    }

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/sunclaw-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/sunclaw-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/sunclaw-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when SUNCLAW_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    const previousSunClaw = process.env.SUNCLAW_AGENT_DIR;
    const previousPi = process.env.PI_CODING_AGENT_DIR;
    delete process.env.SUNCLAW_AGENT_DIR;
    process.env.PI_CODING_AGENT_DIR = "/tmp/sunclaw-legacy-agent";
    mocks.resolveAgentDir.mockClear();
    try {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    } finally {
      if (previousSunClaw === undefined) {
        delete process.env.SUNCLAW_AGENT_DIR;
      } else {
        process.env.SUNCLAW_AGENT_DIR = previousSunClaw;
      }
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/sunclaw-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/sunclaw-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/sunclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/sunclaw-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/sunclaw-agent-custom/auth-profiles.json",
        });
      },
    );
  });
      ...
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          ...
          effective: {
            kind: "profiles",
            detail: "/tmp/sunclaw-agent/auth-profiles.json",
          },
        },
      ]);
      expect(localRuntime.exit).not.toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/sunclaw-agent/workspace"
          ? {
