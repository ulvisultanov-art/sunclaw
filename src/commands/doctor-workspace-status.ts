...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("sunclaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("sunclaw tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: SunClawConfig) {
