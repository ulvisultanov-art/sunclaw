...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("sunclaw security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("sunclaw security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      "  Docs: https://docs.sunclaw.complex.az/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.sunclaw.complex.az/faq",
    "Troubleshooting: https://docs.sunclaw.complex.az/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("sunclaw status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("sunclaw logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("sunclaw node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("sunclaw status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("sunclaw gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`sunclaw devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("sunclaw devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("sunclaw devices list")}`),
  ];
}
