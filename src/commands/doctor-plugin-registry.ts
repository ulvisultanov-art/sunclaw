...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "sunclaw.plugin.json"));
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("sunclaw doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("sunclaw doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
export async function maybeRepairManagedNpmSunClawPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  const npmRoots = listManagedPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audits = await Promise.all(
      npmRoots.map((npmRoot) => auditSunClawPeerDependenciesInManagedNpmRoot({ npmRoot })),
    );
    const issues = audits.flatMap((audit) => audit.issues);
    if (issues.length > 0) {
      note(
        [
          "Managed npm SunClaw host peer links need repair:",
          ...issues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("sunclaw doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (repaired > 0) {
    note(
      `Repaired SunClaw host peer link(s) for ${repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm SunClaw host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmSunClawPeerLinks = await maybeRepairManagedNpmSunClawPeerLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("sunclaw doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmSunClawPeerLinks
  ) {
