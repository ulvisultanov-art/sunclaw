...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: SunClawConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => SunClawConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: SunClawConfig, sourceConfig: SunClawConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: SunClawConfig;
      effectiveConfig: SunClawConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sunclaw-agent-" });
}
...
function mockConfig(home: string, storePath: string): SunClawConfig {
  const cfg = {
    agents: {
      defaults: {
