...
import {
  SUNCLAW_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveSunClawWrapperPath,
...
// they share the SUNCLAW_ prefix that is otherwise stripped from preserved
...
const PRESERVED_SUNCLAW_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "SUNCLAW_CLI_CONTAINER_BYPASS",
  "SUNCLAW_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("SUNCLAW_") && !PRESERVED_SUNCLAW_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[SUNCLAW_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${SUNCLAW_WRAPPER_ENV_KEY} because it points to the Windows task script; using the SunClaw gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveSunClawWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [SUNCLAW_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, SUNCLAW_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.SUNCLAW_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("sunclaw gateway install")}\` after fixing the error.`;
}
