...
function collectConfiguredChannelIds(cfg: SunClawConfig): string[] {
  const channels =
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: SunClawConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  const doctorFixCommand = formatCliCommand("sunclaw doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- sunclaw doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
