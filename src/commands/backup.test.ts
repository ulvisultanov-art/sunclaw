...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("sunclaw-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".sunclaw");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.SUNCLAW_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "sunclaw.json"), JSON.stringify({}), "utf8");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".sunclaw");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".sunclaw");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backups-"));
  ...
  it("keeps volatile-skip notices out of json output", async () => {
    const stateDir = path.join(tempHome.home, ".sunclaw");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backups-json-"));
    try {
      const runtime = createBackupTestRuntime();
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".sunclaw");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-cwd-link-"));
