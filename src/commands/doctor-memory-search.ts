...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: SunClawConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: SunClawConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("sunclaw doctor --fix")} or ${formatCliCommand("sunclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("sunclaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: SunClawConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: SunClawConfig): boolean {
 ...
 * Runs as part of `sunclaw doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("sunclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("sunclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("sunclaw memory status --deep")}`,
        ]
          .filter(Boolean)
    ...
    note(
      [
        ...
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`sunclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("sunclaw config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("sunclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("sunclaw config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("sunclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `sunclaw doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("sunclaw configure --section model")}`,
      `- To disable: ${formatCliCommand("sunclaw config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("sunclaw memory status --deep")}`,
    ].join("\n"),
    "Memory search",
