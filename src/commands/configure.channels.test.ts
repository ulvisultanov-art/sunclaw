...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from sunclaw.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from sunclaw.json", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram configuration from ~/.sunclaw/sunclaw.json?",
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.sunclaw/sunclaw.json?");
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram\\nBot configuration from ~/.sunclaw/sunclaw.json?",
    );
    expect(note).toHaveBeenCalledWith(
