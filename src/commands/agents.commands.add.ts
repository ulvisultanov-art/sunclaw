...
export async function agentsAddCommand(
  ...
  if (nonInteractive && !workspaceFlag) {
    runtime.error(
      `Non-interactive agent creation requires --workspace. Re-run ${formatCliCommand("sunclaw agents add <id> --workspace <path>")} or omit flags to use the wizard.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (nonInteractive) {
    if (!nameInput) {
      runtime.error(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("sunclaw agents add <id> --workspace <path>")}.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (agentId === DEFAULT_AGENT_ID) {
      runtime.error(
        `"${DEFAULT_AGENT_ID}" is reserved. Choose another name, or run ${formatCliCommand("sunclaw agents list")} to inspect the default agent.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId) >= 0) {
      runtime.error(
        `Agent "${agentId}" already exists. Run ${formatCliCommand("sunclaw agents list")} to inspect configured agents.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  try {
    await prompter.intro("Add SunClaw agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.sunclaw.complex.az/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
