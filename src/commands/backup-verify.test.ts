...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-sunclaw-backup";
...
async function createArchiveWithManifestContent(
  ...
  const payloadArchivePath =
    options.payloadArchivePath ?? `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/payload.txt`;
  try {
...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-verify-out-"));
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.sunclaw/state.txt`;
      await fs.writeFile(
        manifestPath,
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-sunclaw-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.sunclaw",
            archivePath: `${rootName}/payload/posix/tmp/.sunclaw`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("reports malformed manifest JSON without leaking parser internals", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "sunclaw-backup-bad-manifest-json-",
  ...
  it("rejects oversized manifest entries without retaining the full body", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "sunclaw-backup-huge-manifest-",
  ...
  it("rejects unsafe hardlink targets", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-linkpath-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/hardlink.txt`;
    try {
  ...
  it("accepts root-relative internal hardlink targets from older backups", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-rootless-linkpath-"));
    const archivePath = path.join(tempDir, "backup.tar.gz");
    const rootRelativeTargetPath = "payload/posix/tmp/.sunclaw/target.txt";
  ...
  it("rejects hardlink targets missing from archive entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-backup-missing-linkpath-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/hardlink.txt`;
    const missingTargetPath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/missing-target.txt`;
    try {
      const archive = gzipSync(
        Buffer.concat([
          encodeTarEntry({
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.sunclaw/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.sunclaw/payload.txt`;
    for (const options of [
      {
        tempPrefix: "sunclaw-backup-duplicate-manifest-",
        payloads: [{ fileName: "payload.txt", contents: "payload\n" }],
        buildTarEntries: ({
          manifestPath,
          payloadPaths,
      ...
      {
        tempPrefix: "sunclaw-backup-duplicate-payload-",
        payloads: [
