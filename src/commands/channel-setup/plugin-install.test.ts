...
import { bundledPluginRoot, bundledPluginRootAt } from "sunclaw/plugin-sdk/test-fixtures";
...
const discoverSunClawPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverSunClawPlugins: (args: unknown) => discoverSunClawPlugins(args),
}));
...
const bundledChatNpmSpec = "@sunclaw/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/sunclaw", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/sunclaw-test/${id}`,
    source: `/tmp/sunclaw-test/${id}/index.ts`,
    manifestPath: `/tmp/sunclaw-test/${id}/sunclaw.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadSunClawPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadSunClawPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverSunClawPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: SunClawConfig = { update: { channel } };
...
function expectLoadSunClawPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadSunClawPlugins), callIndex),
    "loadSunClawPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: SunClawConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/sunclaw-ledger-channel";
    process.env.SUNCLAW_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: SunClawConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      ...
      npmResolution: {
        name: "@sunclaw/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@sunclaw/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(requireMockCallArg(installPluginFromNpmSpec, 0), "npm install args", {
      spec: "@sunclaw/wecom@beta",
      expectedPluginId: "wecom-sunclaw-plugin",
    });
    expect(result.cfg.plugins?.installs?.["wecom-sunclaw-plugin"]?.spec).toBe("@sunclaw/wecom");
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/sunclaw", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:sunclaw/clawhub-chat@2026.5.2",
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:sunclaw/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@sunclaw/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-sunclaw-plugin");
  });
  ...
  it("scopes snapshots by activation-declared channel ownership when direct channel lists are empty", () => {
    ...
    loadPluginManifestRegistry.mockImplementation((args: unknown) => {
      if (
        isRecord(args) &&
        args.config === cfg &&
        args.workspaceDir === "/tmp/sunclaw-workspace" &&
