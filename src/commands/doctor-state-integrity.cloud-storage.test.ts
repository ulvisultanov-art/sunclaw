...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "SunClaw", ".sunclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/sunclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".sunclaw");
  ...
  it("anchors cloud detection to OS homedir when SUNCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".sunclaw");
    const originalSunClawHome = process.env.SUNCLAW_HOME;
    process.env.SUNCLAW_HOME = "/tmp/sunclaw-home-override";
