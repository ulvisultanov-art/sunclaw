...
export async function appendStatusAllDiagnosis(params: {
  ...
  if (params.portUsage) {
    ...
    if (!portOk) {
      ...
      if (gatewayPidCount > 1) {
        lines.push(
          `  ${muted(`${gatewayPidCount} SunClaw gateway processes appear to be listening on port ${params.port}; stop stale gateway processes before trusting channel health.`)}`,
        );
      }
      ...
      lines.push(`  ${muted("Detected SunClaw Gateway listener on the configured port.")}`);
    }
  }
  ...
  lines.push("Troubleshooting: https://docs.sunclaw.complex.az/troubleshooting");
  lines.push("");
}
