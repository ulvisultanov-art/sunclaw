    ...
    findLegacyConfigIssues: (raw: unknown, sourceRaw?: unknown, extraRules: LegacyRule[] = []) => {
      ...
      if ("heartbeat" in root) {
        addIssue(
          issues,
          ["heartbeat"],
          'heartbeat is legacy; use agents.defaults.heartbeat and channels.defaults.heartbeat. Run "sunclaw doctor --fix".',
        );
      }
      if ("memorySearch" in root) {
        addIssue(
          issues,
          ["memorySearch"],
          'memorySearch is legacy; use agents.defaults.memorySearch. Run "sunclaw doctor --fix".',
        );
      }
      ...
      if (gateway && "bind" in gateway) {
        addIssue(
          issues,
          ["gateway", "bind"],
          'gateway.bind host aliases are legacy; use the canonical bind mode. Run "sunclaw doctor --fix".',
        );
      }
      ...
      if (sessionThreadBindings && "ttlHours" in sessionThreadBindings) {
        addIssue(
          ...
          'session.threadBindings.ttlHours is legacy; use session.threadBindings.idleHours. Run "sunclaw doctor --fix".',
        );
      }
      ...
      if (xSearch && "apiKey" in xSearch) {
        addIssue(
          ...
          'tools.web.x_search.apiKey is legacy; use plugins.entries.xai.config.webSearch.apiKey. Run "sunclaw doctor --fix".',
        );
      }
      ...
      if (sandbox && "perSession" in sandbox) {
        addIssue(
          ...
          'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope. Run "sunclaw doctor --fix".',
        );
      }
      ...
      for (const [channelId, channelRaw] of Object.entries(channels ?? {})) {
        ...
        if (hasLegacyStreamingAlias(channel)) {
          addIssue(
            ...
            channelId === "googlechat"
              ? `channels.${channelId}.streamMode is legacy and no longer used. Run "sunclaw doctor --fix".`
              : `channels.${channelId}.streamMode, channels.${channelId}.streaming aliases are legacy. Run "sunclaw doctor --fix".`,
          );
        }
        ...
        if (threadBindings && "ttlHours" in threadBindings) {
          addIssue(
            ...
            'channels.<id>.threadBindings.ttlHours is legacy; use channels.<id>.threadBindings.idleHours. Run "sunclaw doctor --fix".',
          );
        }
        if (channelId === "slack") {
          for (const roomRaw of Object.values(asRecord(channel.channels) ?? {})) {
            if ("allow" in (asRecord(roomRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "slack"],
                'channels.slack.channels.<id>.allow is legacy; use enabled. Run "sunclaw doctor --fix".',
              );
            }
          }
        }
          ...
          for (const spaceRaw of Object.values(asRecord(channel.groups) ?? {})) {
            if ("allow" in (asRecord(spaceRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "googlechat"],
                'channels.googlechat.groups.<id>.allow is legacy; use enabled. Run "sunclaw doctor --fix".',
              );
            }
          }
        }
            ...
            for (const roomRaw of Object.values(asRecord(guild?.channels) ?? {})) {
              if ("allow" in (asRecord(roomRaw) ?? {})) {
                addIssue(
                  issues,
                  ["channels", "discord"],
                  'channels.discord.guilds.<id>.channels.<id>.allow is legacy; use enabled. Run "sunclaw doctor --fix".',
                );
              }
            }
          }
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmSunClawPeerLinks: vi.fn(async () => undefined),
    ...
    listPluginDoctorLegacyConfigRules: () => [
      {
        ...
        message:
          'channels.telegram.groupMentionsOnly was removed; use channels.telegram.groups."*".requireMention instead. Run "sunclaw doctor --fix".',
      },
      {
        path: ["talk"],
        message:
          "talk.voiceId/talk.voiceAliases/talk.modelId/talk.outputFormat/talk.apiKey are legacy; use talk.providers.<provider> and run sunclaw doctor --fix.",
...
vi.mock("./doctor-config-preflight.js", async () => {
  ...
  function resolveConfigPath() {
    const stateDir =
      process.env.SUNCLAW_STATE_DIR ||
      (process.env.HOME ? pathLocal.join(process.env.HOME, ".sunclaw") : "");
    return process.env.SUNCLAW_CONFIG_PATH || pathLocal.join(stateDir, "sunclaw.json");
  }
  ...
  it("previews and repairs hooks token reuse of gateway auth", async () => {
    ...
    expect(
      previewNotes.mock.calls.some(
        ([message, title]) =>
          title === "Doctor" &&
          message.includes("sunclaw doctor --fix") &&
  ...
  it("warns when hooks transformsDir points outside the hook transforms root", async () => {
    const doctorWarnings = await collectDoctorWarnings({
      hooks: {
        enabled: true,
        token: "hook-secret",
        transformsDir: "/virtual/.sunclaw/workspace/skills/linear-webhook",
        mappings: [
          {
            match: { path: "linear" },
            action: "agent",
            messageTemplate: "Linear event",
            transform: { module: "./sunclaw-linear-transform.js" },
          },
        ],
      },
    });
    ...
    expect(warning).toContain("/virtual/.sunclaw/workspace/skills/linear-webhook");
    expect(warning).toContain("/virtual/.sunclaw/hooks/transforms");
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            message.includes('Run "sunclaw doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockClear();
