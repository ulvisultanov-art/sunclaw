...
import type { SunClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SunClawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: SunClawConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "SunClaw default (full)";
  const defaultRequestedAskSource = "SunClaw default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("sunclaw approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: SunClawConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: SunClawConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: SunClawConfig): string[] {
  ...
  return [
    "- WARNING: sunclaw.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("sunclaw secrets configure")} or ${formatCliCommand("sunclaw secrets apply")}, then verify with ${formatCliCommand("sunclaw secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.sunclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("sunclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("sunclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("sunclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("sunclaw doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "sunclaw config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("sunclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('sunclaw config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: SunClawConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("sunclaw security audit --deep")}`;
