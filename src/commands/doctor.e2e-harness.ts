...
export const resolveSunClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadSunClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("sunclaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("sunclaw/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveSunClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SUNCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SUNCLAW_UPDATE_IN_PROGRESS;
  process.env.SUNCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-doctor-state-"));
