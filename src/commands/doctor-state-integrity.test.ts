...
type EnvSnapshot = {
  HOME?: string;
  SUNCLAW_HOME?: string;
  SUNCLAW_STATE_DIR?: string;
  SUNCLAW_OAUTH_DIR?: string;
  SUNCLAW_AGENT_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    SUNCLAW_OAUTH_DIR: process.env.SUNCLAW_OAUTH_DIR,
    SUNCLAW_AGENT_DIR: process.env.SUNCLAW_AGENT_DIR,
  };
}
...
function setupSessionState(cfg: SunClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("SUNCLAW_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: SunClawConfig) {
...
async function runStateIntegrityText(cfg: SunClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.SUNCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when SUNCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.SUNCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SunClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when SUNCLAW_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.SUNCLAW_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("sunclaw tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.SUNCLAW_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.SUNCLAW_HOME = symlinkHome;
        process.env.SUNCLAW_STATE_DIR = path.join(symlinkHome, ".sunclaw");
  ...
  it("prints sunclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SunClawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/sunclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/sunclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /sunclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-tui-pointer-clear-"));
    try {
