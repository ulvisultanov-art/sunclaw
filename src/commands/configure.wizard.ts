...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.SUNCLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.SUNCLAW_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from sunclaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<SunClawConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "SunClaw update wizard" : "SunClaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: SunClawConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.sunclaw.complex.az/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("sunclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.SUNCLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.SUNCLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.sunclaw.complex.az/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
