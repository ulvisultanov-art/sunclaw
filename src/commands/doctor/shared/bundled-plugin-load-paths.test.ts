...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "sunclaw/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/sunclaw";
  ...
  it("detects legacy bundled plugin paths that still point at source extensions", () => {
    const packageRoot = path.resolve("app-node-modules", "sunclaw");
  ...
  it("removes stale bundled paths from old versioned SunClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "sunclaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "sunclaw@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "sunclaw",
    );
  ...
  it("removes stale legacy bundled paths from old versioned SunClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "sunclaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; SunClaw loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "sunclaw doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "sunclaw", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
