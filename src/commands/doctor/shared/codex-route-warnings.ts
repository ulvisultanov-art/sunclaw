...
import { AGENT_MODEL_CONFIG_KEYS } from "@sunclaw/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@sunclaw/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@sunclaw/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@sunclaw/normalization-core/string-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: SunClawConfig): string {
...
function collectConfigModelRefs(cfg: SunClawConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: SunClawConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: SunClawConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: SunClawConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: SunClawConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: SunClawConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: SunClawConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: SunClawConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: SunClawConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores SunClaw compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `sunclaw doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an SunClaw runtime policy."
...
function collectCodexAppServerCommandWarnings(cfg: SunClawConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `sunclaw doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
...
}): { cfg: SunClawConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "sunclaw") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "sunclaw") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `sunclaw doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
