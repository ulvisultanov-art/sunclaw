...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createSunClawTestState({
    layout: "state-only",
    prefix: "sunclaw-legacy-oauth-keychain-warn-",
    env: {
      SUNCLAW_AGENT_DIR: undefined,
      SUNCLAW_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: SunClawTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("sunclaw doctor --fix");
