...
function currentSunClawReleaseBase(): string {
...
const mocks = vi.hoisted(() => ({
  ...
  resolveDefaultPluginExtensionsDir: vi.fn(() => "/tmp/sunclaw-plugins"),
  resolveDefaultPluginNpmDir: vi.fn(() => "/tmp/sunclaw-npm"),
  resolvePluginNpmPackageDir: vi.fn(
    ({ npmDir, packageName }: { npmDir?: string; packageName: string }) =>
      path.join(
        npmDir ?? "/tmp/sunclaw-npm",
        "projects",
  ...
  resolvePluginInstallDir: vi.fn(
    (pluginId: string, extensionsDir = "/tmp/sunclaw-plugins") => `${extensionsDir}/${pluginId}`,
  ),
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-plugin-stub-repair-"));
  tempDirs.push(dir);
  return dir;
}
...
function writeLegacyNpmDeclarationStub(params: {
  ...
  fs.writeFileSync(
    path.join(params.pluginDir, "sunclaw.extension.json"),
    JSON.stringify({
...
describe("repairMissingConfiguredPluginInstalls", () => {
  beforeEach(() => {
    ...
    mocks.resolveDefaultPluginExtensionsDir.mockReturnValue("/tmp/sunclaw-plugins");
    mocks.resolveDefaultPluginNpmDir.mockReturnValue("/tmp/sunclaw-npm");
    mocks.resolveProviderInstallCatalogEntries.mockReturnValue([]);
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      targetDir: "/tmp/sunclaw-plugins/matrix",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.complex.az",
        clawhubPackage: "@sunclaw/plugin-matrix",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@sunclaw/plugin-matrix",
        version: "1.2.3",
        resolvedSpec: "@sunclaw/plugin-matrix@1.2.3",
  ...
  it("installs a missing configured SunClaw channel plugin from npm by default", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
        pluginId: "matrix",
        meta: { label: "Matrix" },
        install: {
          npmSpec: "@sunclaw/plugin-matrix@1.2.3",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@sunclaw/plugin-matrix@1.2.3",
      extensionsDir: "/tmp/sunclaw-plugins",
    ...
    expectRecordFields((records as Record<string, unknown>).matrix, {
      source: "npm",
      spec: "@sunclaw/plugin-matrix@1.2.3",
      installPath: "/tmp/sunclaw-plugins/matrix",
      version: "1.2.3",
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from @sunclaw/plugin-matrix@1.2.3.',
    ]);
  ...
  it("uses an explicit ClawHub install spec before npm", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@sunclaw/plugin-matrix@stable",
          npmSpec: "@sunclaw/plugin-matrix@1.2.3",
          expectedIntegrity: "sha512-test",
        },
      },
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: "clawhub:@sunclaw/plugin-matrix@stable",
      expectedPluginId: "matrix",
    });
    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from clawhub:@sunclaw/plugin-matrix@stable.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("falls back to npm when an SunClaw channel plugin artifact is unavailable on ClawHub", async () => {
    ...
    expect(result.changes).toEqual([
      'ClawHub clawhub:@sunclaw/plugin-matrix@stable unavailable for "matrix"; falling back to npm @sunclaw/plugin-matrix@1.2.3.',
      'Installed missing configured plugin "matrix" from @sunclaw/plugin-matrix@1.2.3.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("does not fall back from ClawHub to non-SunClaw npm packages", async () => {
    ...
    expect(result.warnings).toEqual([
      'Failed to install missing configured plugin "matrix" from clawhub:@sunclaw/plugin-matrix@stable: ClawHub artifact download is not available yet.',
    ]);
  });

  it("honors npm-first catalog metadata for missing SunClaw channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "twitch",
      targetDir: "/tmp/sunclaw-plugins/twitch",
      version: "2026.5.2",
      npmResolution: {
        name: "@sunclaw/twitch",
        version: "2026.5.2",
        resolvedSpec: "@sunclaw/twitch@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/twitch"),
      expectedPluginId: "twitch",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "twitch" from ${expectedNpmInstallSpec("@sunclaw/twitch")}.`,
    ]);
  });
  ...
  it("installs missing configured non-channel plugins from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      targetDir: "/tmp/sunclaw-plugins/diagnostics-otel",
      version: "2026.5.2",
      npmResolution: {
        name: "@sunclaw/diagnostics-otel",
        version: "2026.5.2",
        resolvedSpec: "@sunclaw/diagnostics-otel@2026.5.2",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        install: {
          clawhubSpec: "clawhub:@sunclaw/diagnostics-otel",
          npmSpec: "@sunclaw/diagnostics-otel",
          defaultChoice: "npm",
        },
      },
    ]);
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/diagnostics-otel"),
      expectedPluginId: "diagnostics-otel",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "diagnostics-otel" from ${expectedNpmInstallSpec("@sunclaw/diagnostics-otel")}.`,
    ]);
  });
  ...
  it("does not let runtime fallback metadata override official catalog install specs", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "acpx",
      targetDir: "/tmp/sunclaw-plugins/acpx",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/acpx"),
      expectedPluginId: "acpx",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "acpx" from ${expectedNpmInstallSpec("@sunclaw/acpx")}.`,
    ]);
  });
  ...
  it("uses current bundled discovery to remove records before stale snapshots can reinstall official plugins", async () => {
    const records = {
      "google-meet": {
        source: "npm",
        spec: "@sunclaw/google-meet",
        resolvedName: "@sunclaw/google-meet",
      ...
      plugins: [
        {
          id: "google-meet",
          origin: "npm",
          packageName: "@sunclaw/google-meet",
        },
      ],
      diagnostics: [],
    });
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "google-meet",
        label: "Google Meet",
        install: { npmSpec: "@sunclaw/google-meet" },
        sunclaw: {
          id: "google-meet",
          install: { npmSpec: "@sunclaw/google-meet" },
        },
  ...
  it.each([
    [
      ...
      {
        source: "npm",
        spec: "@sunclaw/matrix-fork",
        resolvedName: "@sunclaw/matrix-fork",
        resolvedSpec: "@sunclaw/matrix-fork@1.2.3",
    ...
    [
      ...
      {
        source: "clawhub",
        spec: "clawhub:@sunclaw/matrix-fork@stable",
        clawhubPackage: "@sunclaw/matrix-fork",
        installPath: "/missing/matrix-fork",
      },
    ],
  ])(
  ...
  it("defers missing external payload repair during the package update doctor pass", async () => {
    ...
    expect(result).toEqual({
      changes: [
        'Skipped package-manager repair for configured plugin "discord" during package update; rerun "sunclaw doctor --fix" after the update completes.',
      ],
      warnings: [],
      records,
    });
  ...
  it("updates an existing npm target when stale baseline records miss an installed package", async () => {
    ...
    const packageDir = path.join(npmRoot, "node_modules", "@sunclaw", "discord");
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "1.2.3",
        resolvedSpec: "@sunclaw/discord@1.2.3",
    ...
    const result = await repairMissingConfiguredPluginInstalls({
      ...
      env: {
        SUNCLAW_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    });

    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/discord"),
      expectedPluginId: "discord",
      npmDir: npmRoot,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "discord" from ${expectedNpmInstallSpec("@sunclaw/discord")}.`,
    ]);
    expect(result.warnings).toEqual([]);
  ...
  it("prefers an existing npm payload over ClawHub during post-core repair", async () => {
    const npmRoot = makeTempDir();
    const packageDir = path.join(npmRoot, "node_modules", "@sunclaw", "matrix");
    fs.mkdirSync(packageDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "@sunclaw/matrix", version: "1.2.3" }),
    );
    mocks.resolveDefaultPluginNpmDir.mockReturnValue(npmRoot);
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@sunclaw/matrix",
          npmSpec: "@sunclaw/matrix",
        },
      },
    ]);
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Plugin "@sunclaw/matrix" requires plugin API >=2026.5.18.',
    });
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      ...
      npmResolution: {
        name: "@sunclaw/matrix",
        version: "1.2.3",
        resolvedSpec: "@sunclaw/matrix@1.2.3",
        integrity: "sha512-matrix",
        resolvedAt: "2026-05-01T00:00:00.000Z",
      },
    });
  ...
  it("passes the post-core compatibility host version to ClawHub repair", async () => {
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "whatsapp",
      targetDir: "/tmp/sunclaw-plugins/whatsapp",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.complex.az",
    ...
    const result = await repairMissingConfiguredPluginInstalls({
      ...
      env: {
        SUNCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        SUNCLAW_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    });

    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: expectedClawHubInstallSpec("clawhub:@sunclaw/whatsapp"),
      env: {
        SUNCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        SUNCLAW_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    ...
    expectRecordFields(result.records.whatsapp, {
      ...
      installPath: "/tmp/sunclaw-plugins/whatsapp",
      clawhubPackage: "@sunclaw/whatsapp",
    });
  });

  it("repairs missing external payload during post-core convergence even with SUNCLAW_UPDATE_IN_PROGRESS=1", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@sunclaw/discord",
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        pluginId: "discord",
        meta: { label: "Discord" },
        install: { npmSpec: "@sunclaw/discord" },
      },
    ]);
  ...
  it("installs channel-selected external plugins during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@sunclaw/discord",
        version: "2026.5.17",
        resolvedSpec: "@sunclaw/discord@2026.5.17",
        integrity: "sha512-discord",
  ...
  it("prefers npm over ClawHub during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.17",
        resolvedSpec: "@sunclaw/whatsapp@2026.5.17",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/whatsapp"),
      expectedPluginId: "whatsapp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "whatsapp" from ${expectedNpmInstallSpec("@sunclaw/whatsapp")}.`,
    ]);
    expectRecordFields(result.records.whatsapp, {
      source: "npm",
      installPath: "/tmp/sunclaw-plugins/whatsapp",
  ...
  it("installs a missing third-party downloadable plugin from npm only", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "wecom",
      targetDir: "/tmp/sunclaw-plugins/wecom",
      version: "2026.4.23",
      npmResolution: {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WeCom" },
        install: {
          npmSpec: "@wecom/wecom-sunclaw-plugin@2026.4.23",
        },
      },
    ]);
    ...
    expectRecordFields(installArg, {
      spec: "@wecom/wecom-sunclaw-plugin@2026.4.23",
      expectedPluginId: "wecom",
    });
    expect(installArg).not.toHaveProperty("trustedSourceLinkedOfficialInstall", true);
    expect(result.changes).toEqual([
      'Installed missing configured plugin "wecom" from @wecom/wecom-sunclaw-plugin@2026.4.23.',
    ]);
  });
  ...
  it("installs a missing default Codex runtime plugin from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/sunclaw-plugins/codex",
      version: "2026.5.2",
      npmResolution: {
        name: "@sunclaw/codex",
        version: "2026.5.2",
        resolvedSpec: "@sunclaw/codex@2026.5.2",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/codex"),
    ...
    expectRecordFields((records as Record<string, unknown>).codex, {
      source: "npm",
      spec: "@sunclaw/codex",
      installPath: "/tmp/sunclaw-plugins/codex",
      version: "2026.5.2",
    });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      env: {},
    ...
    expect(result.changes).toEqual([
      `Installed missing configured plugin "codex" from ${expectedNpmInstallSpec("@sunclaw/codex")}.`,
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("refreshes a stale managed Codex runtime plugin selected by the OpenAI Codex route", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@sunclaw/codex", version: "2026.5.6" }),
    );
    const records = {
      codex: {
        source: "npm",
        ...
        resolvedSpec: "@sunclaw/codex@2026.5.6",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@sunclaw/codex",
        version: VERSION,
        resolvedSpec: `@sunclaw/codex@${VERSION}`,
    ...
    expect(result.changes).toEqual([
      `Refreshed stale configured plugin "codex" from ${expectedNpmInstallSpec("@sunclaw/codex")}.`,
    ]);
  ...
  it("does not refresh a converged beta Codex runtime plugin on the second doctor pass", async () => {
    const codexBetaVersion = `${currentSunClawReleaseBase()}-beta.4`;
    const installDir = makeTempDir();
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@sunclaw/codex", version: "2026.5.6" }),
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@sunclaw/codex",
        version: codexBetaVersion,
        resolvedSpec: `@sunclaw/codex@${codexBetaVersion}`,
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@sunclaw/codex@beta",
      expectedPluginId: "codex",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(firstPass.changes).toEqual([
      'Refreshed stale configured plugin "codex" from @sunclaw/codex@beta.',
    ]);
  ...
  it("does not downgrade a newer managed Codex runtime plugin", async () => {
    ...
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@sunclaw/codex", version: "9999.1.1" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@sunclaw/codex",
        resolvedName: "@sunclaw/codex",
        resolvedSpec: "@sunclaw/codex@9999.1.1",
  ...
  it.each([
    ...
    ["environment runtime override", {}, { SUNCLAW_AGENT_RUNTIME: "codex" }],
  ...
  it("still installs a channel catalog plugin when the configured owner is blocked by the allowlist", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "feishu",
      targetDir: "/tmp/sunclaw-plugins/feishu",
      version: "2026.5.2",
      npmResolution: {
        name: "@sunclaw/feishu",
        version: "2026.5.2",
        resolvedSpec: "@sunclaw/feishu@2026.5.2",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/feishu"),
      expectedPluginId: "feishu",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "feishu" from ${expectedNpmInstallSpec("@sunclaw/feishu")}.`,
    ]);
  });
  ...
  it("reinstalls a missing configured plugin from its persisted install record", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@sunclaw/plugin-demo@1.0.0",
        ...
        plugins: {
          installs: {
            demo: {
              source: "npm",
              spec: "@sunclaw/plugin-demo@1.0.0",
              installPath: "/tmp/sunclaw-plugins/demo",
            },
          },
        },
      },
  ...
  it("repairs a broken managed package entry from its attributed registry diagnostic", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@sunclaw/plugin-demo@1.0.0",
        resolvedName: "@sunclaw/plugin-demo",
        resolvedSpec: "@sunclaw/plugin-demo@1.0.0",
  ...
  it("reinstalls a known configured plugin from the catalog when its recorded install path is missing", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@sunclaw/discord",
        installPath: "/tmp/sunclaw-missing-discord-install-record",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      diagnostics: [
        {
          ...
          pluginId: "discord",
          message:
            "channel plugin manifest declares discord without channelConfigs metadata; add sunclaw.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads",
        },
      ],
    });
    mocks.updateNpmInstalledPlugins.mockResolvedValue({
  ...
  it("reinstalls a recorded external web search plugin from provider-only config", async () => {
    const records = {
      brave: {
        source: "npm",
        spec: "@sunclaw/brave-plugin@beta",
  ...
  it("replaces a configured official web search plugin when its installed package is source-only", async () => {
    ...
    const records = {
      brave: {
        source: "clawhub",
        spec: "clawhub:@sunclaw/brave-plugin@2026.5.1-beta.1",
        installPath: installDir,
        clawhubPackage: "@sunclaw/brave-plugin",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.complex.az",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      targetDir: "/tmp/sunclaw-plugins/brave",
      version: "2026.5.12",
      npmResolution: {
        name: "@sunclaw/brave-plugin",
        version: "2026.5.12",
        resolvedSpec: "@sunclaw/brave-plugin@2026.5.12",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/brave-plugin"),
    ...
    expectRecordFields(persistedRecords.brave, {
      source: "npm",
      spec: "@sunclaw/brave-plugin",
      installPath: "/tmp/sunclaw-plugins/brave",
      version: "2026.5.12",
    });
    expect(result).toEqual({
      changes: [
        `Installed missing configured plugin "brave" from ${expectedNpmInstallSpec("@sunclaw/brave-plugin")}.`,
      ],
      warnings: [],
  ...
  it("replaces a configured official channel plugin when only its channel is configured", async () => {
    ...
    const records = {
      slack: {
        source: "clawhub",
        spec: "clawhub:@sunclaw/slack@2026.5.12-beta.1",
        installPath: installDir,
        clawhubPackage: "@sunclaw/slack",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.complex.az",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "slack",
      targetDir: "/tmp/sunclaw-npm/node_modules/@sunclaw/slack",
      version: "2026.5.12",
      npmResolution: {
        name: "@sunclaw/slack",
        version: "2026.5.12",
        resolvedSpec: "@sunclaw/slack@2026.5.12",
    ...
    expect(fs.existsSync(installDir)).toBe(false);
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@sunclaw/slack"),
      expectedPluginId: "slack",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "slack" from ${expectedNpmInstallSpec("@sunclaw/slack")}.`,
    ]);
  });
  ...
  it("keeps a broken official install record when replacement install fails", async () => {
    ...
    expect(result).toEqual({
      changes: [],
      warnings: [
        `Failed to install missing configured plugin "brave" from ${expectedNpmInstallSpec("@sunclaw/brave-plugin")}: network unavailable`,
      ],
  ...
  it("installs a configured external web search plugin from provider-only config", async () => {
    ...
    mocks.resolveOfficialExternalPluginId.mockImplementation(
      (entry: { id?: string; sunclaw?: { plugin?: { id?: string } } }) =>
        entry.sunclaw?.plugin?.id ?? entry.id,
    );
    mocks.resolveOfficialExternalPluginInstall.mockImplementation(
      (entry: { install?: unknown; sunclaw?: { install?: unknown } }) =>
        entry.sunclaw?.install ?? entry.install ?? null,
    );
    mocks.resolveOfficialExternalPluginLabel.mockImplementation(
      (entry: { label?: string; sunclaw?: { plugin?: { label?: string } } }) =>
        entry.sunclaw?.plugin?.label ?? entry.label ?? "plugin",
    );
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      ...
      npmResolution: {
        name: "@sunclaw/brave-plugin",
        version: "2026.5.2",
        resolvedSpec: "@sunclaw/brave-plugin@2026.5.2",
      },
    });
  ...
  it("installs configured external web search plugins from beta on the beta channel", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@sunclaw/brave-plugin",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@sunclaw/brave-plugin@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(persistedRecords.brave, { spec: "@sunclaw/brave-plugin" });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      env: {},
    });
    expect(result.changes).toEqual([
      'Installed missing configured plugin "brave" from @sunclaw/brave-plugin@beta.',
    ]);
  });
  ...
  it("repairs a configured plugin from a legacy npm declaration stub", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "guardrail-bridge",
      targetDir: "/tmp/sunclaw-plugins/guardrail-bridge",
      version: "1.0.0",
      npmResolution: {
    ...
    expectRecordFields((records as Record<string, unknown>)["guardrail-bridge"], {
      source: "npm",
      spec: "@guardrail-bridge/guardrail-bridge@1.0.0",
      installPath: "/tmp/sunclaw-plugins/guardrail-bridge",
      version: "1.0.0",
