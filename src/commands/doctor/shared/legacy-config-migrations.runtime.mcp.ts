...
import {
  ...
  resolveSunClawMcpTransportAlias,
...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use SunClaw transport names; CLI-native type aliases are legacy here. Run "sunclaw doctor --fix".',
  match: (value) =>
    isRecord(value) &&
...
export const LEGACY_CONFIG_MIGRATIONS_RUNTIME_MCP: LegacyConfigMigrationSpec[] = [
  defineLegacyConfigMigration({
    ...
    describe: "Move CLI-native MCP server type aliases to SunClaw transport",
    ...
    apply: (raw, changes) => {
      ...
      for (const [serverName, rawServer] of Object.entries(servers)) {
        ...
        const alias = resolveSunClawMcpTransportAlias(rawServer.type);
