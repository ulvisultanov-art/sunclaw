...
describe("collectCodexRouteWarnings", () => {
  ...
  it("ignores SUNCLAW_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
  ...
  it("warns when Codex app-server command includes inline arguments", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.sunclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.sunclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
  ...
  it("warns when Codex runtime has SunClaw compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
          ...
          list: [
            {
              ...
              model: "anthropic/claude-sonnet-4-6",
              models: {
                "anthropic/claude-sonnet-4-6": { agentRuntime: { id: "sunclaw" } },
              },
            },
          ],
        },
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the SunClaw runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the SunClaw runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise SunClaw/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        models: {
          providers: {
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "sunclaw"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "sunclaw"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves explicit SunClaw runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("clears stale Codex overrides while preserving explicit SunClaw session pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to SunClaw", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(store.main.updatedAt).toBe(1);
    expect(store.main.agentHarnessId).toBe("sunclaw");
    expect(store.main.agentRuntimeOverride).toBe("sunclaw");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "sunclaw" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });
