...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-plugin-deps-cleanup-"));
  });
  ...
  it("collects and removes legacy plugin dependency state roots", async () => {
    ...
    const explicitStageDir = path.join(stateDir, ".sunclaw-install-stage-explicit");
    ...
    const env = {
      ...
      SUNCLAW_PLUGIN_STAGE_DIR: explicitStageDir,
  ...
  it("removes configured plugin stage roots outside SunClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(
      path.join(stageRoot, "node_modules", "ansi-escapes", ".sunclaw-rename-tmp"),
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        SUNCLAW_STATE_DIR: stateDir,
        SUNCLAW_PLUGIN_STAGE_DIR: stageRoot,
      },
      packageRoot,
    });
  ...
  it("refuses arbitrary explicit plugin stage roots outside SunClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses explicit plugin stage paths with parent segments", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const dotDotStage = `${stateDir}${path.sep}..${path.sep}.sunclaw-install-stage-dotdot`;
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        SUNCLAW_STATE_DIR: stateDir,
        SUNCLAW_PLUGIN_STAGE_DIR: dotDotStage,
      },
      packageRoot,
    });
  ...
  it("does not follow symlinked extension roots outside SunClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(path.join(externalPlugin, ".sunclaw-runtime-deps.json"), "{}");
  ...
  it("refuses legacy roots that resolve outside SunClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    ...
    expect(result.changes).toStrictEqual([]);
    expect(result.warnings).toContain(
      `Skipped legacy plugin dependency state ${legacyRuntimeRoot}: resolved outside SunClaw cleanup roots`,
    );
  ...
  it("does not unlink global runtime symlinks through unsafe cleanup roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "prefix", "lib", "node_modules", "sunclaw");
