 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "sunclaw"`, `source: "implicit"`.
...
/**
 * Build a minimal `SunClawConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "sunclaw"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): SunClawConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="sunclaw", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "sunclaw". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "sunclaw" and reflects the actual runtime driving the session.
