        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `sunclaw gateway run` or `sunclaw agent --local`.",
              ]
            : [`Run \`${formatCliCommand("sunclaw gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("sunclaw gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
