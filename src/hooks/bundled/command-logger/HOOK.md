...
homepage: https://docs.sunclaw.complex.az/automation/hooks#command-logger
metadata:
  {
    "sunclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.sunclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.sunclaw/logs/commands.log ~/.sunclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/sunclaw`:
   ```
   /home/username/.sunclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
