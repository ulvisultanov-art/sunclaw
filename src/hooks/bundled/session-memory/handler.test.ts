...
// Avoid calling the embedded SunClaw agent (global command lane); keep this unit test deterministic.
...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => SunClawConfig;
  action?: "new" | "reset";
