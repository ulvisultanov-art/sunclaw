...
import { type SunClawConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<SunClawConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("sunclaw@gmail.com");
