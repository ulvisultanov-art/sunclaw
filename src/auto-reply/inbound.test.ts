...
describe("initSessionState BodyStripped", () => {
  it("prefers BodyForAgent over Body for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-sender-meta-"));
  ...
  it("prefers BodyForAgent over Body for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as SunClawConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bsunclaw\\b", "(invalid", "(a+)+$"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("sunclaw")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("open\u200bclaw")).toBe("sunclaw");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bsunclaw\\b"] } },
    });
    expect(matchesMentionPatterns("SUNCLAW: hi", regexes)).toBe(true);
  });
  ...
  it("scopes configured mention patterns by provider conversation policy", () => {
    ...
    expect(matchesMentionPatterns("sunclaw: hi", allowed)).toBe(true);
    expect(matchesMentionPatterns("sunclaw: hi", denied)).toBe(false);
  });
  ...
  it("preserves mention patterns for callers without scoped policy facts", () => {
    ...
    expect(matchesMentionPatterns("sunclaw", regexes)).toBe(true);
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("sunclaw " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\bsunclaw\\b", "(a+)+$"] },
      },
    });
