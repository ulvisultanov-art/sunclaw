...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("Runtime: SunClaw Default");
  ...
  it("shows the default SunClaw harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "sunclaw-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: SunClaw Default");
    expect(normalized).not.toContain("· sunclaw");
  });
