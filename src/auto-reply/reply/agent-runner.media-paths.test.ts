...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside runAgentTurnWithFallback", async () => {
    // Regression test for sunclaw/sunclaw#68056.
  ...
  it("passes current inbound media paths as native SunClaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "sunclaw-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native SunClaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "sunclaw-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "sunclaw-native-agent-partial-"));
    cleanupPaths.push(tmpDir);
