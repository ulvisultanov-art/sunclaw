...
vi.mock("../../agents/sunclaw-tools.runtime.js", () => ({
  createSunClawTools: (...args: unknown[]) => createSunClawToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createSunClawToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  beforeEach(() => {
    ...
    createSunClawToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createSunClawToolsMock, "createSunClawTools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createSunClawToolsMock, "createSunClawTools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledWith(
