...
import { DELIVERY_NO_REPLY_RUNTIME_CONTRACT } from "sunclaw/plugin-sdk/agent-runtime-test-contracts";
...
const FOLLOWUP_DEBUG = process.env.SUNCLAW_DEBUG_FOLLOWUP_RUNNER_TEST === "1";
...
describe("createFollowupRunner runtime config", () => {
  ...
  it("reuses CLI session bindings for queued room-event followups", async () => {
    ...
    await runner(
      createQueuedRun({
        ...
        currentInboundContext: { text: "[SunClaw room event]" },
        run: {
          config: runtimeConfig,
          sessionId: "session-cli-room-event",
          provider: "anthropic",
  ...
  it("stores queued room-event CLI sessions created from the first ambient run", async () => {
    ...
    const storePath = "/tmp/sunclaw-followup-room-event-cli.json";
    const sessionEntry: SessionEntry = {
      sessionId: "session-cli-room-event",
      updatedAt: Date.now(),
    };
...
describe("createFollowupRunner progress forwarding", () => {
  ...
  it("suppresses queued follow-up progress when verbose progress is disabled", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-followup-progress-off-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
...
describe("createFollowupRunner compaction", () => {
  it("adds verbose auto-compaction notice and tracks count", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-compaction-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("suppresses queued auto-compaction notice when verbose is turned off", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-compaction-quiet-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-compaction-meta-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("refreshes queued followup runs to the rotated transcript", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-compaction-queue-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("does not count failed compaction end events in followup runs", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "sunclaw-compaction-failed-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("injects the post-compaction refresh prompt before followup runs after preflight compaction", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(tmpdir(), "sunclaw-preflight-followup-"));
...
describe("createFollowupRunner messaging delivery and dedupe", () => {
  ...
  it("persists usage even when replies are suppressed", async () => {
    const storePath = "/tmp/sunclaw-followup-usage.json";
  ...
  it("passes queued config into usage persistence during drained followups", async () => {
    const storePath = "/tmp/sunclaw-followup-usage-cfg.json";
    const sessionKey = "main";
    const sessionEntry: SessionEntry = { sessionId: "session", updatedAt: Date.now() };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
  ...
  it("uses providerUsed for snapshot freshness when agent metadata overrides the run provider", async () => {
    const storePath = "/tmp/sunclaw-followup-usage-provider.json";
    const sessionKey = "main";
    const sessionEntry: SessionEntry = { sessionId: "session", updatedAt: Date.now() };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const persistSpy = vi.spyOn(sessionRunAccounting, "persistRunSessionUsage");
  ...
  it("preserves user-facing session model state for queued internal announce fallback", async () => {
    const storePath = "/tmp/sunclaw-followup-internal-announce-usage.json";
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
      sessionId: "session",
      updatedAt: Date.now(),
