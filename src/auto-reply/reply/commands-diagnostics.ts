...
const DIAGNOSTICS_DOCS_URL = "https://docs.sunclaw.complex.az/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "sunclaw gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentSunClawCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this SunClaw session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an SunClaw session file.",
    ) === true
  );
}
