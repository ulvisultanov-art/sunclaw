...
import { scheduleGatewaySigusr1Restart, triggerSunClawRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting SunClaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelPath: string | null = null;
  ...
  const restartMethod = triggerSunClawRestart();
  if (!restartMethod.ok) {
    await removeRestartSentinelFile(sentinelPath);
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting SunClaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
