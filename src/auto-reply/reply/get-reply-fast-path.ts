...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("sunclaw.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("sunclaw.reply.full-runtime");

type ReplyConfigWithMarker = SunClawConfig & {
...
export function markCompleteReplyConfig<T extends SunClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends SunClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends SunClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is SunClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set SUNCLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as SunClawConfig;
}
