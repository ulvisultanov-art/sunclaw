...
vi.mock("../../agents/workspace.js", () => ({
  DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/sunclaw-workspace",
...
describe("getReplyFromConfig fast test bootstrap", () => {
  ...
  it("fails fast on unmarked config overrides in strict fast-test mode", async () => {
    await expect(
      getReplyFromConfig(buildGetReplyCtx(), undefined, {} as SunClawConfig),
  ...
  it("skips getRuntimeConfig, workspace bootstrap, and session bootstrap for marked test configs", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-fast-reply-"));
  ...
  it("still merges partial config overrides against getRuntimeConfig()", async () => {
    vi.stubEnv("SUNCLAW_ALLOW_SLOW_REPLY_TESTS", "1");
    vi.mocked(loadConfigMock).mockReturnValue({
      channels: {
        telegram: {
  ...
  it("marks configs through withFastReplyConfig()", async () => {
    const cfg = withFastReplyConfig({ session: { store: "/tmp/sessions.json" } } as SunClawConfig);
  ...
  it("clears stale ack-only heartbeat pending delivery before running heartbeat", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-heartbeat-pending-clear-"));
  ...
  it("keeps non-ack heartbeat pending delivery without direct replay", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-heartbeat-pending-replay-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
  ...
  it("does not replay stale heartbeat pending delivery", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-heartbeat-pending-suppress-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
  ...
  it("handles native /status before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-native-status-fast-"));
    ...
    expect(reply.text.includes("SunClaw")).toBe(true);
  ...
  it("uses configured agent thinking defaults for native /status", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-native-status-agent-think-"));
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
  ...
  it("uses the target session thinking override for native /status", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-native-status-think-"));
    const storePath = path.join(home, "sessions.json");
    const targetSessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
  ...
  it("handles native slash directives before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-native-slash-fast-"));
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
  ...
  it("continues native slash goal starts with the rewritten command-safe prompt", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-native-goal-fast-"));
    const targetSessionKey = "agent:main:telegram:123";
    const storePath = path.join(home, "sessions.json");
    const cfg = markCompleteReplyConfig({
      agents: {
  ...
  it("uses native command target session keys during fast bootstrap", () => {
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: "/tmp/sessions.json" } } as SunClawConfig,
      agentId: "main",
      commandAuthorized: true,
  ...
  it("keeps the existing session for /reset newline soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-fast-reset-newline-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
    ...
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: storePath } } as SunClawConfig,
      agentId: "main",
      commandAuthorized: true,
      workspaceDir: home,
    });
  ...
  it("keeps the existing session for /reset: soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-fast-reset-colon-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
