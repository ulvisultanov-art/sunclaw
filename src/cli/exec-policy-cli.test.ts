...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: SunClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: SunClawConfig }>
    >(async () => ({
      path: "/tmp/sunclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: SunClawConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/sunclaw.json");
    ...
    expect(output).not.toContain("/tmp/sunclaw.json\nforged");
