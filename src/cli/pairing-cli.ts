...
function parseChannel(raw: unknown, channels: PairingChannel[]): PairingChannel {
  ...
  if (!value) {
    throw new Error(
      `Missing channel. Use ${formatCliCommand("sunclaw pairing list --channel <channel>")}.`,
    );
  }
...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.sunclaw.complex.az/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!channelRaw || !resolvedCode) {
        throw new Error(
          `Usage: ${formatCliCommand("sunclaw pairing approve <channel> <code>")} (or: ${formatCliCommand("sunclaw pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("sunclaw pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
      const accountId = normalizeStringifiedOptionalString(opts.account) ?? "";
      ...
      if (!approved) {
        throw new Error(
          `No pending pairing request found for code "${String(resolvedCode)}". Run ${formatCliCommand(`sunclaw pairing list --channel ${channel}`)} to list pending requests.`,
        );
      }
