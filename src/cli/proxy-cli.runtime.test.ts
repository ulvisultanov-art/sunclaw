...
describe("proxy cli runtime", () => {
  const envKeys = [
    "SUNCLAW_DEBUG_PROXY_DB_PATH",
    "SUNCLAW_DEBUG_PROXY_BLOB_DIR",
    "SUNCLAW_DEBUG_PROXY_CERT_DIR",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "sunclaw-proxy-cli-runtime-"));
    process.env.SUNCLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.SUNCLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.SUNCLAW_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.SUNCLAW_DEBUG_PROXY_ENABLED;
    delete process.env.SUNCLAW_DEBUG_PROXY_SESSION_ID;
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or SUNCLAW_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, SUNCLAW_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or SUNCLAW_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.SUNCLAW_DEBUG_PROXY_DB_PATH!,
      process.env.SUNCLAW_DEBUG_PROXY_BLOB_DIR!,
    );
