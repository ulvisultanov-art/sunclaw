...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.SUNCLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.SUNCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.SUNCLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.SUNCLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.SUNCLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "sunclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "sunclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "sunclaw", "nodes", "list"], "sunclaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "sunclaw", "infer"], "sunclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "sunclaw", "acp", "--help"], "sunclaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "sunclaw", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("sunclaw");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "sunclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("sunclaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "sunclaw", "completion", "--write-state"];
    const program = new Command().name("sunclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "sunclaw", ...args];
    const program = new Command().name("sunclaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "sunclaw", "plugins"];
    const program = new Command().name("sunclaw");
