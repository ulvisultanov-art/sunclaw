  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("sunclaw setup")}\n` +
        `    ${theme.muted("Create config, workspace, and session folders.")}\n` +
        `  ${theme.command("sunclaw setup --wizard")}\n` +
        `    ${theme.muted("Run full onboarding for auth, models, Gateway, and channels.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.sunclaw.complex.az/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/.sunclaw/workspace; stored as agents.defaults.workspace)",
    )
