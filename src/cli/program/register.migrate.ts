...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration SunClaw backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sunclaw migrate list", "Show available migration providers."],
          ["sunclaw migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["sunclaw migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "sunclaw migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "sunclaw migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing SunClaw state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
