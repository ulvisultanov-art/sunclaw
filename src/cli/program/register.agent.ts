...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.sunclaw.complex.az/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['sunclaw agents set-identity --agent main --name "SunClaw" --emoji "🦞"', "Set name + emoji."],
  ["sunclaw agents set-identity --agent main --avatar avatars/sunclaw.png", "Set avatar path."],
  [
    "sunclaw agents set-identity --workspace ~/.sunclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "sunclaw agents set-identity --identity-file ~/.sunclaw/workspace/IDENTITY.md --agent main",
