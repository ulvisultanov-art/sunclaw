...
describe("tryRouteCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.SUNCLAW_DISABLE_ROUTE_FIRST;
    originalHideBanner = process.env.SUNCLAW_HIDE_BANNER;
    delete process.env.SUNCLAW_DISABLE_ROUTE_FIRST;
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.SUNCLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  ...
  it("keeps config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "sunclaw", "status", "--json"])).resolves.toBe(true);
  ...
  it("keeps config guard for the parent tasks JSON list alias", async () => {
    await expect(tryRouteCli(["node", "sunclaw", "tasks", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "sunclaw", "status"])).resolves.toBe(true);
  ...
  it("keeps logs routed to stderr for routed --json commands", async () => {
    ...
    await tryRouteCli(["node", "sunclaw", "agents", "--json"]);
  ...
  it("routes command-run logs to stderr for config-guard-skipping --json routes", async () => {
    ...
    await expect(tryRouteCli(["node", "sunclaw", "models", "status", "--json"])).resolves.toBe(
      true,
    );
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "sunclaw", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "sunclaw", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );

    expect(findRoutedCommandMock).toHaveBeenCalledWith(
      ["status"],
      ["node", "sunclaw", "--log-level", "debug", "status"],
    );
    expect(ensureConfigReadyMock).toHaveBeenCalledTimes(1);
    const configReadyCall = firstConfigReadyCall();
    expect(typeof configReadyCall?.runtime).toBe("object");
  ...
  it("respects SUNCLAW_HIDE_BANNER for routed commands", async () => {
  ...
  it("falls back before bootstrap when the route cannot parse the argv", async () => {
    ...
    await expect(tryRouteCli(["node", "sunclaw", "tasks", "list"])).resolves.toBe(false);
