...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.sunclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SUNCLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.SUNCLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.SUNCLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.SUNCLAW_CONFIG_PATH)) {
    env.SUNCLAW_CONFIG_PATH = path.join(stateDir, "sunclaw.json");
  }

  if (profile === "dev" && !env.SUNCLAW_GATEWAY_PORT?.trim()) {
    env.SUNCLAW_GATEWAY_PORT = "19001";
  }
}
