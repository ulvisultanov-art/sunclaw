...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-SunClawScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-SunClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "sunclaw restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-SunClawListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-SunClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'sunclaw-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `sunclaw-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("sunclaw-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^sunclaw-restart-/);
    ...
    it("uses SUNCLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUNCLAW_PROFILE: "default",
        SUNCLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("sunclaw-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        SUNCLAW_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        SUNCLAW_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped sunclaw gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart sunclaw-gateway.service");
      expect(calls).toContain("--user is-active --quiet sunclaw-gateway.service");
      expect(calls).toContain("is-active --quiet sunclaw-gateway.service");
      expect(calls).not.toContain("--user restart sunclaw-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.sunclaw.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.sunclaw.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.sunclaw/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.sunclaw/logs/gateway-restart.log' 2>&1");
    ...
    it("uses SUNCLAW_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/sunclaw-state/logs' 2>/dev/null && : >>'/tmp/sunclaw-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/sunclaw-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("sunclaw restart attempt source=update target=ai.sunclaw.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.sunclaw.gateway");
      expect(log).toContain("sunclaw restart failed source=update status=42");
      expect(log).not.toContain("sunclaw restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        SUNCLAW_LAUNCHD_LABEL: "ai.sunclaw.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        SUNCLAW_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.sunclaw.$(echo injected)");
      expect(log).not.toContain("target=ai.sunclaw.injected");
    });

    it("uses SUNCLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        SUNCLAW_PROFILE: "default",
        SUNCLAW_LAUNCHD_LABEL: "com.custom.sunclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.sunclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'SunClaw Gateway'");
      expect(content).toContain("function Invoke-SunClawSchtasksWithTimeout");
      expect(content).toContain("function Get-SunClawScheduledTaskState");
      expect(content).toContain("function Invoke-SunClawStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("sunclaw restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".sunclaw\\gateway.cmd"',
      );
      expect(content).toContain("sunclaw restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%SUNCLAW_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses SUNCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        SUNCLAW_PROFILE: "default",
        SUNCLAW_WINDOWS_TASK_NAME: "SunClaw Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'SunClaw Gateway (custom)'");
      expect(content).toContain("Get-SunClawScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-SunClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-SunClawStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUNCLAW_PROFILE: "production",
      });
      expect(content).toContain("sunclaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SUNCLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.sunclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'SunClaw Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        SUNCLAW_LAUNCHD_LABEL: "ai.sunclaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.sunclaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
