...
const mocks = vi.hoisted(() => ({
  ...
  relinkSunClawPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkSunClawPeerDependenciesInManagedNpmRoot:
    mocks.relinkSunClawPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkSunClawPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with SUNCLAW_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as SunClawConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { SUNCLAW_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        SUNCLAW_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { SUNCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as SunClawConfig,
      env: {},
    });
  ...
  it("repairs managed npm sunclaw peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/sunclaw-state/npm",
      "/tmp/sunclaw-state/npm/projects/codex",
    ]);
    mocks.relinkSunClawPeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as SunClawConfig,
      env: { SUNCLAW_STATE_DIR: "/tmp/sunclaw-state" },
    });

    expect(mocks.relinkSunClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/sunclaw-state/npm",
      logger: {},
    });
    expect(mocks.relinkSunClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/sunclaw-state/npm/projects/codex",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired SunClaw host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkSunClawPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @sunclaw/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @sunclaw/discord: ENETUNREACH.',
        guidance: ["Run `sunclaw doctor --fix` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@sunclaw/matrix@beta: ClawHub ClawPack download for @sunclaw/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `sunclaw doctor --fix` to retry plugin repair.",
          "Run `sunclaw plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `sunclaw doctor --fix`."],
        },
        {
