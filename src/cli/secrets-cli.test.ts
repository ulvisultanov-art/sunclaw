...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    ...
    preflight: {
      ...
      changedFiles: options?.changed ? ["/tmp/sunclaw.json"] : [],
      checks: {
        resolvability: true,
...
async function withPlanFile(run: (planPath: string) => Promise<void>) {
  const planPath = path.join(
    os.tmpdir(),
    `sunclaw-secrets-cli-test-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
  );
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/sunclaw.json Bind: loopback Possible causes: - Gateway not yet ready. Run `sunclaw doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("sunclaw gateway status --deep");
