...
const SANDBOX_EXAMPLES = {
  main: [
    ["sunclaw sandbox list", "List all sandbox containers."],
    ["sunclaw sandbox list --browser", "List only browser containers."],
    ["sunclaw sandbox recreate --all", "Recreate all containers."],
    ["sunclaw sandbox recreate --session main", "Recreate a specific session."],
    ["sunclaw sandbox recreate --agent mybot", "Recreate agent containers."],
    ["sunclaw sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["sunclaw sandbox list", "List all sandbox containers."],
    ["sunclaw sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["sunclaw sandbox recreate --all", "Recreate all containers."],
    ["sunclaw sandbox recreate --session main", "Recreate a specific session."],
    ["sunclaw sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["sunclaw sandbox recreate --browser --all", "Recreate only browser containers."],
    ["sunclaw sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["sunclaw sandbox explain", "Show effective sandbox config."],
    ["sunclaw sandbox explain --session agent:main:main", "Explain a specific session."],
    ["sunclaw sandbox explain --agent work", "Explain an agent sandbox."],
    ["sunclaw sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.sunclaw.complex.az/cli/sandbox")}\n`,
    )
    .action(() => {
