...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "sunclaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "sunclaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "sunclaw", "--profile", "work", "gateway", "status"],
