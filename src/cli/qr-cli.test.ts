...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("SUNCLAW_GATEWAY_TOKEN", "");
    vi.stubEnv("SUNCLAW_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("sunclaw devices approve <requestId>");
  });
  ...
  it("uses SUNCLAW_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("SUNCLAW_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
