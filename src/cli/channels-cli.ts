...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sunclaw channels list", "List configured channels."],
          ["sunclaw channels list --all", "Show configured, bundled, and installable channels."],
          ["sunclaw channels add", "Open guided channel setup."],
          ["sunclaw channels status --probe", "Run channel status checks and probes."],
          [
            "sunclaw channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["sunclaw channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.sunclaw.complex.az/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sunclaw channels add", "Open guided setup for available chat channels."],
          [
            "sunclaw channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["sunclaw channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
