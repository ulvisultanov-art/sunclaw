    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sunclaw nodes status", "List known nodes with live status."],
          ["sunclaw nodes pairing pending", "Show pending node pairing requests."],
          ["sunclaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'sunclaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["sunclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.sunclaw.complex.az/cli/nodes")}\n`,
    );
