...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.sunclaw.complex.az/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`sunclaw doctor --fix\` to normalize legacy cron job storage.\n`,
    );
