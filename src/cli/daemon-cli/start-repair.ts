...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveSunClawWrapperPath(installEnv[SUNCLAW_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `sunclaw dashboard` or copy a fresh auth URL with `sunclaw dashboard --no-open`.",
