...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("prepends a single container restart hint when SUNCLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        SUNCLAW_CONTAINER: "sunclaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when SUNCLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `sunclaw gateway`.",
        ],
        { SUNCLAW_CONTAINER: "sunclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when SUNCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { SUNCLAW_CONTAINER_HINT: "sunclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
