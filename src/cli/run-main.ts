...
import type { ConfigFileSnapshot, SunClawConfig } from "../config/types.sunclaw.js";
import { isTruthyEnvValue, normalizeEnv } from "../infra/env.js";
import { isMainModule } from "../infra/is-main.js";
import type { ProxyHandle } from "../infra/net/proxy/proxy-lifecycle.js";
import { ensureSunClawCliOnPath } from "../infra/path-env.js";
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.SUNCLAW_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.SUNCLAW_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: SunClawConfig): SunClawConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return (
    ...
    `Unknown command: sunclaw ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`
  );
}
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.SUNCLAW_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<SunClawConfig> | null = null;
  const readBestEffortCliConfig = async (): Promise<SunClawConfig> => {
  ...
  try {
    ...
    // `sunclaw <typo> --help` surfaces the same Unknown command error as
    // `sunclaw <typo>` instead of silently showing generic top-level help.
    ...
    if (shouldRunBareRootCrestodian) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `sunclaw crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[sunclaw] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "SunClaw hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
