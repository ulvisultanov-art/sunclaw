...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "sunclaw-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __SUNCLAW_VERSION__");
    expect(source).toContain("? __SUNCLAW_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "sunclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "sunclaw", version: "2.3.4" });
  ...
  it("ignores non-sunclaw package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers SUNCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        SUNCLAW_VERSION: "9.9.9",
        SUNCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale SUNCLAW_VERSION for compatibility checks", () => {
    const previous = process.env.SUNCLAW_VERSION;
    const previousService = process.env.SUNCLAW_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.SUNCLAW_VERSION = "2026.3.25";
      process.env.SUNCLAW_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("SUNCLAW_VERSION", previous);
      restoreEnvValue("SUNCLAW_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        SUNCLAW_VERSION: "2026.3.99",
        SUNCLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        SUNCLAW_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        SUNCLAW_VERSION: "2026.3.99",
        SUNCLAW_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
