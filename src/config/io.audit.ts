...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.SUNCLAW_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.SUNCLAW_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.SUNCLAW_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `sunclaw doctor --fix` once the
