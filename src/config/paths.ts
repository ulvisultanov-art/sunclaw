...
/**
 * Nix mode detection: When SUNCLAW_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const NEW_STATE_DIRNAME = ".sunclaw";
const CONFIG_FILENAME = "sunclaw.json";
...
/** Build a homedir thunk that respects SUNCLAW_HOME for the given env. */
 ...
 * Can be overridden via SUNCLAW_STATE_DIR.
 * Default: ~/.sunclaw
 */
export function resolveStateDir(
  ...
  const override = env.SUNCLAW_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.SUNCLAW_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const sunclawOverride = env.SUNCLAW_STATE_DIR?.trim();
  if (sunclawOverride) {
    env.SUNCLAW_STATE_DIR = resolveUserPath(sunclawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `SUNCLAW_INCLUDE_ROOTS` as a
 ...
 * directory containing `sunclaw.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.SUNCLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via SUNCLAW_CONFIG_PATH.
 * Default: ~/.sunclaw/sunclaw.json (or $SUNCLAW_STATE_DIR/sunclaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.SUNCLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.SUNCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const sunclawStateDir = env.SUNCLAW_STATE_DIR?.trim();
  if (sunclawStateDir) {
    const resolved = resolveUserPath(sunclawStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/sunclaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `sunclaw-${uid}` : "sunclaw";
 ...
 *
 * Precedence:
 * - `SUNCLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.SUNCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.SUNCLAW_GATEWAY_PORT?.trim();
