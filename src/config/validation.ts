...
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@sunclaw/net-policy/ip";
...
import type { SunClawConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: SunClawConfig): SunClawConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.sunclaw.complex.az/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: sunclaw plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: sunclaw plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: SunClawConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: SunClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: SunClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: SunClawConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: SunClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SunClawSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as SunClawConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: SunClawConfig | null | undefined;
  ...
  const ensureCompatConfig = (): SunClawConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path: pathEntry,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run sunclaw doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or sunclaw doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run sunclaw doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run sunclaw doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native SunClaw config schema.
