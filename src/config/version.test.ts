...
import {
  compareSunClawVersions,
  isSameSunClawStableFamily,
  parseSunClawVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseSunClawVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseSunClawVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseSunClawVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseSunClawVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseSunClawVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseSunClawVersion("2026.3")).toBeNull();
    expect(parseSunClawVersion("latest")).toBeNull();
  });
});

describe("compareSunClawVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareSunClawVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareSunClawVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareSunClawVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareSunClawVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareSunClawVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareSunClawVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameSunClawStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameSunClawStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameSunClawStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameSunClawStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameSunClawStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
