...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SUNCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-sunclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-sunclaw#general");
  });
  ...
  it("derives session transcripts dir from SUNCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SUNCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SUNCLAW_STATE_DIR differs from stored paths", () => {
