...
import { withTempHome, writeSunClawConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.sunclaw/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeSunClawConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.sunclaw`);
      expect(entries.some((entry) => entry.startsWith("sunclaw.json.clobbered."))).toBe(false);
    });
  });
