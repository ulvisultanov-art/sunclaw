...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.SUNCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveSunClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    SUNCLAW_STATE_DIR: path.join(os.tmpdir(), "sunclaw-config-doc-baseline-state"),
