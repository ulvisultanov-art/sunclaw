...
function collectConfigEnvVarsByTarget(cfg?: SunClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SunClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SunClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SUNCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
