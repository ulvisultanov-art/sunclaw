...
export function listConfiguredSessionStoreAgentIds(cfg: SunClawConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "sunclaw agents list" to see configured agents.`,
      );
    }
    return [
      {
