...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.sunclaw) || !isRecord(entry.sunclaw.channel)) {
      continue;
    }
    const channel = entry.sunclaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: SunClawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: SunClawConfig, pluginId: string) => boolean;
