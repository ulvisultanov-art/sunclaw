...
import { normalizeConfiguredProviderCatalogModelId } from "@sunclaw/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  sunclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  sunclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: SunClawConfig,
  pathSegments: string[],
): { changed: boolean; next: SunClawConfig } {
