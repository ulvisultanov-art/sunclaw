...
describe("sandbox docker config", () => {
  ...
  it("accepts Windows drive-letter binds in sandbox.docker config", () => {
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/sunclaw/src:/src:ro", "D:\\data\\sunclaw\\output:/output:rw"],
            },
          },
        },
      },
    ...
    if (res.ok) {
      expect(res.config.agents?.defaults?.sandbox?.docker?.binds).toEqual([
        "D:/data/sunclaw/src:/src:ro",
        "D:\\data\\sunclaw\\output:/output:rw",
      ]);
    }
  });
...
describe("sandbox browser binds config", () => {
          ...
          sandbox: {
            browser: {
              binds: [
                ...
                "D:/data/sunclaw/chrome:/data/chrome-windows:rw",
              ],
            },
          },
        },
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("sunclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "sunclaw-sandbox-browser-global" },
      agentBrowser: { network: "sunclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("sunclaw-sandbox-browser-agent");
  });
