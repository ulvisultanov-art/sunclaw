...
export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "SUNCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
type FutureConfigGuardParams = {
  ...
  config?: Pick<SunClawConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this SunClaw binary (${currentVersion}) is older than the config last written by SunClaw ${touchedVersion}.`,
    hints: [
