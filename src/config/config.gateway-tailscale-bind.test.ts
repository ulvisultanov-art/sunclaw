...
describe("gateway tailscale bind validation", () => {
  ...
  it("validates Tailscale service names", () => {
    const validRes = validateConfigObject({
      gateway: {
        bind: "loopback",
        tailscale: { mode: "serve", serviceName: "svc:sunclaw-gateway" },
      },
    });
    expect(validRes.ok).toBe(true);

    for (const serviceName of ["sunclaw", "svc:", "svc:-sunclaw", "svc:SunClaw"]) {
      ...
      if (!res.ok) {
        expect(res.issues).toContainEqual({
          ...
          message:
            'Tailscale serviceName must use the "svc:<dns-label>" format, for example "svc:sunclaw"',
        });
      }
    }
  });
