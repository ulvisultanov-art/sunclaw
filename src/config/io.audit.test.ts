...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".sunclaw", "sunclaw.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".sunclaw", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "sunclaw-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".sunclaw", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/sunclaw.json (sha256 prev-hash -> next-hash, backup=/tmp/sunclaw.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      configPath: "/tmp/sunclaw.json",
      env: {
        SUNCLAW_WATCH_MODE: "1",
        SUNCLAW_WATCH_SESSION: "watch-session-1",
        SUNCLAW_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/sunclaw.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".sunclaw", "sunclaw.json")),
        suspicious: [
  ...
  it("redacts the value half of `--flag=value` for secret flags", () => {
    const argv = ["sunclaw", "--token=ghp_realgithubtoken1234567890ABCD", "--port=8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["sunclaw", "--token=***", "--port=8080"]);
  });
  ...
  it("leaves non-secret arguments untouched", () => {
    const argv = ["node", "sunclaw", "gateway", "--port", "8080", "--bind", "lan"];
  ...
  it("masks the next arg after a secret flag even when it looks like another option", () => {
    const argv = ["sunclaw", "--token", "--port", "8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["sunclaw", "--token", "***", "8080"]);
  });

  it("redacts dash-leading secret values after bare secret flags", () => {
    const argv = ["sunclaw", "--password", "-secret-value"];
    expect(redactConfigAuditArgv(argv)).toEqual(["sunclaw", "--password", "***"]);
  });

  it("does not mask when a secret flag is the final arg with no value", () => {
    const argv = ["sunclaw", "--token"];
    expect(redactConfigAuditArgv(argv)).toEqual(["sunclaw", "--token"]);
  });
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "sunclaw", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "sunclaw", "--token", "***"]);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const unredactedRecord = {
      ...
      source: "config-io",
      event: "config.write",
      configPath: path.join(home, ".sunclaw", "sunclaw.json"),
      pid: 1590563,
      ppid: 1590548,
      cwd: home,
      argv: [
        "/usr/bin/node",
        "/usr/local/bin/sunclaw.mjs",
        "config",
        "set",
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/sunclaw.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "sunclaw.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "sunclaw.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;
