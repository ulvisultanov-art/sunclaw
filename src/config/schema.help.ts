...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by SunClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when SunClaw writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for SunClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "SunClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping SunClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "gateway.handshakeTimeoutMs":
    "Pre-auth Gateway WebSocket handshake timeout in milliseconds. Use higher values on loaded or low-powered hosts where local clients can connect during startup warmup. SUNCLAW_HANDSHAKE_TIMEOUT_MS still takes precedence.",
  ...
  "gateway.tailscale.serviceName":
    'Optional Tailscale Service name for Serve mode, such as "svc:sunclaw". The value must use Tailscale\'s svc:<dns-label> format. When set, SunClaw passes it to tailscale serve --service and reports the derived Service URL.',
  "gateway.tailscale.preserveFunnel":
    "When mode='serve' and an externally configured Tailscale Funnel route already covers the gateway port, skip re-applying tailscale serve on startup. Lets operators keep Funnel exposure managed outside SunClaw without losing it across gateway restarts.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to SunClaw's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "talk.realtime.consultRouting":
    "Gateway relay fallback for final user transcripts when the realtime provider skips sunclaw_agent_consult. provider-direct preserves provider replies; force-agent-consult routes through SunClaw.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default SunClaw execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default SunClaw runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this SunClaw agent (for example codex, claude, cursor, gemini, sunclaw).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "sunclaw" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for Chrome DevTools MCP attachment on the selected host or browser node.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as sunclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "tools.experimental.planTool":
    "Enable the experimental structured `update_plan` tool for non-trivial multi-step work tracking. Leave this off unless you explicitly want the tool outside strict-agentic embedded SunClaw runs.",
  "tools.toolSearch":
    "Compact large SunClaw, MCP, and client tool catalogs behind one search/call surface. Set to true for the default code bridge or use the object form to choose the structured fallback.",
  "tools.toolSearch.enabled":
    "Enables Tool Search. When on, SunClaw hides large tool catalogs behind `tool_search_code` or structured search/describe/call tools during embedded runtime runs.",
  ...
  "tools.codeMode":
    "Generic SunClaw code mode. When enabled, agent runs expose only `exec` and `wait` to the model and hide normal tools behind a QuickJS-WASI catalog bridge.",
  "tools.codeMode.enabled":
    "Enables generic code mode. Default is off. When explicitly enabled, SunClaw fails closed if the runtime is unavailable instead of exposing the full tool list.",
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: sunclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /sunclaw).",
  ...
  "gateway.controlUi.allowExternalEmbedUrls":
    "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__sunclaw__/canvas and /__sunclaw__/a2ui documents do not need it.",
  ...
  "gateway.push.apns.relay":
    "External relay settings for relay-backed APNs sends. The gateway uses the hosted SunClaw relay by default, or this custom relay for push.test, wake nudges, and reconnect wakes after a paired official iOS build publishes a relay-backed registration.",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `sunclaw security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, SunClaw enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "audio.transcription.command":
    'Executable + args used to transcribe audio (first token must be a safe binary/path), for example `["whisper-cli", "--model", "small", "{{MediaPath}}"]`. Deprecated `{input}` placeholders are migrated to `{{MediaPath}}` by `sunclaw doctor --fix`.',
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $SUNCLAW_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "skills.load.extraDirs":
    "Additional shared skill roots to scan at lowest precedence. Use this for sibling repos or shared skill packs that should be available without copying them into the SunClaw workspace.",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether SunClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "models.providers.*.agentRuntime":
    "Optional low-level agent runtime policy for this provider. Use provider/model runtime policy instead of agent-wide runtime pins; omitted/default lets SunClaw choose the runtime for the selected provider.",
  "models.providers.*.agentRuntime.id":
    'Provider agent runtime id: "sunclaw", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli". OpenAI on the official endpoint defaults to the Codex harness when omitted.',
  "models.providers.*.localService":
    "Optional on-demand local model server process for this provider. SunClaw probes healthUrl, starts the command when needed, waits for readiness, and then sends the model request.",
  ...
  "models.providers.*.localService.healthUrl":
    "Readiness URL probed before model requests. If omitted, SunClaw uses the provider baseUrl with /models appended.",
  ...
  "models.providers.*.localService.idleStopMs":
    "Milliseconds to keep an SunClaw-started local model server alive after the last request finishes. Set 0 to keep it alive until SunClaw exits.",
  ...
  proxy:
    "Operator-managed forward proxy routing for SunClaw runtime HTTP, HTTPS, WebSocket, and supported raw-egress paths. Use this when central egress control is part of the deployment boundary.",
  "proxy.enabled":
    "Enables operator-managed proxy routing. When enabled, SunClaw fails startup if no managed proxy URL is configured.",
  "proxy.proxyUrl":
  ...
  "models.providers.*.models[].agentRuntime.id":
    'Model agent runtime id: "sunclaw", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "models.providers.*.models[].mediaInput.image.preferredSidePx":
    "Preferred image resize side for balanced compression. Leave unset to use SunClaw's conservative default.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, SunClaw drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.models.*.agentRuntime.id":
    'Default-agent model runtime id: "sunclaw", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.sunclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses SunClaw memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "agents.list.*.models.*.agentRuntime.id":
    'Per-agent model runtime id: "sunclaw", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "agents.defaults.compaction.reserveTokensFloor":
    "Minimum floor enforced for reserveTokens in embedded SunClaw compaction paths (0 disables the floor guard). Use a non-zero floor to avoid over-aggressive compression under fluctuating token estimates.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional embedded SunClaw tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, SunClaw reuses existing precheck recovery to truncate tool results or compact before retrying.",
  "agents.defaults.compaction.midTurnPrecheck.enabled":
    "Enable structured mid-turn context pressure checks for embedded SunClaw tool loops. Default: false. Keep disabled unless long tool-heavy sessions hit context overflow before normal turn-end compaction can run.",
  ...
  "agents.defaults.runRetries":
    "Outer run loop retry iteration boundaries for the embedded SunClaw runner to prevent infinite execution loops during failure recovery.",
  "agents.defaults.runRetries.base":
    "Base number of run retry iterations for the embedded SunClaw runner's outer run loop (default: 24).",
  ...
  "agents.list[].runRetries":
    "Optional per-agent override for the embedded SunClaw runner's outer run loop retry iteration boundaries.",
  ...
  "agents.defaults.embeddedAgent":
    "Embedded SunClaw runner hardening controls for how workspace-local agent settings are trusted and applied in SunClaw sessions.",
  "agents.defaults.embeddedAgent.projectSettingsPolicy":
    'How embedded SunClaw handles workspace-local `.sunclaw/settings.json`: "sanitize" (default) strips shellPath/shellCommandPrefix, "ignore" disables project settings entirely, and "trusted" applies project settings as-is.',
  "agents.defaults.embeddedAgent.executionContract":
    'Embedded SunClaw execution contract: "default" keeps the standard runner behavior, while "strict-agentic" keeps OpenAI/OpenAI Codex GPT-5-family runs acting until they hit a real blocker instead of stopping at plans or filler.',
  "agents.list[].embeddedAgent":
    "Optional per-agent embedded SunClaw overrides. Use this to opt specific agents into stricter GPT-5 execution behavior without changing the global default.",
  "agents.list[].embeddedAgent.executionContract":
    'Optional per-agent embedded SunClaw execution contract override. Set "strict-agentic" to keep that agent acting through plan-only turns on OpenAI/OpenAI Codex GPT-5-family runs, or "default" to inherit the standard runner behavior.',
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage SunClaw MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by SunClaw. Embedded SunClaw and other runtime adapters can consume these servers without storing them inside runtime-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. SunClaw stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "SunClaw projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  ...
  "mcp.servers.*.codex.agents":
    "Optional non-empty SunClaw agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in SunClaw config.",
  ...
  "session.writeLock.acquireTimeoutMs":
    "Milliseconds to wait while acquiring a session transcript write lock before reporting the session as busy. Default: 60000; env override: SUNCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS.",
  "session.writeLock.staleMs":
    "Milliseconds before an existing session transcript lock can be treated as stale and reclaimed. Default: 1800000; env override: SUNCLAW_SESSION_WRITE_LOCK_STALE_MS.",
  "session.writeLock.maxHoldMs":
    "Milliseconds a held in-process session transcript lock may remain held before the watchdog releases it. Default: 300000; env override: SUNCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS.",
  ...
  "session.maintenance.rotateBytes":
    'Deprecated and ignored. Do not use for `sessions.json` growth control; SunClaw no longer creates automatic rotation backups, and "sunclaw doctor --fix" removes this key.',
  ...
  "cron.webhook":
    'Deprecated legacy fallback webhook URL used by `sunclaw doctor --fix` to migrate old jobs with `notify=true`. Runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.',
  "cron.webhookToken":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in SunClaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
