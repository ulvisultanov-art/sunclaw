...
import { resetConfigRuntimeState, type SunClawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        SUNCLAW_MPM_CATALOG_PATHS: undefined,
        SUNCLAW_LOAD_SHELL_ENV: undefined,
        SUNCLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        SUNCLAW_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeSunClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.SUNCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected SUNCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeSunClawConfig(home, config);
