...
const ETC_SUNCLAW_DIR = path.join(ROOT_DIR, "etc", "sunclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "sunclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSunClawPath(...parts: string[]) {
  return path.join(ETC_SUNCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcSunClawPath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "sunclaw-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "sunclaw.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "sunclaw-includes-hardlink-" }, async (tempRoot) => {
    ...
    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "sunclaw-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "sunclaw.json")),
...
describe("SUNCLAW_INCLUDE_ROOTS allowlist", () => {
  ...
  it("still rejects include paths that fall outside every allowed root", () => {
    const obj = { $include: etcSunClawPath("agents.json") };
    expect(() =>
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "sunclaw-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "sunclaw-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
