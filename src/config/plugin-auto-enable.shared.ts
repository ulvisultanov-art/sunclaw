...
function hasConfiguredEmbeddedHarnessRuntime(cfg: SunClawConfig, _env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: SunClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: SunClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: SunClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: SunClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: SunClawConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: SunClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: SunClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: SunClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: SunClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: SunClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: SunClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: SunClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: SunClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: SunClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: SunClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: SunClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: SunClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: SunClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: SunClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: SunClawConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: SunClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: SunClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: SunClawConfig, env: NodeJS.ProcessEnv): boolean {
...
export function configMayNeedPluginAutoEnable(cfg: SunClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: SunClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SunClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: SunClawConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: SunClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
