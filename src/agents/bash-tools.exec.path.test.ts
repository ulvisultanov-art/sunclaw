...
const ENV_KEYS = ["SUNCLAW_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("SUNCLAW_SHELL")) {
        input.onStdout?.(env.SUNCLAW_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SUNCLAW_EXEC_SHELL_SNAPSHOT = "0";
  ...
  it("strips malformed XML arg-value suffixes from exec command and routing options", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-exec-xml-"));
    try {
  ...
  it("sets SUNCLAW_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-sunclaw-shell", {
      command: 'printf "%s" "${SUNCLAW_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/sunclaw-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/sunclaw-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a sunclaw /approve abc123 deny",
    ...
    "sudo SUNCLAW_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root SUNCLAW_APPROVE=1 bash -lc '/approve abc123 allow-once'",
