...
const envKeys = ["SUNCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY", "SUNCLAW_STATE_DIR"] as const;
...
describe("provider auth warm worker", () => {
  ...
  it("preserves runtime-only auth profile snapshots in the worker warm input", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "sunclaw-provider-auth-worker-"));
    tempDirs.push(root);
    const previousEnv = Object.fromEntries(envKeys.map((key) => [key, process.env[key]])) as Record<
      (typeof envKeys)[number],
      string | undefined
    >;
    process.env.SUNCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
    process.env.SUNCLAW_STATE_DIR = path.join(root, "state");
