...
function resolveMainSessionKeyForSandbox(params: { cfg?: SunClawConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: SunClawConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  const explainCommand = runtime.sessionKey
    ? hasUnsafeControlChars(runtime.sessionKey)
      ? `sunclaw sandbox explain --agent ${runtime.agentId}`
      : `sunclaw sandbox explain --session ${shellEscapeSingleArg(runtime.sessionKey)}`
