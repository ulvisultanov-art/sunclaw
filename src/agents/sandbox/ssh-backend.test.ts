...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "sunclaw-test-ssh-config"),
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "sunclaw-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "sunclaw-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/sunclaw");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/sunclaw/sunclaw-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "sunclaw-browser",
          containerPrefix: "sunclaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/sunclaw/sunclaw-ssh-agent-worker");
