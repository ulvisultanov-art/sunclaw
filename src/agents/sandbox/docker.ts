...
import { markSunClawExecEnv } from "../../infra/sunclaw-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; SunClaw will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "sunclaw.sandbox=1");
  args.push("--label", `sunclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `sunclaw.createdAtMs=${createdAtMs}`);
  args.push("--label", `sunclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `sunclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markSunClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "sunclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`sunclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`sunclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("sunclaw sandbox recreate --all");
}
