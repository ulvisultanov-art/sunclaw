...
describe("sanitizeUserFacingText", () => {
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      ...
      "⚠️ 🛠️ gh search issues --repo sunclaw/sunclaw --state open --no-search-pages.jsonl /tmp/sunclaw_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo sunclaw/sunclaw --state open (agent) failed: command timed out",
      "🛠️ run git status",
  ...
  it("strips copied inbound metadata blocks from user-facing assistant text", () => {
    const input = [
      ...
      "```json",
      '{"chat_id":"channel:123","sender":"SunClaw"}',
      "```",
      "",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `SunClaw runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "SunClaw runtime context (internal): is the label used by the old runtime block formatter.";
