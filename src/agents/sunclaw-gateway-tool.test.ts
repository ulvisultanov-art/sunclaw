...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-test-"));

    try {
      await withEnvAsync({ SUNCLAW_STATE_DIR: stateDir, SUNCLAW_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
        ...
        expect(parsed.payload?.doctorHint).toBe(
          "Recommended follow-up: run sunclaw --profile isolated doctor --non-interactive in a terminal or approvals-capable SunClaw surface.",
        );
      });
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/sunclaw-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/sunclaw-bin"] } } }',
      }),
    ).rejects.toThrow(
