...
const planSunClawModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  const call = planSunClawModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planSunClawModelsJson: (...args: unknown[]) => planSunClawModelsJsonMock(...args),
  }));
  ...
  ({ ensureSunClawModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureSunClawModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureSunClawModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureSunClawModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".sunclaw", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".sunclaw", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planSunClawModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureSunClawModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planSunClawModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planSunClawModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureSunClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureSunClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planSunClawModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planSunClawModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureSunClawModelsJson(first);
      await ensureSunClawModelsJson(second);
      await ensureSunClawModelsJson(first);
