...
import { resolveOptionalMediaToolFactoryPlan } from "./sunclaw-tools.media-factory-plan.js";
...
type CreateSunClawToolsOptions = Parameters<
  typeof import("./sunclaw-tools.js").createSunClawTools
>[0];
let createSunClawToolsForTestModule: typeof import("./sunclaw-tools.js").createSunClawTools;
let legacyComfyToolNames: string[];

async function createSunClawToolsForTest(options?: CreateSunClawToolsOptions) {
  return createSunClawToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/sunclaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): SunClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ...
    legacyComfyToolNames = (
      await createSunClawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: SunClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createSunClawToolsForTest({
      config,
      agentDir: "/tmp/sunclaw-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: SunClawConfig = {};
    const workspaceDir = "/tmp/sunclaw-workspace";
