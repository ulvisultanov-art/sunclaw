...
let configOverride: SunClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: SunClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/ulvisultanov-art/sunclaw/issues/31298
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // SUNCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("SunClaw runtime context (internal):");
