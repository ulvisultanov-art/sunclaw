...
} from "@sunclaw/acp-core/runtime/session-identifiers";
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: SunClawConfig;
  agentId: string;
}): NonNullable<NonNullable<SunClawConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an SunClaw config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for SunClaw config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: SunClawConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: SunClawConfig): string[] {
