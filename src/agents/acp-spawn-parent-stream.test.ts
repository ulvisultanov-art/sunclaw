...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/sunclaw/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/sunclaw/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue("/tmp/sunclaw/agents/codex/sessions/sess-123.jsonl");

    const resolved = resolveAcpSpawnStreamLogPath({
      childSessionKey: "agent:codex:acp:child-1",
    });

    expect(resolved).toBe("/tmp/sunclaw/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/sunclaw/agents/codex/sessions/sessions.json");
  });
});
