...
} from "./sunclaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./sunclaw-tools.nodes-workspace-guard.js";
import {
...
type SunClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: SunClawConfig;
};

const defaultSunClawToolsDeps: SunClawToolsDeps = {
  callGateway,
};

let sunClawToolsDeps: SunClawToolsDeps = defaultSunClawToolsDeps;

export function createSunClawTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? sunClawToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("sunclaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("sunclaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("sunclaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("sunclaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("sunclaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : sunClawToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForSunClawTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentSunClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentSunClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: sunClawToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentSunClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("sunclaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveSunClawPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("sunclaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("sunclaw-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<SunClawToolsDeps>) {
    sunClawToolsDeps = overrides
      ? {
          ...defaultSunClawToolsDeps,
          ...overrides,
        }
      : defaultSunClawToolsDeps;
  },
