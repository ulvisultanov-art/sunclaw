...
describe("promoteAuthProfileInOrder", () => {
  it("normalizes copied secrets when using the locked upsert path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-upsert-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    const previousOAuthDir = process.env.SUNCLAW_OAUTH_DIR;
    ...
    try {
      ...
      if (previousOAuthDir === undefined) {
        ...
        process.env.SUNCLAW_OAUTH_DIR = previousOAuthDir;
      }
  ...
  it("persists openai oauth credentials inline", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-metadata-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("preserves access-only openai oauth credentials inline", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-access-only-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("preserves legacy OAuth sidecar refs during unrelated auth-store saves", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-sidecar-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("drops legacy OAuth sidecar refs when inline token material changes", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-sidecar-new-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("keeps copied openai oauth profiles inline", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-profile-copy-ref-"));
  ...
  it("moves a relogin profile to the front of an existing per-agent provider order", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-order-promote-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("creates a per-agent provider order when relogin has no existing order", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-order-create-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("preserves config-only fallback ids when creating a relogin order", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-order-config-only-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("keeps implicit round-robin when relogin has no existing order by default", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-order-implicit-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("clears matching lastGood after a stale refresh_token_reused profile", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-clear-lastgood-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
  ...
  it("does not clear lastGood when the failed profile is not the stored profile", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-auth-clear-lastgood-keep-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const previousStateDir = process.env.SUNCLAW_STATE_DIR;
    process.env.SUNCLAW_STATE_DIR = stateDir;
    try {
