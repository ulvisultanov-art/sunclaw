...
function buildCoreToolGroupMap() {
  ...
  const sunclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSunClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:sunclaw": sunclawTools,
