  ...
  it("injects workspace identity context", () => {
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        userTimezone: "UTC",
        contextFiles: [
          { path: "/tmp/sunclaw/SOUL.md", content: "SOUL_CONTEXT_MARKER" },
          { path: "/tmp/sunclaw/IDENTITY.md", content: "IDENTITY_CONTEXT_MARKER" },
          { path: "/tmp/sunclaw/USER.md", content: "USER_CONTEXT_MARKER" },
        ],
      },
    ...
    expect(result.systemPrompt).toContain("## /tmp/sunclaw/SOUL.md");
    expect(result.systemPrompt).toContain("SOUL_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/sunclaw/IDENTITY.md");
    expect(result.systemPrompt).toContain("IDENTITY_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/sunclaw/USER.md");
  ...
  it("preserves bootstrap Project Context", () => {
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        contextFiles: [
          {
            path: "/tmp/sunclaw/BOOTSTRAP.md",
            content: "Reply with BOOTSTRAP_OK.",
          },
          {
            path: "/tmp/sunclaw/SOUL.md",
            content: "SOUL_CONTEXT_MARKER",
          },
          {
            path: "/tmp/sunclaw/IDENTITY.md",
            content: "IDENTITY_CONTEXT_MARKER",
          },
          {
            path: "/tmp/sunclaw/USER.md",
    ...
    expect(result.systemPrompt).toContain("## /tmp/sunclaw/USER.md");
    expect(result.systemPrompt).toContain("USER_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/sunclaw/BOOTSTRAP.md");
