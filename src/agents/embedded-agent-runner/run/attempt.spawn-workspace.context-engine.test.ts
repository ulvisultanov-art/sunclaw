...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createSunClawCodingToolsMock,
        0,
        "createSunClawCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded SunClaw runs when configured", async () => {
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createSunClawCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.sunclawCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "sunclaw.runtime-context",
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("SUNCLAW_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.sunclaw/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.sunclaw/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.sunclaw/skills/smaug",
              source: "sunclaw-workspace",
              sourceInfo: {
                path: "/Users/alice/.sunclaw/skills/smaug/SKILL.md",
                source: "sunclaw-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.sunclaw/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/sunclaw-warning-workspace/AGENTS.md",
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createSunClawCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/sunclaw-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/sunclaw-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("SUNCLAW_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the SunClaw runtime event.");
    expect(result.finalPromptText).toBe("Continue the SunClaw runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the SunClaw runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toContain("[SunClaw room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[SunClaw room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the SunClaw runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[SunClaw room event]");
  });
