...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createSunClawCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createSunClawCodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolveSunClawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createSunClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createSunClawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("sunclaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "sunclaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createSunClawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.SUNCLAW_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.SUNCLAW_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.SUNCLAW_TRAJECTORY;
    } else {
      process.env.SUNCLAW_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
