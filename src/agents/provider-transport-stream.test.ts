...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "sunclaw-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "sunclaw-openai-completions-transport",
      },
      {
        ...
        alias: "sunclaw-azure-openai-responses-transport",
      },
      {
        ...
        alias: "sunclaw-anthropic-messages-transport",
      },
      {
        ...
        alias: "sunclaw-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on SunClaw transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createSunClawTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the SunClaw simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("sunclaw-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the SunClaw transport until SunClaw preserves attribution", () => {
