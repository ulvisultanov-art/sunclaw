...
describe("provider attribution", () => {
  it("resolves the canonical SunClaw product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in SunClaw runtime wrapper.",
      product: "SunClaw",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(resolveProviderAttributionHeaders("NVIDIA", { SUNCLAW_VERSION: "2026.3.22" })).toEqual({
      "X-BILLING-INVOKE-ORIGIN": "SunClaw",
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { SUNCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { SUNCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "sunclaw",
      version: "2026.3.22",
      "User-Agent": "sunclaw/2026.3.22",
    });
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { SUNCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard sunclaw User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "SunClaw",
      version: "2026.3.22",
      headers: {
        originator: "sunclaw",
    ...
    expect(resolveProviderAttributionHeaders("xai", { SUNCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "sunclaw",
      version: "2026.3.22",
      "User-Agent": "sunclaw/2026.3.22",
    });
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ SUNCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
