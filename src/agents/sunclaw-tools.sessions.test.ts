...
import "./test-helpers/fast-sunclaw-tools-sessions.js";
...
function createSunClawTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createSunClawTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createSunClawTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.sunclawReasoningReplay).toBeUndefined();
  });
