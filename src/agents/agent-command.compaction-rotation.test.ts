...
const state = vi.hoisted(() => ({
  cfg: undefined as SunClawConfig | undefined,
...
vi.mock("./agent-scope.js", async () => {
  ...
  return {
    ...
    resolveAgentDir: () => state.agentDir ?? "/tmp/sunclaw-agent",
    resolveDefaultAgentId: () => "main",
    resolveEffectiveModelFallbacks: () => undefined,
    resolveSessionAgentId: () => "main",
    resolveAgentWorkspaceDir: () => state.workspaceDir ?? "/tmp/sunclaw-workspace",
  };
});
...
beforeEach(async () => {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-rotation-e2e-"));
