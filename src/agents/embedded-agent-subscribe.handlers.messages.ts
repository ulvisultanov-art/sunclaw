...
function isTranscriptOnlySunClawAssistantMessage(message: AgentMessage | undefined): boolean {
...
function isOpenAiCompletionsAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const api = normalizeOptionalString((message as { api?: unknown }).api) ?? "";
  return api === "openai-completions" || api === "sunclaw-openai-completions-transport";
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlySunClawAssistantMessage(msg)) {
    return;
  }
