...
const ensureSunClawModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;

vi.mock("sunclaw/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("sunclaw/plugin-sdk/llm")>("sunclaw/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureSunClawModelsJson: (...args: Parameters<typeof ensureSunClawModelsJsonMock>) =>
        ensureSunClawModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("sunclaw/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("sunclaw/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("sunclaw-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureSunClawModelsJsonMock.mockReset();
  ensureSunClawModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  ...
  it("resolves explicit OpenAI SunClaw runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-sunclaw"),
      enqueue: immediateEnqueue,
    });
