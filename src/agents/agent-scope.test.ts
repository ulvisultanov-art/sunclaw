...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SunClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/sunclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SunClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SunClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SunClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: SunClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: SunClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: SunClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: SunClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: SunClawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SunClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: SunClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/sunclaw");
  });

  it("uses SUNCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "sunclaw-home");
    vi.stubEnv("SUNCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as SunClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".sunclaw", "workspace"));
  });

  it("uses SUNCLAW_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses SUNCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "sunclaw-home");
    vi.stubEnv("SUNCLAW_HOME", home);
    // Clear state dir so it falls back to SUNCLAW_HOME
    vi.stubEnv("SUNCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as SunClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".sunclaw", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/sunclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/sunclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-agent-scope-"));
