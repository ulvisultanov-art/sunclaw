...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if SUNCLAW_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${SUNCLAW_PREPEND_PATH}${PATH:+:$PATH}"; unset SUNCLAW_PREPEND_PATH; echo test',
    );
  });
