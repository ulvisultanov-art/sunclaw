...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__sunclawCatalog) ? globalThis.__sunclawCatalog : [];
  const apiFiles = Array.isArray(globalThis.__sunclawApiFiles) ? globalThis.__sunclawApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__sunclawNamespaces) ? globalThis.__sunclawNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__sunclawHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __sunclawSettleBridge: { value: settle },
    __sunclawTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__sunclawResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__sunclawHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "sunclaw-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__sunclawTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__sunclawResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  let output: unknown[] = [];
  try {
    const settle = vm.global.getProp("__sunclawSettleBridge");
    try {
