...
import { completeSimple, type Model } from "sunclaw/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.SUNCLAW_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "SUNCLAW_LIVE_SETUP_TOKEN did not look like a setup-token. Use SUNCLAW_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set SUNCLAW_LIVE_SETUP_TOKEN_VALUE or SUNCLAW_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureSunClawModelsJson(cfg, tokenSource.agentDir);
