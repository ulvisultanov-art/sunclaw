...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    sunclaw: process.env.SUNCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.sunclaw === undefined) {
      delete process.env.SUNCLAW_GATEWAY_TOKEN;
    } else {
      process.env.SUNCLAW_GATEWAY_TOKEN = envSnapshot.sunclaw;
    }
  });
  ...
  it("uses SUNCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SUNCLAW_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
