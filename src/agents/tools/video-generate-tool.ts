...
function hasExplicitVideoGenerationModelConfig(cfg?: SunClawConfig): boolean {
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: SunClawConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
