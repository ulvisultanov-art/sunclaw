...
type SkillDiscoveryMode = "sunclaw" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { sunclaw?: ResourceManifest };
    return pkg.sunclaw ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "sunclaw");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "sunclaw-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "sunclaw"),
      projectMetadata,
    ...
    // User extensions from ~/.sunclaw/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.sunclaw/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "sunclaw"),
      userMetadata,
