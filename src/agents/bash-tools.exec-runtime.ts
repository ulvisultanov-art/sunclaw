...
export const DEFAULT_MAX_OUTPUT = clampWithDefault(
  readEnvInt("SUNCLAW_BASH_MAX_OUTPUT_CHARS", "PI_BASH_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
export const DEFAULT_PENDING_MAX_OUTPUT = clampWithDefault(
  readEnvInt("SUNCLAW_BASH_PENDING_MAX_OUTPUT_CHARS"),
  30_000,
  1_000,
  200_000,
);
  ...
  switch (params.failureKind) {
    ...
    case "overall-timeout":
      return typeof params.timeoutSec === "number" && params.timeoutSec > 0
        ? `Command timed out after ${params.timeoutSec} seconds. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so SunClaw can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`
        : "Command timed out. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so SunClaw can register a pollable process session. Do not rely on shell backgrounding with a trailing &.";
...
function wrapPosixCommandWithPathPrepend(
  ...
  env.SUNCLAW_PREPEND_PATH = pathPrepend.join(path.delimiter);

  return `export PATH="\${SUNCLAW_PREPEND_PATH}\${PATH:+:$PATH}"; unset SUNCLAW_PREPEND_PATH; ${command}`;
}
