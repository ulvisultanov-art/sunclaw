...
describe("SunClaw-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetSunClawOwnedToolHooks();
  });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/sunclaw-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/sunclaw-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/sunclaw-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
