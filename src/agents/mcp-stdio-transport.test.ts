...
import { SunClawStdioClientTransport } from "./mcp-stdio-transport.js";
...
describe("SunClawStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    ...
    const transport = new SunClawStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new SunClawStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
