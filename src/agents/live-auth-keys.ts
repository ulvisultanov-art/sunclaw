...
const GOOGLE_LIVE_SINGLE_KEY = "SUNCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SUNCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "SUNCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SUNCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SUNCLAW_LIVE_${base}_KEY`;
