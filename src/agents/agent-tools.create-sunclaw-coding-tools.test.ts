...
import "./test-helpers/fast-sunclaw-tools.js";
import { createSunClawCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as sunClawPluginTools from "./sunclaw-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createSunClawCodingTools>) {
...
type SunClawCodingTool = ReturnType<typeof createSunClawCodingTools>[number];
type SunClawToolsOptions = NonNullable<Parameters<typeof createSunClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: SunClawCodingTool[], name: string): SunClawCodingTool {
...
function requireToolExecute(tool: SunClawCodingTool): NonNullable<SunClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateSunClawToolsOptions(): SunClawToolsOptions {
  const calls = vi.mocked(createSunClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createSunClawTools call");
  }
  return options;
}
...
describe("createSunClawCodingTools", () => {
  const testConfig: SunClawConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createSunClawCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-hook-channel-"));
  ...
  it("keeps Tool Search controls when core SunClaw tools are not materialized", () => {
    ...
    expect(createSunClawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateSunClawToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:sunclaw"], ["*"]]) {
      const tools = createSunClawCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to SunClaw tool construction", () => {
    const createSunClawToolsMock = vi.mocked(createSunClawTools);
    createSunClawToolsMock.mockClear();
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSunClawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into SunClaw tool construction plans", () => {
    const createSunClawToolsMock = vi.mocked(createSunClawTools);
    createSunClawToolsMock.mockClear();
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSunClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the SunClaw core factory", () => {
    const createSunClawToolsMock = vi.mocked(createSunClawTools);
    createSunClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(sunClawPluginTools, "resolveSunClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSunClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to SunClaw tool factory planning", () => {
    const createSunClawToolsMock = vi.mocked(createSunClawTools);
    createSunClawToolsMock.mockClear();
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSunClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to SunClaw plugin discovery", async () => {
    const createSunClawToolsMock = vi.mocked(createSunClawTools);
    createSunClawToolsMock.mockClear();
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSunClawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createSunClawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateSunClawToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("sunclaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("sunclaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createSunClawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createSunClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createSunClawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: SunClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createSunClawCodingTools({
    ...
    const anthropicTools = createSunClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: SunClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSunClawCodingTools({
    ...
    const denied = createSunClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createSunClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSunClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createSunClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createSunClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createSunClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createSunClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createSunClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createSunClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createSunClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createSunClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createSunClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-read-"));
    try {
      ...
      const contents = "Hello from sunclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "sunclaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "sunclaw-workspace"),
    ...
    const tools = createSunClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-canonical-"));
    try {
      const tools = createSunClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-legacy-alias-"));
    try {
      const tools = createSunClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-structured-write-"));
    try {
      const tools = createSunClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-structured-edit-"));
    try {
