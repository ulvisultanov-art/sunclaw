...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(async () => {
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `sunclaw-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
  ...
  it("materializes attachments under explicit cwd when native subagent cwd is provided", async () => {
    const explicitWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `sunclaw-subagent-cwd-attachments-${process.pid}-${Date.now()}-`),
    );
    try {
      const { spawnSubagentDirect } = subagentSpawnModule;
      const result = await spawnSubagentDirect(
      ...
      const explicitAttachmentsRoot = path.join(explicitWorkspaceDir, ".sunclaw", "attachments");
      const targetAttachmentsRoot = path.join(workspaceDirOverride, ".sunclaw", "attachments");
  ...
  it("normalizes explicit cwd before materializing native subagent attachments", async () => {
    const homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `sunclaw-subagent-home-attachments-${process.pid}-${Date.now()}-`),
    );
    ...
    try {
      ...
      expect(result.status).toBe("accepted");
      const attachmentsRoot = path.join(expectedCwd, ".sunclaw", "attachments");
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".sunclaw", "attachments");
