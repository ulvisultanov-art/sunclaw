 ...
 *
 * @see https://github.com/ulvisultanov-art/sunclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /sunclaw\.json|sunclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify sunclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
