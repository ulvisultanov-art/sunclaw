...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("sunclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("sunclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("sunclaw-sbx-");
