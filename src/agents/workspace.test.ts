...
beforeEach(async () => {
  testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-workspace-state-"));
  vi.stubEnv("SUNCLAW_STATE_DIR", testStateDir);
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SUNCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SUNCLAW_HOME: "/srv/sunclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/sunclaw-home"), ".sunclaw", "workspace"));
  });

  it("prefers SUNCLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SUNCLAW_WORKSPACE_DIR: "/srv/sunclaw-workspace",
      SUNCLAW_HOME: "/srv/sunclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/sunclaw-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".sunclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("sunclaw-workspace-");
  ...
  it("does not overwrite a sibling file that is not an SunClaw attestation marker", async () => {
    const tempDir = await makeTempWorkspace("sunclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-workspace-hardlink-"));
    try {
