...
function hasProviderWildcardModelAllowlist(cfg: SunClawConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: SunClawConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: SunClawConfig) {
  return cfg.models?.mode === "replace" && hasProviderWildcardModelAllowlist(cfg);
}

async function loadEmbeddedTuiModelCatalog(cfg: SunClawConfig) {
