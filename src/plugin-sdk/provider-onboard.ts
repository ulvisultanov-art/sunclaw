...
import { findNormalizedProviderKey } from "@sunclaw/model-catalog-core/provider-id";
...
export type { SunClawConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: SunClawConfig, ...args: TArgs) => SunClawConfig;
  applyConfig: (cfg: SunClawConfig, ...args: TArgs) => SunClawConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: SunClawConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: SunClawConfig, preset: TParams) => SunClawConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: SunClawConfig): {
  next: SunClawConfig;
  changed: boolean;
} {
