...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "sunclaw/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "sunclaw/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "sunclaw/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "sunclaw-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "sunclaw-sdk-file-access-compat-" }, async (root) => {
