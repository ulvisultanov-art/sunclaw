...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SUNCLAW_HOME.
  delete process.env.SUNCLAW_HOME;
  process.env.SUNCLAW_STATE_DIR = path.join(base, ".sunclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "sunclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".sunclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
