...
const resolveSunClawPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  beforeEach(() => {
    ...
    resolveSunClawPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "sunclaw-qa-test-api-root-");
    resolveSunClawPackageRootSync.mockReturnValue(sourceRoot);
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "sunclaw-qa-runner-root-");
    resolveSunClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.SUNCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.SUNCLAW_BUNDLED_PLUGINS_DIR).toBe(
    ...
    expect(publicSurfaceCall?.env?.SUNCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.SUNCLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in sunclaw.plugin.json',
    );
  });
});
