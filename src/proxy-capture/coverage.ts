...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[sunclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[sunclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`sunclaw proxy coverage\` for details.`,
  );
}
