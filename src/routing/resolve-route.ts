...
function listAgents(cfg: SunClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SunClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SunClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SunClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SunClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SunClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SunClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SunClawConfig,
  {
    bindingsRef: SunClawConfig["bindings"];
    agentsRef: SunClawConfig["agents"];
    sessionRef: SunClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SunClawConfig): Map<string, ResolvedAgentRoute> {
