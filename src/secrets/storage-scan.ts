...
import { isRecord as isJsonObject } from "@sunclaw/normalization-core/record-coerce";
...
export function listAuthProfileStorePaths(config: SunClawConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.SUNCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
