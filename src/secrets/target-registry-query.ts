...
let compiledSecretTargetRegistryState: {
  ...
  sunClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  sunClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreSunClawTargetState: {
  knownTargetIds: Set<string>;
  sunClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  sunClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelSunClawTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const sunClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "sunclaw.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    sunClawCompiledSecretTargets,
    sunClawTargetsById: buildConfigTargetIdIndex(sunClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreSunClawTargetState() {
  if (compiledCoreSunClawTargetState) {
    return compiledCoreSunClawTargetState;
  }
  const sunClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "sunclaw.json")
    .map(compileTargetRegistryEntry);
  compiledCoreSunClawTargetState = {
    knownTargetIds: new Set(sunClawCompiledSecretTargets.map((entry) => entry.id)),
    sunClawCompiledSecretTargets,
    sunClawTargetsById: buildConfigTargetIdIndex(sunClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(sunClawCompiledSecretTargets),
  };
  return compiledCoreSunClawTargetState;
}
...
function getCompiledChannelSunClawTargets(
  ...
  if (compiledChannelSunClawTargets.has(normalizedChannelId)) {
    return compiledChannelSunClawTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "sunclaw.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelSunClawTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreSunClawTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreSunClawTargetState().sunClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelSunClawTargets(explicitChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().sunClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreSunClawTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreSunClawTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.sunClawCompiledSecretTargets,
    entriesById: registryState.sunClawTargetsById,
  });
