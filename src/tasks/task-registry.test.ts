...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/sunclaw-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/sunclaw-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "sunclaw-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through SunClaw subagent sessions", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.SUNCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      const task = createTaskRecord({
