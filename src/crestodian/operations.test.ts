...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/sunclaw.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @sunclaw/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@sunclaw/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:sunclaw-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:sunclaw-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall sunclaw-demo")).toEqual({
      kind: "plugin-uninstall",
      pluginId: "sunclaw-demo",
    });
  });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/sunclaw.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:sunclaw-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:sunclaw-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:sunclaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:sunclaw-demo",
      },
      { rescue: true, spec: "clawhub:sunclaw-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "sunclaw-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin sunclaw-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("sunclaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin sunclaw-demo",
      },
      { rescue: true, pluginId: "sunclaw-demo" },
    );
  });
