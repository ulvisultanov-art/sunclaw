...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, SunClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe SunClaw Crestodian command.",
  ...
  "Use the provided SunClaw docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `SunClaw docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `SunClaw source: ${
