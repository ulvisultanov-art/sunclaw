...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/sunclaw-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/sunclaw-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("caps project-level parallelism when the Vitest worker budget is conservative", () => {
    ...
    expect(
      resolveParallelFullSuiteConcurrency(58, {
        SUNCLAW_TEST_WORKERS: "1",
      }),
    ).toBe(1);
  });

  it("keeps conservative core full-suite runs on aggregate shards", () => {
    const originalVitestMaxWorkers = process.env.SUNCLAW_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.SUNCLAW_TEST_WORKERS;
    const originalProjectParallel = process.env.SUNCLAW_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.SUNCLAW_TEST_PROJECTS_LEAF_SHARDS;
    try {
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.SUNCLAW_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.SUNCLAW_TEST_WORKERS;
      } else {
        process.env.SUNCLAW_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.SUNCLAW_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.SUNCLAW_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.SUNCLAW_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.SUNCLAW_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
    }
  });
  ...
  it("uses a bounded local default for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          SUNCLAW_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.SUNCLAW_VITEST_FS_MODULE_CACHE_PATH).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.SUNCLAW_VITEST_FS_MODULE_CACHE_PATH).toBe(
  ...
  it("preserves explicit Vitest filesystem module cache paths", () => {
    ...
    expect(
      applyParallelVitestCachePaths(specs, {
        cwd: "/repo",
        env: {
          SUNCLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      }),
    ).toBe(specs);
  });
  ...
  it("routes top-level test helpers to importing repo tests", () => {
    expect(buildVitestRunPlans(["test/helpers/temp-dir.ts"])).toEqual([
      ...
      },
      {
        ...
        forwardedArgs: ["test/sunclaw-launcher.e2e.test.ts"],
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("sunclaw-vitest-include-");
    expect(spec?.env.SUNCLAW_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-sunclaw-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-sunclaw-test.test.ts",
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-contract-include-"));
    try {
      ...
      const specs = createVitestRunSpecs(
        ...
        {
          baseEnv: {
            SUNCLAW_VITEST_INCLUDE_FILE: includeFile,
