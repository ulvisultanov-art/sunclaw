...
import {
  buildSunClawCompileCacheRespawnPlan,
  isSourceCheckoutInstallRoot,
  resolveSunClawCompileCacheDirectory,
  resolveEntryInstallRoot,
  runSunClawCompileCacheRespawnPlan,
  shouldEnableSunClawCompileCache,
...
describe("entry compile cache", () => {
  ...
  it("resolves install roots from source and dist entry paths", () => {
    expect(resolveEntryInstallRoot("/repo/sunclaw/src/entry.ts")).toBe("/repo/sunclaw");
    expect(resolveEntryInstallRoot("/repo/sunclaw/dist/entry.js")).toBe("/repo/sunclaw");
    expect(resolveEntryInstallRoot("/pkg/sunclaw/entry.js")).toBe("/pkg/sunclaw");
  });

  it("treats git and source entry markers as source checkouts", async () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-source-");
    await fs.writeFile(path.join(root, ".git"), "gitdir: .git/worktrees/sunclaw\n", "utf8");

    expect(isSourceCheckoutInstallRoot(root)).toBe(true);
  });

  it("disables compile cache for source-checkout installs", async () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-src-entry-");
  ...
  it("keeps compile cache enabled for packaged installs unless disabled by env", () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-package-");

    expect(shouldEnableSunClawCompileCache({ env: {}, installRoot: root })).toBe(true);
    expect(
      shouldEnableSunClawCompileCache({
  ...
  it("scopes packaged compile cache by package install metadata", async () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-package-key-");
    const packageJsonPath = path.join(root, "package.json");
    await fs.writeFile(packageJsonPath, '{"version":"2026.4.29"}\n', "utf8");

    const directory = resolveSunClawCompileCacheDirectory({
      env: { NODE_COMPILE_CACHE: path.join(root, ".node-cache") },
      installRoot: root,
    });

    expect(directory).toContain(path.join(".node-cache", "sunclaw"));
  ...
  it("builds a one-shot no-cache respawn plan when source checkout inherits NODE_COMPILE_CACHE", async () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-respawn-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");

    const plan = buildSunClawCompileCacheRespawnPlan({
      currentFile: path.join(root, "dist", "entry.js"),
      env: { NODE_COMPILE_CACHE: "/tmp/sunclaw-cache" },
  ...
  it("does not respawn packaged installs when NODE_COMPILE_CACHE is configured", () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-package-respawn-");
  ...
  it("does not respawn source checkouts twice", async () => {
    const root = makeTempDir(tempDirs, "sunclaw-compile-cache-respawn-once-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");
    ...
    expect(
      buildSunClawCompileCacheRespawnPlan({
        currentFile: path.join(root, "dist", "entry.js"),
        env: {
          NODE_COMPILE_CACHE: "/tmp/sunclaw-cache",
          SUNCLAW_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
        },
        installRoot: root,
      }),
  ...
  it("runs compile-cache respawn plans with the child-process bridge", () => {
    ...
    runSunClawCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/sunclaw/dist/entry.js", "status"],
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
      {
  ...
  it("marks signal-terminated compile-cache respawn children as failed without forcing another exit", () => {
    ...
    runSunClawCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/sunclaw/dist/entry.js"],
        env: {},
      },
      {
        spawn: spawn as unknown as typeof import("node:child_process").spawn,
