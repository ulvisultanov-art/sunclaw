...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 SunClaw 配置",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `sunclaw doctor`。",
      previewTitle: "迁移预览",
      source: "迁移来源",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 SUNCLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密码存入 SunClaw 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 SunClaw 配置",
    ...
    security: {
      ...
      beta: "SunClaw 是一个爱好项目，仍处于 beta 阶段。请预期会有边缘问题。",
      confirm: "我理解 SunClaw 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 SunClaw。",
      learnMore: "了解更多",
      notMultitenant: "SunClaw 默认不是敌对多租户安全边界。",
      personalAgent: "默认情况下，SunClaw 是个人 agent：一个可信操作者边界。",
    ...
    skills: {
      ...
      docsLine: "文档：https://docs.sunclaw.complex.az/skills",
      enterEnv: "输入 {env}",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "SunClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 SunClaw 将你加入允许列表。",
      ...
      separatePhoneLabel: "SunClaw 专用独立手机",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：sunclaw channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 SunClaw 用来向 Chat 发送回复的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "SunClaw"',
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 SUNCLAW_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：SUNCLAW_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 SunClaw，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 sunclaw configure --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.sunclaw.complex.az/web/control-ui",
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 SunClaw。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 SunClaw。",
      ...
      gatewayTokenStored:
        "存储位置：$SUNCLAW_CONFIG_PATH（默认 ~/.sunclaw/sunclaw.json）中的 gateway.auth.token，或 SUNCLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 SunClaw。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 SunClaw。",
      ...
      restart: "重启",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.sunclaw.complex.az/security",
      ...
      webDocs: "文档：https://docs.sunclaw.complex.az/tools/web",
      ...
      whatNow: '下一步：https://docs.sunclaw.complex.az/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作区备份",
      workspaceDocs: "文档：https://docs.sunclaw.complex.az/concepts/agent-workspace",
