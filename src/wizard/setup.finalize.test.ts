...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: SunClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: SunClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: SunClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): SunClawConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    ...
    process.env.SUNCLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("SUNCLAW_GATEWAY_PASSWORD", "env-password");
