...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("sunclaw-bundled-");
  const workspaceDir = await createTempDir("sunclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: SunClawConfig) {
