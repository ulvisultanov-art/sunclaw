...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-skill-upload-store-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("skill upload store", () => {
  ...
  beforeAll(async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-skill-upload-store-"));
    try {
