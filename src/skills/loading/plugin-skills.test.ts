    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "sunclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "sunclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "sunclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("sunclaw-");
  const acpxRoot = await tempDirs.make("sunclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("sunclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("sunclaw-");
  const pluginRoot = await tempDirs.make("sunclaw-plugin-");
  const outsideDir = await tempDirs.make("sunclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("sunclaw-");
    const pluginRoot = await tempDirs.make("sunclaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("sunclaw-");
    const pluginRoot = await tempDirs.make("sunclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
