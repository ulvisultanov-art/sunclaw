    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@sunclaw/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that SunClaw can discover during onboarding and install on demand via `sunclaw plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-sunclaw-plugin",
                  description:
                    "SunClaw WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  sunclaw: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-sunclaw-plugin@1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@sunclaw/prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@sunclaw/prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@sunclaw/prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@sunclaw/prerelease-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-clawhub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "clawhub",
                clawhub: {
                  spec: "clawhub:sunclaw/clawhub-chat@2026.5.2",
                  packageName: "sunclaw/clawhub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "SunClaw Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  sunclaw: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "sunclaw-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "sunclaw-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "sunclaw-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredSunClawTmpDir(), "sunclaw-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@sunclaw/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              SUNCLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              SUNCLAW_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@sunclaw/default-env-demo",
