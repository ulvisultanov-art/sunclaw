...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: SunClawConfig, policy: ChannelAccessPolicy) => SunClawConfig;
  resolveAllowlist: (params: { cfg: SunClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: SunClawConfig; resolved: TResolved }) => SunClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: SunClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: SunClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: SunClawConfig, policy: ChannelAccessPolicy): SunClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: SunClawConfig, policy: ChannelAccessPolicy): SunClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: SunClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: SunClawConfig; resolved: string[] }) => {
      calls.push("apply");
