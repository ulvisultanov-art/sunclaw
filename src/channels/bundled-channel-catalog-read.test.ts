...
// resolveSunClawPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/sunclaw-root.js", () => ({
  resolveSunClawPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveSunClawPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
const originalTrustBundledPluginsDir = process.env.SUNCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalTrustBundledPluginsDir === undefined) {
    ...
    process.env.SUNCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
  }
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.SUNCLAW_BUNDLED_PLUGINS_DIR = extensionsRoot;
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "sunclaw" });
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "sunclaw.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // SUNCLAW_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an SUNCLAW_BUNDLED_PLUGINS_DIR override pointing at
