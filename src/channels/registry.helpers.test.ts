...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://docs.sunclaw.complex.az");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("sunclaw-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["sunclaw-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("sunclaw-weixin");
    expect(getRegisteredChannelPluginMeta("SUNCLAW-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
