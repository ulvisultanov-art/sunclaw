...
import type { SunClawConfig, ConfigFileSnapshot } from "../config/config.js";
  ...
  try {
    const result = await withDockerProbeTimeout(params.timeoutMs, (signal) =>
      params.execDockerRawFn(
        ["ps", "-a", "--filter", "label=sunclaw.sandboxBrowser=1", "--format", "{{.Names}}"],
    ...
    const result = await withDockerProbeTimeout(params.timeoutMs, (signal) =>
      params.execDockerRawFn(
        [
          "inspect",
          "-f",
          '{{ index .Config.Labels "sunclaw.configHash" }}\t{{ index .Config.Labels "sunclaw.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("sunclaw sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected sunclaw.browserConfigEpoch=${browserHashEpoch}.`,
      remediation: `${formatCliCommand("sunclaw sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("sunclaw sandbox recreate --browser --all")} (add --force to skip prompt), ` +
...
function buildSandboxBrowserDockerProbeTimeoutFinding(timeoutMs: number): SecurityAuditFinding {
  return {
    ...
    detail:
      ...
      "SunClaw skipped any remaining sandbox browser container drift checks for this status run.",
    remediation:
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `sunclaw security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    try {
      ...
      findings.push({
        ...
        detail:
          ...
          "The extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via `sunclaw.extensions`.",
        remediation:
          "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your SunClaw extensions state directory.",
      });
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all sunclaw.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your SunClaw extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (resolveSkillSource(entry.skill) === "sunclaw-bundled") {
        continue;
      }
      ...
      const summary = await getCodeSafetySummary({
        ...
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `sunclaw security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
