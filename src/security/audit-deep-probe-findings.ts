...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("sunclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("sunclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set SUNCLAW_GATEWAY_TOKEN/SUNCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("sunclaw security audit --deep")}".`,
    });
  }
  return findings;
}
