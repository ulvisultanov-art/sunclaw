...
function summarizeGroupPolicy(cfg: SunClawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: SunClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SunClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SunClawConfig): boolean {
...
function isBrowserEnabled(cfg: SunClawConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: SunClawConfig): SecurityAuditFinding[] {
