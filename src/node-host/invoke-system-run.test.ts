...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  beforeAll(() => {
    sharedFixtureRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-node-host-fixtures-")),
    );
    sharedSunClawHome = path.join(sharedFixtureRoot, "sunclaw-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedSunClawHome, { recursive: true });
  ...
  beforeEach(() => {
    ...
    process.env.SUNCLAW_HOME = sharedSunClawHome;
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedSunClawHome;
    const previousSunClawHomeLocal = process.env.SUNCLAW_HOME;
    process.env.SUNCLAW_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousSunClawHomeLocal === undefined) {
        delete process.env.SUNCLAW_HOME;
      } else {
        process.env.SUNCLAW_HOME = previousSunClawHomeLocal;
      }
    }
  }
  ...
  it("uses auto reviewer for system.run approval misses when exec mode is auto", async () => {
    const tmp = createFixtureDir("sunclaw-system-run-auto-review-");
  ...
  it("does not auto-review direct system.run approval misses without an approval plan", async () => {
    const tmp = createFixtureDir("sunclaw-system-run-auto-review-no-plan-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it("does not auto-review direct system.run security audit suppression edits", async () => {
    const tmp = createFixtureDir("sunclaw-system-run-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "sunclaw" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
          mode: "auto",
  ...
  it("defers to human approval when system.run auto reviewer asks", async () => {
    const tmp = createFixtureDir("sunclaw-system-run-auto-review-ask-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("sunclaw-approved-wrapper-");
  ...
  it("denies shell payload carriers in allowlist mode without explicit approval", async () => {
    ...
    > = [
      ...
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "sunclaw status; id"]
            : ["/bin/sh", "-lc", "sunclaw status; id"],
        approvalRequired: true,
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "sunclaw status; cat /etc/passwd"]
            : ["/bin/sh", "-lc", "sunclaw status; cat /etc/passwd"],
        approvalRequired: true,
      },
      {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const { runCommand, sendInvokeResult } = await runSystemInvoke({
        ...
        security: "allowlist",
        ask: "off",
        command: ["/bin/sh", "-lc", "head -c${IFS}16${IFS}${SUNCLAW_CONFIG_PATH}"],
        rawCommand: "head -c${IFS}16${IFS}${SUNCLAW_CONFIG_PATH}",
      });
  ...
  it("denies abbreviated PowerShell encoded payloads even when the wrapper is allowlisted", async () => {
    const binDir = createFixtureDir("sunclaw-pwsh-allowlist-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "sunclaw-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "sunclaw-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("sunclaw-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("sunclaw-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("sunclaw-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("sunclaw-approval-cwd-drift-");
    const fallback = createFixtureDir("sunclaw-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "sunclaw-approval-script-drift-" : "sunclaw-approval-script-stable-",
      );
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("sunclaw-approval-tsx-script-drift-");
        ...
        const missingBindingTmp = createFixtureDir("sunclaw-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `sunclaw-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("does not persist allow-always approvals for strict inline-eval carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("sunclaw-inline-eval-bin-");
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("sunclaw-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "awk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("sunclaw-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("sunclaw-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("sunclaw-cmd-wrapper-allow-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("sunclaw-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "cd ."],
      cwd: tempDir,
    });
