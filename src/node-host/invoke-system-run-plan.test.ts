...
beforeAll(() => {
  sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sunclaw-run-plan-fixtures-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "sunclaw-node-import-inline-equals-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "sunclaw-ruby-require-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "sunclaw-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "sunclaw-perl-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "sunclaw-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "sunclaw-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "sunclaw-pnpm-dlx-unknown-prefix-value-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32")("handles approval hardening cases", () => {
    for (const testCase of cases) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("sunclaw-approval-hardening-");
        const oldPath = process.env.PATH;
  ...
  it("captures mutable runtime operands in approval plans", () => {
    const tmp = createFixtureDir("sunclaw-approval-script-plan-");
    withFakeRuntimeBins({
  ...
  it("captures mutable shell script operands in approval plans", () => {
    withScriptOperandPlanFixture(
      {
        tmpPrefix: "sunclaw-approval-script-plan-",
      },
      (fixture, tmp) => {
        expectMutableFileOperandApprovalPlan(fixture, tmp);
      },
  ...
  it("keeps fail-closed behavior for relative native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("sunclaw-shell-relative-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for writable absolute native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("sunclaw-shell-absolute-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for owner-controlled read-only absolute binaries", () => {
    ...
    const tmp = createFixtureDir("sunclaw-shell-owned-readonly-binding-");
  ...
  it("keeps fail-closed behavior for symlinked binaries with writable targets", () => {
    ...
    const tmp = createFixtureDir("sunclaw-shell-symlink-binary-binding-");
  ...
  it("keeps fail-closed behavior for mutable or ambiguous shell payload files", () => {
    for (const testCase of [
      {
        tmpPrefix: "sunclaw-shell-script-binding-",
        fileName: "run.sh",
        body: "#!/bin/sh\necho SAFE\n",
      },
      {
        tmpPrefix: "sunclaw-shell-empty-binding-",
        fileName: "empty",
        body: "",
      },
      {
        tmpPrefix: "sunclaw-shell-mz-text-binding-",
        fileName: "mz-script",
        body: "MZ not really a PE file\n",
      },
      {
        tmpPrefix: "sunclaw-shell-nul-header-binding-",
        fileName: "nul-script",
  ...
  it("keeps fail-closed behavior when the shell payload probe stops seeing a file", () => {
    ...
    const tmp = createFixtureDir("sunclaw-shell-race-binding-");
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "sunclaw-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = createFixtureDir("sunclaw-pnpm-dlx-shell-mode-");
        fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
        expect(
          resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind mutable local files", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const casesResult = [
          {
            prefix: "sunclaw-pnpm-dlx-package-bin-",
            command: ["pnpm", "dlx", "cowsay", "hello"],
          },
          {
            prefix: "sunclaw-pnpm-dlx-package-runtime-token-",
            command: ["pnpm", "dlx", "cowsay", "node"],
          },
          {
            prefix: "sunclaw-pnpm-dlx-package-runtime-token-multi-",
            command: ["pnpm", "dlx", "cowsay", "node", "hello"],
          },
          {
            prefix: "sunclaw-pnpm-dlx-package-file-",
          ...
          {
            prefix: "sunclaw-pnpm-dlx-package-data-tail-",
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "sunclaw-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    ...
    for (const testCase of casesValue) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("sunclaw-shell-option-value-");
        const scriptPath = path.join(tmp, "run.sh");
        fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
  ...
  it("captures fish script operands with plus-prefixed filenames", () => {
    ...
    for (const testCase of casesLocal) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("sunclaw-fish-plus-script-");
