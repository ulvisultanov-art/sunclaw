...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sunclaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SUNCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("SUNCLAW_HOME", "/srv/sunclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/sunclaw-home", "cron", "jobs.json"));
  });
});
