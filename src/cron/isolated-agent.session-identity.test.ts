...
describe("runCronIsolatedAgentTurn session identity", () => {
  ...
  it("passes resolved agentDir to runEmbeddedAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call.agentDir).toBe(path.join(home, ".sunclaw", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".sunclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
  ...
  it("passes sessionFile to isolated cron runs", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call.sessionFile).toContain(path.join(home, ".sunclaw", "agents", "main", "sessions"));
  ...
  it("uses lightweight bootstrap context for command-style cron payloads", async () => {
    await withTempHome(async (home) => {
      await runCronTurn(home, {
        jobPayload: {
          kind: "agentTurn",
          message: "cd /srv/sunclaw && ./scripts/nightly-report.sh",
        },
      });
